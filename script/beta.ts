...
const model = "deepagent-code/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/deepagent-code")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with deepagentCode...`)
  ...
  try {
    await $`deepagent-code run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  deepagent-code failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with deepagent-code")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with deepagentCode...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/deepagent-code`.",
