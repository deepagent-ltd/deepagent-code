...
const repo = process.env.GH_REPO ?? "deepagent-ltd/deepagent-code"
const bot = ["actions-user", "github-actions[bot]", "deepagent-code", "deepagent-code-agent[bot]"]
const team = [
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/deepagent-code packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/deepagent-code/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/deepagent-code/")) areas.add("core")
