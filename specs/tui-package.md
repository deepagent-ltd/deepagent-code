...
Move the canonical DeepAgent Code terminal application from
`packages/deepagent-code/src/cli/cmd/tui` into a self-contained workspace package while
...
packages/deepagent-code ---\
                      > @deepagent-code/tui -> @deepagent-code/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/deepagent-code`,
`packages/cli`, or `@deepagent-code/core`.

The SDK is the TUI's DeepAgent Code boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/deepagent-code`.
- Do not replace private `packages/deepagent-code` imports with `@deepagent-code/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@deepagent-code/tui`.

## Ownership Boundary

### `@deepagent-code/tui` Owns
...
- DeepAgent Code domain data displayed by the TUI
...
```text
packages/deepagent-code/src/cli/cmd/tui
```

Its private dependency on `packages/deepagent-code` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/deepagent-code/src/*`.
...
Status: Completed. The private `@deepagent-code/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@deepagent-code/tui`.
...
- The package has no dependency on `deepagent-code`, `@deepagent-code/cli`, or
  `@deepagent-code/core`.
...
primitives now live in `@deepagent-code/tui`. The legacy host consumes explicit
...
and build inputs through `@deepagent-code/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads DeepAgent Code globals or process state;
...
Replace process-global DeepAgent Code state with resolved TUI inputs.
...
- Keep environment reads needed by legacy command or worker startup in
  `packages/deepagent-code` adapters.
...
`@deepagent-code/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/deepagent-code` initially.
...
canonical in `@deepagent-code/tui`. Configured references resolve through the new
...
adapters rather than DeepAgent Code domain implementations.

Make the SDK the only DeepAgent Code domain boundary used by the TUI.
...
- Domain-facing TUI code imports DeepAgent Code data and operations only from
  `@deepagent-code/sdk`.
...
`@deepagent-code/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/deepagent-code`.
...
- The package has no imports from `packages/deepagent-code`, `packages/cli`, or
  `@deepagent-code/core`.
...
public `@deepagent-code/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/deepagent-code`.
- Keep the legacy embedded worker and server startup in `packages/deepagent-code`.
- Change those adapters to load config, create transport inputs, and call the
  public `@deepagent-code/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/deepagent-code`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@deepagent-code/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@deepagent-code/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for DeepAgent Code domain state.
...
cd packages/deepagent-code && bun typecheck
...
rg '@deepagent-code/core|packages/deepagent-code|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/deepagent-code/src packages/deepagent-code/test
```
...
- Build the current-platform `packages/deepagent-code` binary.
