...
Both `action` and `resource` use deepagent-code's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.deepagent-code` files is intentionally deferred until `.deepagent-code` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether deepagent-code permits an operation through its managed execution paths.
