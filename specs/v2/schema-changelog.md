...
This document covers meaningful contract changes introduced on the `feat/deepagent-code-embedded-api` branch since its divergence from `origin/dev`. Mechanical file moves and internal refactors are omitted unless they changed stored data, replay behavior, public HTTP or SDK shapes, or model-facing tool contracts.
...
- Canonical tagged tool file sources in `@deepagent-code/llm`.
...
- Spill oversized model-facing tool text into globally unique files under DeepAgent Code's shared tool-output directory.
...
- Canonicalize ambient instruction traversal boundaries, honor `DEEPAGENT_CODE_DISABLE_PROJECT_CONFIG`, and make non-empty aggregate updates explicitly supersede previously loaded instructions.
