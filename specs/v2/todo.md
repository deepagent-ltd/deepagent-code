...
The deepagent-code server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@deepagent-code/llm` tool loop after replacing its
...
- plugins get global "deepagent-code" instance like in that post i showed
- deepagent-code instance has stuff like `deepagent-code.session.prompt()` or
  `deepagent-code.tool.register({...})`
...
- decide whether to preserve deprecated `@deepagent-code/llm` orchestration exports
