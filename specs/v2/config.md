...
| Field        | Current Purpose                                     | Status | Notes                                                                          |
| ------------ | --------------------------------------------------- | ------ | ------------------------------------------------------------------------------ |
| `shell`      | Default shell for terminal and shell tool execution | keep   | Port as effective config; shared shell choice is used throughout deepagent-code.     |
...
The configured `plugins` list represents package-loaded plugins only. Local plugin code remains discovered from plugin directories such as `.deepagent-code/plugins/`; v2 does not port arbitrary configured local paths or file URLs into this field.
...
Policy evaluation will consume authored config documents in reverse order while preserving statement order inside each document. The precedence of `.deepagent-code` policy sources remains open until `.deepagent-code` configuration is reviewed.
...
| Field | Current Purpose                       | Status   | Notes                                                                                                                                             |
| ----- | ------------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mcp` | MCP server definitions and enablement | redesign | Keep deepagent-code's explicit local/remote server entry format, nested under `mcp.servers`; use `disabled` for inactive entries and move timeout here. |

Keep the deepagent-code MCP server entry format instead of adopting the common `mcpServers` copy/paste shape. Local servers remain explicit `type: "local"` entries with command arrays and `environment`; remote servers remain explicit `type: "remote"` entries with `url`, `headers`, and optional `oauth`. Nest the server map under `mcp.servers` so protocol-wide settings such as default timeout can live under the same subsystem.
