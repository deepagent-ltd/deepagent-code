...
- `packages/deepagent-code` becomes thinner over time: UI, server routes, CLI, storage glue, and legacy compatibility should call the core services instead of owning domain logic directly.
...
Core should not import from `packages/deepagent-code`. If a type or concept is needed by core, move or remodel the domain shape in core first.
...
When porting an deepagent-code service:
...
Keep schemas stable and explicit. Do not rely on deepagent-code config shapes as core domain shapes unless the config shape is actually the domain model.
