# Remove `packages/deepagent-code/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/deepagent-code/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the deepagent-code legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/deepagent-code/src/storage/db.ts` are concentrated in 22 source files:

- `packages/deepagent-code/src/account/repo.ts`
- `packages/deepagent-code/src/cli/cmd/db.ts`
- `packages/deepagent-code/src/cli/cmd/import.ts`
- `packages/deepagent-code/src/cli/cmd/stats.ts`
- `packages/deepagent-code/src/control-plane/workspace.ts`
- `packages/deepagent-code/src/index.ts`
- `packages/deepagent-code/src/node.ts`
- `packages/deepagent-code/src/permission/index.ts`
- `packages/deepagent-code/src/project/project.ts`
- `packages/deepagent-code/src/server/projectors.ts`
- `packages/deepagent-code/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/deepagent-code/src/server/shared/fence.ts`
- `packages/deepagent-code/src/session/message-v2.ts`
- `packages/deepagent-code/src/session/projectors.ts`
...
- `packages/deepagent-code/src/share/share-next.ts`
- `packages/deepagent-code/src/storage/db.ts`
- `packages/deepagent-code/src/sync/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy deepagent-code database wrapper; `packages/deepagent-code/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the deepagent-code wrapper.
...
Status: Completed. `SyncEvent` and the deepagent-code projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy deepagent-code database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy deepagent-code database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy deepagent-code database wrapper.
...
All migration groups are complete or superseded. `packages/deepagent-code/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No deepagent-code data-migration group remains.

The previous deepagent-code `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate deepagent-code data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/deepagent-code`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/deepagent-code/src`
- `bun typecheck` from `packages/deepagent-code`
- Relevant package tests from `packages/deepagent-code`, not the repo root
