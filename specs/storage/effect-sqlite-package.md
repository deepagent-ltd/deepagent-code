...
Create a small workspace package that vendors the Drizzle `effect-sqlite` adapter shape for our repo. This is not an deepagent-code storage abstraction. It is a local package that ports the Drizzle Effect SQLite implementation so we can use it before/independently of upstream release timing.

`packages/deepagent-code` will use it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No deepagent-code paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- `@deepagent-code/effect-drizzle-sqlite`
...
- Opencode-specific path/channel selection stays in `packages/deepagent-code`.

## Opencode Adoption Notes

These are not package requirements, but they matter for the later deepagent-code adoption PR.

The current `packages/deepagent-code/src/storage/db.ts` has two non-obvious semantics that the deepagent-code wrapper must preserve when it consumes this adapter:
...
The deepagent-code wrapper can implement that using Effect context instead of `LocalContext`:
...
Do not remove this behavior while moving deepagent-code to Effect SQLite. `SyncEvent.run` depends on transaction composability and `behavior: "immediate"` for sequencing correctness.

## Migration Strategy

1. Add `@deepagent-code/effect-drizzle-sqlite` with a minimal in-memory/file SQLite test schema.
...
4. Add `@deepagent-code/effect-drizzle-sqlite` as a dependency of `packages/deepagent-code`.
5. Port `packages/deepagent-code/src/storage/db.ts` to be a thin compatibility wrapper over the adapter plus deepagent-code-specific transaction/post-commit context.
...
8. Only then build domain stores like session/message/project stores on top of deepagent-code's storage wrapper.
...
An Effect Drizzle SQLite package lets us vendor the adapter once. Then deepagent-code can build its own storage wrapper on top, and `SessionStorage`, `MessageStorage`, event store, and projector writes can all share the same transaction and migration model.
...
- Should `afterCommit` stay deepagent-code-specific until event publishing moves? Default answer: yes.
...
- Do CLI/admin raw SQL and sqlite shell stay in `packages/deepagent-code`, or does the storage package expose backend capabilities for them?
...
- Use a tiny test schema, not deepagent-code domain tables.
- Prove Effect Drizzle SQLite queries, transactions, and migrations.
- Do not migrate `packages/deepagent-code` yet except possibly adding the dependency if needed for typechecking.

That gives us a focused place to validate the Effect SQLite approach before disturbing deepagent-code's current database runtime.
