...
1. If `.deepagent-code/references/effect-smol` is missing, clone `https://github.com/Effect-TS/effect-smol` there. Do this in the project, not in the skill folder.
2. Search `.deepagent-code/references/effect-smol` for exact APIs, examples, tests, and naming patterns before answering or implementing Effect-specific code.
...
- Do not answer from memory. Verify against `.deepagent-code/references/effect-smol` or nearby code first.

## Testing Patterns

- Use `testEffect(...)` from `packages/deepagent-code/test/lib/effect.ts` for tests that exercise Effect services, layers, runtime context, scoped resources, or platform integrations.
...
- Run tests from package directories such as `packages/deepagent-code`; never run package tests from the repo root.
