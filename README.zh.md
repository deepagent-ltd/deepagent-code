...
仓库当前准备发布 Core V2 与 Desktop 2.0 的协同 alpha 版本。Core 和 Desktop 分别验证、分别发布，因此在公开信息中保留清晰的产品标签。

| 组件 | 版本 | Git 标签 | 范围 |
|---|---|---|---|
| **Core 运行时** | `2.0.0-alpha.0` | `core-v2.0alpha` | 持久 V2 会话、存储、供应商所有权、上下文、工具与恢复 |
| **Desktop 应用** | `2.0.0-alpha.0` | `desktop-v2.0alpha` | Electron 外壳、渲染器、内置 CLI、更新器与打包后的用户体验 |

这些 alpha 标签指向本轮已经完成验证的源码线，不代表稳定 API 兼容承诺，也不代表已经进入生产数据库迁移窗口。GitHub Action 仍使用独立的 `github-v1.4.7` 版本线，待该组件单独发布时再升级。

## 仓库结构

| 路径 | 职责 |
|---|---|
| `packages/core` | 共享的持久运行时与数据库权威 |
| `packages/deepagent-code` | CLI、服务器、会话编排与内置运行时 |
| `packages/app` | Desktop 渲染器与面向用户的应用状态 |
| `packages/desktop` | Electron 进程、打包、更新器与原生集成 |
| `packages/sdk/js` | 对外 JavaScript SDK 接口 |

根 README 介绍产品与快速开始流程；各组件 README 负责本地包开发；`CHANGELOG.md` 记录面向用户的公开版本变化。
