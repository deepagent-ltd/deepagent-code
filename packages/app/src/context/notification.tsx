...
export const { use: useNotification, provider: NotificationProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    // 16.5 API-APP-PACKAGE P5 - per-session durable notification journals replace the
    // volatile GlobalBus listener: each known session's V2 journal is subscribed via the P2
    ...
    const journal = createNotificationJournalSubscription({
      ...
      sessionsOf: (directory: string) => {
        ...
        return childStore.session.map((item) => item.id)
      },
      handlers: {
        onIdle: (directory, sessionID, time) => {
          handleSessionIdle(directory ?? "", { properties: { sessionID } }, time)
        },
        onError: (directory, sessionID, error, time) => {
          // The V2 journal carries the typed UnknownError shape ({type:"unknown",message}),
          // structurally identical to the legacy unknown error envelope the notification path
          // consumes; the cast is the type-level bridge, the runtime shape is preserved.
          handleSessionError(
            directory ?? "",
            {
              properties: {
                sessionID,
                error: error as unknown as EventSessionError["properties"]["error"],
              },
            },
            time,
          )
    ...
    createEffect(() => {
      // Track reactively: the session list (child store) and the active directory drive the
      // known-session set; the subscription rebuilds only when the set actually changes.
      // The refresh handles the unknown-directory case (closes every cursor).
      const directory = currentDirectory()
      const sessionIDs = directory
        ? serverSync.child(directory, { bootstrap: false })[0].session.map((item) => item.id)
        : []
      void sessionIDs.length
      journal.refresh()
    })
