    ...
    queryKey: [directory, "path"],
      queryFn: async () => ({
        ...
        agent: {
          ...
          directories: { data: "", cache: "", state: "", tmp: "", runs: "", artifacts: "", output: "", log: "" },
          coverage: [],
        },
      }),
  }),
...
describe("createChildStoreManager", () => {
  ...
  test("provides the requested directory while the path query is pending", () => {
    ...
    try {
      ...
      expect(store.path.agent.schemaVersion).toBe("deepagent_generic_agent_runtime.v1")
      expect(store.path.agent.implementation).toBe("visible_skeleton")
      expect(store.path.agent.directories.runs).toBe("")
    } finally {
      dispose()
    }
  })
