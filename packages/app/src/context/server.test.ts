...
test("active server removal falls back across built-in and persisted servers", () => {
  ...
  expect(
    nextServerAfterRemoval(
      [local, remote],
      ServerConnection.Key.make("http://10.0.0.5:4097"),
  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://deepagentCode.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://deepagentCode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://deepagentCode.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://deepagentCode.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://deepagentCode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://deepagentCode.example.com"),
      ),
    ).toEqual({
