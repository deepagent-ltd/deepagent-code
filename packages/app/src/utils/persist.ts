...
const GLOBAL_STORAGE = `${APP_STORAGE}.global.dat`
const LEGACY_GLOBAL_STORAGE = `${LEGACY_APP_STORAGE}.global.dat`
const LOCAL_PREFIX = `${APP_STORAGE}.`
const LEGACY_LOCAL_PREFIX = `${LEGACY_APP_STORAGE}.`
...
function legacyAppWorkspaceStorage(dir: string) {
  const head = (dir.slice(0, 12) || "workspace").replace(/[^a-zA-Z0-9._-]/g, "-")
  const sum = checksum(dir) ?? "0"
