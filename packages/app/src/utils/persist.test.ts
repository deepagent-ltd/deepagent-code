...
class MemoryStorage implements Storage {
  ...
  getItem(key: string) {
    ...
    if (key.startsWith("deepagent.throw")) throw new Error("storage get failed")
  ...
  setItem(key: string, value: string) {
    ...
    if (key.startsWith("deepagent.quota")) throw new DOMException("quota", "QuotaExceededError")
    if (key.startsWith("deepagent.throw")) throw new Error("storage set failed")
  ...
  removeItem(key: string) {
    ...
    if (key.startsWith("deepagent.throw")) throw new Error("storage remove failed")
...
describe("persist localStorage resilience", () => {
  test("does not cache values as persisted when quota write and eviction fail", () => {
    const storageApi = persistTesting.localStorageWithPrefix("deepagent.quota.scope")
    storageApi.setItem("value", '{"value":1}')

    expect(storage.getItem("deepagent.quota.scope:value")).toBeNull()
    expect(storageApi.getItem("value")).toBeNull()
  })

  test("disables only the failing scope when storage throws", () => {
    const bad = persistTesting.localStorageWithPrefix("deepagent.throw.scope")
    ...
    const healthy = persistTesting.localStorageWithPrefix("deepagent.safe.scope")
    healthy.setItem("value", '{"value":3}')
    expect(storage.getItem("deepagent.safe.scope:value")).toBe('{"value":3}')
  })

  test("failing fallback scope does not poison direct storage scope", () => {
    const broken = persistTesting.localStorageWithPrefix("deepagent.throw.scope2")
  ...
  test("workspace storage sanitizes Windows filename characters", () => {
    ...
    expect(result).toStartWith("deepagent.workspace.")
