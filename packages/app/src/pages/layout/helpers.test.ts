...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("deepagent-code://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("deepagent-code://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("deepagent-code://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("deepagent-code://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("deepagent-code://open-project")).toBeUndefined()
    expect(parseDeepLink("deepagent-code://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "deepagent-code://open-project?directory=/a",
      "deepagent-code://other?directory=/b",
      "deepagent-code://open-project?directory=/c",
    ])
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("deepagent-code://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("deepagent-code://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "deepagent-code://new-session?directory=/a",
      "deepagent-code://open-project?directory=/b",
      "deepagent-code://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __DEEPAGENT_CODE__: {
        deepLinks: ["deepagent-code://open-project?directory=/a"],
      },
    } as unknown as Window & { __DEEPAGENT_CODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["deepagent-code://open-project?directory=/a"])
