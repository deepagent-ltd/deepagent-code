...
export namespace Timeline {
  ...
  // activity progress projection Fix-E (restored after 1ae96499 removed it): per-Activity revision convergence. For
  ...
  function latestActivityProgress<T extends { messageID: string; part: Part }>(
    ...
    if (visibility)
      return refs.filter((ref) => {
        // Tool (and structural) parts are one-shot evidence of the calls being made, not
        // revisioned streaming output — activity progress projection's per-activity revision convergence applies
        // to text/reasoning only, so an older revision's tool calls stay visible.
        if (ref.part.type !== "text" && ref.part.type !== "reasoning") return true
    ...
    return refs.filter((ref) => {
      // Tool/structural parts are one-shot evidence and must not be hidden by the
      // per-activity revision convergence (see the visibility branch above).
      if (ref.part.type !== "text" && ref.part.type !== "reasoning") return true
