/**
 * BUG-407-008 — spec §7.2 App projection deterministic regression tests (scenarios 18-29,
...
// ---------------------------------------------------------------------------
// Precise incident fixture (docs/bug-407-008.md §2.1/§2.2):
...
const user = {
  ...
  sessionID: "ses_0149b8afffffWlu80cVGdzFI9s",
  role: "user",
  agent: "build",
...
describe("BUG-407-008 §7.2 App projection — incident fixture (timeline layer)", () => {
  test("18: precise incident fixture — revision 8/9 reasoning+tool with identical reasoning", async () => {
    ...
    // Only the latest revision (9) renders; superseded revision 8 is hidden.
    ...
    expect(visible).not.toContain("prt_tool_rev8")
    expect(visible).toEqual(["prt_ff2015c8e00132UPp8LegJCQQ0", "prt_tool_rev9"])
  })
  ...
  test("19: terminal revision 10 has zero parts and state=interrupted", async () => {
    ...
    // Revision 8/9 are hidden once the terminal revision 10 arrives; nothing renderable remains.
