...
export type WslDeepagentCodeCheck = {
  distro: string
...
export type WslJob =
  ...
  | { kind: "probe-deepagent-code"; distro: string; startedAt: number }
  | { kind: "install-deepagent-code"; distro: string; startedAt: number }
...
export type WslServersPlatform = {
  ...
  probeDeepagentCode(name: string): Promise<void>
  installDeepagentCode(name: string): Promise<void>
