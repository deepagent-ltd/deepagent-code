...
import { showToast } from "@deepagent-code/ui/toast"
...
const STEPS: WslServerStep[] = ["wsl", "distro", "deepagent-code"]
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const addDisabled = createMemo(() => {
    ...
    return store.adding || job.kind !== "probe-deepagent-code"
  })
      ...
      >
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "deepagent-code"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.deepagent-code")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
          ...
          <Show when={activeStep() === "deepagent-code" && allReady() && selectedDistro()}>
