...
describe("buildRequestParts", () => {
    ...
    expect(
      result.requestParts.some(
        (part) =>
          part.type === "text" &&
          part.synthetic &&
          part.metadata?.deepagentCodeComment &&
          (part.metadata.deepagentCodeComment as { comment?: string }).comment === "check this",
      ),
    ).toBe(true)
  ...
  test("handles macOS paths correctly", () => {
    ...
    const result = buildRequestParts({
      ...
      messageID: "msg_mac_1",
      sessionID: "ses_mac_1",
      sessionDirectory: "/Users/example/Projects/deepagent-code",
    })
    ...
    if (filePart?.type === "file") {
      ...
      expect(() => new URL(filePart.url)).not.toThrow()
      // Should be a normal Unix path
      expect(filePart.url).toBe("file:///Users/example/Projects/deepagent-code/README.md")
    }
  })
