...
export const setStatus = async (client: ReviewClient, action: "approve" | "reject-ids", ids: string[]): Promise<void> => {
...
export const DialogReview: Component<{ client: ReviewClient }> = (props) => {
  ...
  const invert = () => {
    const cur = selected()
    setSelected(new Set(allItems().map((i) => i.id).filter((id) => !cur.has(id))))
  }
  ...
  const apply = async (action: "approve" | "reject-ids") => {
    ...
    try {
      ...
      showToast({ variant: "error", title: language.t("review.title"), description: error instanceof Error ? error.message : String(error) })
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <Dialog size="x-large" variant="settings" title={language.t("review.title")}>
      <div class="flex h-full flex-col gap-3" data-component="review-dialog">
        <p class="text-12-regular text-v2-text-text-faint">{language.t("review.description")}</p>

        <div class="min-h-0 flex-1 overflow-y-auto rounded-lg border border-v2-border-border-muted">
          <Show
            when={!items.loading}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div class="flex min-w-0 flex-col gap-0.5">
                        <span class="text-13-medium text-v2-text-text-base">{item.summary}</span>
