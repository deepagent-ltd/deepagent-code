import { useFilteredList } from "@deepagent-code/ui/hooks"
...
import { createPromptSubmit, type DeepAgentPromptPrepareResult, type FollowupDraft } from "./prompt-input/submit"
import { ScenarioToggle } from "./prompt-input/scenario-toggle"
...
import { ImagePreview } from "@deepagent-code/ui/image-preview"
...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const promptText = (value: Prompt) => value.map((part) => ("content" in part ? part.content : "")).join("")
  ...
  const finishDraftReview = (result: { editedGoal: string } | false) => {
    ...
    requestAnimationFrame(() => {
      editorRef.focus()
      setCursorPosition(editorRef, current.originalCursor)
      queueScroll()
    })
  }

  onCleanup(() => {
    draftReviewResolve?.(false)
    draftReviewResolve = undefined
  })

  const startDraftPrepare = () => {
    if (!draftPreparePrompt) {
  ...
  const confirmCurrentDraft = () => {
    const editedGoal = promptText(prompt.current()).trim()
    if (!editedGoal) return
    finishDraftReview({ editedGoal })
  }

  const confirmPromptDraft = (draft: DeepAgentPromptPrepareResult) =>
    new Promise<{ editedGoal: string } | false>((resolve) => {
      if (draftReview()) finishDraftReview(false)
      const original = draftPreparePrompt ?? {
        prompt: prompt.current(),
        cursor: prompt.cursor() ?? promptLength(prompt.current()),
      }
      draftPreparePrompt = undefined
      ...
      setStore("mode", "normal")
      setStore("popover", null)
      prompt.set(makeTextPrompt(editable), editable.length)
      requestAnimationFrame(() => {
        editorRef.focus()
        setCursorPosition(editorRef, editable.length)
        queueScroll()
      })
    })
  ...
  const draftReviewBar = () => (
    <Show when={draftReview()} keyed>
      {(_review) => (
        <div
          data-component="prompt-draft-review"
          class="mx-2 mt-2 flex min-w-0 items-center gap-3 rounded-md border border-border-weak-base bg-background-base px-3 py-2"
        >
          <div class="min-w-0 flex-1">
            <div class="truncate text-13-medium text-text-strong">
              {language.t("dialog.provider.deepagent.promptConfirm.title")}
            </div>
            <div class="truncate text-12-regular text-text-base">
              {language.t("dialog.provider.deepagent.promptConfirm.edit.description")}
            </div>
          </div>
          <Button data-action="prompt-draft-cancel" type="button" size="small" variant="ghost" onClick={() => finishDraftReview(false)}>
            {language.t("common.cancel")}
          </Button>
          <Button
            data-action="prompt-draft-confirm"
            type="button"
            size="small"
            variant="primary"
            disabled={promptText(prompt.current()).trim().length === 0}
                ...
                <div class="relative max-h-[180px] overflow-y-auto no-scrollbar" ref={(el) => (scrollRef = el)}>
                  <div
                    ...
                    classList={{
                      ...
                      "cursor-wait text-v2-text-text-muted": draftPreparing(),
                    }}
                  />
                  <div
