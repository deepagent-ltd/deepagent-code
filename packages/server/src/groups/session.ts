...
export const SessionGroup = HttpApiGroup.make("server.session")
  ...
  .add(
    // §16.5 API-APP-PACKAGE P1 — durable session event cursor. The event journal is the
    // authority; a consumer reconnects with `after=<cursor>` and receives exactly the tail. The
    // cursor is opaque and server-issued; gap/reset handling follows in the SDK cursor primitive.
    HttpApiEndpoint.get("session.events", "/api/session/:sessionID/events", {
      params: { sessionID: SessionV2.ID },
      ...
      .annotateMerge(
        OpenApi.annotations({
          ...
          description: "Durable cursor replay of a session's event journal; reconnect with after=<cursor> to resume.",
        }),
      ),
  )
  .add(
    // 16.5 API-APP-PACKAGE P4 - durable journal watermark for snapshot-at-watermark
    // consumers (TUI/CLI/ACP): read the high-water cursor, fetch the message snapshot,
    // then subscribe the events stream with after=<cursor>. Every journaled event at or
    // below the cursor is already reflected in the snapshot (projection commits with the
    // journal), so a drain from the cursor is an exact tail with no gap and no replay.
    HttpApiEndpoint.get("session.events.cursor", "/api/session/:sessionID/events/cursor", {
      params: { sessionID: SessionV2.ID },
      success: Schema.Struct({ cursor: Schema.NullOr(Schema.Number) }),
      error: [SessionNotFoundError],
    })
      .middleware(SessionLocationMiddleware)
      .annotateMerge(
        OpenApi.annotations({
          identifier: "v2.session.events.cursor",
          summary: "Read the session event journal watermark",
          description: "Returns the durable event journal high-water cursor (null when the journal is empty).",
        }),
      ),
  )
  .annotateMerge(
