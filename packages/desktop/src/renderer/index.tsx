...
/** Set to true the first time the renderer reaches ready() to avoid duplicate logs. */
let rendererReadyLogged = false
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.DEEPAGENT_CODE_CHANNEL === "prod" &&
...
const deepLinkEvent = "deepagent-code:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__DEEPAGENT_CODE__ ??= {}
  const pending = window.__DEEPAGENT_CODE__.deepLinks ?? []
  window.__DEEPAGENT_CODE__.deepLinks = [...pending, ...urls]
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("deepagent-code.global.dat").getItem("language")
  ...
  function App() {
    ...
    createEffect(() => {
      ...
      const durationMs = Math.round(performance.now() - rendererStartTime)
      console.info("[startup] telemetry", {
        event: "renderer.initialization",
        durationMs,
        cold: true,
      })
    })
