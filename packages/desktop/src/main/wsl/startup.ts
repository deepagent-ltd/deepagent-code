...
export function expectDeepagentCodeVersion(installed: string | null, expected: string, distro = "Debian") {
  if (installed === expected) return
  throw new Error(
    `DeepAgent Code update finished but ${distro} still reports ${installed ?? "no version"}; expected ${expected}`,
  )
}
