  ...
  const refreshDeepagentCodeCheckBackground = (id: string, distro: string) => {
    void checkDeepagentCode(distro)
      ...
      .catch((error) => {
        ...
        logger?.error("wsl deepagent-code check failed", { id, distro, message })
      })
  }
    ...
    await Promise.all(
      state.servers.map((item) =>
        checkDeepagentCode(item.config.distro)
          ...
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl deepagent-code check failed", {
              id: item.config.id,
...
function deepagentCodeCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "deepagent-code is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "deepagent-code is installed but could not run",
    }
  }
  return {
    distro,
