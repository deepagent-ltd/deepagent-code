...
export async function exportDebugLogs() {
  ...
  const output = join(app.getPath("downloads"), `deepagent-code-debug-${stamp()}.zip`)
  try {
...
function serverLogRoots() {
  ...
  return [...new Set([join(xdgData, "deepagent-code", "log"), join(app.getPath("userData"), "deepagent-code", "log")])]
}
