...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.deepagent-code.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `deepagent-code.settings.dat` is special: it maps to the `deepagent-code.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"deepagent-code.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // deepagentCode.settings.dat → the electron settings store ("deepagent-code.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "deepagent-code.global.dat").
  const storeName = filename === "deepagent-code.settings.dat" ? "deepagent-code.settings" : filename
