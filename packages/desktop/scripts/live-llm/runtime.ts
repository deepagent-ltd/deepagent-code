...
export type Runtime = {
  ...
  permissionResponse: "reject" | "observe"
  cleanupRoot: boolean
}
...
export async function launch(name: string, config: LiveConfig, options: LaunchOptions = {}) {
  ...
  if (process.env.DEEPAGENT_CODE_LIVE_LLM_DEBUG_LOG) {
    app.process().stdout?.on("data", (chunk: Buffer) => process.stdout.write("[app-out] " + chunk.toString()))
    app.process().stderr?.on("data", (chunk: Buffer) => process.stdout.write("[app-err] " + chunk.toString()))
  }
  activeApps.add(app)
  ...
  const runtime = {
    ...
    permissionResponse: options.permissionResponse ?? "reject",
...
export async function createSession(runtime: Runtime, title: string, agent = "live-parent") {
  return request<Session>(runtime, "/session", {
    ...
    body: JSON.stringify({
      title,
      agent,
      model: { id: runtime.config.modelID, providerID: runtime.config.providerID },
    }),
  })
}
...
async function monitorPermissions(runtime: Runtime) {
  ...
  while (!runtime.permissionAbort.signal.aborted) {
    ...
    for (const item of pending.filter((request) => !seen.has(request.id))) {
      ...
      if (runtime.permissionResponse === "observe") continue
