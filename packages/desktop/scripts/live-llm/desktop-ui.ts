...
try {
  ...
  try {
    ...
    const persisted = await waitFor(
      ...
      const all = await messages(runtime, session.id).catch(() => [] as Message[])
      throw new Error(
        `${String(error)}\nPersisted snapshot at timeout: ${JSON.stringify({
          messages: all.map((message) => ({
            ...
            parts: message.parts.map((part) => ({
              type: part.type,
              ...("text" in part ? { text: part.text.slice(0, 80) } : {}),
              ...("tool" in part
                ? { tool: part.tool, status: "state" in part ? part.state.status : undefined }
                : {}),
            })),
          })),
        })}`,
    ...
    await waitFor(
      ...
      await runtime.page
        .screenshot({ path: `/tmp/desktop-ui-fail-${Date.now()}.png` })
        .catch(() => undefined)
      throw new Error(
        `${String(error)}\nLive renderer diagnostics: ${JSON.stringify({
          ...
          contextPartIds: await runtime.page.locator('[data-timeline-part-ids]').count(),
          collapsible: await runtime.page.locator('.tool-collapsible').count(),
    ...
    if (process.env.DEEPAGENT_CODE_KEEP_LIVE_SMOKE === "1") {
      console.error(`[desktop-ui] kept root: ${runtime.root}`)
    }
  }
} finally {
  await closeAll()
