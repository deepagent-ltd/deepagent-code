  ...
  export const invite = fn(
    ...
    async ({ email, role, monthlyLimit }) => {
      ...
      try {
        ...
        const { InviteEmail } = await import("@deepagent-code/console-mail/InviteEmail.jsx")
        await AWS.sendEmail({
          to: email,
          subject: `You've been invited to join the ${emailInfo.workspaceName} workspace on DeepAgent Code`,
          body: render(
            // @ts-ignore
            InviteEmail({
              inviter: emailInfo.inviterEmail,
              assetsUrl: `https://deepagent-code.ai/email`,
