...
import { Billing } from "@deepagent-code/console-core/billing.js"
import { Actor } from "@deepagent-code/console-core/actor.js"
import { WorkspaceTable } from "@deepagent-code/console-core/schema/workspace.sql.js"
import { ZenData } from "@deepagent-code/console-core/model.js"
import { Subscription } from "@deepagent-code/console-core/subscription.js"
import { BlackData } from "@deepagent-code/console-core/black.js"
import { UserTable } from "@deepagent-code/console-core/schema/user.sql.js"
import { ModelTable } from "@deepagent-code/console-core/schema/model.sql.js"
import { ProviderTable } from "@deepagent-code/console-core/schema/provider.sql.js"
...
import { LiteData } from "@deepagent-code/console-core/lite.js"
import { Resource } from "@deepagent-code/console-resource"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-deepagent-code-session") ?? ""
    const requestId = input.request.headers.get("x-deepagent-code-request") ?? ""
    const ocClient = input.request.headers.get("x-deepagent-code-client") ?? ""
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWith429Retry(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-deepagent-code-request")
          headers.delete("x-deepagent-code-session")
          headers.delete("x-deepagent-code-project")
          headers.delete("x-deepagent-code-client")
          return headers
        })(),
        body: reqBody,
      })
    ...
    if (modelData.trialEnded)
      throw new ModelError(
        `${t("zen.api.error.trialEnded", {
          ...
          link: "https://deepagent-code.ai/go",
        })}`,
      )
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://deepagent-code.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://deepagent-code.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://deepagent-code.ai/workspace/${authInfo.workspaceID}/members`
