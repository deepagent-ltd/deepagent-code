...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("deepagent-code-find")
  api.delete("deepagent-code-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("deepagent-code-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("deepagent-code-find", new Highlight(...rest))
    return true
  }
