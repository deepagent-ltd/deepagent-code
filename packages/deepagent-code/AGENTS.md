...
- Running `bun dev` from `packages/deepagent-code` starts the live interactive TUI. Do not run it as a blocking foreground command when you need to inspect the result.
- Start it in `tmux` instead: `tmux new-session -d -s deepagent-code-dev 'bun dev'`.
- Capture the current TUI output with: `tmux capture-pane -pt deepagent-code-dev`.
- Stop the session explicitly when done: `tmux kill-session -t deepagent-code-dev`.
...
export class Service extends Context.Service<Service, Interface>()("@deepagent-code/Foo") {}
