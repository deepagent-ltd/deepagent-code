...
export function layer(config: { readonly perGraphTimeoutMs: number; readonly freshTimeoutMs: number }) {
  if (!Number.isSafeInteger(config.freshTimeoutMs) || config.freshTimeoutMs <= 0)
    throw new Error("invalid fresh timeout")
  return Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const query: FederatedContextQuery.Interface["query"] = (request) =>
        Effect.gen(function* () {
          ...
          const repoFresh =
            request.consistency === "stale_ok" || !sources.includes("documents")
              ? true
              : Exit.isSuccess(
                  yield* handle.coordinator
          ...
          const resolved = yield* FederatedContextResolver.make(
            { adapters, perGraphTimeoutMs: config.perGraphTimeoutMs },
            links,
          ).query({
            securityNamespaceId: handle.identity.securityNamespaceId,
            projectScopeKey: handle.identity.projectScopeKey,
            ...
            text:
              request.query ??
              request.ref?.locator?.symbolPath ??
              request.ref?.locator?.heading ??
              request.ref?.locator?.path ??
              "",
          ...
          const result = {
            ...
            snapshotFingerprint: Hash.sha256(
              JSON.stringify({
                statuses: resolved.statuses,
      ...
      input.code
        ...
        .pipe(
          Effect.map((result) => ({
            ...
            status:
              result.freshnessSatisfied || input.request.consistency === "stale_ok"
                ? result.status
  ...
  return {
    ...
    query: (query) =>
      (query.entityIds?.length
        ...
        : input.coordinator.searchDocuments({ query: query.text, limit: Math.min(query.limit ?? 12, 100) })
      ).pipe(
        Effect.match({
          onFailure: () => ({
            candidates: [],
          ...
          onSuccess: (result) => {
            ...
            return {
              ...
              status: input.fresh
                ? candidates.length > 0
                  ? ContextFederation.status.matched("documents", [
                      { source: "repo_documents", revision, state: "ready" },
                    ])
                  : ContextFederation.status.empty("documents", [
                      { source: "repo_documents", revision, state: "ready" },
                    ])
                : ContextFederation.status.partial({
                    graph: "documents",
...
function validate(request: FederatedContextQuery.Request) {
  ...
  if (request.relation && !ContextLinkStore.Relation.literals.includes(request.relation as ContextLinkStore.Relation))
    return "relation"
...
function allowsRef(ref: ContextReference.ContextRef, identity: Identity, request: FederatedContextQuery.Request) {
  if (
    !ContextAuthorization.authorizeScope(ref, request.principal).allowed ||
    !request.egress.graphs.includes(ref.graph)
  )
    return false
  ...
  if (binding.scope === "project" || binding.scope === "session")
    return binding.projectScopeKey === identity.projectScopeKey
  return true
}
