...
import { openEditor } from "@deepagent-code/tui/editor"
import { registerOpencodeKeymap } from "@deepagent-code/tui/keymap"
...
function footerLabels(input: Pick<RunInput, "agent" | "model" | "variant">): FooterLabels {
  const agentLabel = Locale.titlecase(input.agent ?? "auto")
...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "deepagent-code.agent.name": input.agent,
      "deepagent-code.directory": input.directory,
      "deepagent-code.first": input.first,
      "deepagent-code.model.provider": input.model?.providerID,
      "deepagent-code.model.id": input.model?.modelID,
      "deepagent-code.model.variant": input.variant,
      ...
      try {
        ...
        const close = async (next: {
          ...
          return withRunSpan(
            "RunLifecycle.close",
            {
              "deepagent-code.show_exit": next.showExit,
