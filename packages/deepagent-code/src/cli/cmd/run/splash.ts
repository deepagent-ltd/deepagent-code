...
//
// Renders the full deepagent-code entry logo and a compact [O] exit badge, plus
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    ...
    push(lines, body_left, top, "DeepAgent Code", right, undefined, TextAttributes.BOLD)
    if (input.detail) {
      push(
        lines,
        body_left,
  ...
  if (kind === "exit") {
    ...
    push(
      ...
      top + 1,
      `deepagent-code run -i -s ${meta.session_id}`,
      right,
      undefined,
      TextAttributes.BOLD,
    )
