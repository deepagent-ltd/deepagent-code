...
async function runInteractiveRuntime(input: RunRuntimeInput, deps: RunRuntimeDeps = {}): Promise<void> {
  return withRunSpan(
    ...
    {
      "deepagent-code.mode": input.resolveSession ? "local" : "attach",
      "deepagent-code.initial_input": !!input.initialInput,
      "deepagent-code.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        ...
        "deepagent-code.agent.name": state.agent,
        "deepagent-code.model.provider": state.model?.providerID,
        "deepagent-code.model.id": state.model?.modelID,
        "deepagent-code.model.variant": state.activeVariant,
      ...
      const shell = await (deps.createRuntimeLifecycle ?? createRuntimeLifecycle)({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "deepagent-code.model.provider": model.providerID,
            "deepagent-code.model.id": model.modelID,
            "deepagent-code.model.variant": state.activeVariant,
          })
          return {
      ...
      if (Flag.DEEPAGENT_CODE_SHOW_TTFD) {
        footer.append({
          kind: "system",
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "deepagent-code.model.id": state.model?.modelID,
                "deepagent-code.model.variant": state.activeVariant,
                "deepagent-code.prompt.chars": prompt.text.length,
                "deepagent-code.prompt.parts": prompt.parts.length,
                "deepagent-code.prompt.include_files": includeFiles,
                "deepagent-code.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "deepagent-code.directory": input.directory,
      "deepagent-code.initial_input": !!input.initialInput,
      "deepagent-code.demo": input.demo,
    },
    async () => {
