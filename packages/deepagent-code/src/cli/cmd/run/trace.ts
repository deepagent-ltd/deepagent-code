...
//
// Enable with DEEPAGENT_CODE_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.deepagent/code/log/direct/<timestamp>-<pid>.jsonl. Also writes
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.DEEPAGENT_CODE_DIRECT_TRACE) {
    state = false
    return undefined
  }
