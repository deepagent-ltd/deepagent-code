...
import type { Event, ToolPart } from "@deepagent-code/sdk/v2"
...
function emitTask(state: State): void {
  ...
  const part = {
    ...
    state: {
      status: "running",
      input: {
        filePath: "packages/deepagent-code/src/cli/cmd/run/stream.ts",
        offset: 1,
        limit: 200,
      },
      time: {
  ...
  if (kind === "read") {
    ...
    askPermission(state, {
      ...
      always: [target],
        ...
        title: "read",
        output: ["1: {", '2:   "name": "deepagent-code",', '3:   "private": true', "4: }"].join("\n"),
        metadata: {},
      },
    })
    return
