...
export const PrCommand = effectCmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run deepagent-code",
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.body) {
        ...
        if (sessionMatch) {
          ...
          UI.println(`Found deepagent-code session: ${sessionUrl}`)
          UI.println(`Importing session...`)

          const importResult = yield* Effect.promise(() =>
            Process.text(["deepagent-code", "import", sessionUrl], { nothrow: true }),
          )
    ...
    if (code !== 0) return yield* Effect.die(new Error(`deepagent-code exited with code ${code}`))
  }),
})
