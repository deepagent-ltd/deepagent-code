...
// CLI entry point for `deepagent-code run`.
//
// Handles three modes:
...
//      deepagent-code server and runs interactive mode against it.
//
...
function formatRunError(error: unknown) {
  // V2-only profile refusals are typed (LegacyExecutionUnavailable); render their reason/detail
  // instead of a generic object dump so the CLI tells the operator why the turn was refused.
  if (error instanceof LegacyExecutionUnavailable)
    return `Legacy execution unavailable (${error.reason}): ${error.detail} — this build serves V2 sessions only`
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run deepagent-code with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running deepagent-code server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to DEEPAGENT_CODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to DEEPAGENT_CODE_SERVER_USERNAME or 'deepagent-code')",
      })
      .option("dir", {
        type: "string",
