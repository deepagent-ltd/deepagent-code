...
const AGENT_USERNAME = "deepagent-code-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/deepagent-code.yml"
  ...
  yield* Effect.promise(async () => {
    {
      ...
      async function installGitHubApp() {
        ...
        // Open browser
        const url = "https://github.com/apps/deepagent-code-agent"
        const command =
        ...
        async function getInstallation() {
          return await fetch(`https://api.deepagent-code.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`)
            .then((res) => res.json())
...
jobs:
  "deepagent-code":
    if: |
      ...
      startsWith(github.event.comment.body, '/deepagent-code')
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/deepagent-code,/oc")
        .split(",")
    ...
    async function getOidcToken() {
      try {
        return await core.getIDToken("deepagent-code-github-action")
      } catch (error) {
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `deepagent-code/${type}-${hex}-${timestamp}`
      }
      return `deepagent-code/${type}${issueId}-${timestamp}`
    }
    ...
    function footer(opts?: { image?: boolean }) {
      const image = (() => {
        ...
        return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/deepagent-code-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
      })()
      const shareUrl = shareId ? `[deepagent-code session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the deepagent-code infrastructure after your response",
