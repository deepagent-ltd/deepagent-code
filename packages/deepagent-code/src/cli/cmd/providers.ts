...
export const ProvidersLoginCommand = effectCmd({
  ...
  builder: (yargs) =>
    yargs
      .positional("url", {
        describe: "deepagent-code auth provider",
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    if (args.url) {
      ...
      const wellknown = (yield* cliTry(`Failed to load auth provider metadata from ${url}: `, () =>
        fetch(`${url}/.well-known/deepagent-code`).then((x) => x.json()),
      )) as {
    ...
    if (provider === "other") {
      ...
      yield* Prompt.log.warn(
        `This only stores a credential for ${provider} - you will need configure it in deepagent-code.json, check the docs for examples.`,
      )
    }
    ...
    if (provider === "amazon-bedrock") {
      yield* Prompt.log.info(
        "Amazon Bedrock authentication priority:\n" +
          ...
          "Configure via deepagent-code.json options (profile, region, endpoint) or\n" +
