...
import {
  COMPATIBILITY_PROVIDER_LOADERS,
  SUPPORTED_DEEPAGENT_PROVIDER_IDS,
  SUPPORTED_DEEPAGENT_PROVIDER_PACKAGES,
  applyCompatibilityProviderOptions,
  deepagentUpstreamProviderID,
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    "deepagent-code": Effect.fnUntraced(function* (input: Info) {
      const env = yield* dep.env()
      const hasKey = providerEnvKey(input.env, env) !== undefined
      const ok =
        hasKey ||
        Boolean(yield* dep.auth(input.id)) ||
        Boolean((yield* dep.config()).provider?.["deepagent-code"]?.options?.apiKey)
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>, model?: Model) {
          ...
          // 1. options.region from deepagent-code.json provider config
    ...
    "google-vertex-anthropic": Effect.fnUntraced(function* () {
      ...
      return {
        autoload: true,
        options: {
          ...googleVertexAnthropicOptions(project, location),
        },
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `deepagent-code/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `deepagent-code/${InstallationVersion} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `deepagent-code auth cloudflare-ai-gateway`.",
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `deepagent-code/${InstallationVersion} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "deepagent-code",
          },
        },
      }),
    kilo: () =>
    ...
    "snowflake-cortex": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!account || !pat) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              `Snowflake Cortex: missing credentials (${missing}). Set via env var, deepagent-code auth, or provider options.`,
            )
          },
        }
      }
...
export class Service extends Context.Service<Service, Interface>()("@deepagent-code/Provider") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        for (const [providerID, provider] of configProviders) {
          ...
          for (const [groupId, group] of Object.entries(provider.groups ?? {})) {
            ...
            for (const [modelID, model] of Object.entries(groupModelSource)) {
              ...
              const apiID = model.id ?? existingModel?.api.id ?? modelID
              const catalogModel = existingModel ? undefined : catalogSpecFor(apiID, modelID, catalogIndex)
              // Priority: model.provider.npm > group.npm > provider.npm > default
              const apiNpm =
                model.provider?.npm ??
                groupNpm ??
                existingModel?.api.npm ??
        ...
        for (const [id, provider] of Object.entries(database)) {
          ...
          const apiKey = envKey ? envs[envKey] : undefined
          if (!apiKey) continue
    ...
    async function resolveSDK(model: Model, s: State, envs: Record<string, string | undefined>, modelAuth?: Auth.Info) {
      try {
        ...
        applyCompatibilityProviderOptions(model, options)
        ...
        const bundledLoader = COMPATIBILITY_PROVIDER_LOADERS[model.api.npm]
        if (bundledLoader) {
    ...
    const getLanguage = Effect.fn("Provider.getLanguage")(function* (model: Model) {
      ...
      const provider = s.providers[model.providerID]
      const modelAuthID = deepagentModelAuthProviderID(model)
      const modelAuth = modelAuthID ? yield* auth.get(modelAuthID).pipe(Effect.orDie) : undefined
      return yield* EffectPromise.refineRejection(
        async () => {
          const sdk = await resolveSDK(model, s, envs, modelAuth)
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderV2.ID) {
      ...
      const priority = providerID.startsWith("deepagent-code")
        ? ["gpt-5-nano"]
