...
// provider lifecycle gap C: optional session attribution threaded into capture budget
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Snapshot.state")(function* (ctx) {
        ...
        const initializeFromRepository = Effect.fnUntraced(function* () {
          const marker = path.join(state.gitdir, "deepagent-baseline")
          if (yield* exists(marker)) return true
          const indexed = yield* git(args(["ls-files", "-z"]), {
            cwd: state.directory,
            maxOutputBytes: DiffLimits.discoveryOutputBytes,
            timeout: DiffLimits.wallTime,
          })
          if (indexed.code === 0 && indexed.text.length > 0) {
            yield* fs.writeFileString(marker, "index\n").pipe(Effect.orDie)
            return true
          }
          const sourceTree = yield* git(["-C", state.worktree, "rev-parse", "HEAD^{tree}"], {
            cwd: state.worktree,
            timeout: DiffLimits.wallTime,
          })
          const sourceGitDir = yield* git(
            ["-C", state.worktree, "rev-parse", "--path-format=absolute", "--git-common-dir"],
            { cwd: state.worktree, timeout: DiffLimits.wallTime },
          )
          if (sourceTree.code !== 0 || sourceGitDir.code !== 0) {
            log.warn("failed to resolve repository snapshot baseline", {
              worktree: state.worktree,
              treeExitCode: sourceTree.code,
              treeError: sourceTree.stderr,
              gitDirExitCode: sourceGitDir.code,
              gitDirError: sourceGitDir.stderr,
            })
            return false
          }
          const objects = path.join(sourceGitDir.text.trim(), "objects")
          if (!(yield* exists(objects))) {
            log.warn("repository object directory is unavailable for snapshot baseline", { objects })
            return false
          }
          yield* fs.ensureDir(path.join(state.gitdir, "objects", "info")).pipe(Effect.orDie)
          yield* fs
            .writeFileString(path.join(state.gitdir, "objects", "info", "alternates"), `${objects}\n`)
            .pipe(Effect.orDie)
          const baseline = yield* git(args(["read-tree", sourceTree.text.trim()]), {
            cwd: state.worktree,
            timeout: DiffLimits.wallTime,
          })
          if (baseline.code === 0) {
            const refreshed = yield* git(args(["update-index", "--refresh", "--ignore-submodules"]), {
              cwd: state.worktree,
              maxOutputBytes: DiffLimits.discoveryOutputBytes,
              timeout: DiffLimits.wallTime,
            })
            if (refreshed.code !== 0) {
              log.warn("failed to refresh repository snapshot baseline", {
                exitCode: refreshed.code,
                stderr: refreshed.stderr,
              })
              return false
            }
            yield* fs.writeFileString(marker, `${sourceTree.text.trim()}\n`).pipe(Effect.orDie)
            return true
          }
          log.warn("failed to initialize snapshot baseline from repository", {
        ...
        // provider lifecycle gap C: budget failures return a typed degraded attribution
        ...
        const trackOutcome = Effect.fnUntraced(function* (attribution?: Attribution) {
          return yield* locked(
            Effect.gen(function* () {
              ...
              yield* initializeFromRepository()
