...
export class Service extends Context.Service<Service, Interface>()("@deepagent-code/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `deepagent-code auth login` in the terminal",
      name: "Login with deepagent-code",
      id: AuthMethodID,
    }
...
async function loadDirectorySnapshot(sdk: OpencodeClient, directory: string) {
  return ACPProfile.measure("acp.directory.load", async () => {
    ...
    return Directory.build({
      ...
      providers,
      modes,
      defaultModeID: agents.find((agent) => agent.mode === "primary" && agent.hidden !== true)?.name ?? "auto",
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "DeepAgent Code service failure", service })
}
