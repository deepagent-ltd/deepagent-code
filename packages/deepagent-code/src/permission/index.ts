...
export const layerWith = (options: LayerOptions = {}) =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      // The permission-owner lease is the deepagent ActivityAuthority's liveness probe. When the
      // deepagent runtime is disabled there is no authority to serve, so registering a lease would
      // make activity recovery pretend a live authority exists (granting recovery_required) where
      // the host never runs one. Gate the registration (and heartbeat below) on the same env the
      // location graph uses.
      const deepagentEnabled = process.env.DEEPAGENT_ENABLED !== "false" && process.env.DEEPAGENT_ENABLED !== "0"
      if (deepagentEnabled)
        yield* DeepAgentActivityAuthority.heartbeatPermissionOwner({
          ownerID: yield* permissionOwnerID(),
          leaseMs: ownerLeaseMs,
        }).pipe(Effect.provideService(Database.Service, database), Effect.orDie)
      yield* deepagentEnabled
        ? Effect.addFinalizer(() =>
            permissionOwnerID().pipe(
      ...
      const heartbeat = withPermissionOwner(
        Effect.gen(function* () {
          ...
          // Quarantine indeterminate external effects before settling their pending permission
          // requests. Reversing this order can trip the activity terminal fence and abort service
          // startup while a dead owner still has a started effect.
          yield* DeepAgentActivityAuthority.recoverPermissionEffects(recoveryOwnerID).pipe(
            Effect.provideService(Database.Service, database),
            Effect.catchCause((cause) =>
              Effect.logError("permission effect recovery failed; keeping affected activities quarantined", {
                cause,
              }),
            ),
          )
          yield* DeepAgentActivityAuthority.recoverPendingPermissions(recoveryOwnerID).pipe(
            Effect.provideService(Database.Service, database),
            Effect.catchCause((cause) =>
              Effect.logError("pending permission recovery failed; keeping affected requests pending", { cause }),
            ),
          )
        }),
      )
      ...
      const askEffect = Effect.fn("Permission.askEffect")(function* (input: AskInput) {
        ...
        // Facade tool permissions belong to the delegated child execution, not to the
        // activity_start/control call in the parent turn. Task delegation binds owner_session_id
        // after the child Session exists and before enqueue, giving every child tool call one
        // durable facade owner without conflating its call ID with the spawn call ID.
        const facadeActivity = request.tool
          ? yield* database.db
              .select({
                activityID: SessionFacadeActivityTable.activity_id,
                sessionID: SessionFacadeActivityTable.owner_session_id,
                state: SessionFacadeActivityTable.state,
              })
              .from(SessionFacadeActivityTable)
              .where(
                and(
                  eq(SessionFacadeActivityTable.owner_session_id, request.sessionID),
                  eq(SessionFacadeActivityTable.subkind, "task"),
              ...
              .pipe(Effect.orDie)
          : undefined
        // The durable ActivityAuthority is the deepagent runtime's record of requests and
        // decisions. When it is disabled (DEEPAGENT_ENABLED=false) there is no permission-owner
        // lease to authorize against — the same environment-absence rule as activity recovery —
        // so tool permissions fall back to policy-only semantics (allowed requests proceed
        // unrecorded; interactive requests settle through the in-memory pending path).
        const activity = deepagentEnabled
          ? facadeActivity
            ? { ...facadeActivity, activityKind: "facade" as const }
            : legacyActivity
              ? { ...legacyActivity, activityKind: "legacy" as const }
              : undefined
          : undefined
        if (!activity) {
