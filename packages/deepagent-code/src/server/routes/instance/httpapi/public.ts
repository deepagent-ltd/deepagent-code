...
import { DeepAgentCodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for deepagent-code serve and web commands",
  ...
  for (const [path, item] of Object.entries(spec.paths ?? {})) {
    for (const method of ["get", "post", "put", "delete", "patch"] as const) {
      ...
      if (operation.requestBody) {
        ...
        if (!isV2Api && path !== "/session/{sessionID}/continuation-resolution") delete operation.requestBody.required
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(DeepAgentCodeHttpApi)
...
export const PublicApi = DeepAgentCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "deepagent-code",
    version: "1.0.0",
    description: "deepagent-code api",
