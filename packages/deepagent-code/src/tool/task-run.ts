                ...
                if (recoveredCompletion) {
                  const settled = yield* tx
                    ...
                    .set({
                      ...
                      control_state: "closed",
                      reason:
                        structuredCompletionReason(recoveredCompletion.structuredOutputReceipt.transport),
                  ...
                  if (settled) {
                    ...
                    yield* recordTerminalReceiptInTransaction(tx, {
                      run: fromRow(candidate.run),
                      state: "completed",
                      reason:
                        structuredCompletionReason(recoveredCompletion.structuredOutputReceipt.transport),
                      output: recoveredCompletion.output,
                      // Recovered evidence was sealed by the original executor, not the sweeper.
                      ownerToken: candidate.run.execution_owner!,
                      now,
                    })
                  }
...
// Control-plane settlements (close, interrupt, recovery resolution, lease expiry, pre-execution
// failure) have no execution owner; a fixed sentinel identifies the settlement source in receipt
// evidence.
export const CONTROL_PLANE_OWNER = "control-plane"

// One reason mapping for structured-output recoveries: the settlement row and its compensation
// receipt must stay byte-identical, or an exact re-settlement would hit a receipt divergence
// conflict far from the source of the drift.
function structuredCompletionReason(transport: StructuredOutputReceipt["transport"]) {
  if (transport === "structured") return "structured_output_valid"
  if (transport === "text_fallback") return "structured_output_text_fallback"
  return "structured_output_degraded_text"
}
      ...
      db.transaction(
        (tx) =>
          Effect.gen(function* () {
            ...
            // Compensation receipt (§16.3 order 2): compositions wired for the V2 capability port
            // record the durable terminal outcome inside this settlement transaction; unwired
            // compositions stay receipt-less.
            yield* recordTerminalReceiptInTransaction(tx, {
              run: input.run,
              state: input.state,
              reason: input.reason,
              ...
              if (immediateTerminal.includes(oldState)) {
                ...
                if (updated) {
                  yield* tx
                    ...
                    .run()
                    .pipe(Effect.orDie)
                  // A receipt divergence here rolls back the whole subtree close; reachable only
                  // via replayed external evidence, since in-transaction settlements converge.
                  yield* recordTerminalReceiptInTransaction(tx, {
                    run: {
                      runID: row.run_id,
                ...
                Effect.gen(function* () {
                  ...
                  const updated = yield* tx
                    ...
                    .set({
                      ...
                      control_state: "closed",
                      reason:
                        structuredCompletionReason(completion.structuredOutputReceipt.transport),
                  ...
                  yield* recordTerminalReceiptInTransaction(tx, {
                    ...
                    reason:
                      structuredCompletionReason(completion.structuredOutputReceipt.transport),
                    output: completion.output,
                    // Recovered evidence was sealed by the original executor, not startup recovery.
                    ownerToken: run.execution_owner!,
                    now,
                  })
...
export function orderedShutdown(input: { directory: string; now?: number }) {
  return Effect.gen(function* () {
    ...
    for (const run of candidates) {
      ...
      if (canRequeue) {
        yield* requestInterrupt({ runID: run.run_id, reason: "shutdown_interrupt", now }).pipe(
          Effect.tapError((error) => Effect.logWarning("task.shutdown.cancel_conflict", { error })),
          Effect.ignore,
        )
      } else if (isActive) {
        yield* requestInterrupt({ runID: run.run_id, reason: "shutdown_interrupt", now }).pipe(
