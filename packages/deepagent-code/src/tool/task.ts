...
import { boundDegradedRawResult, makeDegradedStructuredOutput } from "./task-structured-output-evidence"
import type { StructuredOutputReceipt } from "./task-run"
...
// The V1 prompt driver retained for PR finalize (item #29 owns its migration onto the V2
// authority). Ordinary task execution no longer routes through here: the durable TaskRunAuthority
// owns every fresh launch, resume, and structured finalization turn.
...
export interface TaskPromptOps {
  ...
  /** SessionPrompt's durable envelope preparation (public service surface); no task-execution
   * consumer remains — retained for the ops() wiring contract. */
  prepareTaskInput?(input: SessionPrompt.PromptInput, timeCreated: number): Effect.Effect<SessionV1.WithParts, unknown>
  // E is unknown, not never: the real prompt fails (provider errors) — pr-finalize's V1 driver
  // relies on that failure channel, and mock ops in tests must be able to throw.
