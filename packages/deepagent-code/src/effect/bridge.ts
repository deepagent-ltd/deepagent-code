...
import { EventRouteRef, InstanceRef, WorkspaceRef } from "./instance-ref"
...
const promiseAbortGraceMs = 2_000

function abortReason(signal: AbortSignal) {
  return signal.reason instanceof Error ? signal.reason : new DOMException("Aborted", "AbortError")
}

function gracefulAbort(signal: AbortSignal | undefined) {
  if (!signal || signal.aborted) return { signal, cleanup: () => undefined }
  const controller = new AbortController()
  let timer: ReturnType<typeof setTimeout> | undefined
  const abort = () => {
    if (timer) return
    timer = setTimeout(() => controller.abort(signal.reason), promiseAbortGraceMs)
  }
...
function captureSync() {
  ...
  const eventRoute = fiber ? Context.getReferenceUnsafe(fiber.context, EventRouteRef) : undefined
  const workspace =
    (fiber ? Context.getReferenceUnsafe(fiber.context, WorkspaceRef) : undefined) ?? WorkspaceContext.workspaceID
  return { instance, eventRoute, workspace }
}
...
export function make(): Effect.Effect<Shape> {
  return Effect.gen(function* () {
    ...
    const eventRoute = (yield* EventRouteRef) ?? captured.eventRoute
    ...
    return {
      promise: <A, E, R>(effect: Effect.Effect<A, E, R>, signal?: AbortSignal) =>
        restoreWorkspace(workspace, () => {
          ...
          const abort = gracefulAbort(signal)
          return Effect.runPromise(wrap(effect), { signal: abort.signal })
            .catch((error) => {
      ...
      run: <A, E, R>(effect: Effect.Effect<A, E, R>) =>
        Effect.callback<A, E>((resume, signal) => {
          const interrupt = restoreWorkspace(workspace, () =>
            Effect.runCallback(wrap(effect), {
              signal,
              onExit: (exit) =>
