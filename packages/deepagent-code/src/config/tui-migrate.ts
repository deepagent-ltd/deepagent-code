...
const TUI_SCHEMA_URL = "https://deepagent-code.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from deepagent-code.json files
...
async function deepagentCodeFiles(input: { directories: string[]; cwd: string }) {
  ...
  if (Flag.DEEPAGENT_CODE_CONFIG) files.push(Flag.DEEPAGENT_CODE_CONFIG)
