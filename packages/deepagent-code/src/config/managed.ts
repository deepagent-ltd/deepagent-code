...
const MANAGED_PLIST_DOMAIN = "ai.deepagent-code.managed"

// Keys injected by macOS/MDM into the managed plist that are not DeepAgent Code config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/deepagent-code"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "deepagent-code")
    default:
      return "/etc/deepagent-code"
  }
}

export function managedConfigDir() {
  return process.env.DEEPAGENT_CODE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
