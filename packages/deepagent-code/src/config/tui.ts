...
import { TuiKeybind } from "@deepagent-code/tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@deepagent-code/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old deepagent-code.json shape) still get their settings applied.
  ...
  const projectFiles = Flag.DEEPAGENT_CODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit DEEPAGENT_CODE_TUI_CONFIG override, if set.
  if (Flag.DEEPAGENT_CODE_TUI_CONFIG) {
    const configFile = Flag.DEEPAGENT_CODE_TUI_CONFIG
