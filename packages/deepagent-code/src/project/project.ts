...
export class Service extends Context.Service<Service, Interface>()("@deepagent-code/Project") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fakeVcs = Schema.decodeUnknownSync(Schema.optional(ProjectVcs))(Flag.DEEPAGENT_CODE_FAKE_VCS)

    const scope = yield* Scope.Scope

    // §16.3 order 5 F1 — project table writer contract (exhaustive inventory):
    //   CREATE-IF-ABSENT  core V2Session.create is the authority (ProjectV2.resolve is pure
    //                     identity resolution, it writes nothing); import/writer/session.ts holds
    //                     one mirror writer with the identical onConflictDoNothing semantics (it
    //                     can never overwrite).
    //   UPDATE / REBIND   THIS service (migrateProjectId, fromDirectory global→directory rebind)
    //                     and cli/cmd/import.ts (session rebind onto the imported project) — all
    //                     control-plane DATA CORRECTIONS (idempotent row repairs in transactions),
    //                     never execution authority and never a fallback for session execution.
    //   DELETE            projectDelete handler (control-plane lifecycle only).
    // Session execution stays on the V2 session/project resolution path throughout.
