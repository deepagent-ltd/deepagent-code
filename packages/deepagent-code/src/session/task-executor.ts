...
import { recordTerminalReceiptInTransaction } from "@/tool/task-run"
      ...
      db.transaction(
        (tx) =>
          Effect.gen(function* () {
            ...
            // Compensation receipt (§16.3 order 2): the executor owns this settlement, so the
            // receipt carries the real owner token and the final (priority-resolved) evidence.
