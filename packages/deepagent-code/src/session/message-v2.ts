...
type DatabaseExecutor =
  ...
  | Parameters<Parameters<Database.Interface["db"]["transaction"]>[0]>[0]

function hydrate(db: DatabaseExecutor, rows: (typeof MessageTable.$inferSelect)[]) {
...
function readHistorySnapshotInTransaction(
  ...
  return Effect.gen(function* () {
    ...
    const replacementMessageIDs = epoch
      ...
      : []
    const providerRecoveryBinding = epoch?.recovery_resolution_id
      ? yield* tx
          .select({
          ...
          .get()
      : undefined
    const continuationRecoveryBinding = epoch?.recovery_resolution_id
      ? yield* tx
          .select({
            resolutionID: CompactionContinuationResolutionTable.resolution_id,
            sourcePromptEpoch: CompactionContinuationResolutionTable.source_prompt_epoch,
            successorPromptEpoch: CompactionContinuationResolutionTable.successor_prompt_epoch,
            successorWindowID: CompactionContinuationResolutionTable.successor_window_id,
            successorHistoryHash: CompactionContinuationResolutionTable.successor_history_hash,
            successorMutationEpoch: CompactionContinuationResolutionTable.successor_mutation_epoch,
          })
          .from(CompactionContinuationResolutionTable)
          .where(
            and(
              eq(CompactionContinuationResolutionTable.session_id, sessionID),
              eq(CompactionContinuationResolutionTable.successor_prompt_epoch, epoch.epoch),
              eq(CompactionContinuationResolutionTable.resolution_id, epoch.recovery_resolution_id),
            ),
          )
          .get()
      : undefined
    const recoveryBinding = providerRecoveryBinding ?? continuationRecoveryBinding
    ...
    const recoveryAdmittedMessageIDs =
      providerRecoveryBinding && epoch?.recovery_resolution_id
        ? new Set(
            (yield* tx
              ...
              .where(
                and(
                  ...
                  eq(SessionIntentTable.mutation_epoch, providerRecoveryBinding.successorMutationEpoch),
                ),
              )
