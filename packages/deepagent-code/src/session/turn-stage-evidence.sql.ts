// provider lifecycle gap C: drizzle-orm type bindings for session_turn_stage_evidence.
// One row per (session_id, activity_id, turn_id), upserted forward-only as the provider turn
...
export const SessionTurnStageEvidenceTable = sqliteTable(
  ...
  (table) => [primaryKey({ columns: [table.session_id, table.activity_id, table.turn_id] })],
)
