...
import { and, asc, eq, inArray, isNull, lte, max, notExists, sql } from "drizzle-orm"
    ...
    .transaction(
      (tx) =>
        Effect.gen(function* () {
          ...
          // 1.4.8.rN: fail-closed admission while a crash-recovered activity still has an
          // INDETERMINATE provider outcome (the crash classifier rewrites dispatching/streaming
          // receipts to indeterminate_after_crash; a settled receipt is a definite outcome).
          // Gates BOTH run_now and deferred admits: a deferred admit would otherwise queue
          // silently behind an unresolved outcome. Exact retries reconcile admission before
          // materializeTurn and never reach this gate.
          const blockedIndeterminate = yield* tx
            ...
            .innerJoin(
              ...
              eq(SessionToolRequestReceiptTable.user_message_id, SessionActivityAdmissionTable.admitted_message_id),
            )
            .where(
              and(
            ...
            .get()
            .pipe(Effect.orDie)
          if (blockedIndeterminate)
            return yield* Effect.fail(
              new Conflict({
                intentID: input.receipt.intentID,
                reason: "legacy activity has an indeterminate provider outcome after recovery",
              }),
            )
          ...
          if (!existingActivity) {
            const active = yield* tx
              ...
              .where(
                and(
                  eq(SessionLegacyActivityTable.session_id, input.receipt.sessionID),
                  inArray(SessionLegacyActivityTable.state, ["active", "recovery_required"] as const),
                ),
              )
              .get()
              .pipe(Effect.orDie)
  ...
  const active = yield* db
    ...
    .where(
      and(
        ...
        notExists(
          db
            .select({ activityID: SessionLegacyActivityTerminalTable.activity_id })
            .from(SessionLegacyActivityTerminalTable)
            .where(eq(SessionLegacyActivityTerminalTable.activity_id, SessionLegacyActivityTable.activity_id)),
        ),
      ),
    )
    .all()
  ...
  return yield* Effect.forEach(active, (activity) =>
    Effect.gen(function* () {
      ...
      const objective = yield* db
        ...
        .get()
        .pipe(Effect.orDie)
      // The recovery authority (deepagent ActivityAuthority) gates HUMAN-approved recovery via a
      // live permission-owner lease. When the authority is unavailable (no live lease — e.g. the
      // deepagent runtime is disabled), a plain "active" objective must still be crash-classified:
      // the refusal is an environment absence, not a decision. "needs_human" retains the gate.
      const permissionRecovered =
        objective && ["active", "needs_human"].includes(objective.state) && recoverActivity
          ? yield* recoverActivity({
              ...
              // An authority absence (no live permission-owner lease — e.g. the deepagent runtime
              // disabled) is NOT an approval: fall into the crash-classifier's own decision.
              Effect.catch(() => Effect.succeed(false)),
            )
          : false
      // Only an objective explicitly awaiting human recovery keeps the approval gate; plain
      // "active" objectives proceed to the crash classifier so a bound run is terminalized.
      if (objective && objective.state === "needs_human" && recoverActivity && !permissionRecovered)
        return undefined
      return yield* db
        .transaction(
          (tx) =>
            Effect.gen(function* () {
              const current = yield* tx
                ...
                .get()
              if (!current || !["active", "recovery_required"].includes(current.state)) return undefined
              if (permissionRecovered && current.state !== "recovery_required") return undefined
              const authorityRecovered = current.state === "recovery_required"
              const run = yield* tx
                .select()
                .from(SessionLegacyActivityRunTable)
              ...
              const terminalState = preDispatch
                ? (authorityRecovered ? ("recovery_required" as const) : ("failed" as const))
                : authorityRecovered
                  ? ("recovery_required" as const)
                  : !run
                    ? ("recovery_required" as const)
                    : ("recovery_required" as const)
              const reasonCode = authorityRecovered
                ? (current.terminalReason ?? recoveryReason)
                : !run
              ...
              if (terminalRun)
                yield* tx
                  ...
                  .run()
              const updated = authorityRecovered
                ? { activityID: activity.activityID }
                : yield* tx
                    .update(SessionLegacyActivityTable)
