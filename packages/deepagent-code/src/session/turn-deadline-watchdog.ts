// provider lifecycle gap C (conservative §8.3-3/4): provider pre-dispatch deadline watchdog.
...
export namespace TurnDeadlineWatchdog {
  ...
  const dispatchEvidenceStages = new Set<string>(["request_prepared", "provider_dispatch_started", "terminal_settled"])

  const settleOne = (input: SweepInput, activity: typeof SessionLegacyActivityTable.$inferSelect) =>
    Effect.gen(function* () {
      ...
      const boundary = yield* SessionPromptIntent.freezeProviderInputBoundary(runIdentity).pipe(
        ...
        Effect.catchTag("SessionPromptIntent.Conflict", () => Effect.succeed({ kind: "pending_steer" } as const)),
      )
      if (boundary.kind !== "ready") return { kind: "skipped", activityID: activity.activity_id, reason: boundary.kind }
      ...
      yield* TurnStageEvidence.record(db, {
        sessionID: activity.session_id,
        activityID: activity.activity_id,
        turnID: `watchdog:${activity.activity_id}`,
      ...
      const stale = yield* db
        ...
        .where(
          and(
            ...
            lte(SessionLegacyActivityTable.created_at, now - input.deadlineMs),
          ),
        )
        .all()
