...
// a restart no longer loses the blob: ordinary Responses turns and the next
// compaction read it back with a durable replay boundary (§4.3).
...
export type EncryptedContentRecord = {
  ...
  readonly sourceEndMessageID: string | undefined
...
const hasPerRunStore = (): boolean => {
  if (!boundDb) return false
  return Effect.runSync(
    boundDb
      .get<{ name: string }>(sql`SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'session_compaction_encrypted_blob'`)
      .pipe(Effect.orDie),
  ) !== undefined
}
...
const readRecord = (sessionID: string): EncryptedContentRecord | undefined => {
  ...
  if (hasPerRunStore()) {
    const head = Effect.runSync(
      boundDb
        .select({
          encryptedContent: SessionCompactionEncryptedBlobTable.encrypted_content,
          providerID: SessionCompactionEncryptedBlobTable.provider_id,
          modelID: SessionCompactionEncryptedBlobTable.model_id,
          sourceRunID: SessionCompactionEncryptedBlobTable.source_run_id,
          sourceEndMessageID: SessionCompactionEncryptedBlobTable.source_end_message_id,
          createdAt: SessionCompactionEncryptedBlobTable.created_at,
        })
        .from(SessionCompactionEncryptedHeadTable)
        .innerJoin(
          SessionCompactionEncryptedBlobTable,
          eq(SessionCompactionEncryptedHeadTable.blob_id, SessionCompactionEncryptedBlobTable.blob_id),
        )
        .where(eq(SessionCompactionEncryptedHeadTable.session_id, sessionID))
        .get()
        .pipe(Effect.orDie),
    )
    if (head)
      return {
        encryptedContent: head.encryptedContent,
        providerID: head.providerID,
        modelID: head.modelID ?? undefined,
        sourceRunID: head.sourceRunID ?? undefined,
        sourceEndMessageID: head.sourceEndMessageID ?? undefined,
  ...
  return {
    ...
    sourceEndMessageID: row.source_end_message_id ?? undefined,
...
const writeRecord = (sessionID: string, value: string, attribution: EncryptedContentAttribution | undefined) => {
  ...
  const sourceEndMessageID = attribution?.sourceEndMessageID ?? null
  if (!boundDb) {
    memory.set(sessionID, {
      ...
      sourceEndMessageID: sourceEndMessageID ?? undefined,
      createdAt: now,
      updatedAt: now,
    })
    return
  }
  if (sourceRunID && hasPerRunStore()) {
    const existing = Effect.runSync(
      boundDb
        .select()
        .from(SessionCompactionEncryptedBlobTable)
        .where(eq(SessionCompactionEncryptedBlobTable.blob_id, sourceRunID))
        .get()
        .pipe(Effect.orDie),
    )
    if (
      ...
      (existing.session_id !== sessionID ||
        existing.encrypted_content !== value ||
        existing.provider_id !== providerID ||
        existing.model_id !== modelID ||
        existing.source_end_message_id !== sourceEndMessageID)
    )
      throw new Error(`remote compaction blob ${sourceRunID} is immutable and already contains different content`)
    if (!existing)
      Effect.runSync(
        boundDb
    ...
    Effect.runSync(
      boundDb
        ...
        .values({ session_id: sessionID, blob_id: sourceRunID, updated_at: now })
        .onConflictDoUpdate({
          target: SessionCompactionEncryptedHeadTable.session_id,
          set: { blob_id: sourceRunID, updated_at: now },
        })
        .run()
        .pipe(Effect.orDie),
    )
  ...
  if (!sourceRunID && hasPerRunStore())
    Effect.runSync(
      boundDb
        .delete(SessionCompactionEncryptedHeadTable)
        .where(eq(SessionCompactionEncryptedHeadTable.session_id, sessionID))
        .run()
        .pipe(Effect.orDie),
    )
  // Keep the old row as a compatibility projection. It is not used for
  // committed-run provenance once the per-run migration is installed.
  Effect.runSync(
    boundDb
...
export const resolveRemoteCompactionReplay = (sessionID: string, model: Provider.Model) => {
  const record = EncryptedContentStore.getRecord(sessionID)
  if (!record) return undefined
  if (!supportsRemoteCompaction(model) || record.providerID !== model.providerID || !record.sourceEndMessageID) {
    EncryptedContentStore.clear(sessionID)
    return undefined
  }
  return record
}

export const projectRemoteCompactionReplay = <Message extends { readonly info: { readonly id: string } }>(
  sessionID: string,
  model: Provider.Model,
  messages: readonly Message[],
) => {
  const record = resolveRemoteCompactionReplay(sessionID, model)
  if (!record) return undefined
  const boundary = messages.findIndex((message) => message.info.id === record.sourceEndMessageID)
  if (boundary >= 0) return { record, messages: messages.slice(boundary + 1) }
  EncryptedContentStore.clear(sessionID)
  return undefined
}
...
export type RemoteCompactionInput = {
  ...
  /** Production compaction persists only inside its run commit transaction. */
