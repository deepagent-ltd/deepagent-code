// BUG-407-012 gap C: durable provider-turn stage evidence.
// One row per (session_id, activity_id) records the furthest stage a provider turn
...
export namespace TurnStageEvidence {
  ...
  const upsertSql = (input: RecordInput, now: number) => {
    ...
    return sql`
      INSERT INTO session_turn_stage_evidence (session_id, activity_id, stage, details, stage_at, updated_at)
      VALUES (${input.sessionID}, ${input.activityID}, ${input.stage}, ${details}, ${stageAt}, ${now})
  ...
  export const latest = (
    db: Db,
    input: { readonly sessionID: string; readonly activityID?: string },
  ) =>
    db
      .select()
      ...
      .where(
        input.activityID
          ? and(
              ...
              eq(SessionTurnStageEvidenceTable.activity_id, input.activityID),
            )
          : eq(SessionTurnStageEvidenceTable.session_id, input.sessionID),
      )
  ...
  export const recent = (
    db: Db,
    input: { readonly sessionID: string; readonly activityID?: string; readonly limit?: number },
  ) =>
    db
      .select()
      .from(SessionTurnStageEvidenceTable)
