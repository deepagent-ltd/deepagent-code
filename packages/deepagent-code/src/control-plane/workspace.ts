...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // §16.3 order 5 F2 — session warp contract: placement changes and change-copying require the
    // durable transfer admission machinery (session_transfer_operation admit → source fence →
    // target receipt → activate, order 5 F4). Until that lands, EVERY mutation case fails closed
    // with a TYPED refusal (recovery projection guard first, then copy_changes / placement_change)
    // — there is deliberately no legacy move fallback; exact-placement retries stay zero-write
    // no-ops. Workspace sync itself already runs on V2 tables + a durable sync cursor; GlobalBus
    // carries sync status notifications only, never authority.
