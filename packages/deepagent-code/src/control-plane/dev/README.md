This is a plugin to simulate a remote environment locally. Add this to `.deepagent-code/deepagent-code.jsonc`:

```json
  "plugin": ["../packages/deepagent-code/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate DeepAgent Code server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/deepagent-code/script/run-workspace-server
```

With the plugin install, you can now run DeepAgent Code and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
