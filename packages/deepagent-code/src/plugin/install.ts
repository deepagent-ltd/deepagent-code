...
export type PatchDeps = {
  ...
  files: (dir: string, name: "deepagent-code" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".deepagent-code")
}

function patchName(kind: Kind): "deepagent-code" | "tui" {
  if (kind === "server") return "deepagent-code"
  return "tui"
}
