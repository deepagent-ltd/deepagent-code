...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@deepagent-code/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import * as Log from "@deepagent-code/core/util/log"
import { errorData, errorMessage } from "@deepagent-code/tui/util/error"
import { isRecord } from "@deepagent-code/tui/util/record"
...
import { hasTheme, upsertTheme } from "@deepagent-code/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@deepagent-code/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@deepagent-code/core/v1/config/plugin"
import { createCommandShim } from "@deepagent-code/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@deepagent-code/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".deepagent-code"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".deepagent-code", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".deepagent-code", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.DEEPAGENT_CODE_PURE ? [] : pluginOrigins
    if (Flag.DEEPAGENT_CODE_PURE && pluginOrigins.length) {
