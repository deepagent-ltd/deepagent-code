...
function authHeaders() {
  return {
    ...
    "User-Agent": `deepagent-code/${InstallationVersion}`,
  }
}
...
export function buildAuthorizeUrl(
  ...
  // clients. `referrer=deepagent-code` lets xAI attribute deepagent-code-originated
...
<html>
  <head>
    <title>DeepAgent Code - xAI Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
...
<html>
  <head>
    <title>DeepAgent Code - xAI Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
async function startOAuthServer(): Promise<{ port: number; redirectUri: string }> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server.listen(OAUTH_PORT, OAUTH_HOST, () => {
      ...
      // behavior and crash the entire deepagent-code process. Matches the silent-
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            headers.set("User-Agent", `deepagent-code/${InstallationVersion}`)
