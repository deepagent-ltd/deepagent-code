...
- Author package entrypoint is `@deepagent-code/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/deepagent-code-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@deepagent-code/plugin/tui`.
- `@deepagent-code/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@deepagent-code/plugin/tui"
...
```json
{
  "name": "@acme/deepagent-code-plugin",
  "type": "module",
  "main": "./dist/server.js",
  "exports": {
    "./server": {
...
- The value is a semver range checked against the running DeepAgent Code version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.deepagent-code` is absent, no check is performed (backward compatible).
...
- `deepagent-code plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `deepagent-code plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.deepagent-code` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.deepagent-code`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.deepagent-code`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.deepagent-code` dir or `DEEPAGENT_CODE_CONFIG_DIR`, DeepAgent Code installs `@deepagent-code/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@deepagent-code/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus DeepAgent Code's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
DeepAgent Code registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, DeepAgent Code pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@deepagent-code/plugin/tui`.
...
- `message` is required; `title` defaults to `"deepagent-code"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `deepagent-code.default`.
...
- Local plugins persist installed themes under the local `.deepagent-code/themes` area near the plugin config source.
...
- `--pure` / `DEEPAGENT_CODE_PURE` skips external TUI plugins only.
