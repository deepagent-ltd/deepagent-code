...
Delete `packages/deepagent-code/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/deepagent-code/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/deepagent-code/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/deepagent-code`
- TUI plugin loader tests in `packages/deepagent-code` if runtime plugin API wiring changed
