...
Practical reference for a future `packages/server` split after the deepagent-code
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/deepagent-code`.
...
Until enough shared service code lives outside `packages/deepagent-code`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/deepagent-code`

It should not import `packages/deepagent-code` services while `packages/deepagent-code`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on deepagent-code-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/deepagent-code` implementation details.
