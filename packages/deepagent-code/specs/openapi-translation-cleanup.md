...
Trim `packages/deepagent-code/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/deepagent-code/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/deepagent-code`.
- `[x]` `bun typecheck` from `packages/deepagent-code`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/deepagent-code/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/deepagent-code`.
- `[x]` `bun dev generate > /tmp/deepagent-code-openapi.json` from `packages/deepagent-code`.
...
- Edit `packages/deepagent-code/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/deepagent-code-openapi.json` from `packages/deepagent-code`.
...
- Check source schemas in files like `packages/deepagent-code/src/session/schema.ts`, `packages/deepagent-code/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/deepagent-code/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/deepagent-code/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
