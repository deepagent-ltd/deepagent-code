...
test("public share upload, token download, and revoke", async () => {
  ...
  try {
    ...
    const bytes = await createSessionBundle({ snapshot, tier: "session_logs", redact: false,
      logs: { events: [{ api_key: "top-secret", DATABASE_URL: "postgres://alice:pwd@db.local/prod", headers: { cookie: "sid=public-share-secret" } }], inputs: [], providerTurns: [] } })
    ...
    expect(JSON.stringify(localShared)).not.toContain("sid=nested-share-secret")
    ...
    expect(JSON.stringify(bundle)).not.toContain("sid=nested-share-secret")
