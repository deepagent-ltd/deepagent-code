...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.DEEPAGENT_CODE_WEBSEARCH_PROVIDER
    ...
    try {
      ...
      if (original === undefined) delete process.env.DEEPAGENT_CODE_WEBSEARCH_PROVIDER
      else process.env.DEEPAGENT_CODE_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for deepagent-code or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderV2.ID.make("deepagent-code"), { exa: false, parallel: false })).toBe(true)
