...
describe("ServerAgentExecutor", () => {
  ...
  it("maps the V2-only legacy execution refusal to AGENT_EXECUTION_UNAVAILABLE", async () => {
    const refusal = new LegacyExecutionUnavailable({
      sessionID: "ses_test",
      reason: "v2_only_profile",
      detail: "V2-only profile: legacy execution is closed",
    })
    const result = await run(
      makeLayer({ prompt: () => Effect.fail(refusal as never) }),
    )
    expect(result.success).toBe(false)
    expect(result.timeout).toBe(false)
    expect(result.error?.code).toBe("AGENT_EXECUTION_UNAVAILABLE")
    expect(result.error?.message).toBe("v2_only_profile: V2-only profile: legacy execution is closed")
    expect(result.error?.retryable).toBe(false)
  })
