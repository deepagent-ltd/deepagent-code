...
describe("ContextFederationDiagnostics", () => {
  test("returns opaque evidence and requires durable terminal evidence before settle", async () => {
    ...
    await Effect.runPromise(Effect.gen(function* () {
      ...
      expect(first.metrics.alerts).toContainEqual({ graph: "documents", state: "degraded", reasonCode: "source_error" })
      ...
      const missingEvidence = yield* service.resolveAttempt({
...
function seed(db: Database.Interface["db"]) {
  ...
  return Effect.gen(function* () {
    ...
    yield* db.insert(SessionContextSelectionTable).values({
    ...
    yield* db.insert(SessionProviderAttemptTable).values({
