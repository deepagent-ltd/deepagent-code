...
describe("acp permissions", () => {
  it("sends requestPermission and replies with the selected outcome", async () => {
    ...
    harness.subscription.handlePermission(permissionAsked("ses_a", "perm_1", { tool: { messageID: "msg_1", callID: "call_1" } }))
  ...
  it("rejects non-selected outcomes", async () => {
    ...
    harness.subscription.handlePermission(permissionAsked("ses_a", "perm_cancelled"))
  ...
  it("rejects when requestPermission fails", async () => {
    ...
    harness.subscription.handlePermission(permissionAsked("ses_a", "perm_failed"))
  ...
  it("does not let a blocked session A permission block session B message updates", async () => {
    ...
    harness.subscription.handlePermission(permissionAsked("ses_a", "perm_blocked"))
    await pollUntil(() => harness.requests.length === 1, "blocked permission was never requested")

    await harness.subscription.deliver({
      id: "evt_text_b",
      type: "session.next.text.ended",
      seq: 1,
      data: { sessionID: "ses_b", assistantMessageID: "msg_b", textID: "part_b", text: "session_b_message" },
    })
  ...
  it("serializes permission requests per session", async () => {
    ...
    harness.subscription.handlePermission(permissionAsked("ses_a", "perm_1"))
    harness.subscription.handlePermission(permissionAsked("ses_a", "perm_2"))
  ...
  it("normalizes permission.v2.asked into the ACP vocabulary", async () => {
    const harness = createHarness()
