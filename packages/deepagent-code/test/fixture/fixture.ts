...
export async function tmpdir<T>(options?: TmpDirOptions<T>) {
  ...
  if (options?.git) {
    ...
    await $`git config user.email "test@deepagent-code.test"`.cwd(dirpath).quiet()
    await $`git config user.name "Test"`.cwd(dirpath).quiet()
    await $`git commit --allow-empty -m "root commit ${dirpath}"`.cwd(dirpath).quiet()
  }
  if (options?.config) {
    await Bun.write(
      path.join(dirpath, "deepagent-code.json"),
      JSON.stringify({
...
export function tmpdirScoped<E = never, R = never>(options?: {
  ...
  return Effect.gen(function* () {
    ...
    if (options?.git) {
      ...
      yield* git("config", "user.email", "test@deepagent-code.test")
    ...
    if (options?.config) {
      ...
      yield* Effect.promise(() =>
        fs.writeFile(
          ...
          JSON.stringify({ $schema: "https://ai.deepagent.ltd/config.schema.json", ...resolved }),
        ),
      )
    }
