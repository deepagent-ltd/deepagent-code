...
const githubBase = <A, E, R>(url: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.DEEPAGENT_CODE_REPO_CLONE_GITHUB_BASE_URL
      process.env.DEEPAGENT_CODE_REPO_CLONE_GITHUB_BASE_URL = url
      return previous
    }),
    () => self,
    (previous) =>
      Effect.sync(() => {
        if (previous) process.env.DEEPAGENT_CODE_REPO_CLONE_GITHUB_BASE_URL = previous
        else delete process.env.DEEPAGENT_CODE_REPO_CLONE_GITHUB_BASE_URL
      }),
  )
...
describe("reference", () => {
  it.live("resolves supported local and git config forms", () =>
    Effect.gen(function* () {
      const root = path.resolve("deepagent-code-reference-root")
    ...
    provideTmpdirInstance(
      (_dir) =>
        Effect.gen(function* () {
          ...
          const cache = path.join(Global.Path.repos, "github.com", "deepagent-code-reference-test", "repo")
          ...
          const remoteDir = path.join(remoteRoot, "deepagent-code-reference-test")
        ...
        }),
      {
        config: {
          reference: {
            docs: "deepagent-code-reference-test/repo",
          },
        },
      },
    ),
  ...
  references.live("refreshes configured git references on new instance init", () =>
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const cache = path.join(Global.Path.repos, "github.com", "deepagent-code-reference-refresh", "repo")
      yield* fs.remove(cache, { recursive: true }).pipe(Effect.ignore)
      yield* Effect.addFinalizer(() => fs.remove(cache, { recursive: true }).pipe(Effect.ignore))

      const source = yield* tmpdirScoped({ git: true })
      const remoteRoot = yield* tmpdirScoped()
      const remoteDir = path.join(remoteRoot, "deepagent-code-reference-refresh")
      const remoteRepo = path.join(remoteDir, "repo.git")
      ...
      yield* githubBase(
        ...
        provideTmpdirInstance(
            ...
            }),
          {
            config: {
              reference: {
                docs: "deepagent-code-reference-refresh/repo",
              },
            },
          },
        ),
