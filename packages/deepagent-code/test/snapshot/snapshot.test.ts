...
// event maintenance §14.2 release acceptance: a 4,500-file discovery must trip the candidate budget
...
it.instance(
  "initializes a repository baseline above the candidate budget and keeps unchanged tracking fast",
  Effect.gen(function* () {
    const tmp = yield* TestInstance
    const snapshot = yield* Snapshot.Service
    const trackedFiles = Snapshot.DiffLimits.captureCandidateFiles + 50
    yield* Effect.forEach(
      Array.from({ length: trackedFiles }, (_, index) => index),
      (index) => write(`${tmp.directory}/tracked/file-${String(index).padStart(5, "0")}.txt`, "baseline"),
      { concurrency: 200, discard: true },
    )
    yield* exec(tmp.directory, ["git", "add", "."])
    yield* exec(tmp.directory, ["git", "commit", "-m", "large tracked baseline"])

    const first = yield* snapshot.trackOutcome({ sessionId: "ses_large_baseline" })
    expect(first.degraded).toBeUndefined()
    expect(first.hash).toBeTruthy()
    const started = performance.now()
    const second = yield* snapshot.trackOutcome({ sessionId: "ses_large_baseline" })
