...
const env = Layer.mergeAll(
  Permission.layer.pipe(Layer.provide(database), Layer.provide(events), Layer.provide(RuntimeFlags.defaultLayer)),
  database,
  events,
...
it.instance(
  "askEffect - delegated facade child tools resolve through owner_session_id",
  () =>
    Effect.gen(function* () {
      const { db } = yield* Database.Service
      const project = yield* db.select({ id: ProjectTable.id }).from(ProjectTable).get().pipe(Effect.orDie)
      if (!project) return yield* Effect.die(new Error("test project is missing"))
      const parentSessionID = SessionID.make("ses_facade_permission_parent")
      const childSessionID = SessionID.make("ses_facade_permission_child")
      ...
      yield* db
        ...
        .values({
          activity_id: "facade-permission-runtime",
          subkind: "task",
          parent_session_id: parentSessionID,
          owner_session_id: childSessionID,
          owner_token: "facade-runtime",
          spawn_tool_call_id: "call-activity-start",
          state: "active",
      ...
      const grant = yield* permission.askEffect({
        ...
        metadata: {},
        always: [],
        tool: { messageID: MessageID.make("msg_facade_child"), callID: "call-child-bash" },
        effectToolName: "bash",
        ruleset: [{ permission: "bash", pattern: "*", action: "allow" }],
      })
      expect(grant).toMatchObject({
        activityKind: "facade",
        activityID: "facade-permission-runtime",
        sessionID: childSessionID,
        toolCallID: "call-child-bash",
      })
      expect(
        yield* db
          .select({
            activityKind: SessionActivityPermissionRequestTable.activity_kind,
            activityID: SessionActivityPermissionRequestTable.activity_id,
            sessionID: SessionActivityPermissionRequestTable.session_id,
            toolCallID: SessionActivityPermissionRequestTable.tool_call_id,
          })
          .from(SessionActivityPermissionRequestTable)
          .where(eq(SessionActivityPermissionRequestTable.request_id, grant!.requestID))
          .get()
          .pipe(Effect.orDie),
      ).toEqual({
        activityKind: "facade",
