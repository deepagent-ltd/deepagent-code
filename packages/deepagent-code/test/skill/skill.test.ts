...
const withHome = <A, E, R>(home: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const prev = process.env.DEEPAGENT_CODE_TEST_HOME
    ...
    (prev) =>
      Effect.sync(() => {
        process.env.DEEPAGENT_CODE_TEST_HOME = prev
      }),
  )

describe("skill", () => {
  it.live("discovers skills from .deepagent-code/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".deepagent-code", "skill", "test-skill", "SKILL.md"),
              `---
name: test-skill
      ...
      (dir) =>
        withHome(
          ...
          Effect.gen(function* () {
            yield* Effect.promise(() =>
              Bun.write(
                path.join(dir, ".deepagent-code", "skill", "dir-skill", "SKILL.md"),
                `---
name: dir-skill
...
`,
            ...
            expect(dirs).toContain(path.join(dir, ".deepagent-code", "skill", "dir-skill"))
  ...
  it.live("discovers multiple skills from .deepagent-code/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".deepagent-code", "skill", "skill-one", "SKILL.md"),
                `---
name: skill-one
...
`,
              ...
              Bun.write(
                path.join(dir, ".deepagent-code", "skill", "skill-two", "SKILL.md"),
                `---
name: skill-two
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".deepagent-code", "skill", "no-frontmatter", "SKILL.md"),
              `# No Frontmatter
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".deepagent-code", "skill", "manual-skill", "SKILL.md"),
              `---
name: manual-skill
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".deepagent-code", "skill", "deepagent-code-skill", "SKILL.md"),
                `---
name: deepagent-code-skill
description: A skill in the .deepagent-code/skill directory.
---

# DeepAgent Code Skill
`,
              ),
            ]),
          )
          ...
          const skill = yield* Skill.Service
          const list = (yield* skill.all()).filter((s) => s.location !== "<built-in>")
          expect(list.map((s) => s.name)).toEqual(["deepagent-code-skill"])
        }),
      { git: true },
    ),
  )
...
`,
              ...
              Bun.write(
                path.join(dir, ".deepagent-code", "skill", "agent-skill", "SKILL.md"),
                `---
name: deepagent-code-skill
description: A skill in the .deepagent-code/skill directory.
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".deepagent-code", "skills", "agent-skill", "SKILL.md"),
                `---
name: deepagent-code-skill
description: A skill in the .deepagent-code/skills directory.
---
