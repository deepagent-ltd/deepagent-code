...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "deepagent-code")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "deepagent-code")
      const wrongCache = path.join(parentDir, ".git", "deepagent-code")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "deepagent-code")
      const cacheB = path.join(bareB, "deepagent-code")
      const wrongCache = path.join(parentDir, ".git", "deepagent-code")
