    ...
    it.instance(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(info.branch).toBe(`deepagent-code/${info.name}`)
    ...
    it.instance(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(info.branch).toBe("deepagent-code/my-feature")
        }),
      { git: true },
    )
    ...
    it.instance(
      ...
      () =>
        Effect.gen(function* () {
          ...
          const svc = yield* Worktree.Service
          yield* git(test.directory, ["branch", "deepagent-code/my-feature"])
    ...
    it.instance(
      ...
      () =>
        withCreatedWorktree(undefined, ({ info }) =>
          Effect.gen(function* () {
            expect(info.name).toBeDefined()
            expect(info.branch ?? "").toStartWith("deepagent-code/")
      ...
      () =>
        withCreatedWorktree({ name: "test-workspace" }, ({ info }) =>
          Effect.gen(function* () {
            ...
            expect(info.branch).toBe("deepagent-code/test-workspace")
          }),
        ),
      { git: true },
    )
