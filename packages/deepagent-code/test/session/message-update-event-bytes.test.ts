/**
 * event maintenance §13.3 / REL-002 — a giant user-message summary must not multiply through the
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      // event maintenance §13.3: cumulative event bytes must not multiply with the patch body —
