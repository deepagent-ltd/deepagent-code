...
describe("session.llm-native.request", () => {
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("deepagent-code") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("deepagent-code") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
  ...
  test("prefers stored deepagent-code auth over provider api key", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("deepagent-code") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("deepagent-code"),
  ...
  it.effect("injects remote compaction state through the DeepSeek provider namespace", () =>
    Effect.gen(function* () {
      const prepared = yield* prepareNativeRequest({
        model: deepseekModel,
        apiKey: "test-deepseek-key",
        messages: [{ role: "user", content: "continue" }],
        providerOptions: { deepseek: { compactionEncryptedContent: "enc-deepseek-normal" } },
      })
      expect(prepared.body).toMatchObject({
        input: [
          { type: "compaction", encrypted_content: "enc-deepseek-normal" },
