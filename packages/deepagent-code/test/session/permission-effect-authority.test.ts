...
test("AI SDK abort interrupts a bridged tool before it can orphan its permission effect", async () => {
  const bridge = await Effect.runPromise(EffectBridge.make())
  const startedEffect = await Effect.runPromise(Deferred.make<void>())
  ...
  await Effect.runPromise(Deferred.await(startedEffect))
  controller.abort()
  expect(await running.then(() => false, () => true)).toBe(true)
  expect(rotations).toBe(1)
  expect(settlements).toBe(0)
})

test("an already-aborted AI SDK signal prevents bridged tool execution", async () => {
  const bridge = await Effect.runPromise(EffectBridge.make())
  const controller = new AbortController()
  let executions = 0
  controller.abort()
