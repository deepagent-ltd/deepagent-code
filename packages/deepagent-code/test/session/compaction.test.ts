...
describe("session.compaction.process", () => {
  ...
  it.instance(
    "records a durable resolution when continuation recovery fails closed",
    Effect.gen(function* () {
      const ssn = yield* SessionNs.Service
      const compact = yield* SessionCompaction.Service
      const session = yield* ssn.create({})
      const continuationMessage = yield* createUserMessage(session.id, "continue")
      const sourceAuthority = yield* MessageV2.promptHistoryProjectionEffect(session.id)
      expect(sourceAuthority.worldStateBaselineHash).toBeUndefined()
      const { db } = yield* Database.Service
      const now = Date.now()
      yield* db
        ...
        .values({
          session_id: session.id,
          encrypted_content: "continuation-test-blob",
          provider_id: "test",
          model_id: "test-model",
          source_run_id: "run-continuation-fail-closed",
          created_at: now,
          updated_at: now,
        })
        .run()
      ...
      yield* db
        ...
        .values({
          blob_id: "run-continuation-fail-closed",
          session_id: session.id,
          encrypted_content: "continuation-test-blob",
          provider_id: "test",
          model_id: "test-model",
      ...
      yield* db
        ...
        .values({ session_id: session.id, blob_id: "run-continuation-fail-closed", updated_at: now })
        .run()
        .pipe(Effect.orDie)
      yield* db
        .insert(CompactionRunTable)
        .values({
          run_id: "run-continuation-fail-closed",
          session_id: session.id,
          from_prompt_epoch: 0,
          trigger: "manual",
          state: "requested",
          created_at: now,
          source_window_id: "window-continuation-test",
          source_effective_history_hash: "hash-continuation-test",
          source_message_count: 1,
          source_projection_version: 1,
          continuation_state: "pending",
          compaction_mode: "remote_compact",
          encrypted_content_session: session.id,
          encrypted_content_blob_id: "run-continuation-fail-closed",
          remote_provider_id: "test",
      ...
      yield* db
        .update(CompactionRunTable)
        .set({ state: "summarizing" })
        .where(eq(CompactionRunTable.run_id, "run-continuation-fail-closed"))
        .run()
        .pipe(Effect.orDie)
      yield* db
        .update(CompactionRunTable)
      ...
      yield* db
        ...
        .values({
          artifact_id: "artifact-continuation-fail-closed",
          run_id: "run-continuation-fail-closed",
          session_id: session.id,
          message_id: continuationMessage.id,
          part_id: null,
          kind: "continue",
          state: "committed",
          created_at: now,
      ...
      yield* compact.failContinuationClosed({ runID: "run-continuation-fail-closed", reason: "test_recovery_gap" })

      const run = yield* db
        .select({
          continuationState: CompactionRunTable.continuation_state,
          errorCode: CompactionRunTable.continuation_error_code,
          terminalAt: CompactionRunTable.continuation_terminal_at,
        })
        .from(CompactionRunTable)
        .where(eq(CompactionRunTable.run_id, "run-continuation-fail-closed"))
        .get()
        .pipe(Effect.orDie)
      expect(run).toMatchObject({ continuationState: "failed", errorCode: "test_recovery_gap" })
      ...
      expect(
        yield* db
          ...
          .where(eq(CompactionContinuationFailureTable.run_id, "run-continuation-fail-closed"))
          .get()
          .pipe(Effect.orDie),
      ).toEqual({ sourceState: "pending", reason: "test_recovery_gap" })
      expect(
        yield* db
          ...
          .get()
          .pipe(Effect.orDie),
      ).toEqual({ state: "recovery_required", reason: "test_recovery_gap" })

      const pending = yield* compact.describeContinuationResolutions(session.id)
      expect(pending).toHaveLength(1)
      expect(pending[0]).toMatchObject({
        runID: "run-continuation-fail-closed",
        sourceState: "pending",
      ...
      expect(
        yield* compact
          .resolveContinuation({
            sessionID: session.id,
            runID: "run-continuation-fail-closed",
            failureID: pending[0]!.failureID,
            commandID: "command-continuation-abandon-without-risk",
            decision: "abandoned",
            reason: "operator chose to skip the continuation",
            actorID: "test-operator",
          })
          .pipe(Effect.flip),
      ).toMatchObject({ code: "risk_not_acknowledged" })
      const replayed = yield* compact.resolveContinuation({
        sessionID: session.id,
        runID: "run-continuation-fail-closed",
        failureID: pending[0]!.failureID,
        commandID: "command-continuation-replay",
        decision: "replay",
        reason: "operator confirmed provider was not dispatched",
        actorID: "test-operator",
      })
      expect(replayed).toMatchObject({ decision: "replay", shouldReplay: true })
      expect(
        yield* db
      ...
      expect(
        yield* db
          .select({
            decision: CompactionContinuationResolutionTable.decision,
            actorID: CompactionContinuationResolutionTable.actor_id,
            successorPromptEpoch: CompactionContinuationResolutionTable.successor_prompt_epoch,
          })
          .from(CompactionContinuationResolutionTable)
          .where(eq(CompactionContinuationResolutionTable.failure_id, pending[0]!.failureID))
          .get()
          .pipe(Effect.orDie),
      ).toEqual({ decision: "replay", actorID: "test-operator", successorPromptEpoch: 1 })
      expect(
        yield* db
          .select({
          ...
          .get()
          .pipe(Effect.orDie),
      ).toMatchObject({ epoch: 1, baselineHash: expect.any(String) })
      expect(
        yield* db
          ...
          .where(
            and(
              eq(SessionWorldStateBaselineTable.session_id, session.id),
              eq(SessionWorldStateBaselineTable.prompt_epoch, 1),
            ),
          )
          .all()
          .pipe(Effect.orDie),
      ).toSatisfy((rows) => rows.length > 0 && rows.every((row) => row.provenance === "recovery_recollected"))
      const recoveredAuthority = yield* MessageV2.promptHistoryProjectionEffect(session.id)
      expect(recoveredAuthority).toMatchObject({
        epoch: 1,
        orderedMessageIDs: [continuationMessage.id],
        recoveryResolutionID: replayed.resolutionID,
        worldStateBaselineHash: expect.any(String),
      })
      expect(yield* MessageV2.promptWorldStateProjectionEffect(session.id)).toMatchObject({
        hash: recoveredAuthority.worldStateBaselineHash,
      })
      const exactRetry = yield* compact.resolveContinuation({
        sessionID: session.id,
        runID: "run-continuation-fail-closed",
        failureID: pending[0]!.failureID,
        commandID: "command-continuation-replay",
      ...
      expect(exactRetry).toMatchObject({ resolutionID: replayed.resolutionID, shouldReplay: false })
      expect(
        yield* compact
          .resolveContinuation({
            sessionID: session.id,
          ...
          .pipe(Effect.flip),
      ).toMatchObject({ code: "command_id_conflict" })
      expect(
        yield* compact
          .resolveContinuation({
            sessionID: session.id,
      ...
      yield* compact.failContinuationClosed({
        runID: "run-continuation-fail-closed",
        reason: "second_pre_dispatch_restart",
      })
      const secondFailure = yield* compact.describeContinuationResolutions(session.id)
      expect(secondFailure).toHaveLength(1)
      expect(secondFailure[0]).toMatchObject({ sourceState: "pending", replaySupported: true })
      const abandoned = yield* compact.resolveContinuation({
        sessionID: session.id,
        runID: "run-continuation-fail-closed",
        failureID: secondFailure[0]!.failureID,
        commandID: "command-continuation-abandon",
        decision: "abandoned",
        reason: "operator accepted skipping the synthetic continuation",
        riskAcknowledged: true,
        actorID: "test-operator",
      })
      expect(abandoned).toMatchObject({ decision: "abandoned", shouldReplay: false })
      expect(
        yield* db
          .select({
            decision: CompactionContinuationResolutionTable.decision,
            riskAcknowledged: CompactionContinuationResolutionTable.risk_acknowledged,
            successorPromptEpoch: CompactionContinuationResolutionTable.successor_prompt_epoch,
          })
          .from(CompactionContinuationResolutionTable)
          .where(eq(CompactionContinuationResolutionTable.failure_id, secondFailure[0]!.failureID))
          .get()
          .pipe(Effect.orDie),
      ).toEqual({ decision: "abandoned", riskAcknowledged: true, successorPromptEpoch: 2 })
      expect(yield* MessageV2.promptHistoryProjectionEffect(session.id)).toMatchObject({
        epoch: 2,
        orderedMessageIDs: [],
        recoveryResolutionID: abandoned.resolutionID,
      })
      expect(
        yield* db
          .select({ provenance: SessionWorldStateBaselineTable.provenance })
          ...
          .where(
            and(
              eq(SessionWorldStateBaselineTable.session_id, session.id),
              eq(SessionWorldStateBaselineTable.prompt_epoch, 2),
            ),
          )
          .all()
          .pipe(Effect.orDie),
      ).toSatisfy((rows) => rows.length > 0 && rows.every((row) => row.provenance === "recovery_copied"))
    }),
  )
...
describe("session.compaction.stripSummaryAnalysis", () => {
  ...
  test("collapses blank lines left behind by stripping", () => {
    const out = SessionCompaction.stripSummaryAnalysis("intro\n\n\n<analysis>x</analysis>\n\n\n<summary>kept</summary>")
...
describe("session.compaction.process remote mode (UPD-005 Gap 2)", () => {
  ...
  itCompaction.instance(
    ...
    Effect.gen(function* () {
      ...
      // Explicit application remains an idempotent fixture guard.
      ...
      const result = yield* SessionCompaction.use
        ...
        .pipe(
          withCompaction({
            ...
            executor: compactExecutor("enc-remote-commit"),
      ...
      expect(blob?.source_end_message_id).toBe(msg.id)
  ...
  itCompaction.instance(
    "continues an automatic remote compaction instead of swallowing the active prompt",
    Effect.gen(function* () {
      const db = (yield* Database.Service).db
      const ssn = yield* SessionNs.Service
      const session = yield* ssn.create({})
      const msg = yield* createRemoteUserMessage(session.id, "answer this after compacting")
      const result = yield* SessionCompaction.use
        .process({
          parentID: msg.id,
          messages: yield* ssn.messages({ sessionID: session.id }),
        ...
        .pipe(
          withCompaction({
            provider: remoteProvider,
            executor: compactExecutor("enc-auto-continue"),
            flags: { experimentalNativeLlm: true },
          }),
        )
      ...
      expect(
        yield* db
          .select({ boundary: SessionCompactionEncryptedContentTable.source_end_message_id })
          .from(SessionCompactionEncryptedContentTable)
          .where(eq(SessionCompactionEncryptedContentTable.session_id, session.id))
          .get(),
      ).toEqual({ boundary: msg.id })
    }),
  )

  itCompaction.instance(
    "feeds only post-boundary messages into the next remote compaction",
    Effect.gen(function* () {
      const ssn = yield* SessionNs.Service
      const session = yield* ssn.create({})
        ...
        RequestExecutor.Service.of({
          execute: (request) =>
            Effect.gen(function* () {
              ...
              bodies.push((yield* Effect.promise(() => web.json())) as Record<string, unknown>)
              ordinal++
              return HttpClientResponse.fromWeb(
                request,
                new Response(
                  JSON.stringify({
                    output: [{ type: "compaction", id: `cmp_${ordinal}`, encrypted_content: `enc-${ordinal}` }],
                  }),
                  { headers: { "content-type": "application/json" } },
                ),
              )
      ...
      const first = yield* createRemoteUserMessage(session.id, "first compacted turn")
      expect(
        yield* SessionCompaction.use
          ...
          .pipe(withCompaction({ provider: remoteProvider, executor, flags: { experimentalNativeLlm: true } })),
      ).toBe("stop")

      const second = yield* createRemoteUserMessage(session.id, "second incremental turn")
      expect(
        yield* SessionCompaction.use
          .process({
            parentID: second.id,
      ...
      expect(JSON.stringify(secondInput)).toContain("second incremental turn")
      expect(JSON.stringify(secondInput)).not.toContain("first compacted turn")
    }),
  )
})
  ...
  test("exempts committed remote runs that carry a blob pointer", () => {
    expect(
      SessionCompaction.remoteCompactionInformationHoleExempt({
        ...
        encrypted_content_blob_id: "run_remote",
      }),
    ).toBe(true)
  })
  ...
  test("keeps every other shape subject to the information-hole check", () => {
    ...
    expect(
      SessionCompaction.remoteCompactionInformationHoleExempt({
        ...
        encrypted_content_session: null,
        encrypted_content_blob_id: null,
      }),
    ).toBe(false)
    expect(
      SessionCompaction.remoteCompactionInformationHoleExempt({
