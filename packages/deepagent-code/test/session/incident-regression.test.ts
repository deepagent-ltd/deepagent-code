/**
 * QUAL-002 incident regression suite for the BUG-407 family.
 *
 * BUG-407-010 (receipt startup recovery shape): a provider turn that reached
 ...
 * idempotent.
 *
 * BUG-407-011 (activity terminal authority shape): rejecting the pending
 ...
 *
 * BUG-407-009 (abandoned resolution shape): an indeterminate receipt resolved
...
// ---------------------------------------------------------------------------
// BUG-407-010 shape: receipt startup recovery (crash after dispatch/stream)
// ---------------------------------------------------------------------------
...
function seedCrashFixture(db: Database.Interface["db"]) {
  return Effect.gen(function* () {
    ...
    yield* db
      ...
      .run()
    yield* db
      .insert(SessionActivityTable) // fixture-exempt: seeds active activity for BUG-407-010 crash fixture
      .values({
        activity_id: crashActivityId,
        session_id: crashSessionId,
        ordinal: 0,
...
describe("BUG-407-010 incident regression: provider receipt startup recovery", () => {
...
// ---------------------------------------------------------------------------
// BUG-407-011 shape: question rejection terminalizes the legacy activity and
...
incidentPrompt.instance(
  "BUG-407-011: question dismissal terminalizes the activity authority and unblocks the next prompt admission",
  () =>
    Effect.gen(function* () {
      ...
      const chat = yield* sessions.create({
        title: "BUG-407-011 question dismissal",
...
const seedIndeterminateProviderTurn = Effect.fn("IncidentRegression.seedIndeterminateProviderTurn")(
  function* (input: {
    ...
    yield* db
      ...
      .pipe(Effect.orDie)
    yield* db
      .insert(SessionActivityTable) // fixture-exempt: seeds active activity for BUG-407-009 abandoned-resolution fixture
      .values({
    ...
    yield* db
      ...
      .pipe(Effect.orDie)
    yield* db
      .insert(SessionProviderAttemptTable) // fixture-exempt: seeds prepared attempt with stale owner for BUG-407-009 crash fixture
      .values({
      ...
      .pipe(Effect.orDie)
    yield* db
      .insert(SessionToolRequestReceiptTable) // fixture-exempt: seeds preparing receipt for BUG-407-009 crash fixture
      .values({
...
resolution.instance(
  "BUG-407-009: abandoned resolution unblocks the session, permits cutoff forks, and is idempotent",
  () =>
    Effect.gen(function* () {
      const sessions = yield* Session.Service
