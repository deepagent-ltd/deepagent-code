...
describe("remote compaction encrypted_content staging", () => {
  ...
  test("projects only messages after the durable replay boundary", () => {
    const sessionID = "ses_replay_projection"
    RemoteCompact.EncryptedContentStore.set(sessionID, "enc-boundary", {
      providerID: "openai",
      sourceEndMessageID: "msg_old",
    })
    const projected = RemoteCompact.projectRemoteCompactionReplay(sessionID, openaiModel, [
      { info: { id: "msg_old" }, value: "compacted" },
      { info: { id: "msg_new" }, value: "replay" },
    ])
    expect(projected?.record.encryptedContent).toBe("enc-boundary")
    expect(projected?.messages).toEqual([{ info: { id: "msg_new" }, value: "replay" }])
    RemoteCompact.EncryptedContentStore.clear(sessionID)
  })

  test("clears an unusable replay when its boundary is missing", () => {
    const sessionID = "ses_missing_boundary"
    RemoteCompact.EncryptedContentStore.set(sessionID, "enc-stale", {
      providerID: "openai",
      sourceEndMessageID: "msg_missing",
    })
    expect(RemoteCompact.projectRemoteCompactionReplay(sessionID, openaiModel, [{ info: { id: "msg_new" } }])).toBeUndefined()
...
// Gap 1 durability: with a bound Database handle the blob and replay boundary
// survive a process restart (fresh Database instance over the same file).
describe("remote compaction encrypted_content durability (Gap 1)", () => {
  ...
  it.instance(
    "does not replay the legacy singleton when the per-run head is missing",
    Effect.gen(function* () {
      const test = yield* TestInstance
      const file = join(test.directory, "remote-compact-missing-head.db")
      yield* Effect.addFinalizer(() => Effect.sync(() => RemoteCompact.EncryptedContentStore.bind(undefined)))

      yield* Effect.gen(function* () {
        const { db } = yield* Database.Service
        yield* DatabaseMigration.apply(db)
        yield* seedSession(db, "ses_missing_head")
        yield* db.run(`
          INSERT INTO session_compaction_encrypted_content (
            session_id, encrypted_content, provider_id, created_at, updated_at
          ) VALUES ('ses_missing_head', 'legacy-fail-open', 'openai', 1, 1)
        `)
        RemoteCompact.EncryptedContentStore.bind(db)
        expect(RemoteCompact.EncryptedContentStore.get("ses_missing_head")).toBeUndefined()
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        RemoteCompact.EncryptedContentStore.set(sessionID, "enc-durable", {
          ...
          sourceEndMessageID: "msg-boundary-1",
        })
      ...
      yield* Effect.gen(function* () {
        ...
        expect(record?.sourceEndMessageID).toBe("msg-boundary-1")

        // A second immutable run advances the session head while preserving the
        // earlier run blob as durable provenance.
        RemoteCompact.EncryptedContentStore.set(sessionID, "enc-durable-2", {
          providerID: "openai",
          sourceRunID: "run-persist-2",
        })
