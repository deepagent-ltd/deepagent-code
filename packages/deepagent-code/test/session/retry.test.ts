...
describe("session.retry.retryable", () => {
  ...
  test("maps free limits to retry message", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
    ...
    expect(SessionRetry.retryable(error, "deepagent-code")).toEqual({ message: "Free usage exceeded" })
  })

  test("maps subscription limits to retry message", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
    ...
    expect(SessionRetry.retryable(error, "deepagent")).toEqual({
      message:
  ...
  test("maps subscription limits without limit metadata", () => {
    ...
    expect(SessionRetry.retryable(error, "deepagent")?.message).toBe(
