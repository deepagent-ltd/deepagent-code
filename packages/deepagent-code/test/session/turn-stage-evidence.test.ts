...
describe("TurnStageEvidence", () => {
  it.effect("records the provider-turn stage sequence forward-only on a state-machine driven activity", () =>
    Effect.gen(function* () {
      ...
      // One row per (session_id, activity_id, turn_id), holding the furthest stage.
  ...
  it.effect("keeps stage evidence for separate provider turns on one activity", () =>
    Effect.gen(function* () {
      yield* setup
      const { db } = yield* Database.Service
      const { run } = yield* liveActivity("intent_stage_turn_identity")
      ...
      yield* TurnStageEvidence.record(db, {
        sessionID,
        activityID: run.activityID,
        turnID: "provider-turn-b",
        stage: "snapshot_started",
        now: 201,
      })

      const rows = yield* TurnStageEvidence.recent(db, { sessionID, activityID: run.activityID })
      expect(rows.map((row) => row.turn_id).sort()).toEqual(["provider-turn-a", "provider-turn-b"])
      expect(
        yield* TurnStageEvidence.latest(db, {
          sessionID,
          activityID: run.activityID,
          turnID: "provider-turn-a",
        }),
      ).toMatchObject({ turn_id: "provider-turn-a", stage: "provider_dispatch_started" })
    }),
  )
...
describe("TurnDeadlineWatchdog", () => {
  ...
  it.effect("flag ON: fails a stale activity without dispatch evidence through the existing terminal path", () =>
    Effect.gen(function* () {
      ...
      expect(yield* TurnStageEvidence.latest(db, { sessionID, activityID: run.activityID })).toMatchObject({
        stage: "terminal_settled",
