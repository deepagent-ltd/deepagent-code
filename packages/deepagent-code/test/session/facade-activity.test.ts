...
const fakeGoals = {
  ...
  status: () =>
    Effect.succeed({
      goalId: "goal_1",
      planDocId: "plan_1",
      phase: goalFakeState.phase,
      running: goalFakeState.phase === "running",
    }),
...
const panelJobs = new Map<string, { status: string; output?: string }>()

const fakeBackground = {
  get: (id: string) => Effect.succeed(panelJobs.has(id) ? ({ id, ...panelJobs.get(id)! } as any) : undefined),
  cancel: (id: string) =>
    Effect.sync(() => {
...
describe("facade-activity.goal delegation", () => {
  ...
  itGoal.instance("status/result stay bounded while active; terminal phase converges via settle", () =>
    Effect.gen(function* () {
      ...
      expect((yield* facade.control({ sessionID: parentSessionID, subkind: "goal", action: "pause" })).applied).toBe(
        true,
      )
      expect((yield* facade.control({ sessionID: parentSessionID, subkind: "goal", action: "resume" })).applied).toBe(
        true,
      )
      ...
      expect((yield* facade.control({ sessionID: parentSessionID, subkind: "goal", action: "stop" })).applied).toBe(
        true,
      )
...
describe("facade-activity.runner honesty (no fabricated runners)", () => {
  it.instance(
    "goal start without GoalManager settles the admitted row as recovery_required (no lingering active)",
    () =>
      Effect.gen(function* () {
        yield* setup
        const facade = yield* FacadeActivity.Service
        const error = yield* facade
          .start({ sessionID: parentSessionID, subkind: "goal", objective: "x" })
          .pipe(Effect.flip)
        ...
        // …and the recovery row must NOT block a retry (fail-closed applies to ACTIVE rows only).
        const retry = yield* facade
          .start({ sessionID: parentSessionID, subkind: "goal", objective: "y" })
          .pipe(Effect.flip)
  ...
  it.instance("empty objective is rejected before any row is written", () =>
    Effect.gen(function* () {
      ...
      const facade = yield* FacadeActivity.Service
      const error = yield* facade
        .start({ sessionID: parentSessionID, subkind: "goal", objective: "   " })
        .pipe(Effect.flip)
  ...
  it.instance("panel start refuses without its runner dependencies", () =>
    Effect.gen(function* () {
      ...
      const facade = yield* FacadeActivity.Service
      const error = yield* facade
        .start({ sessionID: parentSessionID, subkind: "panel", objective: "x" })
        .pipe(Effect.flip)
      expect(error).toBeInstanceOf(FacadeActivityRunnerUnavailable)
    }),
  )
...
describe("facade-activity recovery", () => {
  it.instance("startup recovers an active row owned by a previous process", () =>
    Effect.gen(function* () {
      yield* setup
      const { db } = yield* Database.Service
      const activityID = "job_crashed_owner"
      yield* db
        .insert(SessionFacadeActivityTable)
        .values({
      ...
      expect((yield* facade.result({ sessionID: parentSessionID, subkind: "goal" })).terminal).toBe(false)
      const control = yield* facade.control({ sessionID: parentSessionID, subkind: "goal", action: "stop" })
      expect(control.applied).toBe(false)
      expect(control.detail).toContain("explicit recovery")
      const retry = yield* facade
        .start({ sessionID: parentSessionID, subkind: "goal", objective: "retry" })
        .pipe(Effect.flip)
      expect(retry).toBeInstanceOf(FacadeActivityRunnerUnavailable)
...
describe("facade-activity.panel result bound", () => {
  itPanelBound.instance("valid JSON verdicts are returned as bounded previews when oversized", () =>
    Effect.gen(function* () {
      yield* setup
      const { db } = yield* Database.Service
      const activityID = "job_panel_large_verdict"
      yield* db
        .insert(SessionFacadeActivityTable)
        .values({
          activity_id: activityID,
          subkind: "panel",
          parent_session_id: parentSessionID,
          objective_text: "bounded panel",
          budget_json: clampFacadeBudget(),
          state: "settled",
          reason_code: "panel_approve",
          source: "activity_facade",
          created_at: Date.now(),
          settled_at: Date.now(),
          mutation_epoch: 1,
        ...
        .run()
        .pipe(Effect.orDie)
      const output = JSON.stringify({ decision: "approve", findings: [{ summary: "x".repeat(200) }] })
      panelJobs.set(activityID, { status: "completed", output })
      const facade = yield* FacadeActivity.Service
      const projection = yield* facade.result({ sessionID: parentSessionID, subkind: "panel" })
      expect(projection.terminal).toBe(true)
      expect(projection.truncated).toBe(true)
      expect(typeof projection.verdict).toBe("string")
      expect(String(projection.verdict).length).toBeGreaterThan(32)
      expect(String(projection.verdict)).toContain("[truncated]")
      panelJobs.clear()
    }),
  )
})
