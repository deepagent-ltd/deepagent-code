/**
 * BUG-407-008 §7.1 — runtime & read-projection deterministic regressions (#1-#17, #5 lives in
...
const mcp = Layer.succeed(
  ...
  MCP.Service.of({
    ...
    startAuth: () => Effect.die("unexpected MCP auth in bug-407-008 runtime tests"),
    authenticate: () => Effect.die("unexpected MCP auth in bug-407-008 runtime tests"),
    finishAuth: () => Effect.die("unexpected MCP auth in bug-407-008 runtime tests"),
...
const debugStubDie = <A>(): Effect.Effect<A, never, never> =>
  Effect.die("DebugService stub (not used in bug-407-008 runtime tests)")
...
// ---------------------------------------------------------------------------
it.instance(
  "BUG-407-008 #1: reasoning+tool turn settles only the progress authority with NULL text_part_id",
  () =>
    Effect.gen(function* () {
      ...
      yield* llm.push(
        reply()
          ...
          .tool("bash", { command: "echo bug-407-008", description: "emit marker", workdir: path.resolve(dir) }),
      )
...
// ---------------------------------------------------------------------------
it.instance(
  "BUG-407-008 #2: provisional projection appears before provider dispatch without waiting for parts",
  () =>
    Effect.gen(function* () {
      const { dir, llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
      ...
      const marker = path.join(dir, "bug407008-p2-dispatch.json")
...
// ---------------------------------------------------------------------------
it.instance(
  "BUG-407-008 #3: crash between beginProgress commit and provisional event still hydrates provisional",
  () =>
    Effect.gen(function* () {
      const { dir, llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
      ...
      const marker = path.join(dir, "bug407008-p1.json")
...
// ---------------------------------------------------------------------------
it.instance(
  "BUG-407-008 #4: recovery monotonically upgrades the provisional revision after a P2 crash",
  () =>
    Effect.gen(function* () {
      const { dir, llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
      ...
      const marker = path.join(dir, "bug407008-p2.json")
      yield* llm.text("must never be dispatched")
      const baselineCalls = yield* llm.calls
      ...
      yield* crashScoped({ dir, point: "after_provisional_event_before_provider_dispatch", marker },
        Effect.gen(function* () {
          ...
          const recovered = yield* SessionPromptIntent.recoverActiveActivities("bug407008-recovery-owner", {
...
// ---------------------------------------------------------------------------
it.instance(
  "BUG-407-008 #6: zero-part interrupted assistant projects the terminal state over old progress",
  () =>
    Effect.gen(function* () {
      const { dir, llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
      ...
      const marker = path.join(dir, "bug407008-zero-part.json")
...
const makeSteerPlaceholder = Effect.fn("test.makeSteerPlaceholder")(function* (
  ...
  const marker = path.join(dir, `bug407008-placeholder-${Math.random().toString(36).slice(2)}.json`)
  return yield* crashScoped(
    ...
    Effect.gen(function* () {
      ...
      yield* steer.admit({ sessionID, prompt: new Prompt({ text: "bug-407-008 steer placeholder" }) })
...
// ---------------------------------------------------------------------------
it.instance(
  "BUG-407-008 #7: recovery_required projects over the settled progress row without fabricating authority rows",
  () =>
    Effect.gen(function* () {
      const { dir, llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
      ...
      const marker = path.join(dir, "bug407008-p7.json")
      ...
      const running = yield* crashScoped({ dir, point: "after_progress_settle_before_next_admission", marker },
        Effect.gen(function* () {
          ...
          const recovered = yield* SessionPromptIntent.recoverActiveActivities("bug407008-p7-owner", {
            includeCurrentOwner: true,
            sessionID: session.id,
            source: "same_process_recovery",
          }).pipe(Effect.provideService(Database.Service, database))
...
// ---------------------------------------------------------------------------
it.instance(
  "BUG-407-008 #8: abort after settlement terminalizes with the old revision without touching the progress row",
  () =>
    Effect.gen(function* () {
      const { dir, llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
      ...
      const marker = path.join(dir, "bug407008-p8.json")
...
// ---------------------------------------------------------------------------
it.instance(
  "BUG-407-008 #9: terminalizing a pre-admission activity fabricates no projection, message or revision",
  () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* SessionPromptIntent.recoverActiveActivities("bug407008-p9-owner", {
          includeCurrentOwner: true,
          sessionID: session.id,
          source: "same_process_recovery",
        }).pipe(Effect.provideService(Database.Service, database)),
...
// ---------------------------------------------------------------------------
it.instance(
  "BUG-407-008 #10: updateMessage strips a stale computed activityProgress marker before publishing",
  () =>
    Effect.gen(function* () {
      const { dir, llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
...
// ---------------------------------------------------------------------------
it.instance(
  "BUG-407-008 #11: projector persists message JSON without the computed marker emitted by the publisher",
  () =>
    Effect.gen(function* () {
      const { dir, llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
...
// ---------------------------------------------------------------------------
it.instance(
  "BUG-407-008 #12: fork delivery propagates only the stripped canonical projection, never caller markers",
  () =>
    Effect.gen(function* () {
      const { dir, llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
      ...
      const target = yield* sessions.fork({ sessionID: session.id, intentID: "fork-bug407008-marker-strip" })
...
// ---------------------------------------------------------------------------
it.instance(
  "BUG-407-008 #13: late marker-less updates never regress the canonical computed marker",
  () =>
    Effect.gen(function* () {
      const { dir, llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
...
// ---------------------------------------------------------------------------
it.instance(
  "BUG-407-008 #14: message pages are batch-hydrated (no N+1) and correct across empty/mixed/cross-activity pages",
  () =>
    Effect.gen(function* () {
      const { dir, llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
...
// ---------------------------------------------------------------------------
it.instance(
  "BUG-407-008 #15: get and all single-message read paths agree on the computed projection",
  () =>
    Effect.gen(function* () {
      const { dir, llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
...
test("BUG-407-008 #16: incident DB copy opens read-only without migration and yields revision 8/9/10 projections", () => {
...
// ---------------------------------------------------------------------------
it.instance(
  "BUG-407-008 #17: settlement/interrupt/recovery/retiral callers publish once with progress, never without",
  () =>
    Effect.gen(function* () {
      const { dir, llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
      ...
      const interruptMarker = path.join(dir, "bug407008-p17-interrupt.json")
      yield* llm.push(
        reply()
      ...
      expect(
        yield* SessionPromptIntent.recoverActiveActivities("bug407008-p17-owner", {
          includeCurrentOwner: true,
