...
describe("SessionPromptIntent", () => {
  ...
  it.effect("same-process recovery converges a permission-quarantined activity through run and terminal authority", () =>
    Effect.gen(function* () {
      yield* setup
      const first = yield* claim({
      ...
      expect(
        yield* DeepAgentActivityAuthority.recoverPermissionEffects("prompt-recovery-new-owner").pipe(
          Effect.provideService(Database.Service, database),
        ),
      ).toBe(1)

      const recoveryOptions = { includeCurrentOwner: true, source: "same_process_recovery" as const }
      expect(yield* SessionPromptIntent.recoverActiveActivities(first.receipt.ownerToken, recoveryOptions)).toEqual([
        { activityID: activity.activityID, sessionID },
      ])
      const { db } = database
      expect(
        yield* db
      ...
      expect(
        yield* db
          .select({ state: SessionLegacyActivityRunTable.state })
          .from(SessionLegacyActivityRunTable)
      ...
      expect(
        yield* db
          .select({ state: SessionLegacyActivityTerminalTable.state, source: SessionLegacyActivityTerminalTable.source })
          .from(SessionLegacyActivityTerminalTable)
          .where(eq(SessionLegacyActivityTerminalTable.activity_id, activity.activityID))
          .get()
          .pipe(Effect.orDie),
      ).toEqual({ state: "recovery_required", source: "same_process_recovery" })
      expect(yield* SessionPromptIntent.recoverActiveActivities(first.receipt.ownerToken, recoveryOptions)).toEqual([])
    }),
  )
