...
import { LLMEvent, LLMResponse } from "@deepagent-code/llm"
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.DEEPAGENT_CODE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    canRecord: () => Boolean(envValue("DEEPAGENT_CODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("DEEPAGENT_CODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  {
    id: "deepagent-code-proxy",
    name: "DeepAgent Code proxy",
    ...
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.make("deepagent-code"),
        name: "DeepAgent Code Zen",
        env: ["DEEPAGENT_CODE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.DEEPAGENT_CODE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.DEEPAGENT_CODE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.DEEPAGENT_CODE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    config: (model) =>
      providerConfig({
        ...
        model,
        options: {
          apiKey: envValue("DEEPAGENT_CODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("DEEPAGENT_CODE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set DEEPAGENT_CODE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
