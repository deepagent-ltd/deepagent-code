...
import { createTuiAttention } from "@deepagent-code/tui/attention"
import type { TuiConfig } from "@deepagent-code/tui/config"
...
describe("createTuiAttention", () => {
  ...
  test("supports focused-only requests", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "deepagent-code", message: "focused" }])
  })
  ...
  test("notification can deliver while focused when requested", async () => {
    ...
    expect(audio.playCalls).toBe(1)
    expect(renderer.notifications).toEqual([{ title: "deepagent-code", message: "hello" }])
  })
  ...
  test("notifies while blurred", async () => {
    ...
    expect(await attention.notify({ title: "deepagentCode", message: "hello", sound: false })).toEqual({
      ok: true,
      notification: true,
      sound: false,
    })
    expect(renderer.notifications).toEqual([{ title: "deepagentCode", message: "hello" }])
  })
  ...
  test("can play sound always while notification is blurred-only", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "deepagent-code", message: "hello again" }])
  })
  ...
  test("plays named sounds from the active sound pack", async () => {
    ...
    dispose()
    expect(attention.soundboard.current()).toBe("deepagent-code.default")
  })
