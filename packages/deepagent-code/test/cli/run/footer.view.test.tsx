...
import { OpencodeKeymapProvider, registerOpencodeKeymap } from "@deepagent-code/tui/keymap"
import {
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "deepagentCode", modelID: "gpt-5" },
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "deepagentCode", modelID: "gpt-5" })
  ...
  try {
    ...
    expect(frame).toContain("Search")
    expect(frame).toContain("deepagentCode")
    expect(frame).toContain("GPT-5")
