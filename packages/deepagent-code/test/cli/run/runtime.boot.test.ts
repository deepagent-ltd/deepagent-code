...
import { createOpencodeClient, type OpencodeClient, type Provider } from "@deepagent-code/sdk/v2"
import type { Resolved } from "@deepagent-code/tui/config"
  ...
  test("prefers configured providers for model selector data", async () => {
    ...
    const list = spyOn(sdk.provider, "list").mockImplementation(() =>
      Promise.resolve({
        data,
        error: undefined,
        request: new Request("https://deepagent-code.test"),
