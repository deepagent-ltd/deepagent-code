...
import { isZedTerminal, offsetToPosition, resolveZedDbPath, resolveZedSelection } from "@deepagent-code/tui/editor-zed"
...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.DEEPAGENT_CODE_ZED_DB
  process.env.DEEPAGENT_CODE_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.DEEPAGENT_CODE_ZED_DB
    else process.env.DEEPAGENT_CODE_ZED_DB = previous
    home.mockRestore()
  }
})
