...
export function createAcpClient(input: Pick<CliFixture, "deepagentCode">, env?: Record<string, string>) {
  return Effect.gen(function* () {
    return createJsonRpcAcpClient(yield* input.deepagentCode.acp(env ? { env } : undefined))
  })
}
  ...
  return Effect.gen(function* () {
    return expectOk(
      yield* acp.request<InitializeResponse>("initialize", {
        ...
        clientInfo: { name: "deepagent-code-local-acp", version: "0.1.0" },
      }),
    )
  })
}
