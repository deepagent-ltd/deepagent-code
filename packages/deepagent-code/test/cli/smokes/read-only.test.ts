...
describe("deepagentCode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // DEEPAGENT_CODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ deepagentCode }) =>
      Effect.gen(function* () {
        const r = yield* deepagentCode.spawn(["mcp", "list"])
        deepagentCode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ deepagentCode }) =>
      Effect.gen(function* () {
        const r = yield* deepagentCode.spawn(["providers", "list"])
        deepagentCode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ deepagentCode }) =>
      Effect.gen(function* () {
        const r = yield* deepagentCode.spawn(["models"])
        deepagentCode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ deepagentCode }) =>
      Effect.gen(function* () {
        const r = yield* deepagentCode.spawn(["agent", "list"])
        deepagentCode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh DEEPAGENT_CODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ deepagentCode }) =>
      Effect.gen(function* () {
        const r = yield* deepagentCode.spawn(["session", "list"])
        deepagentCode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ deepagentCode }) =>
      Effect.gen(function* () {
        const r = yield* deepagentCode.spawn(["stats"])
        deepagentCode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under DEEPAGENT_CODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ deepagentCode }) =>
      Effect.gen(function* () {
        const r = yield* deepagentCode.spawn(["db", "path"])
        deepagentCode.expectExit(r, 0, "db path")
