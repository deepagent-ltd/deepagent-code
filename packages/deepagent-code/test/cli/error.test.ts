...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/deepagent-code.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/deepagent-code.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/deepagent-code.jsonc", dir: ".deepagent-code", suggestion: "deepagentCode" },
        expected:
          'Directory ".deepagent-code" in /tmp/deepagent-code.jsonc is not valid. Rename the directory to "deepagentCode" or remove it. This is a common typo.',
      },
      {
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.deepagent-code.ai/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.deepagent-code.ai/auth/device/code.")
