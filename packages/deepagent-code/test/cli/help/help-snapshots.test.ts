...
// terminal sizes. The default deepagentCode tui command is excluded —
// `deepagentCode --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `deepagentCode --help` prints today;
...
// top-level help on `--help` and exits 1; not a real deepagentCode command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("deepagentCode CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ deepagentCode }) =>
      Effect.gen(function* () {
        const topLevel = yield* deepagentCode.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* deepagentCode.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`deepagentCode ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`deepagentCode ${argv.join(" ")} --help`)
        }
