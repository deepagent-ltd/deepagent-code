...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.DEEPAGENT_CODE_TEST ?? "")'), {
      env: {
        DEEPAGENT_CODE_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "DEEPAGENT_CODE_TEST_SHELL"], {
      shell: true,
      env: {
        DEEPAGENT_CODE_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("DEEPAGENT_CODE_TEST_SHELL=ok")
  })
