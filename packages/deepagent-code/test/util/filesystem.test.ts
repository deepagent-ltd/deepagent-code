...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "deepagent-code.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "deepagent-code.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "deepagent-code.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "deepagent-code.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["deepagent-code.json", "deepagent-code.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "deepagent-code.json"),
        path.join(project, "deepagent-code.jsonc"),
      ])
    })
  })
