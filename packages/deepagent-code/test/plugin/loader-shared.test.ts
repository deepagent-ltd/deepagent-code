...
const { readPackageThemes, resetPluginTargetResolutionCache } = await import("../../src/plugin/shared")
const { EventV2Bridge } = await import("../../src/event-v2-bridge")
const { Npm } = await import("@deepagent-code/core/npm")
const { TestConfig } = await import("../fixture/config")
const { RuntimeFlags } = await import("../../src/effect/runtime-flags")

afterEach(async () => {
  resetPluginTargetResolutionCache()
...
function load(dir: string, flags?: Parameters<typeof RuntimeFlags.layer>[0]) {
  const source = path.join(dir, "deepagent-code.json")
  return Effect.gen(function* () {
...
describe("plugin.loader.shared", () => {
  ...
  it.live("rejects npm server export that resolves outside plugin directory", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(path.join(dir, "deepagent-code.json"), JSON.stringify({ plugin: ["acme-plugin"] }, null, 2))
      ...
      (_tmp) =>
        Effect.gen(function* () {
          ...
          try {
            ...
            expect(pkgs).not.toContain("deepagent-code-openai-codex-auth@1.0.0")
            expect(pkgs).not.toContain("deepagent-code-copilot-auth@1.0.0")
          } finally {
            install.mockRestore()
          }
        }),
  ...
  it.live("backs off repeated resolution of one failed npm plugin", () =>
    Effect.gen(function* () {
      const install = spyOn(Npm, "add").mockRejectedValue(new Error("registry unavailable"))
      try {
        const plan = { spec: "missing-plugin@9.9.9", options: undefined, deprecated: false }
        expect((yield* Effect.promise(() => PluginLoader.resolve(plan, "server"))).ok).toBe(false)
        expect((yield* Effect.promise(() => PluginLoader.resolve(plan, "server"))).ok).toBe(false)
  ...
  it.live("continues loading plugins when plugin init throws", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(path.join(dir, "deepagent-code.json"), JSON.stringify({ plugin: [file, ok] }, null, 2))
  ...
  it.live("continues loading plugins when plugin import fails", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(path.join(dir, "deepagent-code.json"), JSON.stringify({ plugin: [missing, ok] }, null, 2))
...
`,
        ...
        await Bun.write(path.join(dir, "deepagent-code.json"), JSON.stringify({ plugin: [aSpec, bSpec] }, null, 2))
