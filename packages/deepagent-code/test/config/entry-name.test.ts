...
describe("configEntryNameFromPath", () => {
  ...
  // the agent key (e.g. `.config/deepagent-code/agents/build`). Anchoring at the
  ...
  test("regression #25713: caller passes relative path; parent /agent/ segment is irrelevant", () => {
    const dir = "/home/agent/.config/deepagent-code"
    const item = "/home/agent/.config/deepagent-code/agents/build.md"
  ...
  test("regression #25713: parent /agents/ segment is irrelevant", () => {
    const dir = "/srv/agents/team/.config/deepagent-code"
    const item = "/srv/agents/team/.config/deepagent-code/agents/build.md"
    const relative = posix.relative(dir, item)
    expect(configEntryNameFromPath(relative, AGENT_PREFIXES)).toBe("build")
  })
})
