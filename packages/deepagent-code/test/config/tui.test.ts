...
const cleanState = Effect.gen(function* () {
  ...
  delete process.env.DEEPAGENT_CODE_CONFIG
  delete process.env.DEEPAGENT_CODE_TUI_CONFIG
...
it.instance("keeps server and tui plugin merge semantics aligned", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      const local = path.join(test.directory, ".deepagent-code")
      yield* fs.makeDirectory(local, { recursive: true })

      yield* fs.writeJson(path.join(Global.Path.config, "deepagent-code.json"), {
        plugin: [["shared-plugin@1.0.0", { source: "global" }], "global-only@1.0.0"],
      })
      yield* fs.writeJson(path.join(Global.Path.config, "tui.json"), {
        plugin: [["shared-plugin@1.0.0", { source: "global" }], "global-only@1.0.0"],
      })
      yield* fs.writeJson(path.join(local, "deepagent-code.json"), {
  ...
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(
        path.join(test.directory, ".deepagent-code", "tui.json"),
...
it.instance("migrates tui-specific keys from deepagent-code.json when tui.json does not exist", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
      const source = path.join(test.directory, "deepagent-code.json")
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "deepagent-code.json.tui-migration.bak"))).toBe(true)
...
it.instance("migrates project legacy tui keys even when global tui.json already exists", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      const test = yield* TestInstance
      yield* fs.writeJson(path.join(Global.Path.config, "tui.json"), { theme: "global" })
      yield* fs.writeJson(path.join(test.directory, "deepagent-code.json"), {
      ...
      const server = JSON.parse(yield* fs.readFileString(path.join(test.directory, "deepagent-code.json")))
      expect(server.theme).toBeUndefined()
      expect(server.tui).toBeUndefined()
    }),
  ),
...
it.instance("skips migration when deepagent-code.jsonc is syntactically invalid", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
  ...
  "username": "still-broken"
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "deepagent-code.jsonc.tui-migration.bak"))).toBe(false)
      const source = yield* fs.readFileString(path.join(test.directory, "deepagent-code.jsonc"))
...
it.instance("skips migration when tui.json already exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
      yield* fs.writeJson(path.join(test.directory, "deepagent-code.json"), { theme: "legacy" })
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "deepagent-code.json.tui-migration.bak"))).toBe(false)
    }),
  ),
)
...
it.instance("migration backup preserves JSONC comments", () =>
  ...
  "tui": {
    ...
    "scroll_speed": 1.5
      ...
      const backup = yield* fs.readFileString(path.join(test.directory, "deepagent-code.jsonc.tui-migration.bak"))
...
it.instance("migrates legacy tui keys across multiple deepagent-code.json levels", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
      ...
      yield* fs.writeJson(path.join(test.directory, "deepagent-code.json"), { theme: "root-theme" })
      yield* fs.writeJson(path.join(nested, "deepagent-code.json"), { theme: "nested-theme" })
...
it.instance("project config takes precedence over DEEPAGENT_CODE_TUI_CONFIG (matches DEEPAGENT_CODE_CONFIG)", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("DEEPAGENT_CODE_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("does not derive tui path from DEEPAGENT_CODE_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
      ...
      yield* fs.writeJson(path.join(customDir, "deepagent-code.json"), { model: "test/model" })
      yield* fs.writeJson(path.join(customDir, "tui.json"), { theme: "should-not-load" })

      yield* withEnv(
        "DEEPAGENT_CODE_CONFIG",
        path.join(customDir, "deepagent-code.json"),
        Effect.gen(function* () {
          const config = yield* getTuiConfig(test.directory)
          expect(config.theme).toBeUndefined()
        }),
...
it.instance("loads .deepagent-code/tui.json", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("silently skips malformed tui.json - load failures degrade to {}", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(path.join(test.directory, ".deepagent-code", "tui.json"), JSON.stringify({ theme: "fallback" }))
