...
function completion(item: Sse, model: string) {
  const content = flow(item)
    .filter((part): part is Extract<Flow, { type: "text" }> => part.type === "text")
    .map((part) => part.text)
    .join("")
  const usage = flow(item).find((part): part is Extract<Flow, { type: "usage" }> => part.type === "usage")?.usage
  return {
    id: "chatcmpl-test",
...
function json(value: unknown) {
  return HttpServerResponse.text(JSON.stringify(value), { contentType: "application/json" })
}
...
function hit(url: string, headers: Record<string, string>, body: unknown) {
  return {
      ...
      const handle = Effect.fn("TestLLMServer.handle")(function* (mode: "chat" | "responses") {
        ...
        if (isTitleRequest(body)) {
          ...
          if (body && typeof body === "object" && (body as Record<string, unknown>).stream !== true)
            return json(completion(auto, modelFrom(body)))
          return send(auto)
        }
        ...
        if (body && typeof body === "object" && (body as Record<string, unknown>).stream !== true)
          return json(completion(next, modelFrom(body)))
        if (next.reset) {
          yield* reset(next)
