// Subprocess test harness for the deepagentCode CLI. Spawns the real binary against
...
//
// Configuration flows through deepagentCode's built-in test affordances:
//   - DEEPAGENT_CODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - DEEPAGENT_CODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - DEEPAGENT_CODE_DISABLE_PROJECT_CONFIG : skip walking up for deepagent-code.json
//   - DEEPAGENT_CODE_PURE                : skip external plugin discovery + install
//   - DEEPAGENT_CODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `deepagentCode.run` is fully wired. The shape supports adding more
// builders (`deepagentCode.serve(opts)`, `deepagentCode.acp(opts)`, `deepagentCode.auth(...)`)
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    DEEPAGENT_CODE_CONFIG_CONTENT: configJson,
...
// Typed equivalent of constructing argv for `deepagentCode run`. New flags should
...
// `deepagentCode serve` is a long-lived process — it never exits on its own.
...
export type OpencodeCli = {
  ...
  // Spawn `deepagentCode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `deepagentCode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly deepagentCode: OpencodeCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const serve = Effect.fn("deepagentCode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `deepagentCode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("deepagentCode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    // and hit endpoints on `deepagentCode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "deepagentCode") {
...
// every call site. Use this for any test that needs the deepagentCode CLI fixture.
//
...
// resources (e.g. `deepagentCode.serve`) without an extra `Effect.scoped` wrapper —
