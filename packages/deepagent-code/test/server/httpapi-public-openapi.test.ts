...
describe("PublicApi OpenAPI v2 errors", () => {
  ...
  test("documents compaction continuation resolution routes", () => {
    const spec = OpenApi.fromApi(PublicApi) as OpenApiSpec
    const route = spec.paths["/session/{sessionID}/continuation-resolution"]

    expect(route?.get?.operationId).toBe("session.continuationResolutionList")
    expect(Object.keys(route?.get?.responses ?? {})).toEqual(expect.arrayContaining(["200", "400", "404"]))
    expect(route?.post?.operationId).toBe("session.continuationResolutionResolve")
    expect(route?.post?.requestBody?.required).toBe(true)
    expect(Object.keys(route?.post?.responses ?? {})).toEqual(expect.arrayContaining(["200", "400", "404", "409"]))
  })
