...
const auth = { username: "deepagent-code", password: "listen-secret" }
...
async function startListener() {
  Flag.DEEPAGENT_CODE_SERVER_PASSWORD = auth.password
  Flag.DEEPAGENT_CODE_SERVER_USERNAME = auth.username
  process.env.DEEPAGENT_CODE_SERVER_PASSWORD = auth.password
  process.env.DEEPAGENT_CODE_SERVER_USERNAME = auth.username
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-deepagent-code-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-deepagent-code-ticket": "1" }),
...
describe("HttpApi Server.listen", () => {
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-deepagent-code-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  test("graceful stop is bounded while a client holds an active connection", async () => {
    // Bun's server.close(callback) only fires once ALL connections complete; an SSE/streaming
    // client may hold a socket indefinitely, so an unbounded graceful wait would hang stop()
    // forever. The server layer bounds the wait to its close budget, then destroys the tracked
    // sockets and completes.
    const listener = await startListener()
    const held = await new Promise<net.Socket>((resolve, reject) => {
      const socket = net.connect(listener.port, "127.0.0.1", () => resolve(socket))
      socket.once("error", reject)
    })
    try {
      const started = Date.now()
      await withTimeout(listener.stop(), 10_000, "timed out stopping listener with a held connection")
