...
const noAuthLayer = ServerAuth.Config.layer({ password: Option.none(), username: "deepagent-code" })
const secretLayer = ServerAuth.Config.layer({ password: Option.some("secret"), username: "deepagent-code" })
  ...
  itSecret.live("requires configured password for basic auth", () =>
    Effect.gen(function* () {
      const [missing, badPassword, good] = yield* Effect.all(
        [
          getProbe(),
          getProbe({ authorization: basic("deepagent-code", "wrong") }),
  ...
  itSecret.live("prefers auth token query credentials over basic auth", () =>
    Effect.gen(function* () {
      const response = yield* HttpClientRequest.get(
        `/probe?auth_token=${encodeURIComponent(token("deepagent-code", "secret"))}`,
      ).pipe(HttpClientRequest.setHeader("authorization", basic("deepagent-code", "wrong")), HttpClient.execute)
  ...
  itSecret.live("preserves handler errors when basic auth succeeds", () =>
    Effect.gen(function* () {
      const response = yield* HttpClientRequest.get("/missing").pipe(
        HttpClientRequest.setHeader("authorization", basic("deepagent-code", "secret")),
        HttpClient.execute,
      )
