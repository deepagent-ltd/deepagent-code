  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() => mkdir(path.join(test.directory, "packages", "deepagent-code"), { recursive: true }))
        ...
        const current = yield* withSession({ title: "current" }).pipe(
          provideInstance(path.join(test.directory, "packages", "deepagent-code")),
        )
    ...
    () =>
      Effect.gen(function* () {
        ...
        const ids = (yield* SessionNs.Service.use((session) =>
          session.list({ directory: path.join(test.directory, "packages", "deepagent-code") }),
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const test = yield* TestInstance
        const dir = path.join(test.directory, "packages", "deepagent-code")
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() =>
          mkdir(path.join(test.directory, "packages", "deepagent-code", "src", "deep"), { recursive: true }),
        )
        yield* Effect.promise(() => mkdir(path.join(test.directory, "packages", "app"), { recursive: true }))
        ...
        const current = yield* withSession({ title: "current" }).pipe(
          provideInstance(path.join(test.directory, "packages", "deepagent-code", "src")),
        )
        const deeper = yield* withSession({ title: "deeper" }).pipe(
          provideInstance(path.join(test.directory, "packages", "deepagent-code", "src", "deep")),
        )
        const sibling = yield* withSession({ title: "sibling" }).pipe(
          provideInstance(path.join(test.directory, "packages", "app")),
        )
        ...
        if (process.platform === "win32") {
          const windowsPathIDs = (yield* SessionNs.Service.use((session) =>
            session.list({ path: "packages\\deepagent-code\\src" }),
          )).map((session) => session.id)
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* Effect.promise(() =>
          mkdir(path.join(test.directory, "packages", "deepagent-code", "src"), { recursive: true }),
        )
        yield* Effect.promise(() => mkdir(path.join(test.directory, "packages", "app"), { recursive: true }))
        ...
        const pathIDs = (yield* SessionNs.Service.use((session) =>
          session.list({
            directory: path.join(test.directory, "packages", "deepagent-code", "src"),
            path: "packages/deepagent-code/src",
          }),
        )).map((session) => session.id)
        expect(pathIDs).toContain(current.id)
