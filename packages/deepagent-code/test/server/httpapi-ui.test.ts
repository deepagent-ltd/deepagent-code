  ...
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("DEEPAGENT_CODE_SERVER_PASSWORD", original.envPassword)
        restoreEnv("DEEPAGENT_CODE_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            DEEPAGENT_CODE_SERVER_PASSWORD: input?.password,
            DEEPAGENT_CODE_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
            ...
            Layer.succeed(
              ...
              HttpClient.make((request) =>
                Effect.succeed(
                  HttpClientResponse.fromWeb(
                    request,
                    new Response("<html>deepagent-code</html>", {
                      headers: {
  ...
  it.live("allows web UI preflight without auth", () =>
    Effect.gen(function* () {
      const response = yield* app({ password: "secret", username: "deepagent-code" }).request("/", {
        method: "OPTIONS",
        headers: {
