 ...
 *
 * The script intentionally isolates `DEEPAGENT_CODE_DB` before importing modules that touch
...
const configureDeepAgentExercise = () =>
  AgentGateway.configure({ enabled: true, runsDir: path.join(exerciseDataDirectory, "deepagent", "runs") })

const deepagentCandidate = (id: string) => ({
  candidate_id: `strategy_candidate:httpapi:${id}`,
  type: "strategy" as const,
  status: "staged" as const,
  source_run_id: `run_httpapi_${id}`,
  source_round: 1,
  summary: `HTTP API ${id} strategy candidate`,
  evidence_refs: [`evidence:httpapi:${id}`],
  confidence: 0.8,
})
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "deepagent-code.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "deepagent-code.jsonc")).text(),
          )
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-deepagent-code-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-deepagent-code-directory")
  }),
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "deepagent-code", model: "test" })}`,
      headers: ctx.headers(),
    }))
    .json(200, array, "status"),
  ...
  http.protected
    .post("/provider/models/discover", "provider.models.discover")
    .mutating()
    .seeded(() =>
      Effect.acquireRelease(
        Effect.promise(async () => {
          await Bun.write(
            path.join(exerciseDataDirectory, "auth.json"),
            JSON.stringify({ test: { type: "api", key: "stored-provider-key" } }),
          )
          const server = Bun.serve({
            ...
            fetch(request) {
              ...
              if (url.pathname === "/models") {
                if (request.headers.get("authorization") !== "Bearer stored-provider-key") {
          ...
          return { baseURL: String(server.url).replace(/\/$/, ""), stop: () => server.stop(true) }
        }),
        (server) => Effect.sync(() => server.stop()),
      ),
    )
    .at((ctx) => ({
      path: "/provider/models/discover",
      headers: ctx.headers(),
      body: {
        providerID: "test",
    ...
    .json(200, (body) => {
      object(body)
      check(body.providerID === "test", "discover should echo provider id")
      object(body.selected)
      check(body.selected.id === "model-a", "discover should select returned chat model")
      check(Array.isArray(body.models) && body.models.length === 1, "discover should return provider models")
    }),
  http.protected
    .get("/deepagent/reviews", "deepagent.reviews")
    .seeded(() => Effect.sync(configureDeepAgentExercise))
    .json(200, (body) => {
      object(body)
      array(body.reviews)
    }),
  http.protected
    ...
    .at((ctx) => ({
      ...
      body: {
        candidate: ctx.state,
        origin: "run_local",
        verdict: { pass: true, reason: "httpapi exercise", evidence: ["evidence:httpapi:promote"] },
        approval: { approver: "httpapi-exercise", approved: true, note: "route coverage" },
      },
    }))
    .json(200, (body) => {
      object(body)
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: ctx.headers(),
      body: { candidate: ctx.state, reason: "httpapi exercise rejection" },
    }))
    .json(200, (body) => {
      object(body)
      object(body.rejected)
      check(body.rejected.candidateId === "strategy_candidate:httpapi:reject", "reject should preserve candidate id")
      check(typeof body.rejected.fingerprint === "string", "reject should return candidate fingerprint")
    }),
      ...
      (body, ctx) =>
        Effect.gen(function* () {
          ...
          check(
            ...
            "async prompt should return message ID",
          )
