...
function writeProviderAuthPlugin(dir: string) {
  return Effect.gen(function* () {
    ...
    yield* Effect.promise(() => markPluginDependenciesReady(path.join(dir, ".deepagent-code")))

    yield* fs.writeWithDirs(
      path.join(dir, ".deepagent-code", "plugin", "provider-oauth-parity.ts"),
      [
        "export default {",
...
function writeProviderAuthValidationPlugin(dir: string) {
  return Effect.gen(function* () {
    ...
    yield* fs.writeWithDirs(
      path.join(dir, ".deepagent-code", "plugin", "provider-oauth-validation.ts"),
      [
        "export default {",
...
function writeFunctionOptionsPlugin(dir: string) {
  return Effect.gen(function* () {
    ...
    yield* fs.writeWithDirs(
      path.join(dir, ".deepagent-code", "plugin", "provider-function-options.ts"),
      [
        "export default {",
...
function writeProviderModelsMutationPlugin(dir: string) {
  return Effect.gen(function* () {
    ...
    yield* fs.writeWithDirs(
      path.join(dir, ".deepagent-code", "plugin", "provider-models-mutation.ts"),
      [
        "export default {",
  ...
  it.instance(
    ...
    Effect.gen(function* () {
      ...
      const response = yield* requestAuthorize({
        ...
        method: 0,
        inputs: { token: "nope" },
        headers: { "x-deepagent-code-directory": directory, "content-type": "application/json" },
      })
