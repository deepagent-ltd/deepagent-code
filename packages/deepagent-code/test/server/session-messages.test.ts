...
const withoutWatcher = <A, E, R>(effect: Effect.Effect<A, E, R>) => {
  ...
  return Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.DEEPAGENT_CODE_EXPERIMENTAL_DISABLE_FILEWATCHER
      process.env.DEEPAGENT_CODE_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
      return previous
    }),
    () => effect,
    (previous) =>
      Effect.sync(() => {
        if (previous === undefined) delete process.env.DEEPAGENT_CODE_EXPERIMENTAL_DISABLE_FILEWATCHER
        else process.env.DEEPAGENT_CODE_EXPERIMENTAL_DISABLE_FILEWATCHER = previous
      }),
  )
}
