// activity progress projection §7.3 — Provider protocol non-regression tests (items 30–34).
//
// The activity progress projection fix is scoped to App/progress projection; it must not touch
...
describe("activity progress projection §7.3 provider protocol non-regression", () => {
  it.effect("#30 Kimi active continuation keeps the reasoning/tool chain intact in the lowered request", () =>
    Effect.gen(function* () {
      ...
      const prepared = yield* LLMClient.prepare<OpenAIChat.OpenAIChatBody>(
        LLM.request({
          id: "req_activity_progress_projection_kimi_active_continuation",
          model: kimiModel,
  ...
  it.effect("#31 signed reasoning provider preserves signature/redacted block order", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<AnthropicMessages.AnthropicMessagesBody>(
        LLM.request({
          id: "req_activity_progress_projection_signed_reasoning_order",
          model: signedModel,
          messages: [
      ...
      const build = (withAppProjection: boolean) =>
        LLM.request({
          id: withAppProjection
            ? "req_activity_progress_projection_encrypted_with_projection"
            : "req_activity_progress_projection_encrypted_without_projection",
  ...
  it.effect("#33 settled plain reasoning is not re-injected across the store:false terminal boundary", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<OpenAIResponses.OpenAIResponsesBody>(
        LLM.request({
          id: "req_activity_progress_projection_terminal_boundary",
          model: encryptedModel,
          messages: [
            Message.assistant([
  ...
  it.effect("#34 parser keeps full raw reasoning text and tool results for DB/export persistence", () =>
    Effect.gen(function* () {
      ...
      const revision8 = yield* LLMClient.generate(turnRequest("req_activity_progress_projection_raw_rev8")).pipe(
        Effect.provide(
          fixedResponse(revisionStream({ itemID: "rs_rev8", callID: "call_rev8", command: "tail stress.log; ps aux" })),
        ),
      )
      const revision9 = yield* LLMClient.generate(turnRequest("req_activity_progress_projection_raw_rev9")).pipe(
        Effect.provide(
