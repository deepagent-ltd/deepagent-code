      ...
      yield* LLMClient.generate(
        LLM.updateRequest(request, {
          model: Azure.configure({
            baseURL: "https://deepagent-code-test.openai.azure.com/openai/v1/",
            apiKey: "azure-key",
        ...
        Effect.provide(
          dynamicResponse((input) =>
            Effect.gen(function* () {
              const web = yield* HttpClientRequest.toWeb(input.request).pipe(Effect.orDie)
              expect(web.url).toBe("https://deepagent-code-test.openai.azure.com/openai/v1/responses?api-version=v1")
