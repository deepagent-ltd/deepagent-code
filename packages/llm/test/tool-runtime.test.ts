...
// These tests exercise the transport/tool-runtime substrate, not the DeepAgent control plane.
// The DeepAgent global runtime now lives in @deepagent-code/core and registers its client
// middleware only when core is loaded. In the llm package the client seam defaults to identity
// passthrough, so these tests observe raw transport behavior with zero DeepAgent artifacts and
// no gateway configuration is needed.
