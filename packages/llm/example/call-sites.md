...
Kit and Aidan want provider-specific LLM behavior to move out of deepagent-code's AI
...
deepagent-code's dynamic runtime should construct executable models at its app
...
- Effect AI: executable models carry provider requirements and can be resolved by
  an app boundary.
- LiteLLM/deepagent-code config: dynamic `providerID/modelID` branching belongs at the
...
- [ ] Decide whether a tiny `Provider.define(...)` helper is warranted after two
      ...
      yet painful.
- [x] Update `packages/deepagent-code/src/session/llm/native-request.ts` to construct
...
- [ ] Update tests so direct route/provider tests assert route values are carried
      by executable models, and deepagent-code/native tests assert boundary-based route
      selection.
