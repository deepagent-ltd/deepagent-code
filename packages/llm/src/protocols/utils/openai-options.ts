...
// Internal continuation state returned by /responses/compact. It is lowered as
// an input item and is never copied to an arbitrary JSON request option.
export const compactionEncryptedContent = (request: LLMRequest) => {
  const value =
    request.providerOptions?.[request.model.provider]?.compactionEncryptedContent ??
    options(request)?.compactionEncryptedContent
