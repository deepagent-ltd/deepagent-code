      ...
      const renderer = yield* Effect.acquireRelease(
        Effect.tryPromise(() =>
          createCliRenderer({
            ...
            useMouse: !Flag.DEEPAGENT_CODE_DISABLE_MOUSE && input.config.mouse,
            consoleOptions: {
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  const forkStartupSession = async (sourceSessionID: string, intentKey: string) => {
    while (true) {
      const result = await requestSessionFork({
        key: intentKey,
        request: (intentID) => sdk.client.session.fork({ sessionID: sourceSessionID, intentID }),
      })
      ...
      const retry = await DialogConfirm.show(
        dialog,
        "Retry fork?",
        `${errorMessage(result.error)}\n\nRetry uses the same request identity to avoid creating a duplicate session.`,
        "Continue original",
      )
      if (retry) continue
      route.navigate({ type: "session", sessionID: sourceSessionID })
      return
    }
  }
  ...
  createEffect(() => {
    ...
    if (route.data.type === "home") {
      renderer.setTerminalTitle("DeepAgent Code")
      return
    }
  ...
  createEffect(() => {
    ...
    if (match) {
      ...
      if (args.fork) {
        ...
        void forkStartupSession(match, intentKey)
      } else {
  ...
  createEffect(() => {
    ...
    void forkStartupSession(args.sessionID, intentKey)
  })
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        title: "Open docs",
        run: () => {
          open("https://deepagent-code.ai/docs").catch(() => {})
          dialog.clear()
        },
        category: "System",
      },
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to DeepAgent Code v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    ...
    >
      <Show when={Flag.DEEPAGENT_CODE_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
