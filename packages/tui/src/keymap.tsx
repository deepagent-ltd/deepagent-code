...
export const DEEPAGENT_CODE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const DEEPAGENT_CODE_MODE_KEY = "deepagent-code.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(DEEPAGENT_CODE_MODE_KEY, DEEPAGENT_CODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(DEEPAGENT_CODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(DEEPAGENT_CODE_MODE_KEY, stack.at(-1)?.mode ?? DEEPAGENT_CODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? DEEPAGENT_CODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(DEEPAGENT_CODE_MODE_KEY, undefined)
...
export function getOpencodeModeStack(keymap: OpenTuiKeymap) {
  ...
  if (!value) throw new Error("DeepAgent mode stack is not registered for this keymap")
  return value
}
