...
test("addTheme keeps first theme for duplicate names", () => {
  ...
  const one = structuredClone(DEFAULT_THEMES["deepagent-code"])
  const two = structuredClone(DEFAULT_THEMES["deepagent-code"])
...
test("resolveTheme rejects circular color refs", () => {
  const item = structuredClone(DEFAULT_THEMES["deepagent-code"])
