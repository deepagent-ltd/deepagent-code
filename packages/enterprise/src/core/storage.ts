...
import { lazy } from "@deepagent-code/core/util/lazy"
...
export namespace Storage {
  ...
  function s3(): Adapter {
    const bucket = process.env.DEEPAGENT_CODE_STORAGE_BUCKET!
    const region = process.env.DEEPAGENT_CODE_STORAGE_REGION || "us-east-1"
    const client = new AwsClient({
      region,
      accessKeyId: process.env.DEEPAGENT_CODE_STORAGE_ACCESS_KEY_ID!,
      secretAccessKey: process.env.DEEPAGENT_CODE_STORAGE_SECRET_ACCESS_KEY!,
    })
    return createAdapter(client, `https://s3.${region}.amazonaws.com`, bucket)
  }

  function r2() {
    const accountId = process.env.DEEPAGENT_CODE_STORAGE_ACCOUNT_ID!
    const client = new AwsClient({
      accessKeyId: process.env.DEEPAGENT_CODE_STORAGE_ACCESS_KEY_ID!,
      secretAccessKey: process.env.DEEPAGENT_CODE_STORAGE_SECRET_ACCESS_KEY!,
    })
    return createAdapter(client, `https://${accountId}.r2.cloudflarestorage.com`, process.env.DEEPAGENT_CODE_STORAGE_BUCKET!)
  }

  const adapter = lazy(() => {
    const type = process.env.DEEPAGENT_CODE_STORAGE_ADAPTER
