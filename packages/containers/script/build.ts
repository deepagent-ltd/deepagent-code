...
const reg = process.env.REGISTRY ?? "ghcr.io/deepagent-ltd"
...
const setup = async () => {
  ...
  if (list.includes("deepagent-code")) {
    await $`docker buildx use deepagent-code`
    return
  }
  await $`docker buildx create --name deepagent-code --use`
}
