---
title: Konfiguration
description: Ved at bruge DeepAgent Code JSON konfig.
---

Du kan konfigurere DeepAgent Code ved hjælp af en JSON-konfigurationsfil.

---

## Format

DeepAgent Code understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/deepagent-code`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/deepagent-code/deepagent-code.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`DEEPAGENT_CODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`deepagent-code.json` i projekt) - projektspecifikke indstillinger
5. **`.deepagent-code` mapper** - agent, kommandoer, plugins
6. **Inline config** (`DEEPAGENT_CODE_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.deepagent-code` og `~/.config/deepagent-code` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/deepagent-code`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale DeepAgent Code-konfiguration i `~/.config/deepagent-code/deepagent-code.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `deepagent-code.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når DeepAgent Code starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `DEEPAGENT_CODE_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `DEEPAGENT_CODE_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.deepagent-code` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.deepagent-code` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`ai.deepagent.ltd/config.schema.json`**](https://ai.deepagent.ltd/config.schema.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `deepagent-code serve` og `deepagent-code web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din DeepAgent Code-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `deepagent-code.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din DeepAgent Code-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger DeepAgent Code at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din DeepAgent Code-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/deepagent-code/agents/` eller `.deepagent-code/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil DeepAgent Code falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`deepagent-code run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/deepagent-code/commands/` eller `.deepagent-code/commands/`. [Learn more here](/docs/commands).
...
DeepAgent Code vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard deepagent-code **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide DeepAgent Code med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.deepagent-code/plugins/` eller `~/.config/deepagent-code/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse DeepAgent Code til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
