---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i DeepAgent Code.
---

import config from "../../../../config.mjs"
export const console = config.console

DeepAgent Code bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i DeepAgent Code-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.deepagent/code/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i DeepAgent Code-konfigurationen.
...
DeepAgent Code Zen er en liste over modeller leveret af DeepAgent Code-teamet, som er blevet testet og verificeret til at fungere godt med DeepAgent Code. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med DeepAgent Code Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg deepagent-code og gå til [deepagent-code.ai/auth](https://deepagent-code.ai/auth).
...
Det fungerer som alle andre udbydere i DeepAgent Code og er helt valgfrit at bruge.

---

## DeepAgent Code Go

DeepAgent Code Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af DeepAgent Code-teamet, som er testet og verificeret til at fungere godt med DeepAgent Code.

1. Kør kommandoen `/connect` i TUI, vælg `DeepAgent Code Go`, og gå til [deepagent-code.ai/auth](https://deepagent-code.ai/zen).
...
Sådan bruger du Amazon Bedrock med DeepAgent Code:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører deepagent-code:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `deepagent-code.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i DeepAgent Code understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere deepagent-code til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at deepagent-code skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem deepagent-code-konfigurationen.
...
2. Vælg din godkendelsesmetode:
   ...
   3. Navn: `DeepAgent Code`, omfang: `api`
...
gemme tokenet i deepagent-code auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
DeepAgent Code bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse DeepAgent Code
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`deepagent-code.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [deepagent-code-gitlab-auth](https://www.npmjs.com/package/deepagent-code-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `deepagent-code.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med DeepAgent Code:
...
Sådan bruger du Google Vertex AI med DeepAgent Code:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører deepagent-code.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem deepagent-code, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [deepagent-code-helicone-session](https://github.com/H2Shami/deepagent-code-helicone-session) plugin for automatisk at logge hver DeepAgent Code-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver DeepAgent Code-samtale opført som en separat session.
...
Du kan konfigurere deepagent-code til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere deepagent-code til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere deepagent-code til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for DeepAgent Code. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/deepagent-code) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med DeepAgent Code:
...
3. Kopier API-nøglen til brug i DeepAgent Code.
...
6. **Vigtigt**: Før du bruger skymodeller i DeepAgent Code, skal du hente modelinformationen lokalt:
...
DeepAgent Code Zen er en liste over testede og verificerede modeller leveret af DeepAgent Code-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>DeepAgent Code Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **DeepAgent Code Zen**.

   ```txt
   /connect
   ```

3. Indtast DeepAgent Code API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem deepagent-code-konfigurationen.
...
5. Du kan også tilpasse dem gennem deepagent-code-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med DeepAgent Code:
...
Du kan også tilpasse modeller gennem deepagent-code-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med deepagent-code. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `deepagent-code.json`-filen i projektkataloget:
...
`limit`-felterne lader DeepAgent Code forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `deepagent-code auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek deepagent-code-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i deepagent-code-konfigurationen.
