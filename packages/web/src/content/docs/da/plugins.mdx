---
title: Plugins
description: Skriv dine egne plugins for at udvide DeepAgent Code.
---

Plugins giver dig mulighed for at udvide DeepAgent Code ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre DeepAgent Codes standardadfærd.
...
- `.deepagent-code/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/deepagent-code/node_modules/`.
...
1. Global konfiguration (`~/.config/deepagent-code/deepagent-code.json`)
2. Projektkonfiguration (`deepagent-code.json`)
3. Global plugin-mappe (`~/.config/deepagent-code/plugins/`)
4. Projekt plugin bibliotek (`.deepagent-code/plugins/`)
...
DeepAgent Code kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En deepagent-code SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide deepagent-code.
...
:::note
Hvis du bruger DeepAgent Code desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå deepagent-code i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til deepagent-code:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som deepagent-code kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for deepagent-code sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://deepagent-code.ai/docs/sdk) for detaljer.
