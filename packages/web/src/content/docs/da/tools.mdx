...
Her er alle de innebygde verktøyene som er tilgængelige i DeepAgent Code.
...
:::note
Dette verktøyet er bare tilgængelig når `DEEPAGENT_CODE_EXPERIMENTAL_LSP_TOOL=true` (eller `DEEPAGENT_CODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger DeepAgent Code-leverandøren eller når miljøvariabelen `DEEPAGENT_CODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
