...
Du kan tilføje eksterne værktøjer til DeepAgent Code ved hjælp af _Model Context Protocol_ eller MCP. DeepAgent Code understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [DeepAgent Code Config](https://deepagent-code.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/deepagent-code`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
DeepAgent Code håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil DeepAgent Code:
...
Hvis serveren kræver godkendelse, vil DeepAgent Code bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `deepagent-code mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil DeepAgent Code opbevare tokens sikkert i `~/.deepagent/code/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem DeepAgent Code-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde DeepAgent Code til din Sentry-konto.
