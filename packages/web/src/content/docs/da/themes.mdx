...
Med DeepAgent Code kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger DeepAgent Code vårt eget `deepagent-code`-tema.
...
DeepAgent Code støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/deepagent-code/themes/*.json` eller `$XDG_CONFIG_HOME/deepagent-code/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.deepagent-code/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.deepagent-code/themes/*.json`
