---
title: Server
description: Interager med deepagent-code-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `deepagent-code serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en deepagent-code-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `deepagent-code.local` |
...
Indstil `DEEPAGENT_CODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `deepagent-code`, eller indstil `DEEPAGENT_CODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `deepagent-code serve` og `deepagent-code web`.
...
Når du kører `deepagent-code` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug deepagent-code-serveren til at interagere med deepagent-code programmatisk.
:::

Denne arkitektur lader deepagent-code understøtte flere klienter og lader dig interagere med deepagent-code programmatisk.

Du kan køre `deepagent-code serve` for at starte en selvstændig server. Hvis du har
deepagent-code TUI kørende, vil `deepagent-code serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af DeepAgent Code [IDE](/docs/ide) plugins.
...
DeepAgent Code-serveren viser følgende API'er.
