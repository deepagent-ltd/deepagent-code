...
Der er to typer agenter i DeepAgent Code; primære agenter og subagenter.
...
DeepAgent Code leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
DeepAgent Code leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

DeepAgent Code leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i DeepAgent Code's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Konfigurer agent i din `deepagent-code.json`-konfigurationsfil:
...
- Globalt: `~/.config/deepagent-code/agents/`
- Pr. projekt: `.deepagent-code/agents/`
...
Hvis ingen temperatur er angivet, bruger DeepAgent Code modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale DeepAgent Code-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din DeepAgent Code-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [DeepAgent Code Zen](/docs/zen), vil du bruge `deepagent-code/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `deepagent-code models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/deepagent-ltd/deepagent-code).
:::
