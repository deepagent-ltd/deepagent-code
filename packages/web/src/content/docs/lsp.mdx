---
title: LSP Servers
description: DeepAgent Code integrates with your LSP servers.
---

DeepAgent Code can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

DeepAgent Code comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `DEEPAGENT_CODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and deepagent-code opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your deepagent-code config.
...
```json title="deepagent-code.json" {5-8}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="deepagent-code.json" {5-13}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "lsp": {
    "custom-lsp": {
