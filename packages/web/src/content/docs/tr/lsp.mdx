...
description: DeepAgent Code, LSP sunucularınızla bütünleşir.
---

DeepAgent Code, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

DeepAgent Code, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`DEEPAGENT_CODE_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve deepagent-code bir dosyayı açtığında:
...
LSP sunucularını, deepagent-code yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
