...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. deepagent-code yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
deepagent-code içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `DEEPAGENT_CODE_EXPERIMENTAL_LSP_TOOL=true` (veya `DEEPAGENT_CODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca deepagent-code provider kullanılırken veya `DEEPAGENT_CODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

deepagent-code başlatırken etkinleştirmek için:
