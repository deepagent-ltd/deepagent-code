...
description: deepagent-code dile özgü biçimlendiriciler kullanır.
---

deepagent-code, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

deepagent-code, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, deepagent-code bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

deepagent-code bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri deepagent-code yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
