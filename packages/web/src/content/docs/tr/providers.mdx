---
title: Sağlayıcılar
description: deepagent-code'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

deepagent-code, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı deepagent-code yapılandırmanızda yapılandırın.
...
`~/.deepagent/code/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları deepagent-code'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
DeepAgent Code Zen, deepagent-code ekibi tarafından sağlanan modellerin bir listesidir.
deepagent-code ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz DeepAgent Code Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, deepagent-code'u seçin ve [ai.deepagent.ltd/auth](https://ai.deepagent.ltd/auth)'ye gidin.
...
deepagent-code'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## DeepAgent Code Go

DeepAgent Code Go, deepagent-code ile iyi çalıştığı test edilmiş ve doğrulanmış, deepagent-code ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `DeepAgent Code Go`'yu seçin ve [ai.deepagent.ltd/auth](https://ai.deepagent.ltd/zen) adresine gidin.
...
Amazon Bedrock'u deepagent-code ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   deepagent-code'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `deepagent-code.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi deepagent-code'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
deepagent-code'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   deepagent-code'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   DeepAgent Code yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
deepagent-code, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. deepagent-code'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`deepagent-code.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [deepagent-code-gitlab-auth](https://www.npmjs.com/package/deepagent-code-gitlab-auth) ana sayfasında.

##### Yapılandırma

`deepagent-code.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi deepagent-code'la kullanmak için:
...
Google Vertex AI'yi deepagent-code ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları deepagent-code çalıştırılırken ayarlayın.
...
Helicone'un deepagent-code aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her deepagent-code görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [deepagent-code-helicone-session](https://github.com/H2Shami/deepagent-code-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her deepagent-code görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
deepagent-code'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla deepagent-code'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla deepagent-code'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini deepagent-code için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/deepagent-code)'a bakın.
:::
...
Ollama Cloud'u deepagent-code ile kullanmak için:
...
3. deepagent-code'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: deepagent-code'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
DeepAgent Code Zen, deepagent-code ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>DeepAgent Code Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **DeepAgent Code Zen**'i arayın.

   ```txt
   /connect
   ```

3. deepagent-code API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   deepagent-code yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca deepagent-code yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi DeepAgent Code ile kullanmak için:
...
Modelleri deepagent-code yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı deepagent-code ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `deepagent-code.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları deepagent-code'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `deepagent-code auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için deepagent-code yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, deepagent-code yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
