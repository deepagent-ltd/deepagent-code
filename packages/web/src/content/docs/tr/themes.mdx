...
deepagent-code ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak deepagent-code kendi `deepagent-code` temasını kullanır.
...
deepagent-code birden fazla yerleşik temayla gelir.
...
- deepagent-code'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
deepagent-code, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/deepagent-code/themes/*.json` veya `$XDG_CONFIG_HOME/deepagent-code/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.deepagent-code/themes/*.json`
4. **Geçerli çalışma dizini** - `./.deepagent-code/themes/*.json`
