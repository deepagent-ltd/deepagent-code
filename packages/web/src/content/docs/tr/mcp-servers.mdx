...
_Model Bağlam Protokolü_ veya MCP kullanarak deepagent-code'a harici araçlar ekleyebilirsiniz. deepagent-code hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [deepagent-code Config](https://deepagent-code.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/deepagent-code` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
deepagent-code, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde deepagent-code şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa deepagent-code, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `deepagent-code mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından deepagent-code, belirteçleri `~/.deepagent/code/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra deepagent-code'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi deepagent-code yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve deepagent-code'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
