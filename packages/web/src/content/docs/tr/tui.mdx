---
title: TUI
description: DeepAgent Code terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

DeepAgent Code, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

DeepAgent Code'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
DeepAgent Code TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
DeepAgent Code'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
DeepAgent Code'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `deepagent-code.json` dosyasından ayrıdır.
...
- `leader_timeout` - DeepAgent Code'un leader key sonrasında ne kadar bekleyeceğini kontrol eder. Varsayılan değer `2000`.
...
Özel bir TUI yapılandırma yolu yüklemek için `DEEPAGENT_CODE_TUI_CONFIG` kullanın.

### Attention

Attention, DeepAgent Code bir yanıt beklediğinde, izin onayı gerektiğinde, oturum hatası oluştuğunda veya oturum tamamlandığında TUI'nin sizi ses ve masaüstü bildirimiyle uyarmasını sağlar. `attention.enabled` ile etkinleştirin; yerleşik olaylar gerçekleştiğinde ses çalar. Masaüstü bildirimleri yalnızca terminal penceresi odakta değilken gönderilir ve subagent olayları için kullanılmaz.
...
- `sound_pack` - Kullanılacak sound pack ID. Varsayılan değer `deepagent-code.default`.
