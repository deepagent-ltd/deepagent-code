---
title: Özel Araçlar
description: LLM'un deepagent-code'da çağırabileceği araçları birleştirebilir.
---

Özel araçlar, LLM'un konuşmalar sırasında arayabileceği, oluşturduğunuz işlevlerdir. deepagent-code'un [built-in tools](/docs/tools) ile birlikte `read`, `write` ve `bash` gibi çalışırlar.
...
- Yerel olarak bunları projenizin `.deepagent-code/tools/` dizinine yerleştirerek.
- Veya küresel olarak bunları `~/.config/deepagent-code/tools/` içine yerleştirerek.
