...
DeepAgent Code Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, DeepAgent Code'daki diğer sağlayıcılar gibi çalışır. DeepAgent Code Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve DeepAgent Code'u kullanmak için bunu kullanmanıza gerek yoktur.
...
DeepAgent Code Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

DeepAgent Code Go, DeepAgent Code'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>DeepAgent Code Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `DeepAgent Code Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye DeepAgent Code Go'ya abone olabilir.
:::
...
DeepAgent Code Go aşağıdaki limitleri içerir:
...
DeepAgent Code yapılandırmanızdaki [model id](/docs/config/#models) formatı `deepagent-code-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `deepagent-code-go/kimi-k2.6` kullanmalısınız.
...
DeepAgent Code Go'yu şu amaçlarla oluşturduk:
...
4. DeepAgent Code ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
