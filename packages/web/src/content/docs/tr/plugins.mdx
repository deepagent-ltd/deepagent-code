---
title: Eklentiler
description: DeepAgent Code'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek DeepAgent Code'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya DeepAgent Code'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.deepagent-code/plugins/` - Proje düzeyinde eklentiler
- `~/.config/deepagent-code/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/deepagent-code/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/deepagent-code/deepagent-code.json`)
2. Project config (`deepagent-code.json`)
3. Global eklenti dizini (`~/.config/deepagent-code/plugins/`)
4. Proje eklenti dizini (`.deepagent-code/plugins/`)
...
DeepAgent Code bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir DeepAgent Code SDK istemcisi.
...
DeepAgent Code'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
DeepAgent Code masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
DeepAgent Code'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca DeepAgent Code'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, DeepAgent Code'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra DeepAgent Code'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://deepagent-code.ai/docs/sdk) bakın.
