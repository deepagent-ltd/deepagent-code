---
title: Sunucu
description: deepagent-code sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`deepagent-code serve` komutu, deepagent-code istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `deepagent-code.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `DEEPAGENT_CODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `deepagent-code` değeridir; değiştirmek isterseniz `DEEPAGENT_CODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `deepagent-code serve` hem de `deepagent-code web` için geçerlidir.
...
`deepagent-code` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
deepagent-code ile programatik etkileşim için deepagent-code sunucusunu kullanın.
:::

Bu mimari, deepagent-code'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`deepagent-code serve` ile bağımsız bir sunucu başlatabilirsiniz. deepagent-code TUI açıksa, `deepagent-code serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum deepagent-code [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
deepagent-code sunucusu aşağıdaki API'leri sunar.
