...
deepagent-code, belirli bir eylemin otomatik olarak mı çalıştırılacağına, size sorulacağına veya engelleneceğine karar vermek için `permission` yapılandırmasını kullanır.
...
deepagent-code'un başlatıldığı çalışma dizini dışındaki yollara dokunan araç çağrılarına izin vermek için `external_directory` kullanın. Bu, girdi olarak bir yolu alan tüm araçlar için geçerlidir (örneğin `read`, `edit`, `glob`, `grep` ve birçok `bash` komutu).
...
deepagent-code izinleri araç adına ve birkaç güvenlik önlemine göre anahtarlanır:
...
Hiçbir şey belirtmezseniz deepagent-code izin verilen varsayılanlardan başlar:
...
deepagent-code onay istediğinde kullanıcı arayüzü üç sonuç sunar:

- `once` — yalnızca bu isteği onayla
- `always` — önerilen kalıplarla eşleşen gelecekteki istekleri onaylayın (geçerli deepagent-code oturumunun geri kalanı için)
