...
There are two types of agents in DeepAgent Code; primary agents and subagents.
...
DeepAgent Code comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
DeepAgent Code comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

DeepAgent Code comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into DeepAgent Code's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `deepagent-code.json` config file:
...
- Per-project: `.deepagent-code/agents/`
...
If no temperature is specified, DeepAgent Code uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global DeepAgent Code config and the project specific config.
...
The model ID in your DeepAgent Code config uses the format `provider/model-id`. For example, if you're using [DeepAgent Code Zen](/docs/zen), you would use `deepagent-code/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `deepagent-code models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/deepagent-ltd/deepagent-code).
:::
