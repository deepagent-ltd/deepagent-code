...
DeepAgent Code Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u DeepAgent Code-u. Pretplatite se na DeepAgent Code Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili DeepAgent Code.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa DeepAgent Code-om.
:::
...
DeepAgent Code Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

DeepAgent Code Go radi kao bilo koji drugi provajder u DeepAgent Code-u.

1. Prijavite se na **<a href={console}>DeepAgent Code Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `DeepAgent Code Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na DeepAgent Code Go.
:::
...
DeepAgent Code Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj DeepAgent Code konfiguraciji
koristi format `deepagent-code-go/<model-id>`. Na primjer, za Kimi K2.6, koristili biste
`deepagent-code-go/kimi-k2.6` u svojoj konfiguraciji.
...
Napravili smo DeepAgent Code Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz DeepAgent Code koristite i bilo kojeg drugog provajdera.
