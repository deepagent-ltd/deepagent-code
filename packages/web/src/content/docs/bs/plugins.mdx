---
title: Dodaci
description: Napišite vlastite dodatke za proširenje DeepAgent Code.
---

Dodaci vam omogućavaju da proširite DeepAgent Code spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja DeepAgent Code.
...
- `.deepagent-code/plugins/` - Dodaci na nivou projekta
- `~/.config/deepagent-code/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/deepagent-code/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/deepagent-code/deepagent-code.json`)
2. Konfiguracija projekta (`deepagent-code.json`)
3. Globalni direktorij dodataka (`~/.config/deepagent-code/plugins/`)
4. Direktorij dodataka projekta (`.deepagent-code/plugins/`)
...
DeepAgent Code pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: DeepAgent Code SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje DeepAgent Code.
...
Spriječite deepagent-code da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u deepagent-code:
...
Pomoćnik `tool` kreira prilagođeni alat koji deepagent-code može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za deepagent-code zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://ai.deepagent.ltd/docs/sdk) za detalje.
