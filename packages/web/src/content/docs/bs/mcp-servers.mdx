...
Možete dodati vanjske alate u DeepAgent Code koristeći _Model Context Protocol_, ili MCP. DeepAgent Code podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [DeepAgent Code Config](https://deepagent-code.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/deepagent-code`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
DeepAgent Code automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, DeepAgent Code će:
...
Ako server zahtijeva autentifikaciju, DeepAgent Code će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `deepagent-code mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, DeepAgent Code sigurno čuva tokene u `~/.deepagent/code/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u DeepAgent Code, zajedno s ugrađenim alatima. Možete njima upravljati kroz DeepAgent Code konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete DeepAgent Code sa vašim Sentry nalogom.
