---
title: Server
description: Komunicirajte s deepagent-code serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `deepagent-code serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji deepagent-code klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `deepagent-code.local` |
...
Postavite `DEEPAGENT_CODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `deepagent-code`, ili postavite `DEEPAGENT_CODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `deepagent-code serve` i za `deepagent-code web`.
...
Kada pokrenete `deepagent-code`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite deepagent-code server za programsku interakciju sa deepagent-code-om.
:::

Ova arhitektura omogucava deepagent-code podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `deepagent-code serve` da startate standalone server. Ako je deepagent-code TUI vec pokrenut, `deepagent-code serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste DeepAgent Code [IDE](/docs/ide) pluginovi.
...
deepagent-code server izlaže sljedece API-je.
