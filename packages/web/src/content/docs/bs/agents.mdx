...
Postoje dvije vrste agenata u DeepAgent Code: primarni agenti i podagenti.
...
DeepAgent Code dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
DeepAgent Code dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

DeepAgent Code dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u DeepAgent Code-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `deepagent-code.json`:
...
- Globalno: `~/.config/deepagent-code/agents/`
- Po projektu: `.deepagent-code/agents/`
...
Ako temperatura nije navedena, DeepAgent Code koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu DeepAgent Code konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj DeepAgent Code konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [DeepAgent Code Zen](/docs/zen), koristili biste `deepagent-code/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `deepagent-code models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/deepagent-ltd/deepagent-code).
:::
