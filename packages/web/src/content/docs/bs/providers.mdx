---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u DeepAgent Code.
---

import config from "../../../../config.mjs"
export const console = config.console

DeepAgent Code koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj DeepAgent Code konfiguraciji.
...
u `~/.deepagent/code/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem DeepAgent Code
config.
...
DeepAgent Code Zen je lista modela koje je obezbedio DeepAgent Code tim koji su bili
testirano i potvrđeno da dobro radi sa DeepAgent Code. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa DeepAgent Code Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite deepagent-code i idite na [ai.deepagent.ltd/auth](https://ai.deepagent.ltd/auth).
...
Radi kao i svaki drugi provajder u DeepAgent Code i potpuno je opcionalan za korištenje.

---

## DeepAgent Code Go

DeepAgent Code Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža DeepAgent Code tim i koji su testirani i verificirani da dobro rade s DeepAgent Code-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `DeepAgent Code Go` i idite na [ai.deepagent.ltd/auth](https://ai.deepagent.ltd/zen).
...
Da biste koristili Amazon Bedrock s DeepAgent Code:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete deepagent-code:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `deepagent-code.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u DeepAgent Code.
:::
...
Možete konfigurirati deepagent-code za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi deepagent-code ispravno radio.
   :::
...
Također možete dodati modele kroz svoju deepagent-code konfiguraciju.
...
3.  Naziv: `DeepAgent Code`, opseg: `api`
...
da pohrani token u deepagent-code auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
DeepAgent Code koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate DeepAgent Code
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`deepagent-code.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [deepagent-code-gitlab-auth](https://www.npmjs.com/package/deepagent-code-gitlab-auth).

##### Konfiguracija

Prilagodite putem `deepagent-code.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s deepagent-code:
...
Za korištenje Google Vertex AI s DeepAgent Code:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće deepagent-code.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem DeepAgent Code, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [deepagent-code-helicone-session](https://github.com/H2Shami/deepagent-code-helicone-session) da automatski prijavite svaki DeepAgent Code razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki DeepAgent Code razgovor naveden kao zasebna sesija.
...
Možete konfigurirati deepagent-code za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati deepagent-code za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati deepagent-code za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za DeepAgent Code. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/deepagent-code) za detalje.
:::
...
Da biste koristili Ollama Cloud s DeepAgent Code:
...
3. Kopirajte API ključ za korištenje u DeepAgent Code.
...
6. **Važno**: Prije upotrebe modela oblaka u DeepAgent Code, morate lokalno povući informacije o modelu:
...
DeepAgent Code Zen je lista testiranih i verifikovanih modela koju je obezbedio DeepAgent Code tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>DeepAgent Code Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **DeepAgent Code Zen**.

```txt
   /connect
```

3. Unesite svoj DeepAgent Code API ključ.
...
Također možete dodati dodatne modele putem vaše deepagent-code konfiguracije.
...
5. Također ih možete prilagoditi putem vaše deepagent-code konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa DeepAgent Codeom:
...
Također možete prilagoditi modele kroz svoju deepagent-code konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s deepagent-code-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `deepagent-code.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju DeepAgent Code da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `deepagent-code auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite DeepAgent Code konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj deepagent-code konfiguraciji.
