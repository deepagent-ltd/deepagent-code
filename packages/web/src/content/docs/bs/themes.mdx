...
U DeepAgent Code mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, DeepAgent Code koristi nasu `deepagent-code` temu.
...
DeepAgent Code dolazi sa vise ugradenih tema.
...
- Zele da DeepAgent Code odgovara izgledu njihovog terminala
...
DeepAgent Code podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/deepagent-code/themes/*.json` ili `$XDG_CONFIG_HOME/deepagent-code/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.deepagent-code/themes/*.json`
4. **Trenutni radni direktorij** - `./.deepagent-code/themes/*.json`
