...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. DeepAgent Code dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u DeepAgent Code.
...
:::note
Ovaj alat je dostupan samo kada je `DEEPAGENT_CODE_EXPERIMENTAL_LSP_TOOL=true` (ili `DEEPAGENT_CODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz DeepAgent Code provajdera ili kada je varijabla `DEEPAGENT_CODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju DeepAgent Code:
