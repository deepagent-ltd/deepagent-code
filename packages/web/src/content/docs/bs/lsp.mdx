---
title: LSP serveri
description: DeepAgent Code se integriše sa vašim LSP serverima.
---

DeepAgent Code se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

DeepAgent Code dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `DEEPAGENT_CODE_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i deepagent-code otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj deepagent-code konfiguraciji.
