...
Agent skills omogucavaju DeepAgent Code da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
DeepAgent Code pretrazuje ove lokacije:

- Konfiguracija projekta: `.deepagent-code/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/deepagent-code/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, DeepAgent Code ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.deepagent-code/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/deepagent-code/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.deepagent-code/skills/git-release/SKILL.md` ovako:
...
DeepAgent Code navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `deepagent-code.json`:
...
**Za ugradene agente** (u `deepagent-code.json`):
