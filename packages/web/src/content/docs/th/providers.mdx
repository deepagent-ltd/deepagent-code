---
title: ผู้ให้บริการ
description: ใช้ผู้ให้บริการ LLM ใน DeepAgent Code
---

import config from "../../../../config.mjs"
export const console = config.console

DeepAgent Code ใช้ [AI SDK](https://ai-sdk.dev/) และ [Models.dev](https://models.dev) เพื่อรองรับผู้ให้บริการ **75+ LLM** และรองรับการใช้งานโมเดลท้องถิ่น

หากต้องการเพิ่มผู้ให้บริการ คุณต้อง:

1. เพิ่มคีย์ API สำหรับผู้ให้บริการโดยใช้คำสั่ง `/connect`
2. กำหนดค่าผู้ให้บริการในการกำหนดค่า DeepAgent Code ของคุณ
...
ใน `~/.deepagent/code/auth.json`.

---

### การกำหนดค่า

คุณสามารถปรับแต่งผู้ให้บริการได้ผ่านทางส่วน `provider` ใน DeepAgent Code ของคุณ
กำหนดค่า
...
DeepAgent Code Zen คือรายชื่อโมเดลที่จัดทำโดยทีมงาน DeepAgent Code ที่ได้รับ
ทดสอบและตรวจสอบแล้วว่าทำงานได้ดีกับ DeepAgent Code [เรียนรู้เพิ่มเติม](/docs/zen)

:::tip
หากคุณยังใหม่ เราขอแนะนำให้เริ่มต้นด้วย DeepAgent Code Zen
:::

1. เรียกใช้คำสั่ง `/connect` ใน TUI เลือก deepagent-code และไปที่ [ai.deepagent.ltd/auth](https://ai.deepagent.ltd/auth)
...
มันทำงานเหมือนกับผู้ให้บริการรายอื่นใน DeepAgent Code และเป็นทางเลือกในการใช้งานโดยสมบูรณ์

---

## DeepAgent Code Go

DeepAgent Code Go คือแผนการสมัครสมาชิกราคาประหยัดที่ให้การเข้าถึงโมเดลการเขียนโค้ดแบบเปิดยอดนิยมที่เชื่อถือได้ ซึ่งจัดเตรียมโดยทีมงาน DeepAgent Code ที่ได้รับการทดสอบและตรวจสอบแล้วว่าทำงานได้ดีกับ DeepAgent Code

1. เรียกใช้คำสั่ง `/connect` ใน TUI เลือก `DeepAgent Code Go` และไปที่ [ai.deepagent.ltd/auth](https://ai.deepagent.ltd/zen)
...
วิธีใช้ Amazon Bedrock กับ DeepAgent Code:
...
2. **กำหนดค่าการตรวจสอบสิทธิ์** โดยใช้วิธีใดวิธีหนึ่งต่อไปนี้:
   ...
   ตั้งค่าหนึ่งในตัวแปรสภาพแวดล้อมเหล่านี้ขณะเรียกใช้ deepagent-code:
   ...
   สำหรับการกำหนดค่าเฉพาะโครงการหรือแบบถาวร ให้ใช้ `deepagent-code.json`:
...
:::info
การใช้การสมัครสมาชิก Claude Pro/Max ใน DeepAgent Code ไม่ได้รับการสนับสนุนอย่างเป็นทางการจาก [Anthropic](https://anthropic.com)
:::
...
คุณสามารถกำหนดค่า deepagent-code ให้ใช้โมเดลท้องถิ่นผ่าน [Atomic Chat](https://atomic.chat) ซึ่งเป็นแอปพลิเคชันเดสก์ท็อปที่เรียกใช้ LLM ในเครื่องภายใต้เซิร์ฟเวอร์ API ที่เข้ากันได้กับ OpenAI (ปลายทางเริ่มต้น `http://127.0.0.1:1337/v1`)
...
2. ไปที่ [Azure AI Foundry](https://ai.azure.com/) และปรับใช้โมเดล
   ...
   :::note
   ชื่อการปรับใช้จะต้องตรงกับชื่อรุ่นเพื่อให้ deepagent-code ทำงานได้อย่างถูกต้อง
   :::
...
5. รันคำสั่ง `/models` เพื่อเลือกรุ่น
   ...
   คุณยังสามารถเพิ่มโมเดลผ่านการกำหนดค่า deepagent-code ของคุณได้
...
3.  ชื่อ: `DeepAgent Code`, ขอบเขต: `api`
...
เพื่อจัดเก็บโทเค็นในที่เก็บข้อมูลการรับรองความถูกต้องของ deepagent-code
:::

##### GitLab ที่โฮสต์เอง

:::note[บันทึกการปฏิบัติตาม]
DeepAgent Code ใช้โมเดลขนาดเล็กสำหรับงาน AI บางอย่าง เช่น การสร้างชื่อเซสชัน
มีการกำหนดค่าให้ใช้ gpt-5-nano ตามค่าเริ่มต้น ซึ่งโฮสต์โดย Zen เพื่อล็อค DeepAgent Code
หากต้องการใช้เฉพาะอินสแตนซ์ที่โฮสต์บน GitLab ของคุณเอง ให้เพิ่มสิ่งต่อไปนี้ในของคุณ
`deepagent-code.json` ไฟล์. ขอแนะนำให้ปิดใช้งานการแชร์เซสชันด้วย
...
เอกสารประกอบเพิ่มเติมเกี่ยวกับหน้าแรกของ [deepagent-code-gitlab-auth](https://www.npmjs.com/package/deepagent-code-gitlab-auth)

##### การกำหนดค่า

ปรับแต่งผ่าน `deepagent-code.json`:
...
วิธีใช้การสมัครสมาชิก GitHub Copilot ด้วย deepagent-code:
...
วิธีใช้ Google Vertex AI กับ DeepAgent Code:
...
2. ตั้งค่าตัวแปรสภาพแวดล้อมที่ต้องการ:
   ...
   ตั้งค่าในขณะที่เรียกใช้ deepagent-code
...
ในกรณีที่คุณเห็นคุณสมบัติหรือรุ่นจาก Helicone ที่ไม่ได้กำหนดค่าโดยอัตโนมัติผ่าน deepagent-code คุณสามารถกำหนดค่าด้วยตนเองได้ตลอดเวลา
...
คุณลักษณะ [เซสชัน](https://docs.helicone.ai/features/sessions) ของ Helicone ช่วยให้คุณสามารถจัดกลุ่มคำขอ LLM ที่เกี่ยวข้องเข้าด้วยกันได้ ใช้ปลั๊กอิน [deepagent-code-helicone-session](https://github.com/H2Shami/deepagent-code-helicone-session) เพื่อบันทึกการสนทนา DeepAgent Code แต่ละรายการเป็นเซสชันใน Helicone โดยอัตโนมัติ
...
ปลั๊กอินจะแทรกส่วนหัว `Helicone-Session-Id` และ `Helicone-Session-Name` ลงในคำขอของคุณ ในหน้าเซสชันของ Helicone คุณจะเห็นการสนทนา DeepAgent Code แต่ละรายการแยกเป็นเซสชันแยกกัน
...
คุณสามารถกำหนดค่า deepagent-code เพื่อใช้โมเดลท้องถิ่นผ่านยูทิลิตี้ [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server
...
คุณสามารถกำหนดค่า deepagent-code เพื่อใช้โมเดลท้องถิ่นผ่าน LM Studio
...
คุณสามารถกำหนดค่า deepagent-code เพื่อใช้โมเดลท้องถิ่นผ่าน Ollama

:::tip
Ollama สามารถกำหนดค่าตัวเองสำหรับ DeepAgent Code ได้โดยอัตโนมัติ ดู [เอกสารการบูรณาการ Ollama](https://docs.ollama.com/integrations/deepagent-code) เพื่อดูรายละเอียด
:::
...
วิธีใช้ Ollama Cloud กับ DeepAgent Code:
...
3. คัดลอกคีย์ API เพื่อใช้ใน DeepAgent Code
...
6. **สำคัญ**: ก่อนที่จะใช้โมเดลคลาวด์ใน DeepAgent Code คุณต้องดึงข้อมูลโมเดลในเครื่อง:
...
DeepAgent Code Zen คือรายการโมเดลที่ได้รับการทดสอบและตรวจสอบแล้วโดยทีมงาน DeepAgent Code [เรียนรู้เพิ่มเติม](/docs/zen)

1. ลงชื่อเข้าใช้ **<a href={console}>DeepAgent Code Zen</a>** และคลิก **สร้าง API คีย์**

2. เรียกใช้คำสั่ง `/connect` และค้นหา **DeepAgent Code Zen**

   ```txt
   /connect
   ```

3. ป้อนคีย์ DeepAgent Code API ของคุณ
...
4. โมเดล OpenRouter จำนวนมากถูกโหลดไว้ล่วงหน้าตามค่าเริ่มต้น รันคำสั่ง `/models` เพื่อเลือกโมเดลที่คุณต้องการ
   ...
   คุณยังสามารถเพิ่มโมเดลเพิ่มเติมผ่านการกำหนดค่า deepagent-code ของคุณได้
...
5. คุณยังสามารถปรับแต่งได้ผ่านการกำหนดค่า deepagent-code ของคุณ นี่คือตัวอย่างการระบุผู้ให้บริการ
...
วิธีใช้ [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) กับ DeepAgent Code:
...
คุณยังสามารถปรับแต่งโมเดลผ่านการกำหนดค่า deepagent-code ของคุณได้ นี่คือตัวอย่างการระบุลำดับการกำหนดเส้นทางผู้ให้บริการ
...
:::tip
คุณสามารถใช้ผู้ให้บริการที่เข้ากันได้กับ OpenAI กับ deepagent-code ได้ ผู้ให้บริการ AI สมัยใหม่ส่วนใหญ่เสนอ API ที่เข้ากันได้กับ OpenAI
:::
...
4. สร้างหรืออัปเดตไฟล์ `deepagent-code.json` ของคุณในไดเร็กทอรีโครงการของคุณ:
...
ฟิลด์ `limit` ช่วยให้ DeepAgent Code เข้าใจว่าคุณมีบริบทเหลืออยู่เท่าใด ผู้ให้บริการมาตรฐานจะดึงข้อมูลเหล่านี้จาก models.dev โดยอัตโนมัติ
...
1. **ตรวจสอบการตั้งค่าการรับรองความถูกต้อง**: รัน `deepagent-code auth list` เพื่อดูว่ามีการเพิ่มข้อมูลรับรอง
...
2. สำหรับผู้ให้บริการที่กำหนดเอง ให้ตรวจสอบการกำหนดค่า deepagent-code และ:
   - ตรวจสอบให้แน่ใจว่า ID ผู้ให้บริการที่ใช้ในคำสั่ง `/connect` ตรงกับ ID ในการกำหนดค่า deepagent-code ของคุณ
...
4. OpenRouter หลายรุ่นถูกโหลดไว้ล่วงหน้าตามค่าเริ่มต้น ให้รันคำสั่ง `/models` เพื่อเลือกรุ่นที่คุณต้องการ
   ...
   คุณยังสามารถเพิ่มโมเดลเพิ่มเติมผ่านการกำหนดค่า deepagent-code ของคุณ
...
วิธีใช้ [Scaleway Generative API](https://www.scaleway.com/en/docs/generative-apis/) กับ DeepAgent Code:
...
คุณยังสามารถปรับแต่งโมเดลผ่านการกำหนดค่า deepagent-code ของคุณได้อีกด้วย นี่คือตัวอย่างการระบุลำดับการกำหนดเส้นทางของผู้ให้บริการ
...
:::tip
คุณสามารถใช้ผู้ให้บริการที่เข้ากันได้กับ OpenAI ด้วย deepagent-code ผู้ให้บริการ AI สมัยใหม่ส่วนใหญ่เสนอ API ที่เข้ากันได้กับ OpenAI
:::
...
4. สร้างหรืออัปเดตไฟล์ `deepagent-code.json` ในไดเรกทอรีโครงการของคุณ:
...
ช่อง `limit` ช่วยให้ DeepAgent Code เข้าใจบริบทที่คุณเหลืออยู่ ผู้ให้บริการมาตรฐานจะดึงข้อมูลเหล่านี้จาก models.dev โดยอัตโนมัติ
...
1. **ตรวจสอบการตั้งค่าการรับรองความถูกต้อง**: เรียกใช้ `deepagent-code auth list` เพื่อดูว่ามีข้อมูลรับรองหรือไม่
...
2. สำหรับผู้ให้บริการแบบกำหนดเอง ให้ตรวจสอบการกำหนดค่า deepagent-code และ:
   - ตรวจสอบให้แน่ใจว่า ID ผู้ให้บริการที่ใช้ในคำสั่ง `/connect` ตรงกับ ID ในการกำหนดค่า deepagent-code ของคุณ
