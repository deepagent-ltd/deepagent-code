---
title: 設定
description: 使用 DeepAgent Code JSON 設定。
---

您可以使用 JSON 設定檔來設定 DeepAgent Code。

---

## 格式

DeepAgent Code 支援 **JSON** 和 **JSONC**（帶註解的 JSON）格式。
...
1. **遠端設定**（來自 `.well-known/deepagent-code`）- 組織預設值
2. **全域設定**（`~/.config/deepagent-code/deepagent-code.json`）- 使用者偏好
3. **自訂設定**（`DEEPAGENT_CODE_CONFIG` 環境變數）- 自訂覆寫
4. **專案設定**（專案中的 `deepagent-code.json`）- 專案特定設定
5. **`.deepagent-code` 目錄** - 代理、指令、外掛程式
6. **內嵌設定**（`DEEPAGENT_CODE_CONFIG_CONTENT` 環境變數）- 執行時覆寫

這意味著專案設定可以覆寫全域預設值，全域設定可以覆寫遠端組織預設值。

:::note
`.deepagent-code` 和 `~/.config/deepagent-code` 目錄的子目錄使用**複數名稱**：`agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/` 和 `themes/`。為了向後相容，也支援單數名稱（例如 `agent/`）。
:::
...
組織可以透過 `.well-known/deepagent-code` 端點提供預設設定。當您使用支援該功能的供應商進行身分驗證時，會自動擷取此設定。
...
將全域 DeepAgent Code 設定放在 `~/.config/deepagent-code/deepagent-code.json` 中。使用全域設定來設定使用者層級的偏好，例如主題、供應商或快捷鍵。
...
在專案根目錄中新增 `deepagent-code.json`。專案設定在標準設定檔中具有最高優先級——它會覆寫全域設定和遠端設定。

:::tip
將專案特定設定放在專案的根目錄中。
:::

當 DeepAgent Code 啟動時，它會在當前目錄中尋找設定檔，或向上遍歷到最近的 Git 目錄。
...
使用 `DEEPAGENT_CODE_CONFIG` 環境變數指定自訂設定檔路徑。
...
使用 `DEEPAGENT_CODE_CONFIG_DIR` 環境變數指定自訂設定目錄。該目錄會像標準 `.deepagent-code` 目錄一樣被搜尋代理、指令、模式和外掛程式，並且應遵循相同的結構。
...
自訂目錄在全域設定和 `.deepagent-code` 目錄之後載入，因此**可以覆寫**它們的設定。

---

## Schema

設定檔具有在 [**`ai.deepagent.ltd/config.schema.json`**](https://ai.deepagent.ltd/config.schema.json) 中定義的 Schema。
...
使用 `DEEPAGENT_CODE_TUI_CONFIG` 指向自訂 TUI 設定檔。

`deepagent-code.json` 中的舊版 `theme`、`keybinds` 和 `tui` 鍵已被棄用，並將在可能的情況下自動遷移。
...
您可以透過 `server` 選項為 `deepagent-code serve` 和 `deepagent-code web` 指令設定伺服器設定。
...
- `mdns` - 啟用 mDNS 服務探索。這允許網路上的其他裝置發現您的 DeepAgent Code 伺服器。
- `mdnsDomain` - mDNS 服務的自訂網域名稱。預設為 `deepagent-code.local`。適用於在同一網路上執行多個實例的情境。
...
您可以透過 `provider`、`model` 和 `small_model` 選項在 DeepAgent Code 設定中設定要使用的供應商和模型。
...
`small_model` 選項為標題生成等輕量級任務設定單獨的模型。預設情況下，如果您的供應商有更便宜的模型可用，DeepAgent Code 會嘗試使用該模型，否則會回退到您的主模型。
...
您還可以使用 `~/.config/deepagent-code/agents/` 或 `.deepagent-code/agents/` 中的 Markdown 檔案定義代理。[在此了解更多](/docs/agents)。
...
預設代理必須是主代理（不能是子代理）。可以是內建代理（如 `"build"` 或 `"plan"`），也可以是您定義的[自訂代理](/docs/agents)。如果指定的代理不存在或是子代理，DeepAgent Code 將回退到 `"build"` 並發出警告。

此設定適用於所有介面：TUI、CLI（`deepagent-code run`）、桌面應用程式和 GitHub Action。
...
您還可以使用 `~/.config/deepagent-code/commands/` 或 `.deepagent-code/commands/` 中的 Markdown 檔案定義指令。[在此了解更多](/docs/commands)。
...
DeepAgent Code 啟動時會自動下載新版本。您可以使用 `autoupdate` 選項停用此功能。
...
預設情況下，DeepAgent Code **允許所有操作**，無需明確批准。您可以使用 `permission` 選項變更此行為。
...
[外掛程式](/docs/plugins)透過自訂工具、掛鉤和整合來擴展 DeepAgent Code。

將外掛程式檔案放置在 `.deepagent-code/plugins/` 或 `~/.config/deepagent-code/plugins/` 中。您還可以透過 `plugin` 選項從 npm 載入外掛程式。
...
當您希望限制 DeepAgent Code 僅使用特定供應商，而不是逐一停用其他供應商時，此選項非常有用。
