...
建立 `.deepagent-code/commands/test.md`：
...
您可以透過 DeepAgent Code 設定或在 `commands/` 目錄中建立 markdown 檔案來新增自訂指令。

---

### JSON

在 DeepAgent Code [設定](/docs/config)中使用 `command` 選項：
...
- 全域：`~/.config/deepagent-code/commands/`
- 專案層級：`.deepagent-code/commands/`
...
DeepAgent Code 包含多個內建指令，如 `/init`、`/undo`、`/redo`、`/share`、`/help`；[了解更多](/docs/tui#commands)。
