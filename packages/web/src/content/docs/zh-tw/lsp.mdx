---
title: LSP 伺服器
description: DeepAgent Code 與您的 LSP 伺服器整合。
---

DeepAgent Code 可以與語言伺服器協定（LSP）伺服器整合，將診斷資訊作為 agent 的回饋。

---

## 內建支援

DeepAgent Code 內建了多種適用於主流語言的 LSP 伺服器：
...
:::note
您可以將 `DEEPAGENT_CODE_DISABLE_LSP_DOWNLOAD` 環境變數設定為 `true` 來停用 LSP 伺服器的自動下載。
:::
...
啟用 LSP 且 DeepAgent Code 開啟檔案時，它會：
...
您可以透過 DeepAgent Code 設定檔中的 `lsp` 部分來啟用並自訂 LSP 伺服器。
