---
title: 分享
description: 分享您的 DeepAgent Code 對話。
---

DeepAgent Code 的分享功能允許您建立指向 DeepAgent Code 對話的公開連結，方便與團隊成員協作或向他人尋求幫助。
...
DeepAgent Code 支援三種分享模式，用於控制對話的共享方式：

---

### 手動模式（預設）

預設情況下，DeepAgent Code 使用手動分享模式。工作階段不會自動共享，但您可以使用 `/share` 指令手動分享：
...
要在團隊中對特定專案強制執行此設定，請將其新增到專案的 `deepagent-code.json` 檔案中並提交到 Git。
...
[了解更多](/docs/enterprise)關於在您的組織中使用 DeepAgent Code 的資訊。
