...
With DeepAgent Code you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, DeepAgent Code uses our own `deepagent-code` theme.
...
DeepAgent Code comes with several built-in themes.
...
- Want DeepAgent Code to match their terminal's appearance
...
DeepAgent Code supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/deepagent-code/themes/*.json` or `$XDG_CONFIG_HOME/deepagent-code/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.deepagent-code/themes/*.json`
4. **Current working directory** - Defined in `./.deepagent-code/themes/*.json`
