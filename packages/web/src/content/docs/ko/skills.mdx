...
Agent Skill let deepagent-code discover reusable instruction from your repo 또는 홈 디렉토리.
...
deepagent-code 이 위치를 검색:

- 프로젝트 구성: `.deepagent-code/skills/<name>/SKILL.md`
- 글로벌 구성: `~/.config/deepagent-code/skills/<name>/SKILL.md`
...
Project-local paths의 경우, deepagent-code는 git worktree에 도달 할 때까지 현재 작업 디렉토리에서 걷습니다.
그것은 `skills/*/SKILL.md`에 있는 어떤 어울리는 `.deepagent-code/` 및 어떤 어울리는 `.claude/skills/*/SKILL.md` 또는 `.agents/skills/*/SKILL.md`를 방법 적재합니다.

세계적인 정의는 또한 `~/.config/deepagent-code/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` 및 `~/.agents/skills/*/SKILL.md`에서 적재됩니다.
...
이처럼 `.deepagent-code/skills/git-release/SKILL.md` 만들기:
...
deepagent-code는 `skill` 도구 설명에서 사용할 수있는 기술을 나열합니다.
...
기술 에이전트가 `deepagent-code.json`의 패턴 기반 권한을 사용하여 액세스 할 수있는 제어 :
...
** 내장 에이전트 ** (`deepagent-code.json`에서):
