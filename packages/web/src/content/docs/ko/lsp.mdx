---
title: LSP 서버
description: DeepAgent Code는 LSP 서버와 통합됩니다.
---

DeepAgent Code는 Language Server Protocol(LSP) 서버와 통합해 진단 정보를 agent의 피드백으로 사용할 수 있습니다.

---

## 기본 제공 (Built-in)

DeepAgent Code는 널리 사용되는 언어를 위해 여러 built-in LSP 서버를 제공합니다.
...
:::note
`DEEPAGENT_CODE_DISABLE_LSP_DOWNLOAD` 환경 변수를 `true`로 설정하면 LSP 서버 자동 다운로드를 비활성화할 수 있습니다.
:::
...
LSP가 활성화되어 있고 DeepAgent Code가 파일을 열면 다음과 같이 동작합니다.
...
DeepAgent Code config의 `lsp` 섹션에서 LSP 서버를 활성화하고 사용자 정의할 수 있습니다.
