...
도구는 LLM을 사용하여 코드베이스에서 작업을 수행 할 수 있습니다. deepagent-code는 내장 도구 세트로 제공되지만, [custom tools](/docs/custom-tools) 또는 [MCP 서버](/docs/mcp-servers)로 확장할 수 있습니다.
...
다음은 deepagent-code에서 사용할 수있는 모든 내장 도구입니다.
...
:::note
이 도구는 `DEEPAGENT_CODE_EXPERIMENTAL_LSP_TOOL=true` (또는 `DEEPAGENT_CODE_EXPERIMENTAL=true`) 때만 유효합니다.
:::
...
:::note
이 도구는 deepagent-code 제공 업체 또는 `DEEPAGENT_CODE_ENABLE_EXA` 환경 변수가 모든 진실 값 (예 : `true` 또는 `1`)로 설정할 때만 사용할 수 있습니다.

deepagent-code를 실행할 때 활성화:
