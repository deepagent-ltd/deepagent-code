---
title: Providers
description: Using any LLM provider in DeepAgent Code.
---

import config from "../../../config.mjs"
export const console = config.console

DeepAgent Code uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your DeepAgent Code config.
...
in `~/.deepagent/code/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your DeepAgent Code
config.
...
```json title="deepagent-code.json" {6}
{
...
DeepAgent Code Zen is a list of models provided by the DeepAgent Code team that have been
tested and verified to work well with DeepAgent Code. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with DeepAgent Code Zen.
:::

1. Run the `/connect` command in the TUI, select `DeepAgent Code Zen`, and head to [ai.deepagent.ltd/auth](https://ai.deepagent.ltd/zen).
...
It works like any other provider in DeepAgent Code and is completely optional to use.

---

## DeepAgent Code Go

DeepAgent Code Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the DeepAgent Code team that have been
tested and verified to work well with DeepAgent Code.

1. Run the `/connect` command in the TUI, select `DeepAgent Code Go`, and head to [ai.deepagent.ltd/auth](https://ai.deepagent.ltd/zen).
...
To use Amazon Bedrock with DeepAgent Code:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running deepagent-code:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY deepagent-code

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile deepagent-code

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX deepagent-code
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `deepagent-code.json`:
...
DeepAgent Code. Anthropic explicitly prohibits this.

Previous versions of DeepAgent Code came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in DeepAgent Code with zero setup:
...
You can configure deepagent-code to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="deepagent-code.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://ai.deepagent.ltd/config.schema.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for deepagent-code to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX deepagent-code
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX deepagent-code
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your deepagent-code config.
...
DeepAgent Code supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; DeepAgent Code uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize DeepAgent Code. Sign in and approve.

   :::note
   DeepAgent Code requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, DeepAgent Code surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — DeepAgent Code forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in DeepAgent Code is experimental. Features, configuration, and
behavior may change in future releases.
:::

DeepAgent Code integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
2. Choose your authentication method:
   ...
   3. Name: `DeepAgent Code`, Scopes: `api`
...
to store token in deepagent-code auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
DeepAgent Code uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock DeepAgent Code
to only use your own GitLab-hosted instance, add the following to your
`deepagent-code.json` file. It is also recommended to disable session sharing.
...
More documentation on [deepagent-code-gitlab-auth](https://www.npmjs.com/package/deepagent-code-gitlab-auth) homepage.

##### Configuration

Customize through `deepagent-code.json`:
...
When a `duo-workflow-*` model is selected, DeepAgent Code will:
...
4. Route tool execution requests through DeepAgent Code's permission-gated tool system
...
{
  "$schema": "https://ai.deepagent.ltd/config.schema.json",
  "plugin": ["deepagent-code-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with deepagent-code:
...
To use Google Vertex AI with DeepAgent Code:
...
2. Set the required environment variables:
   ...
   Set them while running deepagent-code.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id deepagent-code
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through deepagent-code, you can always configure it yourself.
...
```jsonc title="~/.config/deepagent-code/deepagent-code.jsonc"
{
  "$schema": "https://ai.deepagent.ltd/config.schema.json",
  "provider": {
    "helicone": {
    ...
    "helicone": {
      ...
      "options": {
        ...
        "headers": {
          ...
          "Helicone-User-Id": "deepagent-code",
        },
      },
    },
  },
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [deepagent-code-helicone-session](https://github.com/H2Shami/deepagent-code-helicone-session) plugin to automatically log each DeepAgent Code conversation as a session in Helicone.

```bash
npm install -g deepagent-code-helicone-session
```
...
```json title="deepagent-code.json"
{
  "plugin": ["deepagent-code-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each DeepAgent Code conversation listed as a separate session.
...
You can configure deepagent-code to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="deepagent-code.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://ai.deepagent.ltd/config.schema.json",
  "provider": {
    "llama.cpp": {
...
You can configure deepagent-code to use local models through LM Studio.

```json title="deepagent-code.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://ai.deepagent.ltd/config.schema.json",
  "provider": {
    "lmstudio": {
...
You can configure deepagent-code to use local models through Ollama.

:::tip
Ollama can automatically configure itself for DeepAgent Code. See the [Ollama integration docs](https://docs.ollama.com/integrations/deepagent-code) for details.
:::

```json title="deepagent-code.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://ai.deepagent.ltd/config.schema.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with DeepAgent Code:
...
3. Copy the API key for use in DeepAgent Code.
...
6. **Important**: Before using cloud models in DeepAgent Code, you must pull the model information locally:
...
DeepAgent Code Zen is a list of tested and verified models provided by the DeepAgent Code team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>DeepAgent Code Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **DeepAgent Code Zen**.

   ```txt
   /connect
   ```

3. Enter your DeepAgent Code API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your deepagent-code config.
...
5. You can also customize them through your deepagent-code config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' deepagent-code
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group deepagent-code
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with DeepAgent Code:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=deepagent-code&utm_medium=docs&utm_campaign=cortex-provider).

deepagent-code's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
Alternatively, set environment variables before starting deepagent-code:
...
The model catalog is provided automatically. A minimal `deepagent-code.json` is all that's needed:
...
You can also customize models through your deepagent-code config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. DeepAgent Code opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
DeepAgent Code refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when DeepAgent Code is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running DeepAgent Code — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. DeepAgent Code prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. DeepAgent Code polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with deepagent-code. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in deepagent-code.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `deepagent-code.json` file in your project directory:

   ```json title="deepagent-code.json" ""myprovider"" {5-15}
   {
     "$schema": "https://ai.deepagent.ltd/config.schema.json",
     "provider": {
       "myprovider": {
...
```json title="deepagent-code.json" {9,11,17-20}
{
  "$schema": "https://ai.deepagent.ltd/config.schema.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow DeepAgent Code to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `deepagent-code auth list` to see if the credentials
...
2. For custom providers, check the deepagent-code config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your deepagent-code config.
