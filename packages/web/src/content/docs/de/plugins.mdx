---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um DeepAgent Code zu erweitern.
---

Mit Plugins können Sie DeepAgent Code erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von DeepAgent Code zu ändern.
...
- `.deepagent-code/plugins/` – Plugins auf Projektebene
- `~/.config/deepagent-code/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/deepagent-code/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/deepagent-code/deepagent-code.json`)
2. Projektkonfiguration (`deepagent-code.json`)
3. Globales Plugin-Verzeichnis (`~/.config/deepagent-code/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.deepagent-code/plugins/`)
...
DeepAgent Code führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein DeepAgent Code-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie DeepAgent Code erweitern können.
...
:::note
Wenn Sie die DeepAgent Code-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass DeepAgent Code `.env`-Dateien liest:
...
Plugins können DeepAgent Code auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das DeepAgent Code aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für DeepAgent Code zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://deepagent-code.ai/docs/sdk).
