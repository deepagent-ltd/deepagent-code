---
title: LSP Server
description: DeepAgent Code lässt sich in Ihren LSP-Server integrieren.
---

DeepAgent Code kann sich in Language Server Protocol (LSP)-Server integrieren, um Diagnosen als Feedback für den Agenten zu verwenden.

---

## Integriert

DeepAgent Code verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `DEEPAGENT_CODE_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn LSP aktiviert ist und DeepAgent Code eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer DeepAgent Code-Konfiguration aktivieren und anpassen.
