...
Sie können externe Tools zu DeepAgent Code hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. DeepAgent Code unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [DeepAgent Code Config](https://deepagent-code.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/deepagent-code`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
DeepAgent Code übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird DeepAgent Code Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von DeepAgent Code beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `deepagent-code mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert DeepAgent Code die Token sicher in `~/.deepagent/code/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in DeepAgent Code verfügbar. Sie können sie also wie jedes andere Tool über die DeepAgent Code-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und DeepAgent Code mit Ihrem Sentry-Konto zu verbinden.
