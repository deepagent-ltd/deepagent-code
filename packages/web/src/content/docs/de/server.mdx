---
title: Server
description: Nutze den deepagent-code-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `deepagent-code serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein deepagent-code-Client nutzen kann.
...
Setze `DEEPAGENT_CODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `deepagent-code`, kann aber mit `DEEPAGENT_CODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `deepagent-code serve` und `deepagent-code web`.
...
Wenn du `deepagent-code` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den deepagent-code-Server, um deepagent-code programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `deepagent-code serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `deepagent-code serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die DeepAgent Code-[IDE](/docs/ide)-Plugins.
...
Der deepagent-code-Server stellt folgende APIs bereit.
