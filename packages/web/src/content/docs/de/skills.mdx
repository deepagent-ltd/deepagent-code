...
Agent Skills erlauben DeepAgent Code, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
DeepAgent Code sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft DeepAgent Code vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.deepagent-code/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/deepagent-code/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.deepagent-code/skills/git-release/SKILL.md` zum Beispiel so:
...
DeepAgent Code listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `deepagent-code.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `deepagent-code.json`):
