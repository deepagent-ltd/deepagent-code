---
title: TUI
description: Arbeite mit DeepAgent Code in der Terminal-Oberflaeche.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

DeepAgent Code bietet eine interaktive Terminaloberflaeche (TUI), um mit einem LLM an deinen Projekten zu arbeiten.

Ohne weitere Argumente startet DeepAgent Code die TUI im aktuellen Verzeichnis.
...
In der DeepAgent Code-TUI kannst du `/` plus Befehlsnamen eingeben, um Aktionen schnell auszufuehren.
Zum Beispiel:
...
Fuegt einen Provider zu DeepAgent Code hinzu. Erlaubt die Auswahl aus verfuegbaren Providern und das Hinzufuegen ihrer API-Keys.
...
Beendet DeepAgent Code. _Aliase_: `/quit`, `/q`
...
Dies ist getrennt von `deepagent-code.json`, welche das Server-/Runtime-Verhalten konfiguriert.
...
- `leader_timeout` - Steuert, wie lange DeepAgent Code nach der leader key wartet. Standard ist `2000`.
...
Verwende `DEEPAGENT_CODE_TUI_CONFIG`, um einen benutzerdefinierten TUI-Konfigurationspfad zu laden.

### Attention

Attention lässt das TUI dich benachrichtigen, wenn DeepAgent Code auf eine Antwort wartet, eine Berechtigung bestätigt werden muss, ein Sitzungsfehler aufgetreten ist oder eine Sitzung abgeschlossen wurde. Aktiviere es mit `attention.enabled`; integrierte Ereignisse spielen beim Auftreten einen Sound ab. Desktop-Benachrichtigungen werden nur gesendet, wenn das Terminalfenster nicht fokussiert ist, und nicht für subagent-Ereignisse verwendet.
...
- `sound_pack` - Zu verwendende sound pack ID. Standard ist `deepagent-code.default`.
