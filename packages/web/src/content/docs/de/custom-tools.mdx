...
description: Erstellen Sie Tools, die der LLM in DeepAgent Code aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von DeepAgent Code wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.deepagent-code/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/deepagent-code/tools/` platzieren.
