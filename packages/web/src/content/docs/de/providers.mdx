---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in DeepAgent Code.
---

import config from "../../../../config.mjs"
export const console = config.console

DeepAgent Code verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer DeepAgent Code-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem DeepAgent Code anpassen
config.
...
DeepAgent Code Zen ist eine Liste von Modellen, die vom DeepAgent Code-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit DeepAgent Code funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit DeepAgent Code Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie DeepAgent Code aus und gehen Sie zu [deepagent-code.ai/auth](https://deepagent-code.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in DeepAgent Code und ist völlig optional.

---

## DeepAgent Code Go

DeepAgent Code Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom DeepAgent Code-Team getestet und verifiziert wurden, dass sie gut mit DeepAgent Code funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `DeepAgent Code Go` und gehen Sie zu [deepagent-code.ai/auth](https://deepagent-code.ai/zen).
...
So verwenden Sie Amazon Bedrock mit DeepAgent Code:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie DeepAgent Code ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `deepagent-code.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in DeepAgent Code wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können deepagent-code so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit DeepAgent Code ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre DeepAgent Code-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `DeepAgent Code`, Bereiche: `api`
...
um das Token im DeepAgent Code-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
DeepAgent Code verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von DeepAgent Code
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`deepagent-code.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [deepagent-code-gitlab-auth](https://www.npmjs.com/package/deepagent-code-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `deepagent-code.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit DeepAgent Code:
...
So verwenden Sie Google Vertex AI mit DeepAgent Code:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie DeepAgent Code ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über DeepAgent Code konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [deepagent-code-helicone-session](https://github.com/H2Shami/deepagent-code-helicone-session)-Plugin, um jede DeepAgent Code-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede DeepAgent Code-Konversation als separate Sitzung aufgeführt.
...
Sie können DeepAgent Code für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können DeepAgent Code über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können DeepAgent Code über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für DeepAgent Code konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/deepagent-code).
:::
...
So verwenden Sie Ollama Cloud mit DeepAgent Code:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in DeepAgent Code.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in DeepAgent Code verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
DeepAgent Code Zen ist eine Liste getesteter und verifizierter Modelle, die vom DeepAgent Code-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>DeepAgent Code Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **DeepAgent Code Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren DeepAgent Code API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre DeepAgent Code-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre DeepAgent Code-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit DeepAgent Code:
...
Sie können Modelle auch über Ihre DeepAgent Code-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit DeepAgent Code verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `deepagent-code.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann DeepAgent Code erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `deepagent-code auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die DeepAgent Code-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer DeepAgent Code-Konfiguration übereinstimmt.
