...
DeepAgent Code bringt eingebaute Tools mit und laesst sich ueber [Custom Tools](/docs/custom-tools) oder [MCP-Server](/docs/mcp-servers) erweitern.
...
Hier sind alle in DeepAgent Code verfuegbaren eingebauten Tools.
...
:::note
Dieses Tool ist nur verfuegbar, wenn `DEEPAGENT_CODE_EXPERIMENTAL_LSP_TOOL=true` (oder `DEEPAGENT_CODE_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der DeepAgent Code-Provider genutzt wird oder die Umgebungsvariable `DEEPAGENT_CODE_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.

Zum Aktivieren beim Start von DeepAgent Code:
