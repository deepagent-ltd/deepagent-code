---
title: Regeln
description: Lege eigene Anweisungen fuer deepagent-code fest.
---

Du kannst deepagent-code ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in deepagent-code den Befehl `/init` aus.
...
So kann deepagent-code sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht deepagent-code sie zu erweitern.
...
deepagent-code kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/deepagent-code/AGENTS.md` speichern.
Diese gelten in allen deepagent-code-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt DeepAgent Code dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/deepagent-code/AGENTS.md` existiert)
...
Beim Start sucht deepagent-code in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/deepagent-code/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `deepagent-code.json` oder in `~/.config/deepagent-code/deepagent-code.json` angeben.
...
deepagent-code parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit deepagent-code.json

Empfohlen ist das `instructions`-Feld in der `deepagent-code.json`:
...
Du kannst deepagent-code explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `deepagent-code.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
