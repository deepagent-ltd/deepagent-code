...
Mit DeepAgent Code kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet DeepAgent Code das `deepagent-code`-Theme.
...
DeepAgent Code liefert mehrere eingebaute Themes mit.
...
- DeepAgent Code optisch ans Terminal anpassen wollen
...
DeepAgent Code unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/deepagent-code/themes/*.json` oder `$XDG_CONFIG_HOME/deepagent-code/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.deepagent-code/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.deepagent-code/themes/*.json`
