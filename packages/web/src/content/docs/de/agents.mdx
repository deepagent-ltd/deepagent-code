...
Es gibt zwei Arten von Agenten in DeepAgent Code; Primäragenten und Subagenten.
...
DeepAgent Code verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `deepagent-code.json`-Konfigurationsdatei:
...
- Pro Projekt: `.deepagent-code/agents/`
...
Wenn keine Temperatur angegeben ist, verwendet DeepAgent Code modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale DeepAgent Code-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer DeepAgent Code-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [DeepAgent Code Zen](/docs/zen) verwenden, würden Sie `deepagent-code/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `deepagent-code models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/deepagent-ltd/deepagent-code).
:::
