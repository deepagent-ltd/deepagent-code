---
title: Konfiguration
description: Verwenden Sie die Konfiguration DeepAgent Code JSON.
---

Sie können DeepAgent Code mithilfe einer JSON-Konfigurationsdatei konfigurieren.

---

## Format

DeepAgent Code unterstützt die Formate **JSON** und **JSONC** (JSON mit Kommentaren).
...
Wenn Ihre globale Konfiguration beispielsweise `theme: "deepagent-code"` und `autoupdate: true` und Ihre Projektkonfiguration `model: "anthropic/claude-sonnet-4-5"` festlegt, umfasst die endgültige Konfiguration alle drei Einstellungen.
...
1. **Remote-Konfiguration** (von `.well-known/deepagent-code`) – Organisationsstandards
2. **Globale Konfiguration** (`~/.config/deepagent-code/deepagent-code.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`DEEPAGENT_CODE_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`deepagent-code.json` im Projekt) – projektspezifische Einstellungen
5. **`.deepagent-code` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`DEEPAGENT_CODE_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
:::note
Die Verzeichnisse `.deepagent-code` und `~/.config/deepagent-code` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Organisationen können die Standardkonfiguration über den Endpunkt `.well-known/deepagent-code` bereitstellen. Dieser wird automatisch abgerufen, wenn Sie sich bei einem Anbieter authentifizieren, der dies unterstützt.
...
Platzieren Sie Ihre globale DeepAgent Code-Konfiguration in `~/.config/deepagent-code/deepagent-code.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `deepagent-code.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.

:::tip
Platzieren Sie die projektspezifische Konfiguration im Stammverzeichnis Ihres Projekts.
:::

Wenn DeepAgent Code gestartet wird, sucht es im aktuellen Verzeichnis nach einer Konfigurationsdatei oder durchsucht das nächstgelegene Git-Verzeichnis.
...
Geben Sie mithilfe der Umgebungsvariablen `DEEPAGENT_CODE_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `DEEPAGENT_CODE_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
Umgebungsvariable. Dieses Verzeichnis wird nach Agenten, Befehlen,
Modi und Plugins genau wie das Standardverzeichnis `.deepagent-code` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.deepagent-code` geladen, sodass es deren Einstellungen **überschreiben** kann.

---

## Schema

Die Konfigurationsdatei verfügt über ein Schema, das in [**`ai.deepagent.ltd/config.schema.json`**](https://ai.deepagent.ltd/config.schema.json) definiert ist.
...
Sie können Servereinstellungen für die Befehle `deepagent-code serve` und `deepagent-code web` über die Option `server` konfigurieren.
...
- `mdns` – mDNS-Diensterkennung aktivieren. Dadurch können andere Geräte im Netzwerk Ihres DeepAgent Code-Servers erkannt werden.
- `mdnsDomain` – Benutzerdefinierter Domänenname für den mDNS-Dienst. Standardmäßig ist `deepagent-code.local`. Nützlich für die Ausführung mehrerer Instanzen im selben Netzwerk.
...
Sie können die Anbieter und Modelle, die Sie in Ihrer DeepAgent Code-Konfiguration verwenden möchten, über die Optionen `provider`, `model` und `small_model` konfigurieren.
...
Die Option `small_model` konfiguriert ein separates Modell für einfache Aufgaben wie die Titelgenerierung. Standardmäßig versucht DeepAgent Code, ein günstigeres Modell zu verwenden, sofern eines bei Ihrem Anbieter verfügbar ist, andernfalls greift es auf Ihr Hauptmodell zurück.
...
Sie können das Thema, das Sie in Ihrer DeepAgent Code-Konfiguration verwenden möchten, über die Option `theme` konfigurieren.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/deepagent-code/agents/` oder `.deepagent-code/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](/docs/agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift DeepAgent Code mit einer Warnung auf `"build"` zurück.

Diese Einstellung gilt für alle Schnittstellen: TUI, CLI (`deepagent-code run`), Desktop-App und GitHub Aktion.
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/deepagent-code/commands/` oder `.deepagent-code/commands/` definieren. [Mehr erfahren](/docs/commands).
...
DeepAgent Code lädt beim Start automatisch alle neuen Updates herunter. Sie können dies mit der Option `autoupdate` deaktivieren.
...
DeepAgent Code erlaubt standardmäßig alle Vorgänge, ohne dass eine ausdrückliche Genehmigung erforderlich ist. Sie können dies mit der Option `permission` ändern.
...
[Plugins](/docs/plugins) erweitert DeepAgent Code mit benutzerdefinierten Tools, Hooks und Integrationen.

Platzieren Sie Ihre Plugin-Dateien in `.deepagent-code/plugins/` oder `~/.config/deepagent-code/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
...
Dies ist nützlich, wenn Sie DeepAgent Code darauf beschränken möchten, nur bestimmte Anbieter zu verwenden, anstatt sie einzeln zu deaktivieren.
