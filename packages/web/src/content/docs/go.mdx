...
DeepAgent Code Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

Go works like any other provider in DeepAgent Code. You subscribe to DeepAgent Code Go and
...
DeepAgent Code Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

DeepAgent Code Go works like any other provider in DeepAgent Code.

1. You sign in to **<a href={console}>DeepAgent Code Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `DeepAgent Code Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to DeepAgent Code Go.
:::
...
DeepAgent Code Go includes the following limits:
...
uses the format `deepagent-code-go/<model-id>`. For example, for Kimi K2.6, you would
use `deepagent-code-go/kimi-k2.6` in your config.
...
We created DeepAgent Code Go to:
...
4. Have **no lock-in** by allowing you to use any other provider with DeepAgent Code as well.
