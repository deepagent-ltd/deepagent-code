...
С помощью deepagent-code вы можете выбрать одну из нескольких встроенных тем, использовать тему, которая адаптируется к теме вашего терминала, или определить свою собственную тему.

По умолчанию deepagent-code использует нашу собственную тему `deepagent-code`.
...
deepagent-code поставляется с несколькими встроенными темами.
...
- Хотите, чтобы deepagent-code соответствовал внешнему виду их терминала
...
deepagent-code поддерживает гибкую систему тем на основе JSON, которая позволяет пользователям легко создавать и настраивать темы.
...
2. **Каталог конфигурации пользователя** – определяется в `~/.config/deepagent-code/themes/*.json` или `$XDG_CONFIG_HOME/deepagent-code/themes/*.json`.
3. **Корневой каталог проекта** – определено в `<project-root>/.deepagent-code/themes/*.json`.
4. **Текущий рабочий каталог** – определено в `./.deepagent-code/themes/*.json`.
