---
title: Zen
description: Подобранный список моделей, предоставленный DeepAgent Code.
---
...
DeepAgent Code Zen — это список протестированных и проверенных моделей, предоставленный командой DeepAgent Code.

Zen работает как любой другой провайдер в DeepAgent Code. Вы входите в DeepAgent Code Zen и получаете
свой ключ API. Это **полностью необязательно**, и вам не нужно использовать его,
чтобы пользоваться DeepAgent Code.
...
:::tip
Мы протестировали выбранную группу моделей и провайдеров, которые хорошо работают с DeepAgent Code.
:::
...
DeepAgent Code Zen — это AI-шлюз, который дает вам доступ к этим моделям.

---

## Как это работает

DeepAgent Code Zen работает как любой другой провайдер в DeepAgent Code.

1. Вы входите в **<a href={console}>DeepAgent Code Zen</a>**, добавляете платежные
   данные и копируете свой ключ API.
2. Вы запускаете команду `/connect` в TUI, выбираете DeepAgent Code Zen и вставляете свой ключ API.
...
[идентификатор модели](/docs/config/#models) в вашей конфигурации DeepAgent Code
использует формат `deepagent-code/<model-id>`. Например, для GPT 5.5 вам нужно
использовать `deepagent-code/gpt-5.5` в своей конфигурации.
...
- DeepSeek V4 Flash Free доступна в DeepAgent Code ограниченное время. Команда использует это время, чтобы собирать отзывы и улучшать модель.
- MiMo-V2.5 Free доступна в DeepAgent Code ограниченное время. Команда использует это время, чтобы собирать отзывы и улучшать модель.
- Nemotron 3 Ultra Free доступна в DeepAgent Code ограниченное время. Команда использует это время, чтобы собирать отзывы и улучшать модель.
- Big Pickle — это скрытая модель, которая доступна бесплатно в DeepAgent Code ограниченное время. Команда использует это время, чтобы собирать отзывы и улучшать модель.
...
Мы создали DeepAgent Code Zen, чтобы:
...
4. Обеспечить **отсутствие привязки**, позволяя вам использовать его с любым другим агентом для программирования. И всегда позволять вам использовать любого другого провайдера с DeepAgent Code.
