---
title: Провайдеры
description: Использование любого провайдера LLM в deepagent-code.
---

import config from "../../../../config.mjs"
export const console = config.console

deepagent-code использует [AI SDK](https://ai-sdk.dev/) и [Models.dev](https://models.dev) для поддержки **более 75 поставщиков LLM** и поддерживает запуск локальных моделей.

Чтобы добавить провайдера, вам необходимо:

1. Добавьте ключи API для провайдера с помощью команды `/connect`.
2. Настройте провайдера в вашей конфигурации deepagent-code.
...
в `~/.deepagent/code/auth.json`.

---

### Настройка

Вы можете настроить поставщиков через раздел `provider` в вашем deepagent-code.
конфиг.
...
DeepAgent Code Zen — это список моделей, предоставленный командой deepagent-code, которые были
протестировано и проверено на хорошую работу с deepagent-code. [Подробнее](/docs/zen).

:::tip
Если вы новичок, мы рекомендуем начать с DeepAgent Code Zen.
:::

1. Запустите команду `/connect` в TUI, выберите `DeepAgent Code Zen` и перейдите по адресу [deepagent-code.ai/auth](https://deepagent-code.ai/zen).
...
Он работает как любой другой поставщик в deepagent-code и его использование совершенно необязательно.

---

## DeepAgent Code Go

DeepAgent Code Go — это недорогой план подписки, обеспечивающий надежный доступ к популярным открытым моделям кодирования, предоставляемым командой deepagent-code, которые были
протестированы и проверены на хорошую работу с deepagent-code.

1. Запустите команду `/connect` в TUI, выберите `DeepAgent Code Go` и перейдите по адресу [deepagent-code.ai/auth](https://deepagent-code.ai/zen).
...
Чтобы использовать Amazon Bedrock с deepagent-code:
...
2. **Настройте аутентификацию** одним из следующих способов:
   ...
   Установите одну из этих переменных среды при запуске deepagent-code:
   ...
   Для конкретной или постоянной конфигурации проекта используйте `deepagent-code.json`:
...
:::info
Использование вашей подписки Claude Pro/Max в deepagent-code официально не поддерживается [Anthropic](https://anthropic.com).
:::
...
Вы можете настроить deepagent-code для работы с локальными моделями через [Atomic Chat](https://atomic.chat) — десктопное приложение, которое запускает локальные LLM за OpenAI-совместимым API-сервером (конечная точка по умолчанию `http://127.0.0.1:1337/v1`).
...
2. Перейдите в [Azure AI Foundry](https://ai.azure.com/) и разверните модель.
   ...
   :::примечание
   Для правильной работы deepagent-code имя развертывания должно совпадать с именем модели.
   :::
...
5. Запустите команду `/models`, чтобы выбрать модель.
   ...
   Вы также можете добавлять модели через конфигурацию deepagent-code.
...
2. Выберите метод аутентификации:
   ...
   3. Имя: `DeepAgent Code`, Области применения: `api`
...
для хранения токена в хранилище аутентификации deepagent-code.
:::

##### Самостоятельная GitLab

:::note[примечание о соответствии]
deepagent-code использует небольшую модель для некоторых задач ИИ, таких как создание заголовка сеанса.
По умолчанию он настроен на использование gpt-5-nano, размещенного на Zen. Чтобы заблокировать deepagent-code
чтобы использовать только свой собственный экземпляр, размещенный на GitLab, добавьте следующее в свой
`deepagent-code.json` файл. Также рекомендуется отключить совместное использование сеансов.
...
Дополнительная документация на домашней странице [deepagent-code-gitlab-auth](https://www.npmjs.com/package/deepagent-code-gitlab-auth).

##### Конфигурация

Настройте через `deepagent-code.json`:
...
Чтобы использовать Google Vertex AI с deepagent-code:
...
2. Установите необходимые переменные среды:
   ...
   Установите их во время запуска deepagent-code.
...
Если вы видите функцию или модель от Helicone, которая не настраивается автоматически через deepagent-code, вы всегда можете настроить ее самостоятельно.
...
Функция Helicone [Sessions](https://docs.helicone.ai/features/sessions) позволяет группировать связанные запросы LLM вместе. Используйте плагин [deepagent-code-helicone-session](https://github.com/H2Shami/deepagent-code-helicone-session), чтобы автоматически регистрировать каждый диалог deepagent-code как сеанс в Helicone.
...
Плагин вставляет в ваши запросы заголовки `Helicone-Session-Id` и `Helicone-Session-Name`. На странице «Сеансы» Helicone вы увидите каждый диалог deepagent-code, указанный как отдельный сеанс.
...
Вы можете настроить deepagent-code для использования локальных моделей с помощью [утилиты llama-server llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Вы можете настроить deepagent-code для использования локальных моделей через LM Studio.
...
Вы можете настроить deepagent-code для использования локальных моделей через Ollama.

:::tip
Ollama может автоматически настроиться для deepagent-code. Подробности см. в документации по интеграции Ollama](https://docs.ollama.com/integrations/deepagent-code).
:::
...
Чтобы использовать Ollama Cloud с deepagent-code:
...
3. Скопируйте ключ API для использования в deepagent-code.
...
6. **Важно**. Перед использованием облачных моделей в deepagent-code необходимо получить информацию о модели локально:
...
DeepAgent Code Zen — это список протестированных и проверенных моделей, предоставленный командой deepagent-code. [Подробнее](/docs/zen).

1. Войдите в систему **<a href={console}>DeepAgent Code Zen</a>** и нажмите **Создать ключ API**.

2. Запустите команду `/connect` и найдите **DeepAgent Code Zen**.

   ```txt
   /connect
   ```

3. Введите свой ключ API deepagent-code.
...
4. Многие модели OpenRouter предварительно загружены по умолчанию. Запустите команду `/models`, чтобы выбрать нужную.
   ...
   Вы также можете добавить дополнительные модели через конфигурацию deepagent-code.
...
5. Вы также можете настроить их через конфигурацию deepagent-code. Вот пример указания провайдера
...
Чтобы использовать [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) с deepagent-code:
...
Вы также можете настраивать модели через конфигурацию deepagent-code. Ниже приведен пример указания порядка маршрутизации поставщика.
...
4. Создайте или обновите файл `deepagent-code.json` в каталоге вашего проекта:
...
Поля `limit` позволяют deepagent-code понять, сколько контекста у вас осталось. Стандартные поставщики автоматически извлекают их из models.dev.
...
1. **Проверьте настройку аутентификации**: запустите `deepagent-code auth list`, чтобы проверить, верны ли учетные данные.
...
2. Для пользовательских поставщиков проверьте конфигурацию deepagent-code и:
   - Убедитесь, что идентификатор провайдера, используемый в команде `/connect`, соответствует идентификатору в вашей конфигурации deepagent-code.
