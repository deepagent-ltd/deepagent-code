---
title: Конфигурация
description: Использование конфигурации deepagent-code JSON.
---

Вы можете настроить deepagent-code, используя файл конфигурации JSON.

---

## Формат

deepagent-code поддерживает форматы **JSON** и **JSONC** (JSON с комментариями).
...
Например, если ваша глобальная конфигурация устанавливает `theme: "deepagent-code"` и `autoupdate: true`, а конфигурация вашего проекта устанавливает `model: "anthropic/claude-sonnet-4-5"`, окончательная конфигурация будет включать все три параметра.
...
1. **Удаленная конфигурация** (от `.well-known/deepagent-code`) – организационные настройки по умолчанию.
2. **Глобальная конфигурация** (`~/.config/deepagent-code/deepagent-code.json`) — настройки пользователя.
3. **Пользовательская конфигурация** (`DEEPAGENT_CODE_CONFIG` env var) – пользовательские переопределения
4. **Конфигурация проекта** (`deepagent-code.json` в проекте) — настройки, специфичные для проекта.
5. **Каталоги `.deepagent-code`** — агенты, команды, плагины
6. **Встроенная конфигурация** (`DEEPAGENT_CODE_CONFIG_CONTENT` env var) – переопределяет время выполнения
...
:::note
В каталогах `.deepagent-code` и `~/.config/deepagent-code` для подкаталогов используются **множественные имена**: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` и `themes/`. Единственные имена (например, `agent/`) также поддерживаются для обратной совместимости.
:::
...
Организации могут предоставить конфигурацию по умолчанию через конечную точку `.well-known/deepagent-code`. Он извлекается автоматически при аутентификации у провайдера, который его поддерживает.
...
Поместите глобальную конфигурацию deepagent-code в `~/.config/deepagent-code/deepagent-code.json`. Используйте глобальную конфигурацию для общепользовательских настроек, таких как темы, поставщики или привязки клавиш.

Для настроек, специфичных для TUI, используйте `~/.config/deepagent-code/tui.json`.
...
Добавьте `deepagent-code.json` в корень вашего проекта. Конфигурация проекта имеет наивысший приоритет среди стандартных файлов конфигурации — она переопределяет как глобальные, так и удаленные конфигурации.
...
Когда deepagent-code запускается, он ищет файл конфигурации в текущем каталоге или переходит к ближайшему каталогу Git.
...
Укажите собственный путь к файлу конфигурации, используя переменную среды `DEEPAGENT_CODE_CONFIG`.
...
Укажите пользовательский каталог конфигурации, используя переменную среды `DEEPAGENT_CODE_CONFIG_DIR`. В этом каталоге будет выполняться поиск агентов, команд, режимов и плагинов (аналогично стандартному каталогу `.deepagent-code`), и он должен иметь ту же структуру.
...
Пользовательский каталог загружается после каталогов global config и `.deepagent-code`, поэтому он **может переопределить** их настройки.

---

## Схема

Файл конфигурации имеет схему, определенную в [**`ai.deepagent.ltd/config.schema.json`**](https://ai.deepagent.ltd/config.schema.json).

Конфигурация TUI использует [**`ai.deepagent.ltd/tui.json`**](https://ai.deepagent.ltd/tui.json).
...
Используйте `DEEPAGENT_CODE_TUI_CONFIG`, чтобы указать на пользовательский файл конфигурации TUI.

Устаревшие ключи `theme`, `keybinds` и `tui` в `deepagent-code.json` устарели и автоматически переносятся, когда это возможно.
...
Вы можете настроить параметры сервера для команд `deepagent-code serve` и `deepagent-code web` с помощью опции `server`.
...
- `mdns` — включить обнаружение службы mDNS. Это позволит другим устройствам в сети обнаружить ваш сервер deepagent-code.
- `mdnsDomain` — собственное доменное имя для службы mDNS. По умолчанию `deepagent-code.local`. Полезно для запуска нескольких экземпляров в одной сети.
...
Вы можете настроить поставщиков и модели, которые хотите использовать в своей конфигурации deepagent-code, с помощью параметров `provider`, `model` и `small_model`.
...
Опция `small_model` настраивает отдельную модель для облегченных задач, таких как создание заголовков. По умолчанию deepagent-code пытается использовать более дешевую модель, если она доступна у вашего провайдера, в противном случае он возвращается к вашей основной модели.
...
Вы также можете определить агентов, используя файлы Markdown в `~/.config/deepagent-code/agents/` или `.deepagent-code/agents/`. [Подробнее здесь](/docs/agents).
...
Агент по умолчанию должен быть основным агентом (а не субагентом). Это может быть встроенный агент, например `"build"` или `"plan"`, или [пользовательский агент](/docs/agents), который вы определили. Если указанный агент не существует или является субагентом, deepagent-code вернется к `"build"` с предупреждением.

Этот параметр применяется ко всем интерфейсам: TUI, CLI (`deepagent-code run`), настольному приложению и действию GitHub.
...
Вы также можете определять команды, используя файлы Markdown в `~/.config/deepagent-code/commands/` или `.deepagent-code/commands/`. [Подробнее здесь](/docs/commands).
...
deepagent-code автоматически загрузит все новые обновления при запуске. Вы можете отключить это с помощью опции `autoupdate`.
...
По умолчанию deepagent-code **разрешает все операции**, не требуя явного разрешения. Вы можете изменить это, используя опцию `permission`.
...
[Плагины](/docs/plugins) расширяют deepagent-code с помощью пользовательских инструментов, перехватчиков и интеграций.

Поместите файлы плагина в `.deepagent-code/plugins/` или `~/.config/deepagent-code/plugins/`. Вы также можете загружать плагины из npm с помощью опции `plugin`.
...
Это полезно, если вы хотите ограничить deepagent-code использованием только определенных поставщиков, а не отключать их по одному.
