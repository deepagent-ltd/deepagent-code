---
title: Сервер
description: Взаимодействуйте с сервером deepagent-code через HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Команда `deepagent-code serve` запускает автономный HTTP-сервер, который предоставляет конечную точку OpenAPI, которую может использовать клиент с открытым кодом.
...
| `--mdns-domain` | Пользовательское доменное имя для mDNS      | `deepagent-code.local` |
...
Установите `DEEPAGENT_CODE_SERVER_PASSWORD`, чтобы защитить сервер с помощью базовой аутентификации HTTP. Имя пользователя по умолчанию — `deepagent-code` или установите `DEEPAGENT_CODE_SERVER_USERNAME`, чтобы переопределить его. Это относится как к `deepagent-code serve`, так и к `deepagent-code web`.
...
Когда вы запускаете `deepagent-code`, он запускает TUI и сервер. Где находится TUI
...
:::tip
Используйте сервер deepagent-code для программного взаимодействия с открытым кодом.
:::
...
Вы можете запустить `deepagent-code serve`, чтобы запустить автономный сервер. Если у вас есть
TUI с открытым кодом запущен, `deepagent-code serve` запустит новый сервер.
...
Конечную точку [`/tui`](#tui) можно использовать для управления TUI через сервер. Например, вы можете предварительно заполнить или запустить подсказку. Эта настройка используется плагинами deepagent-code [IDE](/docs/ide).
...
Сервер deepagent-code предоставляет следующие API.
