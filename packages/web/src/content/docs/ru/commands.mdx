...
Создайте `.deepagent-code/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию deepagent-code или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем deepagent-code [config](/docs/config):
...
- Глобальный: `~/.config/deepagent-code/commands/`
- Для каждого проекта: `.deepagent-code/commands/`
...
deepagent-code включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
