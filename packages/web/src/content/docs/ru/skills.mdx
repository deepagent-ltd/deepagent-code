...
Навыки агента позволяют deepagent-code обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
deepagent-code выполняет поиск в следующих местах:

- Конфигурация проекта: `.deepagent-code/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/deepagent-code/skills/<name>/SKILL.md`.
...
Для локальных путей проекта deepagent-code переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
Он загружает все соответствующие `skills/*/SKILL.md` в `.deepagent-code/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/deepagent-code/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.deepagent-code/skills/git-release/SKILL.md` следующим образом:
...
deepagent-code перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `deepagent-code.json`:
...
**Для встроенных агентов** (в формате `deepagent-code.json`):
