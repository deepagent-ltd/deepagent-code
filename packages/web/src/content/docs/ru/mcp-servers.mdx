...
Вы можете добавить внешние инструменты в deepagent-code, используя _Model Context Protocol_ или MCP. deepagent-code поддерживает как локальные, так и удаленные серверы.
...
Вы можете определить серверы MCP в своем [deepagent-code Config](https://deepagent-code.ai/docs/config/) в разделе `mcp`. Добавьте каждому MCP уникальное имя. Вы можете обратиться к этому MCP по имени при запросе LLM.
...
Организации могут предоставлять серверы MCP по умолчанию через свою конечную точку `.well-known/deepagent-code`. Эти серверы могут быть отключены по умолчанию, что позволяет пользователям выбирать те, которые им нужны.
...
deepagent-code автоматически обрабатывает аутентификацию OAuth для удаленных серверов MCP. Когда серверу требуется аутентификация, deepagent-code:
...
Если сервер требует аутентификации, deepagent-code предложит вам пройти аутентификацию при первой попытке его использования. Если нет, вы можете [вручную запустить поток ](#authenticating) с помощью `deepagent-code mcp auth <server-name>`.
...
Команда `mcp auth` откроет ваш браузер для авторизации. После того как вы авторизуетесь, deepagent-code надежно сохранит токены в `~/.deepagent/code/mcp-auth.json`.
...
Ваши MCP доступны в виде инструментов deepagent-code наряду со встроенными инструментами. Таким образом, вы можете управлять ими через конфигурацию deepagent-code, как и любым другим инструментом.
...
Откроется окно браузера для завершения процесса OAuth и подключения deepagent-code к вашей учетной записи Sentry.
