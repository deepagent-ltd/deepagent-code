...
Вы можете определить серверы MCP в своем [deepagent-code Config](https://ai.deepagent.ltd/docs/config/) в разделе `mcp`. Добавьте каждому MCP уникальное имя. Вы можете обратиться к этому MCP по имени при запросе LLM.
