---
title: LSP-серверы
description: deepagent-code интегрируется с вашими серверами LSP.
---

deepagent-code может интегрироваться с серверами Language Server Protocol (LSP), чтобы использовать диагностику как обратную связь для агента.

---

## Встроенные

deepagent-code поставляется с несколькими встроенными LSP-серверами для популярных языков:
...
:::note
Вы можете отключить автоматическую загрузку LSP-сервера, установив для переменной среды `DEEPAGENT_CODE_DISABLE_LSP_DOWNLOAD` значение `true`.
:::
...
Когда LSP включен и deepagent-code открывает файл, он:
...
Вы можете включить и настроить серверы LSP через раздел `lsp` в конфигурации deepagent-code.
