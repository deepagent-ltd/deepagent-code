---
title: Плагины
description: Напишите свои собственные плагины для расширения deepagent-code.
---

Плагины позволяют расширять deepagent-code, подключаясь к различным событиям и настраивая поведение. Вы можете создавать плагины для добавления новых функций, интеграции с внешними сервисами или изменения поведения deepagent-code по умолчанию.
...
- `.deepagent-code/plugins/` – плагины уровня проекта.
- `~/.config/deepagent-code/plugins/` — глобальные плагины
...
**Плагины npm** устанавливаются автоматически с помощью Bun при запуске. Пакеты и их зависимости кэшируются в `~/.cache/deepagent-code/node_modules/`.
...
1. Глобальная конфигурация (`~/.config/deepagent-code/deepagent-code.json`)
2. Конфигурация проекта (`deepagent-code.json`)
3. Глобальный каталог плагинов (`~/.config/deepagent-code/plugins/`)
4. Каталог плагинов проекта (`.deepagent-code/plugins/`)
...
deepagent-code запускает `bun install` при запуске для их установки. Затем ваши плагины и инструменты смогут импортировать их.
...
Вот несколько примеров плагинов, которые вы можете использовать для расширения deepagent-code.
...
:::note
Если вы используете настольное приложение deepagent-code, оно может автоматически отправлять системные уведомления, когда ответ готов или когда возникает ошибка сеанса.
:::
...
Плагины также могут добавлять в deepagent-code собственные инструменты:
...
Помощник `tool` создает собственный инструмент, который может вызывать deepagent-code. Он принимает функцию схемы Zod и возвращает определение инструмента:
...
Уровни: `debug`, `info`, `warn`, `error`. Подробности см. в документации SDK](https://deepagent-code.ai/docs/sdk).
