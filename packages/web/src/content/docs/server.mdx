---
title: Server
description: Interact with deepagent-code server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `deepagent-code serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an deepagent-code client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `deepagent-code.local` |
...
Set `DEEPAGENT_CODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `deepagent-code`, or set `DEEPAGENT_CODE_SERVER_USERNAME` to override it. This applies to both `deepagent-code serve` and `deepagent-code web`.
...
When you run `deepagent-code` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the deepagent-code server to interact with deepagent-code programmatically.
:::

This architecture lets deepagent-code support multiple clients and allows you to interact with deepagent-code programmatically.

You can run `deepagent-code serve` to start a standalone server. If you have the
deepagent-code TUI running, `deepagent-code serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the DeepAgent Code [IDE](/docs/ide) plugins.
...
The deepagent-code server exposes the following APIs.
