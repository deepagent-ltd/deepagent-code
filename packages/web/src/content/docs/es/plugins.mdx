---
title: Complementos
description: Escriba sus propios complementos para extender DeepAgent Code.
---

Los complementos le permiten extender DeepAgent Code al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de DeepAgent Code.
...
- `.deepagent-code/plugins/` - Complementos a nivel de proyecto
- `~/.config/deepagent-code/plugins/` - Complementos globales
...
```json title="deepagent-code.json"
{
  ...
  "plugin": ["deepagent-code-helicone-session", "deepagent-code-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/deepagent-code/node_modules/`.
...
1. Configuración global (`~/.config/deepagent-code/deepagent-code.json`)
2. Configuración del proyecto (`deepagent-code.json`)
3. Directorio global de complementos (`~/.config/deepagent-code/plugins/`)
4. Directorio de complementos del proyecto (`.deepagent-code/plugins/`)
...
```json title=".deepagent-code/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
DeepAgent Code ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".deepagent-code/plugins/my-plugin.ts"
...
```js title=".deepagent-code/plugins/example.js"
...
- `client`: Un cliente SDK deepagent-code para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar deepagent-code.
...
```js title=".deepagent-code/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "deepagent-code"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio DeepAgent Code, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que deepagent-code lea archivos `.env`:

```javascript title=".deepagent-code/plugins/env-protection.js"
...
```javascript title=".deepagent-code/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a deepagent-code:

```ts title=".deepagent-code/plugins/custom-tools.ts"
import { type Plugin, tool } from "@deepagent-code/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que deepagent-code puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para deepagent-code junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://deepagent-code.ai/docs/sdk) para obtener más detalles.
...
```ts title=".deepagent-code/plugins/compaction.ts"
import type { Plugin } from "@deepagent-code/plugin"
...
```ts title=".deepagent-code/plugins/custom-compaction.ts"
import type { Plugin } from "@deepagent-code/plugin"
