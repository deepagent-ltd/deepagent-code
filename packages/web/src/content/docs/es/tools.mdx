...
Las herramientas permiten que LLM realice acciones en su código base. DeepAgent Code viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en DeepAgent Code.
...
```json title="deepagent-code.json" {4}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "permission": {
    "bash": "allow"
...
:::note
Esta herramienta solo está disponible cuando `DEEPAGENT_CODE_EXPERIMENTAL_LSP_TOOL=true` (o `DEEPAGENT_CODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor DeepAgent Code o cuando la variable de entorno `DEEPAGENT_CODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar DeepAgent Code:

```bash
DEEPAGENT_CODE_ENABLE_EXA=1 deepagent-code
```
