...
DeepAgent Code Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

Go funciona como cualquier otro proveedor en DeepAgent Code. Te suscribes a DeepAgent Code Go y
...
DeepAgent Code Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

DeepAgent Code Go funciona como cualquier otro proveedor en DeepAgent Code.

1. Inicias sesión en **<a href={console}>DeepAgent Code Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `DeepAgent Code Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a DeepAgent Code Go.
:::
...
DeepAgent Code Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de DeepAgent Code
usa el formato `deepagent-code-go/<model-id>`. Por ejemplo, para Kimi K2.6, usarías
`deepagent-code-go/kimi-k2.6` en tu configuración.
...
Creamos DeepAgent Code Go para:
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con DeepAgent Code.
