...
Con DeepAgent Code puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, DeepAgent Code usa nuestro propio tema `deepagent-code`.
...
DeepAgent Code viene con varios temas integrados.
...
- Quiere que DeepAgent Code coincida con la apariencia de su terminal
...
DeepAgent Code admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/deepagent-code/themes/*.json` o `$XDG_CONFIG_HOME/deepagent-code/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.deepagent-code/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.deepagent-code/themes/*.json`
