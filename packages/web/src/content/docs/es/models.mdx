...
Los proveedores más populares están precargados de forma predeterminada. Si agregó las credenciales de un proveedor mediante el comando `/connect`, estarán disponibles cuando inicie DeepAgent Code.
...
Aqui tienes varios modelos que funcionan bien con DeepAgent Code, sin orden particular. (Esta no es una lista exhaustiva ni necesariamente actualizada):
...
configuracion de DeepAgent Code.
...
Aquí el ID completo es `provider_id/model_id`. Por ejemplo, si usa [DeepAgent Code Zen](/docs/zen), usaría `deepagent-code/gpt-5.1-codex` para GPT 5.1 Codex.
...
Muchos modelos admiten múltiples variantes con diferentes configuraciones. DeepAgent Code se envía con variantes predeterminadas integradas para proveedores populares.

### Variantes integradas

DeepAgent Code se envía con variantes predeterminadas para muchos proveedores:
...
Cuando se inicia DeepAgent Code, busca modelos en el siguiente orden de prioridad:
...
2. La lista de modelos en la configuración DeepAgent Code.
