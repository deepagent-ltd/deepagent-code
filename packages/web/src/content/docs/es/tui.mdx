---
title: TUI
description: Usando la interfaz de usuario del terminal DeepAgent Code.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

DeepAgent Code proporciona una interfaz de terminal interactiva o TUI para trabajar en sus proyectos con un LLM.

Al ejecutar DeepAgent Code se inicia TUI para el directorio actual.
...
Cuando utilice DeepAgent Code TUI, puede escribir `/` seguido de un nombre de comando para ejecutar acciones rápidamente. Por ejemplo:
...
Agregue un proveedor a DeepAgent Code. Le permite seleccionar entre proveedores disponibles y agregar sus claves API.
...
Salga de DeepAgent Code. _Alias_: `/quit`, `/q`
...
Esto es independiente de `deepagent-code.json`, que configura el comportamiento del servidor/tiempo de ejecución.
...
- `leader_timeout`: controla cuánto tiempo espera DeepAgent Code después de la leader key. El valor predeterminado es `2000`.
...
- `sound_pack`: ID del sound pack que se usará. El valor predeterminado es `deepagent-code.default`.
