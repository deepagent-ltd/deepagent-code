---
title: Proveedores
description: Usando cualquier proveedor LLM en DeepAgent Code.
---

import config from "../../../../config.mjs"
export const console = config.console

DeepAgent Code usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración DeepAgent Code.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu DeepAgent Code
configuración.
...
DeepAgent Code Zen es una lista de modelos proporcionados por el equipo DeepAgent Code que han sido
probado y verificado para funcionar bien con DeepAgent Code. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con DeepAgent Code Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione deepagent-code y diríjase a [deepagent-code.ai/auth](https://deepagent-code.ai/auth).
...
Funciona como cualquier otro proveedor en DeepAgent Code y su uso es completamente opcional.

---

## DeepAgent Code Go

DeepAgent Code Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de DeepAgent Code que han sido
probado y verificado para funcionar bien con DeepAgent Code.

1. Ejecute el comando `/connect` en TUI, seleccione `DeepAgent Code Go` y diríjase a [deepagent-code.ai/auth](https://deepagent-code.ai/zen).
...
Para usar Amazon Bedrock con DeepAgent Code:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta deepagent-code:
   ...
   Para una configuración persistente o específica del proyecto, utilice `deepagent-code.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en DeepAgent Code no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar deepagent-code para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que deepagent-code funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración deepagent-code.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `DeepAgent Code`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación deepagent-code.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
DeepAgent Code utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear DeepAgent Code
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `deepagent-code.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [deepagent-code-gitlab-auth](https://www.npmjs.com/package/deepagent-code-gitlab-auth).

##### Configuración

Personalizar a través de `deepagent-code.json`:
...
Para utilizar su suscripción GitHub Copilot con deepagent-code:
...
Para utilizar Google Vertex AI con DeepAgent Code:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta deepagent-code.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de deepagent-code, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [deepagent-code-helicone-session](https://github.com/H2Shami/deepagent-code-helicone-session) para registrar automáticamente cada conversación DeepAgent Code como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación DeepAgent Code enumerada como una sesión separada.
...
Puede configurar deepagent-code para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar deepagent-code para usar modelos locales a través de LM Studio.
...
Puedes configurar deepagent-code para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para DeepAgent Code. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/deepagent-code) para obtener más detalles.
:::
...
Para usar Ollama Cloud con DeepAgent Code:
...
3. Copie la clave API para usarla en DeepAgent Code.
...
6. **Importante**: Antes de usar modelos en la nube en DeepAgent Code, debe obtener la información del modelo localmente:
...
DeepAgent Code Zen es una lista de modelos probados y verificados proporcionada por el equipo DeepAgent Code. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>DeepAgent Code Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **DeepAgent Code Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave DeepAgent Code API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración deepagent-code.
...
5. También puedes personalizarlos a través de tu configuración deepagent-code. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con DeepAgent Code:
...
También puede personalizar modelos a través de su configuración deepagent-code. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con deepagent-code. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `deepagent-code.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a DeepAgent Code comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `deepagent-code auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración deepagent-code y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración deepagent-code.
