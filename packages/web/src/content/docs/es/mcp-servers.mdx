...
Puede agregar herramientas externas a DeepAgent Code utilizando el _Model Context Protocol_ o MCP. DeepAgent Code admite servidores locales y remotos.
...
Puede definir servidores MCP en su [DeepAgent Code Config](https://deepagent-code.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/deepagent-code`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
DeepAgent Code maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, DeepAgent Code:
...
Si el servidor requiere autenticación, DeepAgent Code le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `deepagent-code mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, DeepAgent Code almacenará los tokens de forma segura en `~/.deepagent/code/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en DeepAgent Code, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración DeepAgent Code como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar DeepAgent Code a su cuenta Sentry.
