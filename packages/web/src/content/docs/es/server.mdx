---
title: Servidor
description: Interactuar con el servidor deepagent-code a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `deepagent-code serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente deepagent-code puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `deepagent-code.local` |
...
Configure `DEEPAGENT_CODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `deepagent-code`, o configure `DEEPAGENT_CODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `deepagent-code serve` como a `deepagent-code web`.
...
Cuando ejecuta `deepagent-code`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor deepagent-code para interactuar con deepagent-code mediante programación.
:::

Esta arquitectura permite que deepagent-code admita múltiples clientes y le permite interactuar con deepagent-code mediante programación.

Puede ejecutar `deepagent-code serve` para iniciar un servidor independiente. Si tienes el
deepagent-code TUI ejecutándose, `deepagent-code serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos DeepAgent Code [IDE](/docs/ide).
...
El servidor deepagent-code expone las siguientes API.
