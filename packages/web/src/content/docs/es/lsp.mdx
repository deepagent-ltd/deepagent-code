...
description: DeepAgent Code se integra con sus servidores LSP.
---

DeepAgent Code puede integrarse con servidores Language Server Protocol (LSP) para usar diagnósticos como feedback para el agente.

---

## Incorporados

DeepAgent Code viene con varios servidores LSP integrados para idiomas populares:
...
:::note
Puede deshabilitar las descargas automáticas del servidor LSP configurando la variable de entorno `DEEPAGENT_CODE_DISABLE_LSP_DOWNLOAD` en `true`.
:::
...
Cuando LSP está habilitado y deepagent-code abre un archivo:
...
Puede habilitar y personalizar los servidores LSP a través de la sección `lsp` en su configuración deepagent-code.
