...
description: Cree herramientas que LLM pueda llamar en deepagent-code.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de deepagent-code como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.deepagent-code/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/deepagent-code/tools/`.
...
```ts title=".deepagent-code/tools/database.ts" {1}
...
```ts title=".deepagent-code/tools/math.ts"
import { tool } from "@deepagent-code/plugin"
...
```ts title=".deepagent-code/tools/bash.ts"
import { tool } from "@deepagent-code/plugin"
...
```ts title=".deepagent-code/tools/project.ts" {8}
import { tool } from "@deepagent-code/plugin"
...
```python title=".deepagent-code/tools/add.py"
import sys
...
```ts title=".deepagent-code/tools/python-add.ts" {10}
import { tool } from "@deepagent-code/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".deepagent-code/tools/add.py")
