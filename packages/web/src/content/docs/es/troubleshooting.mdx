...
Para depurar problemas con DeepAgent Code, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\deepagent-code\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `deepagent-code --log-level DEBUG`.

---

## Almacenamiento

deepagent-code almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.deepagent/code/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\deepagent-code`
...
DeepAgent Code Desktop ejecuta un servidor DeepAgent Code local (el sidecar `deepagent-code-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `DeepAgent Code` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/deepagent-code/deepagent-code.jsonc` (o `~/.config/deepagent-code/deepagent-code.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.deepagent/code/deepagent-code.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\deepagent-code\deepagent-code.jsonc`
...
DeepAgent Code también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/deepagent-code/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\deepagent-code\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que DeepAgent Code pueda reconstruirlo.

1. Salga de DeepAgent Code Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/deepagent-code`
- **Linux**: eliminar `~/.cache/deepagent-code` (o ejecutar `rm -rf ~/.cache/deepagent-code`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\deepagent-code`

3. Reinicie el escritorio DeepAgent Code.

---

### Solucionar problemas de conexión del servidor

DeepAgent Code Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `deepagent-code.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `DEEPAGENT_CODE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `DEEPAGENT_CODE_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio DeepAgent Code requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de DeepAgent Code.

---

### Notificaciones que no se muestran

DeepAgent Code Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para DeepAgent Code en la configuración de su sistema operativo, y
...
1. Salga del escritorio DeepAgent Code.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio DeepAgent Code):

- `deepagent-code.settings.dat` (URL del servidor predeterminado de escritorio)
- `deepagent-code.global.dat` y `deepagent-code.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con DeepAgent Code:
...
3. Asegúrese de tener la última versión con `deepagent-code upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `deepagent-code models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\deepagent-code`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. deepagent-code instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\deepagent-code`

2. Reinicie deepagent-code para reinstalar los paquetes del proveedor más recientes.

Esto obligará a deepagent-code a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
deepagent-code detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
