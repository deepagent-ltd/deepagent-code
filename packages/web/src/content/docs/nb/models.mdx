...
De fleste populære leverandørene er forhåndslastet som standard. Hvis du har lagt til legitimasjonen for en leverandør gjennom kommandoen `/connect`, vil den være tilgjengelig når du starter DeepAgent Code.
...
Her er flere modeller som fungerer godt med DeepAgent Code, uten spesiell rekkefølge. (Dette er ikke en uttømmende liste og er heller ikke nødvendigvis oppdatert):
...
Her er hele ID `provider_id/model_id`. For eksempel, hvis du bruker [DeepAgent Code Zen](/docs/zen), vil du bruke `deepagent-code/gpt-5.1-codex` for GPT 5.1 Codex.
...
Mange modeller støtter flere varianter med forskjellige konfigurasjoner. deepagent-code leveres med innebygde standardvarianter for populære leverandører.

### Innebygde varianter

DeepAgent Code leveres med standardvarianter for mange leverandører:
...
Når DeepAgent Code starter opp, ser den etter modeller i følgende prioriterte rekkefølge:

1. Kommandolinjeflagget `--model` eller `-m`. Formatet er det samme som i konfigurasjonsfilen: `provider_id/model_id`.

2. Modelllisten i DeepAgent Code-konfigurasjonen.
