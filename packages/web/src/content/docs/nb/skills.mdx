...
Agentferdigheter lar DeepAgent Code oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
deepagent-code søker etter disse stedene:

- Prosjektkonfigurasjon: `.deepagent-code/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/deepagent-code/skills/<name>/SKILL.md`
...
For prosjektlokale stier går deepagent-code opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.deepagent-code/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/deepagent-code/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.deepagent-code/skills/git-release/SKILL.md` slik:
...
deepagent-code viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `deepagent-code.json`:
...
**For innebygde agenter** (i `deepagent-code.json`):
