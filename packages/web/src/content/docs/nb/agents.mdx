...
Det er to typer agenter i DeepAgent Code; primære agenter og underagenter.
...
DeepAgent Code kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
DeepAgent Code kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

DeepAgent Code kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `deepagent-code.json`:
...
- Per prosjekt: `.deepagent-code/agents/`
...
Hvis ingen temperatur er spesifisert, bruker DeepAgent Code modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale DeepAgent Code-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i DeepAgent Code-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [DeepAgent Code Zen](/docs/zen), vil du bruke `deepagent-code/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `deepagent-code models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/deepagent-ltd/deepagent-code).
:::
