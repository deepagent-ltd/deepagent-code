...
description: Skriv dine egne programtillegg for å utvide DeepAgent Code.
---

Plugins lar deg utvide DeepAgent Code ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til DeepAgent Code.
...
- `.deepagent-code/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/deepagent-code/node_modules/`.
...
1. Global konfigurasjon (`~/.config/deepagent-code/deepagent-code.json`)
2. Prosjektkonfigurasjon (`deepagent-code.json`)
3. Global plugin-katalog (`~/.config/deepagent-code/plugins/`)
4. Prosjektpluginkatalog (`.deepagent-code/plugins/`)
...
DeepAgent Code kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En DeepAgent Code SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide DeepAgent Code.
...
:::note
Hvis du bruker DeepAgent Code-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre DeepAgent Code fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til DeepAgent Code:
...
`tool`-hjelperen lager et tilpasset verktøy som DeepAgent Code kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for DeepAgent Code sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://deepagent-code.ai/docs/sdk) for detaljer.
