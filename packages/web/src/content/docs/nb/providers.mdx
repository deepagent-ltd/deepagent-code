---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i DeepAgent Code.
---

import config from "../../../../config.mjs"
export const console = config.console

DeepAgent Code bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i DeepAgent Code-konfigurasjonen.
...
i `~/.deepagent/code/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i DeepAgent Code
konfig.
...
DeepAgent Code Zen er en liste over modeller levert av DeepAgent Code-teamet som har vært
testet og verifisert for å fungere godt med DeepAgent Code. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med DeepAgent Code Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `DeepAgent Code Zen` og gå til [deepagent-code.ai/auth](https://deepagent-code.ai/zen).
...
Det fungerer som alle andre leverandører i DeepAgent Code og er helt valgfritt å bruke.

---

## DeepAgent Code Go

DeepAgent Code Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av DeepAgent Code-teamet som har vært
testet og verifisert for å fungere godt med DeepAgent Code.

1. Kjør kommandoen `/connect` i TUI, velg `DeepAgent Code Go`, og gå til [deepagent-code.ai/auth](https://deepagent-code.ai/zen).
...
Slik bruker du Amazon Bedrock med DeepAgent Code:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører DeepAgent Code:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `deepagent-code.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i deepagent-code støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere deepagent-code til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at DeepAgent Code skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom DeepAgent Code-konfigurasjonen.
...
for å lagre token i DeepAgent Code auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
DeepAgent Code bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse DeepAgent Code
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`deepagent-code.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [deepagent-code-gitlab-auth](https://www.npmjs.com/package/deepagent-code-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `deepagent-code.json`:
...
Slik bruker du GitHub Copilot-abonnementet med DeepAgent Code:
...
Slik bruker du Google Vertex AI med DeepAgent Code:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører DeepAgent Code.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom DeepAgent Code, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [deepagent-code-helicone-session](https://github.com/H2Shami/deepagent-code-helicone-session) plugin for automatisk å logge hver DeepAgent Code-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver DeepAgent Code-samtale oppført som en separat økt.
...
Du kan konfigurere DeepAgent Code for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere DeepAgent Code for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere DeepAgent Code for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for DeepAgent Code. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/deepagent-code) for detaljer.
:::
...
Slik bruker du Ollama Cloud med DeepAgent Code:
...
3. Kopier API-nøkkelen for bruk i DeepAgent Code.
...
6. **Viktig**: Før du bruker skymodeller i DeepAgent Code, må du hente modellinformasjonen lokalt:
...
DeepAgent Code Zen er en liste over testede og verifiserte modeller levert av DeepAgent Code-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>DeepAgent Code Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **DeepAgent Code Zen**.

   ```txt
   /connect
   ```

3. Skriv inn DeepAgent Code API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom DeepAgent Code-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom DeepAgent Code-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med DeepAgent Code:
...
Du kan også tilpasse modeller gjennom DeepAgent Code-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med DeepAgent Code. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `deepagent-code.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar DeepAgent Code forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `deepagent-code auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk DeepAgent Code-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i DeepAgent Code-konfigurasjonen.
