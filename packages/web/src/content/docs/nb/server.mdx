---
title: Tjener
description: Samhandle med DeepAgent Code-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `deepagent-code serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en DeepAgent Code-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `deepagent-code.local` |
...
Sett `DEEPAGENT_CODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `deepagent-code` som standard, eller sett `DEEPAGENT_CODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `deepagent-code serve` og `deepagent-code web`.
...
Når du kjører `deepagent-code` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk DeepAgent Code-serveren til å samhandle med DeepAgent Code programmatisk.
:::

Denne arkitekturen lar DeepAgent Code støtte flere klienter og lar deg samhandle med DeepAgent Code programmatisk.

Du kan kjøre `deepagent-code serve` for å starte en frittstående server. Hvis du har
DeepAgent Code TUI kjører, vil `deepagent-code serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av DeepAgent Code [IDE](/docs/ide) plugins.
...
deepagent-code-serveren viser følgende APIer.
