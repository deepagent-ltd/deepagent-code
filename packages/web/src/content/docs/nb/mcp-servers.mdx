...
Du kan legge til eksterne verktøy i DeepAgent Code ved å bruke _Model Context Protocol_ eller MCP. DeepAgent Code støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [DeepAgent Code Config](https://deepagent-code.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="deepagent-code.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/deepagent-code`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="deepagent-code.jsonc" {15}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
DeepAgent Code håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil DeepAgent Code:
...
Hvis serveren krever autentisering, vil DeepAgent Code be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `deepagent-code mcp auth <server-name>`.
...
```json title="deepagent-code.json" {7-11}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
deepagent-code mcp auth my-oauth-server
```
...
```bash
deepagent-code mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil DeepAgent Code lagre tokens sikkert i `~/.deepagent/code/mcp-auth.json`.
...
```json title="deepagent-code.json" {7}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "mcp": {
    "my-api-key-server": {
...
deepagent-code mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i DeepAgent Code, sammen med innebygde verktøy. Så du kan administrere dem gjennom DeepAgent Code-konfigurasjonen som alle andre verktøy.
...
```json title="deepagent-code.json" {14}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "mcp": {
    "my-mcp-foo": {
...
```json title="deepagent-code.json" {11, 14-18}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="deepagent-code.json" {4-8}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
deepagent-code mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble DeepAgent Code til Sentry-kontoen din.
...
```json title="deepagent-code.json" {4-7}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="deepagent-code.json" {7-9}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver DeepAgent Code-samtale oppført som en separat økt.
