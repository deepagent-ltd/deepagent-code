---
title: Konfigurasjon
description: Ved å bruke deepagent-code JSON-konfigurasjonen.
---

Du kan konfigurere DeepAgent Code ved å bruke en JSON konfigurasjonsfil.

---

## Format

DeepAgent Code støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "deepagent-code"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/deepagent-code`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/deepagent-code/deepagent-code.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`DEEPAGENT_CODE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`deepagent-code.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.deepagent-code` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`DEEPAGENT_CODE_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.deepagent-code` og `~/.config/deepagent-code` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/deepagent-code`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale DeepAgent Code-konfigurasjon i `~/.config/deepagent-code/deepagent-code.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `deepagent-code.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når DeepAgent Code starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `DEEPAGENT_CODE_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `DEEPAGENT_CODE_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.deepagent-code` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.deepagent-code`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`ai.deepagent.ltd/config.schema.json`**](https://ai.deepagent.ltd/config.schema.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `deepagent-code serve` og `deepagent-code web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage DeepAgent Code-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `deepagent-code.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i DeepAgent Code-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver DeepAgent Code å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/deepagent-code/agents/` eller `.deepagent-code/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil DeepAgent Code falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`deepagent-code run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/deepagent-code/commands/` eller `.deepagent-code/commands/`. [Les mer her](/docs/commands).
...
DeepAgent Code vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater DeepAgent Code alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider DeepAgent Code med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.deepagent-code/plugins/` eller `~/.config/deepagent-code/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense DeepAgent Code til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
