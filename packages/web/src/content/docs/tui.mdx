---
title: TUI
description: Using the DeepAgent Code terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

DeepAgent Code provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running DeepAgent Code starts the TUI for the current directory.
...
When using the DeepAgent Code TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to DeepAgent Code. Allows you to select from available providers and add their API keys.
...
Exit DeepAgent Code. _Aliases_: `/quit`, `/q`
...
This is separate from `deepagent-code.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long DeepAgent Code waits after the leader key. Defaults to `2000`.
...
Use `DEEPAGENT_CODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `deepagent-code.default`.
