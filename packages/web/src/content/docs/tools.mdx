...
Tools allow the LLM to perform actions in your codebase. DeepAgent Code comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in DeepAgent Code.
...
:::note
This tool is only available when `DEEPAGENT_CODE_EXPERIMENTAL_LSP_TOOL=true` (or `DEEPAGENT_CODE_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the DeepAgent Code provider or when the `DEEPAGENT_CODE_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching DeepAgent Code:
