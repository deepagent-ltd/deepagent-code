...
You can add external tools to DeepAgent Code using the _Model Context Protocol_, or MCP. DeepAgent Code supports both local and remote servers.
...
You can define MCP servers in your [DeepAgent Code Config](https://deepagent-code.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/deepagent-code` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
DeepAgent Code automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, DeepAgent Code will:
...
If the server requires authentication, DeepAgent Code will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `deepagent-code mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, DeepAgent Code will store the tokens securely in `~/.deepagent/code/mcp-auth.json`.
...
Your MCPs are available as tools in DeepAgent Code, alongside built-in tools. So you can manage them through the DeepAgent Code config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect DeepAgent Code to your Sentry account.
