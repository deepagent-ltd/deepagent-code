---
title: 分享
description: 分享您的 DeepAgent Code 对话。
---

DeepAgent Code 的分享功能允许您创建指向 DeepAgent Code 对话的公开链接，方便与团队成员协作或向他人寻求帮助。
...
DeepAgent Code 支持三种分享模式，用于控制对话的共享方式：

---

### 手动模式（默认）

默认情况下，DeepAgent Code 使用手动分享模式。会话不会自动共享，但您可以使用 `/share` 命令手动分享：
...
要在团队中对特定项目强制执行此设置，请将其添加到项目的 `deepagent-code.json` 文件中并提交到 Git。
...
[了解更多](/docs/enterprise)关于在您的组织中使用 DeepAgent Code 的信息。
