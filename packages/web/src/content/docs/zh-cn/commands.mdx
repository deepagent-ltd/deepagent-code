...
创建 `.deepagent-code/commands/test.md`：
...
你可以通过 DeepAgent Code 配置或在 `commands/` 目录中创建 markdown 文件来添加自定义命令。

---

### JSON

在 DeepAgent Code [配置](/docs/config)中使用 `command` 选项：
...
- 全局：`~/.config/deepagent-code/commands/`
- 项目级：`.deepagent-code/commands/`
...
deepagent-code 包含多个内置命令，如 `/init`、`/undo`、`/redo`、`/share`、`/help`；[了解更多](/docs/tui#commands)。
