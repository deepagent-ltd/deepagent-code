...
你可以通过 _Model Context Protocol_（MCP）为 DeepAgent Code 添加外部工具。DeepAgent Code 同时支持本地和远程服务器。
...
你可以在 [DeepAgent Code 配置](https://deepagent-code.ai/docs/config/)的 `mcp` 字段下定义 MCP 服务器。为每个 MCP 指定一个唯一的名称，在提示词中可以通过该名称来引用对应的 MCP。
...
组织可以通过其 `.well-known/deepagent-code` 端点提供默认的 MCP 服务器。这些服务器可能默认处于禁用状态，允许用户按需启用。
...
DeepAgent Code 会自动处理远程 MCP 服务器的 OAuth 身份验证。当服务器需要身份验证时，DeepAgent Code 将：
...
如果服务器需要身份验证，DeepAgent Code 会在你首次使用时提示你进行认证。你也可以使用 `deepagent-code mcp auth <server-name>` [手动触发认证流程](#authenticating)。
...
`mcp auth` 命令会打开浏览器进行授权。授权完成后，DeepAgent Code 会将 Token 安全地存储在 `~/.deepagent/code/mcp-auth.json` 中。
...
你的 MCP 在 DeepAgent Code 中作为工具使用，与内置工具并列。因此，你可以像管理其他工具一样，通过 DeepAgent Code 配置来管理它们。
...
这会打开浏览器窗口完成 OAuth 流程，将 DeepAgent Code 连接到你的 Sentry 账户。
