---
title: LSP 服务器
description: DeepAgent Code 与你的 LSP 服务器集成。
---

DeepAgent Code 可以与语言服务器协议（LSP）服务器集成，将诊断信息作为 agent 的反馈。

---

## 内置支持

DeepAgent Code 内置了多种适用于主流语言的 LSP 服务器：
...
:::note
你可以将 `DEEPAGENT_CODE_DISABLE_LSP_DOWNLOAD` 环境变量设置为 `true` 来禁用 LSP 服务器的自动下载。
:::
...
启用 LSP 且 deepagent-code 打开文件时，它会：
...
你可以通过 deepagent-code 配置文件中的 `lsp` 部分来启用并自定义 LSP 服务器。
