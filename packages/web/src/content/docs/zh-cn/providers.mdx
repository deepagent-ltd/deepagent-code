...
DeepAgent Code 使用 [AI SDK](https://ai-sdk.dev/) 和 [Models.dev](https://models.dev)，支持 **75+ LLM 提供商**，同时也支持运行本地模型。

要添加提供商，你需要：

1. 使用 `/connect` 命令添加提供商的 API 密钥。
2. 在 DeepAgent Code 配置中设置该提供商。
...
你可以通过 DeepAgent Code 配置中的 `provider` 部分来自定义提供商。
...
DeepAgent Code Zen 是由 DeepAgent Code 团队提供的模型列表，这些模型已经过测试和验证，能够与 DeepAgent Code 良好配合使用。[了解更多](/docs/zen)。

:::tip
如果你是新用户，我们建议从 DeepAgent Code Zen 开始。
:::

1. 在 TUI 中执行 `/connect` 命令，选择 deepagent-code，然后前往 [deepagent-code.ai/auth](https://deepagent-code.ai/auth)。
...
它的使用方式与 DeepAgent Code 中的其他提供商完全相同，且完全可选。
...
2. 使用以下方法之一**配置身份验证**：
   ...
   ***
   ...
   如需项目级别或持久化的配置，请使用 `deepagent-code.json`：
...
:::info
在 DeepAgent Code 中使用 Claude Pro/Max 订阅不是 [Anthropic](https://anthropic.com) 官方支持的用法。
:::
...
你可以通过 [Atomic Chat](https://atomic.chat) 配置 deepagent-code 以使用本地模型。Atomic Chat 是一款桌面应用程序，它在 OpenAI 兼容的 API 服务器后面运行本地 LLM（默认端点 `http://127.0.0.1:1337/v1`）。
...
2. 前往 [Azure AI Foundry](https://ai.azure.com/) 并部署一个模型。
   ...
   :::note
   部署名称必须与模型名称一致，DeepAgent Code 才能正常工作。
   :::
...
2. 选择你的身份验证方式：
   ...
   2. 点击 **Add new token**
   3. 名称填写 `DeepAgent Code`，范围选择 `api`
   4. 复制令牌（以 `glpat-` 开头）
   5. 在终端中输入该令牌
...
:::note
你也可以通过指定 `GITLAB_TOKEN` 环境变量来避免将令牌存储在 DeepAgent Code 的认证存储中。
:::

##### 自托管 GitLab

:::note[合规说明]
DeepAgent Code 会使用一个小模型来执行部分 AI 任务，例如生成会话标题。默认情况下使用由 Zen 托管的 gpt-5-nano。如果你需要让 DeepAgent Code 仅使用你自己的 GitLab 托管实例，请在 `deepagent-code.json` 文件中添加以下内容。同时建议禁用会话共享。
...
更多文档请参阅 [deepagent-code-gitlab-auth](https://www.npmjs.com/package/deepagent-code-gitlab-auth) 主页。

##### 配置

通过 `deepagent-code.json` 进行自定义配置：
...
要在 DeepAgent Code 中使用你的 GitHub Copilot 订阅：
...
如果 Helicone 的某些功能或模型未通过 DeepAgent Code 自动配置，你随时可以手动配置。
...
Helicone 的 [Sessions](https://docs.helicone.ai/features/sessions) 功能允许你将相关的 LLM 请求归为一组。使用 [deepagent-code-helicone-session](https://github.com/H2Shami/deepagent-code-helicone-session) 插件可以自动将每个 DeepAgent Code 对话记录为 Helicone 中的一个会话。
...
该插件会在你的请求中注入 `Helicone-Session-Id` 和 `Helicone-Session-Name` 请求头。在 Helicone 的 Sessions 页面中，你可以看到每个 DeepAgent Code 对话都作为独立的会话列出。
...
你可以通过 [llama.cpp](https://github.com/ggml-org/llama.cpp) 的 llama-server 工具配置 DeepAgent Code 使用本地模型。
...
你可以通过 LM Studio 配置 DeepAgent Code 使用本地模型。
...
你可以通过 Ollama 配置 DeepAgent Code 使用本地模型。

:::tip
Ollama 可以自动为 DeepAgent Code 进行配置。详见 [Ollama 集成文档](https://docs.ollama.com/integrations/deepagent-code)。
:::
...
3. 复制 API 密钥以便在 DeepAgent Code 中使用。
...
6. **重要**：在 DeepAgent Code 中使用云端模型之前，必须先将模型信息拉取到本地：
...
DeepAgent Code Zen 是由 DeepAgent Code 团队提供的经过测试和验证的模型列表。[了解更多](/docs/zen)。

1. 登录 **<a href={console}>DeepAgent Code Zen</a>** 并点击 **Create API Key**。

2. 执行 `/connect` 命令并搜索 **DeepAgent Code Zen**。

   ```txt
   /connect
   ```

3. 输入你的 DeepAgent Code API 密钥。
...
4. 默认已预加载了许多 OpenRouter 模型，执行 `/models` 命令选择你想要的模型。
   ...
   你也可以通过 DeepAgent Code 配置添加更多模型。
...
5. 你还可以通过 DeepAgent Code 配置自定义模型。以下是指定提供商的示例：
...
你也可以通过 DeepAgent Code 配置自定义模型。以下是指定提供商路由顺序的示例。
...
:::tip
你可以在 DeepAgent Code 中使用任何 OpenAI 兼容的提供商。大多数现代 AI 提供商都提供 OpenAI 兼容的 API。
:::
...
4. 在项目目录中创建或更新 `deepagent-code.json` 文件：
...
`limit` 字段让 DeepAgent Code 了解你还剩余多少上下文空间。标准提供商会自动从 models.dev 拉取这些信息。
...
1. **检查认证设置**：运行 `deepagent-code auth list` 查看该提供商的凭据是否已添加到配置中。

   这不适用于 Amazon Bedrock 等依赖环境变量进行认证的提供商。

2. 对于自定义提供商，请检查 DeepAgent Code 配置并确认：
   - `/connect` 命令中使用的提供商 ID 与 DeepAgent Code 配置中的 ID 一致。
