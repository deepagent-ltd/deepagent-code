...
通过 DeepAgent Code，您可以从多个内置主题中进行选择，使用能自动适配终端主题的主题，或者定义您自己的自定义主题。

默认情况下，DeepAgent Code 使用我们自己的 `deepagent-code` 主题。
...
DeepAgent Code 支持灵活的基于 JSON 的主题系统，让用户可以轻松创建和自定义主题。
...
1. **内置主题** — 嵌入在二进制文件中
2. **用户配置目录** — 定义在 `~/.config/deepagent-code/themes/*.json` 或 `$XDG_CONFIG_HOME/deepagent-code/themes/*.json`
3. **项目根目录** — 定义在 `<project-root>/.deepagent-code/themes/*.json`
4. **当前工作目录** — 定义在 `./.deepagent-code/themes/*.json`
