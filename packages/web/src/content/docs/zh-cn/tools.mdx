...
工具允许 LLM 在您的代码库中执行操作。DeepAgent Code 自带一组内置工具，您也可以通过[自定义工具](/docs/custom-tools)或 [MCP 服务器](/docs/mcp-servers)来扩展它。
...
:::note
该工具仅在设置 `DEEPAGENT_CODE_EXPERIMENTAL_LSP_TOOL=true`（或 `DEEPAGENT_CODE_EXPERIMENTAL=true`）时可用。
:::
...
:::note
该工具仅在使用 DeepAgent Code 提供商时，或当 `DEEPAGENT_CODE_ENABLE_EXA` 环境变量设置为任意真值（例如 `true` 或 `1`）时可用。

在启动 DeepAgent Code 时启用：
