...
As ferramentas permitem que o LLM execute ações em sua base de código. O deepagent-code vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no deepagent-code.
...
:::note
Esta ferramenta está disponível apenas quando `DEEPAGENT_CODE_EXPERIMENTAL_LSP_TOOL=true` (ou `DEEPAGENT_CODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor deepagent-code ou quando a variável de ambiente `DEEPAGENT_CODE_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o deepagent-code:
