---
title: Provedores
description: Usando qualquer provedor LLM no deepagent-code.
---

import config from "../../../../config.mjs"
export const console = config.console

deepagent-code usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do deepagent-code.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.deepagent/code/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do deepagent-code.
...
DeepAgent Code Zen é uma lista de modelos fornecidos pela equipe do deepagent-code que foram testados e verificados para funcionar bem com o deepagent-code. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o DeepAgent Code Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `DeepAgent Code Zen` e acesse [deepagent-code.ai/zen](https://deepagent-code.ai/zen).
...
Funciona como qualquer outro provedor no deepagent-code e é completamente opcional.

---

## DeepAgent Code Go

DeepAgent Code Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do deepagent-code que foram
testados e verificados para funcionar bem com o deepagent-code.

1. Execute o comando `/connect` no TUI, selecione `DeepAgent Code Go` e acesse [deepagent-code.ai/zen](https://deepagent-code.ai/zen).
...
Para usar o Amazon Bedrock com o deepagent-code:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o deepagent-code:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `deepagent-code.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no deepagent-code não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o deepagent-code para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o deepagent-code funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do deepagent-code.
...
2. Escolha seu método de autenticação:
   ...
   3. Nome: `DeepAgent Code`, Escopos: `api`
...
armazenar o token no armazenamento de autenticação do deepagent-code.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
deepagent-code usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o deepagent-code
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `deepagent-code.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [deepagent-code-gitlab-auth](https://www.npmjs.com/package/deepagent-code-gitlab-auth).

##### Configuração

Personalize através do `deepagent-code.json`:
...
Para usar sua assinatura do GitHub Copilot com o deepagent-code:
...
Para usar o Google Vertex AI com o deepagent-code:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o deepagent-code.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do deepagent-code, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [deepagent-code-helicone-session](https://github.com/H2Shami/deepagent-code-helicone-session) para registrar automaticamente cada conversa do deepagent-code como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do deepagent-code listada como uma sessão separada.
...
Você pode configurar o deepagent-code para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o deepagent-code para usar modelos locais através do LM Studio.
...
Você pode configurar o deepagent-code para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o deepagent-code. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/deepagent-code) para detalhes.
:::
...
Para usar o Ollama Cloud com o deepagent-code:
...
3. Copie a chave da API para uso no deepagent-code.
...
6. **Importante**: Antes de usar modelos em nuvem no deepagent-code, você deve puxar as informações do modelo localmente:
...
DeepAgent Code Zen é uma lista de modelos testados e verificados fornecidos pela equipe do deepagent-code. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>DeepAgent Code Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **DeepAgent Code Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do deepagent-code.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do deepagent-code.
...
5. Você também pode personalizá-los através da sua configuração do deepagent-code. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o deepagent-code:
...
Você também pode personalizar modelos através da sua configuração do deepagent-code. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o deepagent-code. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no deepagent-code.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `deepagent-code.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o deepagent-code entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `deepagent-code auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do deepagent-code e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do deepagent-code.
