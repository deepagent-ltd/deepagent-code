...
Você pode adicionar ferramentas externas ao deepagent-code usando o _Model Context Protocol_, ou MCP. O deepagent-code suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do deepagent-code](https://deepagent-code.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/deepagent-code`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O deepagent-code lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o deepagent-code irá:
...
Se o servidor requer autenticação, o deepagent-code solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `deepagent-code mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o deepagent-code armazenará os tokens de forma segura em `~/.deepagent/code/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no deepagent-code, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do deepagent-code como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o deepagent-code à sua conta do Sentry.
