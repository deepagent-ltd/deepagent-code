...
description: O deepagent-code integra-se aos seus servidores LSP.
---

O deepagent-code pode se integrar a servidores Language Server Protocol (LSP) para usar diagnósticos como feedback para o agente.

---

## Integrados

O deepagent-code vem com vários servidores LSP integrados para linguagens populares:
...
:::note
Você pode desabilitar os downloads automáticos do servidor LSP definindo a variável de ambiente `DEEPAGENT_CODE_DISABLE_LSP_DOWNLOAD` como `true`.
:::
...
Quando o LSP está habilitado e o deepagent-code abre um arquivo, ele:
...
Você pode habilitar e personalizar os servidores LSP através da seção `lsp` na sua configuração do deepagent-code.
