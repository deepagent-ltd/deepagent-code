---
title: Configuração
description: Usando a configuração JSON do deepagent-code.
---

Você pode configurar o deepagent-code usando um arquivo de configuração JSON.

---

## Formato

O deepagent-code suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
1. **Configuração remota** (de `.well-known/deepagent-code`) - padrões organizacionais
2. **Configuração global** (`~/.config/deepagent-code/deepagent-code.json`) - preferências do usuário
3. **Configuração personalizada** (`DEEPAGENT_CODE_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`deepagent-code.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.deepagent-code`** - agentes, comandos, plugins
6. **Configuração inline** (`DEEPAGENT_CODE_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.deepagent-code` e `~/.config/deepagent-code` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/deepagent-code`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do deepagent-code em `~/.config/deepagent-code/deepagent-code.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/deepagent-code/tui.json`.
...
Adicione `deepagent-code.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Quando o deepagent-code é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `DEEPAGENT_CODE_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `DEEPAGENT_CODE_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.deepagent-code`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.deepagent-code`, então ele **pode substituir** suas configurações.

---

## Esquema

O esquema de configuração do servidor/tempo de execução é definido em [**`ai.deepagent.ltd/config.schema.json`**](https://ai.deepagent.ltd/config.schema.json).

A configuração do TUI usa [**`ai.deepagent.ltd/tui.json`**](https://ai.deepagent.ltd/tui.json).
...
Use `DEEPAGENT_CODE_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `deepagent-code.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `deepagent-code serve` e `deepagent-code web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor deepagent-code.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `deepagent-code.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do deepagent-code através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o deepagent-code tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/deepagent-code/agents/` ou `.deepagent-code/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o deepagent-code recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`deepagent-code run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/deepagent-code/commands/` ou `.deepagent-code/commands/`. [Saiba mais aqui](/docs/commands).
...
O deepagent-code fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o deepagent-code **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o deepagent-code com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.deepagent-code/plugins/` ou `~/.config/deepagent-code/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o deepagent-code para usar apenas provedores específicos, em vez de desabilitá-los um a um.
