...
Existem dois tipos de agentes no deepagent-code; agentes primários e subagentes.
...
deepagent-code vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
DeepAgent Code vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

DeepAgent Code vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do DeepAgent Code, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `deepagent-code.json`:
...
- Global: `~/.config/deepagent-code/agents/`
- Por projeto: `.deepagent-code/agents/`

```markdown title="~/.config/deepagent-code/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o deepagent-code usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do deepagent-code quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do deepagent-code usa o formato `provider/model-id`. Por exemplo, se você estiver usando [DeepAgent Code Zen](/docs/zen), você usaria `deepagent-code/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="deepagent-code.json" {3-6,9-12}
{
  "$schema": "https://ai.deepagent.ltd/config.schema.json",
  "tools": {
    "write": true,
...
```json title="deepagent-code.json" {3-5,8-10}
{
  "$schema": "https://ai.deepagent.ltd/config.schema.json",
  "permission": {
    "edit": "deny"
...
```json title="deepagent-code.json" {8}
{
  "$schema": "https://ai.deepagent.ltd/config.schema.json",
  "agent": {
    "build": {
...
```json title="deepagent-code.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Execute `deepagent-code models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/deepagent-ltd/deepagent-code).
:::
...
```markdown title="~/.config/deepagent-code/agents/docs-writer.md"
---
...
```markdown title="~/.config/deepagent-code/agents/security-auditor.md"
---
