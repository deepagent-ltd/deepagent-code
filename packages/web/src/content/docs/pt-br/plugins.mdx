---
title: Plugins
description: Escreva seus próprios plugins para estender o deepagent-code.
---

Plugins permitem que você estenda o deepagent-code conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do deepagent-code.
...
- `.deepagent-code/plugins/` - Plugins em nível de projeto
- `~/.config/deepagent-code/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/deepagent-code/node_modules/`.
...
1. Configuração global (`~/.config/deepagent-code/deepagent-code.json`)
2. Configuração do projeto (`deepagent-code.json`)
3. Diretório de plugins global (`~/.config/deepagent-code/plugins/`)
4. Diretório de plugins do projeto (`.deepagent-code/plugins/`)
...
O deepagent-code executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do deepagent-code para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o deepagent-code.
...
:::note
Se você estiver usando o aplicativo desktop deepagent-code, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o deepagent-code de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao deepagent-code:
...
O helper `tool` cria uma ferramenta personalizada que o deepagent-code pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o deepagent-code junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://deepagent-code.ai/docs/sdk) para detalhes.
