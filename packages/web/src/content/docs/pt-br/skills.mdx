...
As habilidades do agente permitem que o deepagent-code descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O deepagent-code pesquisa nesses locais:

- Configuração do projeto: `.deepagent-code/skills/<name>/SKILL.md`
- Configuração global: `~/.config/deepagent-code/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o deepagent-code sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.deepagent-code/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/deepagent-code/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.deepagent-code/skills/git-release/SKILL.md` assim:
...
O deepagent-code lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `deepagent-code.json`:
...
**Para agentes embutidos** (em `deepagent-code.json`):
