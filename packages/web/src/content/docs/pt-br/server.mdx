---
title: Servidor
description: Interaja com o servidor deepagent-code via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `deepagent-code serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente deepagent-code pode usar.
...
```bash
deepagent-code serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `deepagent-code.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
deepagent-code serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `DEEPAGENT_CODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `deepagent-code`, ou defina `DEEPAGENT_CODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `deepagent-code serve` quanto ao `deepagent-code web`.

```bash
DEEPAGENT_CODE_SERVER_PASSWORD=your-password deepagent-code serve
```
...
Quando você executa `deepagent-code`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor deepagent-code para interagir com o deepagent-code programaticamente.
:::

Essa arquitetura permite que o deepagent-code suporte múltiplos clientes e permite que você interaja com o deepagent-code programaticamente.

Você pode executar `deepagent-code serve` para iniciar um servidor autônomo. Se você tiver o TUI do deepagent-code em execução, `deepagent-code serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do deepagent-code [IDE](/docs/ide).
...
O servidor deepagent-code expõe as seguintes APIs.
