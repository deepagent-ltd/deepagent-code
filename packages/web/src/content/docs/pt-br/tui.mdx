---
title: TUI
description: Usando a interface de usuário de terminal deepagent-code.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

O deepagent-code fornece uma interface de terminal interativa ou TUI para trabalhar em seus projetos com um LLM.

Executar deepagent-code inicia o TUI para o diretório atual.
...
Ao usar o TUI do deepagent-code, você pode digitar `/` seguido pelo nome de um comando para executar ações rapidamente. Por exemplo:
...
Adicione um provedor ao deepagent-code. Permite que você selecione entre os provedores disponíveis e adicione suas chaves de API.
...
Saia do deepagent-code. _Aliases_: `/quit`, `/q`
...
Isso é separado do `deepagent-code.json`, que configura o comportamento do servidor/runtime.
...
- `leader_timeout` - Controla por quanto tempo o DeepAgent Code espera depois da leader key. O padrão é `2000`.
...
Use `DEEPAGENT_CODE_TUI_CONFIG` para carregar um caminho de configuração TUI personalizado.

### Attention

Attention permite que o TUI avise você quando o DeepAgent Code espera uma resposta, precisa da aprovação de uma permissão, encontra um erro de sessão ou conclui uma sessão. Ative com `attention.enabled`; eventos integrados reproduzem sons quando acontecem. As notificações de desktop só são enviadas quando a janela do terminal não está em foco e não são usadas para eventos de subagent.
...
- `sound_pack` - ID do sound pack a ser usado. O padrão é `deepagent-code.default`.
