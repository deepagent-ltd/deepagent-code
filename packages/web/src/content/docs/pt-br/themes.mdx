...
Com o deepagent-code, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o deepagent-code usa nosso próprio tema `deepagent-code`.
...
O deepagent-code vem com vários temas embutidos.
...
- Querem que o deepagent-code corresponda à aparência do seu terminal
...
O deepagent-code suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/deepagent-code/themes/*.json` ou `$XDG_CONFIG_HOME/deepagent-code/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.deepagent-code/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.deepagent-code/themes/*.json`
