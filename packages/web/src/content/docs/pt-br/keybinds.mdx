...
O deepagent-code tem uma lista de atalhos de teclado que você pode personalizar através de `tui.json`.
...
O deepagent-code usa uma tecla `leader` para a maioria dos atalhos de teclado. Isso evita conflitos no seu terminal.
...
O prompt de entrada do aplicativo desktop deepagent-code suporta atalhos comuns de estilo Readline/Emacs para edição de texto. Estes são embutidos e atualmente não são configuráveis via `deepagent-code.json`.
