...
O DeepAgent Code Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no DeepAgent Code. Você assina o DeepAgent Code Go e
obtém a sua chave de API. Ele é **totalmente opcional** e você não precisa usá-lo para
usar o DeepAgent Code.
...
O DeepAgent Code Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O DeepAgent Code Go funciona como qualquer outro provedor no DeepAgent Code.

1. Você entra no **<a href={console}>DeepAgent Code Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `DeepAgent Code Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o DeepAgent Code Go.
:::
...
O DeepAgent Code Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do DeepAgent Code
usa o formato `deepagent-code-go/<model-id>`. Por exemplo, para o Kimi K2.6, você usaria
`deepagent-code-go/kimi-k2.6` na sua configuração.
...
Criamos o DeepAgent Code Go para:
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o DeepAgent Code também.
