...
DeepAgent Code has a list of keybinds that you can customize through `tui.json`.
...
DeepAgent Code uses a `leader` key for many keybinds. This avoids conflicts in your terminal.
...
`leader_timeout` controls how long DeepAgent Code waits for the next key after the leader key. It defaults to `2000` milliseconds.
...
The DeepAgent Code desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `deepagent-code.json`.
