...
مع DeepAgent Code يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم DeepAgent Code سمتنا `deepagent-code`.
...
يأتي DeepAgent Code مع عدة سمات مدمجة.
...
- يريدون أن يطابق DeepAgent Code مظهر terminal لديهم
...
يدعم DeepAgent Code نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/deepagent-code/themes/*.json` أو `$XDG_CONFIG_HOME/deepagent-code/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.deepagent-code/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.deepagent-code/themes/*.json`
