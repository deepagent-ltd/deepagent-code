...
يوجد نوعان من الوكلاء في DeepAgent Code: وكلاء أساسيون ووكلاء فرعيون.
...
يأتي DeepAgent Code مع وكيلين أساسيين مدمجين: **Build** و **Plan**. سنلقي نظرة عليهما أدناه.
...
يأتي DeepAgent Code مع ثلاثة وكلاء فرعيين مدمجين: **General** و **Explore** و **Scout**. سنلقي نظرة على ذلك أدناه.

---

## المدمجة

يأتي DeepAgent Code مع وكيلين أساسيين مدمجين وثلاثة وكلاء فرعيين مدمجين.
...
وكيل للقراءة فقط مخصص للوثائق الخارجية وأبحاث التبعيات. استخدمه عندما تحتاج إلى استنساخ مستودع تبعية داخل ذاكرة التخزين المؤقت المُدارة في DeepAgent Code، أو فحص الشفرة المصدرية لمكتبة، أو إجراء مراجع متقاطعة بين الشفرة المحلية والتنفيذات upstream بدون تعديل مساحة العمل الخاصة بك.
...
هيّئ الوكلاء في ملف إعدادات `deepagent-code.json`:
...
- عام: `~/.config/deepagent-code/agents/`
- لكل مشروع: `.deepagent-code/agents/`
...
إذا لم يتم تحديد temperature، يستخدم DeepAgent Code القيم الافتراضية الخاصة بالنموذج؛ عادة 0 لمعظم النماذج و 0.55 لنماذج Qwen.
...
هذا المسار نسبي لمكان وجود ملف الإعدادات. لذلك يعمل هذا لكل من إعدادات DeepAgent Code العامة وإعدادات المشروع.
...
يستخدم معرّف النموذج في إعدادات DeepAgent Code الصيغة `provider/model-id`. على سبيل المثال، إذا كنت تستخدم [DeepAgent Code Zen](/docs/zen)، فستستخدم `deepagent-code/gpt-5.1-codex` لـ GPT 5.1 Codex.
...
:::tip
شغّل `deepagent-code models` لعرض قائمة بالنماذج المتاحة.
:::
...
:::tip
هل لديك وكيل تود مشاركته؟ [قدّم PR](https://github.com/deepagent-ltd/deepagent-code).
:::
