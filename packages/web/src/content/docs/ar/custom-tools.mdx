...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في deepagent-code.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في deepagent-code مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.deepagent-code/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/deepagent-code/tools/`.
