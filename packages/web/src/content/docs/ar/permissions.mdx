...
يستخدم DeepAgent Code إعداد `permission` لتحديد ما إذا كان إجراءٌ ما سيُنفّذ تلقائيًا، أو سيطلب موافقتك، أو سيتم حظره.
...
استخدم `external_directory` للسماح باستدعاءات الأدوات التي تلمس مسارات خارج دليل العمل الذي بدأ منه DeepAgent Code. ينطبق ذلك على أي أداة تأخذ مسارًا كمدخل (مثل `read` و`edit` و`glob` و`grep` والعديد من أوامر `bash`).
...
تُعرَّف أذونات DeepAgent Code بأسماء الأدوات، بالإضافة إلى بعض حواجز الأمان:
...
إذا لم تحدد شيئًا، يبدأ DeepAgent Code بقيم افتراضية متساهلة:
...
عندما يطلب DeepAgent Code الموافقة، تعرض الواجهة ثلاث نتائج ممكنة:

- `once` — الموافقة على هذا الطلب فقط
- `always` — الموافقة على الطلبات المستقبلية المطابقة للأنماط المقترحة (لباقي جلسة DeepAgent Code الحالية)
- `reject` — رفض الطلب
