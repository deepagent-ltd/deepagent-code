---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع DeepAgent Code.
---

تتيح لك الإضافات توسيع DeepAgent Code عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ DeepAgent Code.
...
- `.deepagent-code/plugins/` - إضافات على مستوى المشروع
- `~/.config/deepagent-code/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/deepagent-code/node_modules/`.
...
1. إعدادات عامة (`~/.config/deepagent-code/deepagent-code.json`)
2. إعدادات المشروع (`deepagent-code.json`)
3. دليل الإضافات العام (`~/.config/deepagent-code/plugins/`)
4. دليل إضافات المشروع (`.deepagent-code/plugins/`)
...
يشغّل DeepAgent Code الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ deepagent-code للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع deepagent-code.
...
:::note
إذا كنت تستخدم تطبيق DeepAgent Code لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع deepagent-code من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى deepagent-code:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ deepagent-code استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ deepagent-code إلى جانب الأدوات المضمنة.
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](https://deepagent-code.ai/docs/sdk) للتفاصيل.
