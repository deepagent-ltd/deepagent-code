...
يمكنك إضافة أدوات خارجية إلى DeepAgent Code باستخدام _Model Context Protocol_، أو MCP. يدعم DeepAgent Code خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات DeepAgent Code](https://deepagent-code.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/deepagent-code` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى DeepAgent Code تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم DeepAgent Code بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك DeepAgent Code المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `deepagent-code mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن DeepAgent Code الرموز بشكل آمن في `~/.deepagent/code/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل DeepAgent Code إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات DeepAgent Code مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط DeepAgent Code بحساب Sentry الخاص بك.
