---
title: الخادم
description: تفاعل مع خادم deepagent-code عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `deepagent-code serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل deepagent-code استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `deepagent-code.local` |
...
عيّن `DEEPAGENT_CODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `deepagent-code`، أو عيّن `DEEPAGENT_CODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `deepagent-code serve` و `deepagent-code web`.
...
عند تشغيل `deepagent-code` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم deepagent-code للتفاعل مع deepagent-code برمجيا.
:::

تتيح هذه البنية لـ deepagent-code دعم عدة عملاء وتمكّنك من التفاعل مع deepagent-code برمجيا.

يمكنك تشغيل `deepagent-code serve` لبدء خادم مستقل. إذا كانت واجهة deepagent-code في terminal (TUI)
قيد التشغيل، فسيبدأ `deepagent-code serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات DeepAgent Code لـ [IDE](/docs/ide).
...
يعرض خادم deepagent-code واجهات API التالية.
