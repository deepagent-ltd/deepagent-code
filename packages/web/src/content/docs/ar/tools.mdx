...
تتيح الأدوات لـ LLM تنفيذ إجراءات داخل قاعدة الشفرة الخاصة بك. يأتي DeepAgent Code مع مجموعة من الأدوات المدمجة، لكن يمكنك توسيعه عبر [أدوات مخصصة](/docs/custom-tools) أو [خوادم MCP](/docs/mcp-servers).
...
فيما يلي جميع الأدوات المدمجة المتاحة في DeepAgent Code.
...
:::note
هذه الأداة متاحة فقط عند ضبط `DEEPAGENT_CODE_EXPERIMENTAL_LSP_TOOL=true` (أو `DEEPAGENT_CODE_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود DeepAgent Code أو عند ضبط متغير البيئة `DEEPAGENT_CODE_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).

للتفعيل عند تشغيل DeepAgent Code:
