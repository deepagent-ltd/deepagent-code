...
DeepAgent Code Go هو اشتراك منخفض التكلفة — **$5 للشهر الأول**، ثم **$10/شهريًا** — يمنحك وصولًا موثوقًا إلى نماذج البرمجة المفتوحة الشائعة.

يعمل Go مثل أي مزود آخر في DeepAgent Code. تشترك في DeepAgent Code Go وتحصل على مفتاح API الخاص بك. وهو **اختياري تمامًا**، ولا تحتاج إلى استخدامه لاستخدام DeepAgent Code.
...
:::tip
اختبرنا مجموعة مختارة من النماذج والمزودين الذين يعملون جيدًا مع DeepAgent Code.
:::
...
يمنحك DeepAgent Code Go الوصول إلى هذه النماذج مقابل **$5 للشهر الأول**، ثم **$10/شهريًا**.

---

## كيف يعمل

يعمل DeepAgent Code Go مثل أي مزود آخر في DeepAgent Code.

1. تسجّل الدخول إلى **<a href={console}>DeepAgent Code Zen</a>**، وتشترك في Go، ثم تنسخ مفتاح API الخاص بك.
2. تشغّل الأمر `/connect` في TUI، وتختار `DeepAgent Code Go`، ثم تلصق مفتاح API الخاص بك.
3. شغّل `/models` في TUI لرؤية قائمة النماذج المتاحة عبر Go.

:::note
يمكن لعضو واحد فقط في كل workspace الاشتراك في DeepAgent Code Go.
:::
...
يتضمن DeepAgent Code Go الحدود التالية:
...
يستخدم [model id](/docs/config/#models) في إعدادات DeepAgent Code لديك التنسيق `deepagent-code-go/<model-id>`. على سبيل المثال، بالنسبة إلى Kimi K2.6، ستستخدم `deepagent-code-go/kimi-k2.6` في إعداداتك.
...
أنشأنا DeepAgent Code Go من أجل:
...
4. **عدم فرض قيود ارتباط** من خلال السماح لك أيضًا باستخدام أي مزود آخر مع DeepAgent Code.
