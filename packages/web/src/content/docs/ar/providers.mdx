---
title: المزوّدون
description: استخدام أي موفّر LLM في DeepAgent Code.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم DeepAgent Code ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات DeepAgent Code.
...
في `~/.deepagent/code/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات DeepAgent Code.
...
DeepAgent Code Zen هي قائمة نماذج يوفّرها فريق DeepAgent Code وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع DeepAgent Code. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع DeepAgent Code Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر deepagent-code، ثم انتقل إلى [ai.deepagent.ltd/auth](https://ai.deepagent.ltd/auth).
...
يعمل مثل أي مزوّد آخر في DeepAgent Code واستخدامه اختياري بالكامل.

---

## DeepAgent Code Go

DeepAgent Code Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق DeepAgent Code، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع DeepAgent Code.

1. شغّل الأمر `/connect` في TUI، واختر `DeepAgent Code Go`، ثم انتقل إلى [ai.deepagent.ltd/auth](https://ai.deepagent.ltd/zen).
...
لاستخدام Amazon Bedrock مع DeepAgent Code:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل deepagent-code:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `deepagent-code.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل DeepAgent Code غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين deepagent-code لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل deepagent-code بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات deepagent-code.
...
تخزين الرمز في مساحة تخزين مصادقة deepagent-code.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم DeepAgent Code نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر DeepAgent Code
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`deepagent-code.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [deepagent-code-gitlab-auth](https://www.npmjs.com/package/deepagent-code-gitlab-auth).

##### التهيئة

خصّص عبر `deepagent-code.json`:
...
لاستخدام اشتراك GitHub Copilot مع deepagent-code:
...
لاستخدام Google Vertex AI مع DeepAgent Code:
...
2. عيّن متغيرات البيئة المطلوبة:
   ...
   عيّنها أثناء تشغيل deepagent-code.
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر deepagent-code، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [deepagent-code-helicone-session](https://github.com/H2Shami/deepagent-code-helicone-session) لتسجيل كل محادثة DeepAgent Code تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة DeepAgent Code مدرجة كجلسة مستقلة.
...
يمكنك ضبط deepagent-code لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط deepagent-code لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط deepagent-code لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع DeepAgent Code. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/deepagent-code) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع DeepAgent Code:
...
3. انسخ مفتاح API لاستخدامه في DeepAgent Code.
...
6. **مهم**: قبل استخدام نماذج السحابة في DeepAgent Code، يجب سحب معلومات النموذج محليا:
...
DeepAgent Code Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق DeepAgent Code. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>DeepAgent Code Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **DeepAgent Code Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ DeepAgent Code.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات deepagent-code.
...
5. يمكنك أيضا تخصيصها عبر إعدادات deepagent-code. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع DeepAgent Code:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات deepagent-code. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع deepagent-code. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `deepagent-code.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ DeepAgent Code بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `deepagent-code auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات deepagent-code وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات deepagent-code.
