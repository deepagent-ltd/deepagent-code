---
title: TUI
description: استخدام واجهة المستخدم TUI في DeepAgent Code.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يوفّر DeepAgent Code واجهة terminal تفاعلية (TUI) للعمل على مشاريعك باستخدام LLM.

يؤدي تشغيل DeepAgent Code إلى بدء واجهة TUI للدليل الحالي.
...
عند استخدام واجهة DeepAgent Code في terminal، يمكنك كتابة `/` متبوعة باسم أمر لتنفيذ الإجراءات بسرعة. مثلا:
...
أضف موفّرا إلى DeepAgent Code. يتيح لك اختيار أحد الموفّرين المتاحين وإضافة مفاتيح API الخاصة بهم.
...
اخرج من DeepAgent Code. _الأسماء المستعارة_: `/quit`, `/q`
...
هذا منفصل عن `deepagent-code.json`، الذي يضبط سلوك الخادم ووقت التشغيل.
...
- `leader_timeout` - يتحكم في مدة انتظار DeepAgent Code بعد الضغط على leader key. القيمة الافتراضية `2000`.
...
- `sound_pack` - sound pack ID المراد استخدامه. القيمة الافتراضية `deepagent-code.default`.
