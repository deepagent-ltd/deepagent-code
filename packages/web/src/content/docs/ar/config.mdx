---
title: الإعدادات
description: استخدام ملف إعدادات DeepAgent Code بصيغة JSON.
---

يمكنك ضبط DeepAgent Code باستخدام ملف إعدادات بصيغة JSON.

---

## التنسيق

يدعم DeepAgent Code تنسيقي **JSON** و**JSONC** (JSON مع تعليقات).
...
على سبيل المثال، إذا كان الإعداد العام يضبط `theme: "deepagent-code"` و`autoupdate: true`، وكان إعداد المشروع يضبط `model: "anthropic/claude-sonnet-4-5"`، فستتضمن الإعدادات النهائية الخيارات الثلاثة جميعها.
...
1. **الإعدادات البعيدة** (من `.well-known/deepagent-code`) - الإعدادات الافتراضية على مستوى المؤسسة
2. **الإعدادات العامة** (`~/.config/deepagent-code/deepagent-code.json`) - تفضيلات المستخدم
3. **إعدادات مخصصة** (`DEEPAGENT_CODE_CONFIG` env var) - تجاوزات مخصصة
4. **إعدادات المشروع** (`deepagent-code.json` داخل المشروع) - إعدادات خاصة بالمشروع
5. **أدلة `.deepagent-code`** - الوكلاء، الأوامر، الإضافات
6. **إعدادات ضمنية** (`DEEPAGENT_CODE_CONFIG_CONTENT` env var) - تجاوزات وقت التشغيل
...
:::note
يستخدم الدليلان `.deepagent-code` و`~/.config/deepagent-code` **أسماء جمع** للأدلة الفرعية: `agents/` و`commands/` و`modes/` و`plugins/` و`skills/` و`tools/` و`themes/`. كما تُدعَم أسماء المفرد (مثل `agent/`) للتوافق مع الإصدارات السابقة.
:::
...
يمكن للمؤسسات توفير إعدادات افتراضية عبر نقطة النهاية `.well-known/deepagent-code`. يتم جلب ذلك تلقائيًا عندما تقوم بالمصادقة مع مزوّد يدعم هذه الميزة.
...
ضع إعدادات DeepAgent Code العامة في `~/.config/deepagent-code/deepagent-code.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
أضف `deepagent-code.json` في جذر مشروعك. تمتلك إعدادات المشروع أعلى أولوية بين ملفات الإعدادات القياسية، إذ تتجاوز الإعدادات العامة والبعيدة معًا.

:::tip
ضع إعدادات المشروع الخاصة في جذر مشروعك.
:::

عند تشغيل DeepAgent Code، يبحث عن ملف إعدادات في الدليل الحالي أو يصعد حتى أقرب دليل Git.
...
حدّد مسار ملف إعدادات مخصصًا باستخدام متغير البيئة `DEEPAGENT_CODE_CONFIG`.
...
حدّد دليل إعدادات مخصصًا باستخدام متغير البيئة `DEEPAGENT_CODE_CONFIG_DIR`.
سيتم البحث داخل هذا الدليل عن الوكلاء والأوامر والأوضاع والإضافات تمامًا مثل
الدليل القياسي `.deepagent-code`، ويجب أن يتبع نفس البنية.
...
يُحمَّل الدليل المخصص بعد الإعدادات العامة وأدلة `.deepagent-code`، لذلك **يمكنه تجاوز** إعداداتها.

---

## المخطط

يحتوي ملف الإعدادات على مخطط مُعرَّف في [**`ai.deepagent.ltd/config.schema.json`**](https://ai.deepagent.ltd/config.schema.json).
...
يمكنك ضبط إعدادات الخادم لأوامر `deepagent-code serve` و`deepagent-code web` عبر الخيار `server`.
...
- `mdns` - تفعيل اكتشاف الخدمة عبر mDNS. يتيح ذلك للأجهزة الأخرى على الشبكة اكتشاف خادم DeepAgent Code.
- `mdnsDomain` - اسم نطاق مخصص لخدمة mDNS. القيمة الافتراضية هي `deepagent-code.local`. مفيد لتشغيل عدة نُسخ على نفس الشبكة.
...
يمكنك ضبط المزوّدين والنماذج التي تريد استخدامها في إعدادات DeepAgent Code عبر الخيارات `provider` و`model` و`small_model`.
...
يضبط الخيار `small_model` نموذجًا منفصلًا للمهام الخفيفة مثل توليد العناوين. افتراضيًا يحاول DeepAgent Code استخدام نموذج أقل تكلفة إذا كان متاحًا لدى مزوّدك، وإلا فسيعود إلى النموذج الرئيسي.
...
يمكنك ضبط السمة التي تريد استخدامها في إعدادات DeepAgent Code عبر الخيار `theme`.
...
يمكنك أيضًا تعريف الوكلاء باستخدام ملفات markdown في `~/.config/deepagent-code/agents/` أو `.deepagent-code/agents/`. [تعرف على المزيد هنا](/docs/agents).
...
يجب أن يكون الوكيل الافتراضي وكيلًا أساسيًا (وليس وكيلًا فرعيًا). يمكن أن يكون وكيلًا مدمجًا مثل `"build"` أو `"plan"`، أو [وكيلًا مخصصًا](/docs/agents) قمت بتعريفه. إذا لم يكن الوكيل المحدد موجودًا أو كان وكيلًا فرعيًا، فسيعود DeepAgent Code إلى `"build"` مع تحذير.

ينطبق هذا الإعداد على جميع الواجهات: TUI وCLI (`deepagent-code run`) وتطبيق سطح المكتب وGitHub Action.
...
يمكنك أيضًا تعريف الأوامر باستخدام ملفات markdown في `~/.config/deepagent-code/commands/` أو `.deepagent-code/commands/`. [تعرف على المزيد هنا](/docs/commands).
...
سيقوم DeepAgent Code بتنزيل أي تحديثات جديدة تلقائيًا عند بدء التشغيل. يمكنك تعطيل ذلك عبر الخيار `autoupdate`.
...
افتراضيًا يسمح deepagent-code **بجميع العمليات** دون الحاجة إلى موافقة صريحة. يمكنك تغيير ذلك عبر الخيار `permission`.
...
[الإضافات](/docs/plugins) توسّع DeepAgent Code بأدوات وخُطافات وتكاملات مخصصة.

ضع ملفات الإضافات في `.deepagent-code/plugins/` أو `~/.config/deepagent-code/plugins/`. يمكنك أيضًا تحميل إضافات من npm عبر الخيار `plugin`.
...
هذا مفيد عندما تريد تقييد DeepAgent Code لاستخدام مزوّدين محددين بدلًا من تعطيلهم واحدًا تلو الآخر.
