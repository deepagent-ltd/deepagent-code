---
title: Serveurs LSP
description: DeepAgent Code s'intègre à vos serveurs LSP.
---

DeepAgent Code peut s'intégrer aux serveurs Language Server Protocol (LSP) pour utiliser les diagnostics comme retour pour l'agent.

---

## Serveurs intégrés

DeepAgent Code est livré avec plusieurs serveurs LSP intégrés pour les langages populaires :
...
:::note
Vous pouvez désactiver les téléchargements automatiques du serveur LSP en définissant la variable d'environnement `DEEPAGENT_CODE_DISABLE_LSP_DOWNLOAD` sur `true`.
:::
...
Lorsque LSP est activé et qu'deepagent-code ouvre un fichier, il :
...
Vous pouvez activer et personnaliser les serveurs LSP via la section `lsp` de votre configuration deepagent-code.
...
```json title="deepagent-code.json" {5-7}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="deepagent-code.json" {5-9}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "lsp": {
    "typescript": {
