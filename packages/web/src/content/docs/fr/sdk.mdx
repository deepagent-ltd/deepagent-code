---
title: SDK
description: Client JS de type sécurisé pour le serveur deepagent-code.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK deepagent-code JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler deepagent-code par programme.
...
```bash
npm install @deepagent-code/sdk
```
...
Créez une instance de deepagent-code :

```javascript
import { createDeepAgentCode } from "@deepagent-code/sdk"

const { client } = await createDeepAgentCode()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `deepagent-code.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createDeepAgentCode } from "@deepagent-code/sdk"

const deepagentCode = await createDeepAgentCode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${deepagentCode.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de deepagent-code, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@deepagent-code/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*deepagent-code" },
})
