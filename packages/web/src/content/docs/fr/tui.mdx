---
title: TUI
description: Utilisation de l’interface utilisateur du terminal DeepAgent Code.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

DeepAgent Code fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de DeepAgent Code démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le DeepAgent Code TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à DeepAgent Code. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Quittez DeepAgent Code. *Alias* : `/quit`, `/q`
...
Ceci est séparé de `deepagent-code.json`, qui configure le comportement du serveur/d'exécution.
...
- `leader_timeout` - Contrôle la durée pendant laquelle DeepAgent Code attend après la leader key. Valeur par défaut : `2000`.
...
- `sound_pack` - ID du sound pack à utiliser. Valeur par défaut : `deepagent-code.default`.
