...
Les outils permettent au LLM d'effectuer des actions dans votre base de code. DeepAgent Code est livré avec un ensemble d'outils intégrés, mais vous pouvez l'étendre avec [outils personnalisés](/docs/custom-tools) ou [serveurs MCP](/docs/mcp-servers).
...
Voici tous les outils intégrés disponibles dans DeepAgent Code.
...
:::note
Cet outil n'est disponible que lorsque `DEEPAGENT_CODE_EXPERIMENTAL_LSP_TOOL=true` (ou `DEEPAGENT_CODE_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur DeepAgent Code ou lorsque la variable d'environnement `DEEPAGENT_CODE_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).

À activer lors du lancement de DeepAgent Code :
