...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez DeepAgent Code.
...
Voici plusieurs modèles qui fonctionnent bien avec DeepAgent Code, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration DeepAgent Code.

```json title="deepagent-code.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [DeepAgent Code Zen](/docs/zen), vous utiliserez `deepagent-code/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="deepagent-code.jsonc" {7-12,19-24}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="deepagent-code.jsonc" {6-21}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "provider": {
    "deepagent-code": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. DeepAgent Code est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

DeepAgent Code est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="deepagent-code.jsonc" {7-18}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque DeepAgent Code démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration DeepAgent Code.
