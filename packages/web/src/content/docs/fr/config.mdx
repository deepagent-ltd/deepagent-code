---
title: Configuration
description: Utilisation de la configuration JSON DeepAgent Code.
---

Vous pouvez configurer DeepAgent Code à l'aide d'un fichier de configuration JSON.

---

## Format

DeepAgent Code prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
1. **Configuration distante** (à partir de `.well-known/deepagent-code`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/deepagent-code/deepagent-code.json`) - préférences utilisateur
3. **Configuration personnalisée** (`DEEPAGENT_CODE_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`deepagent-code.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.deepagent-code`** - agents, commandes, plugins
6. **Configuration en ligne** (`DEEPAGENT_CODE_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.deepagent-code` et `~/.config/deepagent-code` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/deepagent-code`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
Placez votre configuration globale DeepAgent Code dans `~/.config/deepagent-code/deepagent-code.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/deepagent-code/tui.json`.
...
Ajoutez `deepagent-code.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque DeepAgent Code démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `DEEPAGENT_CODE_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `DEEPAGENT_CODE_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.deepagent-code`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.deepagent-code`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`ai.deepagent.ltd/config.schema.json`**](https://ai.deepagent.ltd/config.schema.json).

La configuration TUI utilise [**`ai.deepagent.ltd/tui.json`**](https://ai.deepagent.ltd/tui.json).
...
Utilisez `DEEPAGENT_CODE_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `deepagent-code.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `deepagent-code serve` et `deepagent-code web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur DeepAgent Code.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `deepagent-code.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration DeepAgent Code via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, DeepAgent Code essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/deepagent-code/agents/` ou `.deepagent-code/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, DeepAgent Code reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`deepagent-code run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/deepagent-code/commands/` ou `.deepagent-code/commands/`. [En savoir plus ici](/docs/commands).
...
DeepAgent Code téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, deepagent-code **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent DeepAgent Code avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.deepagent-code/plugins/` ou `~/.config/deepagent-code/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
...
Ceci est utile lorsque vous souhaitez restreindre DeepAgent Code à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
