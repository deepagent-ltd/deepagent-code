---
title: Plugins
description: Écrivez vos propres plugins pour étendre DeepAgent Code.
---

Les plugins vous permettent d'étendre DeepAgent Code en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de DeepAgent Code.
...
- `.deepagent-code/plugins/` - Plugins au niveau du projet
- `~/.config/deepagent-code/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/deepagent-code/node_modules/`.
...
1. Configuration globale (`~/.config/deepagent-code/deepagent-code.json`)
2. Configuration du projet (`deepagent-code.json`)
3. Répertoire global des plugins (`~/.config/deepagent-code/plugins/`)
4. Répertoire des plugins du projet (`.deepagent-code/plugins/`)
...
DeepAgent Code exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK deepagent-code pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre deepagent-code.
...
:::note
Si vous utilisez l'application de bureau DeepAgent Code, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez deepagent-code de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à deepagent-code :
...
L'assistant `tool` crée un outil personnalisé que deepagent-code peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour deepagent-code aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://deepagent-code.ai/docs/sdk) pour plus de détails.
