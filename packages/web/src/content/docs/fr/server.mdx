---
title: Serveur
description: Interagissez avec le serveur deepagent-code via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `deepagent-code serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client deepagent-code peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `deepagent-code.local` |
...
Définissez `DEEPAGENT_CODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `deepagent-code`, ou définissez `DEEPAGENT_CODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `deepagent-code serve` et à `deepagent-code web`.
...
Lorsque vous exécutez `deepagent-code`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur deepagent-code pour interagir avec deepagent-code par programmation.
:::

Cette architecture permet à deepagent-code de prendre en charge plusieurs clients et vous permet d'interagir avec deepagent-code par programme.

Vous pouvez exécuter `deepagent-code serve` pour démarrer un serveur autonome. Si vous avez le
deepagent-code TUI en cours d'exécution, `deepagent-code serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins DeepAgent Code [IDE](/docs/ide).
...
Le serveur deepagent-code expose les API suivantes.
