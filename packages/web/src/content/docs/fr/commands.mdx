...
Créez `.deepagent-code/commands/test.md` :
...
Vous pouvez ajouter des commandes personnalisées via la configuration DeepAgent Code ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre DeepAgent Code [config](/docs/config) :
...
- Par projet : `.deepagent-code/commands/`
...
deepagent-code comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
