---
title: Formateurs
description: DeepAgent Code utilise des formateurs spécifiques au langage.
---

DeepAgent Code formate automatiquement les fichiers après leur écriture ou leur modification à l'aide de formateurs spécifiques au langage. Cela garantit que le code généré suit les styles de code de votre projet.

---

## Formateurs intégrés

DeepAgent Code est livré avec plusieurs formateurs intégrés pour les langages et frameworks populaires. Vous trouverez ci-dessous une liste des formateurs, des extensions de fichiers prises en charge et des commandes ou options de configuration dont il a besoin.
...
Ainsi, si votre projet a `prettier` dans votre `package.json`, DeepAgent Code l'utilisera automatiquement.

---

## Comment ça marche

Lorsque DeepAgent Code écrit ou modifie un fichier, il :
...
Vous pouvez personnaliser les formateurs via la section `formatter` de votre configuration DeepAgent Code.
...
```json title="deepagent-code.json" {5}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "formatter": {
    "prettier": {
...
```json title="deepagent-code.json" {4-14}
{
  "$schema": "https://deepagent-code.ai/config.json",
  "formatter": {
    "prettier": {
