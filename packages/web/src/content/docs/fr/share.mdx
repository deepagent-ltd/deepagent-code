---
title: Partager
description: Partagez vos conversations DeepAgent Code.
---

La fonctionnalité de partage de DeepAgent Code vous permet de créer des liens publics vers vos conversations DeepAgent Code, afin que vous puissiez collaborer avec vos coéquipiers ou obtenir de l'aide d'autres personnes.
...
Lorsque vous partagez une conversation, DeepAgent Code :
...
DeepAgent Code prend en charge trois modes de partage qui contrôlent la manière dont les conversations sont partagées :

---

### Manuel (par défaut)

Par défaut, DeepAgent Code utilise le mode de partage manuel. Les sessions ne sont pas partagées automatiquement, mais vous pouvez les partager manuellement à l'aide de la commande `/share` :
...
Pour appliquer cela au sein de votre équipe pour un projet donné, ajoutez-le au `deepagent-code.json` dans votre projet et enregistrez-vous dans Git.
...
[En savoir plus](/docs/enterprise) sur l'utilisation de deepagent-code dans votre organisation.
