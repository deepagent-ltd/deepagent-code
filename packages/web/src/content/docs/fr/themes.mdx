...
Avec DeepAgent Code, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, DeepAgent Code utilise notre propre thème `deepagent-code`.
...
DeepAgent Code est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que DeepAgent Code corresponde à l'apparence de leur terminal
...
DeepAgent Code prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/deepagent-code/themes/*.json` ou `$XDG_CONFIG_HOME/deepagent-code/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.deepagent-code/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.deepagent-code/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/deepagent-code/themes
vim ~/.config/deepagent-code/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .deepagent-code/themes
vim .deepagent-code/themes/my-theme.json
```
...
{
  "$schema": "https://deepagent-code.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
