...
Pour déboguer les problèmes avec DeepAgent Code, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\deepagent-code\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `deepagent-code --log-level DEBUG`.

---

## Stockage

deepagent-code stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.deepagent/code/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\deepagent-code`
...
DeepAgent Code Desktop exécute un serveur DeepAgent Code local (le side-car `deepagent-code-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `DeepAgent Code` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** : `~/.config/deepagent-code/deepagent-code.jsonc` (ou `~/.config/deepagent-code/deepagent-code.json`)
- **macOS/Linux** (anciennes installations) : `~/.deepagent/code/deepagent-code.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\deepagent-code\deepagent-code.jsonc`
...
DeepAgent Code peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/deepagent-code/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\deepagent-code\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que DeepAgent Code puisse le reconstruire.

1. Quittez complètement DeepAgent Code Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/deepagent-code`
- **Linux** : supprimez `~/.cache/deepagent-code` (ou exécutez `rm -rf ~/.cache/deepagent-code`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\deepagent-code`

3. Redémarrez le bureau DeepAgent Code.

---

### Résoudre les problèmes de connexion au serveur

DeepAgent Code Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `deepagent-code.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `DEEPAGENT_CODE_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `DEEPAGENT_CODE_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, DeepAgent Code Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des lenteurs, des problèmes d'accès aux fichiers ou des problèmes de terminal sous Windows, vérifiez que vous utilisez une version native récente — consultez le [guide Windows](/docs/windows).

---

### Les notifications ne s'affichent pas

DeepAgent Code Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour DeepAgent Code dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau DeepAgent Code.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application DeepAgent Code Desktop) :

- `deepagent-code.settings.dat` (serveur de bureau par défaut URL)
- `deepagent-code.global.dat` et `deepagent-code.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec DeepAgent Code :
...
[**github.com/deepagent-ltd/deepagent-code/issues**](https://github.com/deepagent-ltd/deepagent-code/issues)
...
[**ai.deepagent.ltd/discord**](https://ai.deepagent.ltd/discord)
...
3. Assurez-vous d'avoir la dernière version avec `deepagent-code upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `deepagent-code models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\deepagent-code`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. deepagent-code installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\deepagent-code`

2. Redémarrez deepagent-code pour réinstaller les derniers packages du fournisseur

Cela forcera deepagent-code à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
deepagent-code détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
