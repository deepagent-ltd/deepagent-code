---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en DeepAgent Code.
---

import config from "../../../../config.mjs"
export const console = config.console

DeepAgent Code utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration DeepAgent Code.
...
en `~/.deepagent/code/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre DeepAgent Code
configuration.
...
DeepAgent Code Zen est une liste de modèles fournis par l'équipe DeepAgent Code qui ont été
testé et vérifié pour fonctionner correctement avec DeepAgent Code. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par DeepAgent Code Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez deepagent-code et dirigez-vous vers [ai.deepagent.ltd/auth](https://ai.deepagent.ltd/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans DeepAgent Code et son utilisation est totalement facultative.

---

## DeepAgent Code Go

DeepAgent Code Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe DeepAgent Code qui ont été
testé et vérifié pour fonctionner correctement avec DeepAgent Code.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `DeepAgent Code Go` et rendez-vous sur [ai.deepagent.ltd/auth](https://ai.deepagent.ltd/zen).
...
Pour utiliser Amazon Bedrock avec DeepAgent Code :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de deepagent-code :
...
Pour une configuration spécifique au projet ou persistante, utilisez `deepagent-code.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans DeepAgent Code n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer deepagent-code pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que deepagent-code fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration deepagent-code.
...
3.  Nom : `DeepAgent Code`, Portées : `api`
...
pour stocker le jeton dans le stockage d'authentification deepagent-code.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
DeepAgent Code utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller DeepAgent Code
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `deepagent-code.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [deepagent-code-gitlab-auth](https://www.npmjs.com/package/deepagent-code-gitlab-auth).

##### Configuration

Personnalisez via `deepagent-code.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec deepagent-code :
...
Pour utiliser Google Vertex AI avec DeepAgent Code :
...
Définissez-les lors de l'exécution de deepagent-code.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via deepagent-code, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [deepagent-code-helicone-session](https://github.com/H2Shami/deepagent-code-helicone-session) pour enregistrer automatiquement chaque conversation DeepAgent Code en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation DeepAgent Code répertoriée comme une session distincte.
...
Vous pouvez configurer deepagent-code pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer deepagent-code pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer deepagent-code pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour DeepAgent Code. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/deepagent-code) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec DeepAgent Code :
...
3. Copiez la clé API à utiliser dans DeepAgent Code.
...
6. **Important** : Avant d'utiliser des modèles cloud dans DeepAgent Code, vous devez extraire les informations du modèle localement :
...
DeepAgent Code Zen est une liste de modèles testés et vérifiés fournie par l'équipe DeepAgent Code. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>DeepAgent Code Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **DeepAgent Code Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé DeepAgent Code API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration deepagent-code.
...
5. Vous pouvez également les personnaliser via votre configuration deepagent-code. Voici un exemple de spécification d'un fournisseur
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec DeepAgent Code :
...
Vous pouvez également personnaliser les modèles via votre configuration deepagent-code. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec deepagent-code. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `deepagent-code.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à DeepAgent Code de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `deepagent-code auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration deepagent-code et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration deepagent-code.
