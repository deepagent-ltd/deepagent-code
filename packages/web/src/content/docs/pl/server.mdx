---
title: Serwer
description: Interakcja z serwerem deepagent-code za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `deepagent-code serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient deepagent-code.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `deepagent-code.local` |
...
Ustaw `DEEPAGENT_CODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `deepagent-code` lub ustaw `DEEPAGENT_CODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `deepagent-code serve`, jak i `deepagent-code web`.
...
Kiedy uruchomisz `deepagent-code`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera deepagent-code do programowej interakcji z deepagent-code.
:::

Ta architektura umożliwia obsługę wielu klientów przez deepagent-code i programową interakcję z kodem otwartym.

Możesz uruchomić `deepagent-code serve`, aby uruchomić samodzielny serwer. Jeśli masz
deepagent-code TUI działa, `deepagent-code serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki deepagent-code [IDE](/docs/ide).
...
Serwer deepagent-code udostępnia następujące interfejsy API.
