...
DeepAgent Code Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w DeepAgent Code. Subskrybujesz DeepAgent Code Go i
otrzymujesz swój klucz API. Jest to **całkowicie opcjonalne** i nie musisz z tego korzystać, aby
używać DeepAgent Code.
...
DeepAgent Code Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

DeepAgent Code Go działa jak każdy inny dostawca w DeepAgent Code.

1. Logujesz się do **<a href={console}>DeepAgent Code Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `DeepAgent Code Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować DeepAgent Code Go.
:::
...
DeepAgent Code Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji DeepAgent Code
używa formatu `deepagent-code-go/<model-id>`. Na przykład dla Kimi K2.6 należy użyć
`deepagent-code-go/kimi-k2.6` w swojej konfiguracji.
...
Stworzyliśmy DeepAgent Code Go, aby:
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z DeepAgent Code.
