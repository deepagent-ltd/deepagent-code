...
Możesz dodać dodatkowe informacje do deepagent-code za pomocą _Model Context Protocol_, w skrócie MCP. deepagent-code obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [deepagent-code Config](https://deepagent-code.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/deepagent-code`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
deepagent-code automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, deepagent-code:
...
Jeśli serwer wymaga uwierzytelnienia, deepagent-code poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `deepagent-code mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji deepagent-code bezpieczne przechowa tokeny w `~/.deepagent/code/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w deepagent-code, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie deepagent-code, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć deepagent-code z kontem Sentry.
