---
title: Konfiguracja
description: Ustawienia DeepAgent Code w pliku JSON.
---

Możesz dostosować DeepAgent Code za pomocą pliku konfiguracyjnego JSON.

---

## Format

DeepAgent Code obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "deepagent-code"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/deepagent-code`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/deepagent-code/deepagent-code.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `DEEPAGENT_CODE_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`deepagent-code.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.deepagent-code`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `DEEPAGENT_CODE_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.deepagent-code` i `~/.config/deepagent-code` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/deepagent-code`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację DeepAgent Code w `~/.config/deepagent-code/deepagent-code.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/deepagent-code/tui.json`.
...
Dodaj `deepagent-code.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy DeepAgent Code się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `DEEPAGENT_CODE_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `DEEPAGENT_CODE_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.deepagent-code` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.deepagent-code`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`ai.deepagent.ltd/config.schema.json`**](https://ai.deepagent.ltd/config.schema.json).
...
Użyj `DEEPAGENT_CODE_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `deepagent-code.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `deepagent-code serve` i `deepagent-code web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer DeepAgent Code.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `deepagent-code.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie DeepAgent Code wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/deepagent-code/agents/` lub `.deepagent-code/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, DeepAgent Code powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`deepagent-code run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/deepagent-code/commands/` lub `.deepagent-code/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
DeepAgent Code może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy DeepAgent Code nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie DeepAgent Code **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają DeepAgent Code o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.deepagent-code/plugins/` lub `~/.config/deepagent-code/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie DeepAgent Code tylko do określonych dostawców.
