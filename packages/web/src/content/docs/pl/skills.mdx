...
Umiejętności agenta pozwalają deepagent-code odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
deepagent-code przeszukuje te lokalizacje:

- Project config: `.deepagent-code/skills/<name>/SKILL.md`
- Global config: `~/.config/deepagent-code/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu deepagent-code przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.deepagent-code/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/deepagent-code/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.deepagent-code/skills/git-release/SKILL.md` w ten sposób:
...
deepagent-code wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `deepagent-code.json`:
...
**Dla agentów wbudowanych** (w `deepagent-code.json`):
