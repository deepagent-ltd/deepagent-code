---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w deepagent-code.
---

import config from "../../../../config.mjs"
export const console = config.console

deepagent-code używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji deepagent-code.
...
w `~/.deepagent/code/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim deepagent-code
config.
...
DeepAgent Code Zen to lista modeli dostarczonych przez zespół deepagent-code, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z deepagent-code. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od DeepAgent Code Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `DeepAgent Code Zen` i przejdź do [deepagent-code.ai/auth](https://deepagent-code.ai/zen).
...
Działa jak każdy inny dostawca w deepagent-code i jest całkowicie opcjonalny w użyciu.

---

## DeepAgent Code Go

DeepAgent Code Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół deepagent-code, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z deepagent-code.

1. Uruchom polecenie `/connect` w TUI, wybierz `DeepAgent Code Go` i przejdź do [deepagent-code.ai/auth](https://deepagent-code.ai/zen).
...
Aby używać Amazon Bedrock z deepagent-code:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania deepagent-code:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `deepagent-code.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w deepagent-code nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować deepagent-code do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod deepagent-code działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji deepagent-code.
...
deepagent-code używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować deepagent-code
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`deepagent-code.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [deepagent-code-gitlab-auth](https://www.npmjs.com/package/deepagent-code-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem deepagent-code:
...
Aby używać Google Vertex AI z deepagent-code:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania deepagent-code.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą deepagent-code, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [deepagent-code-helicone-session](https://github.com/H2Shami/deepagent-code-helicone-session), aby automatycznie rejestrować każdą konwersację deepagent-code jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja deepagent-code będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować deepagent-code tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować deepagent-code tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować deepagent-code tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem deepagent-code. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/deepagent-code).
:::
...
Aby korzystać z Ollama Cloud z deepagent-code:
...
3. Skopiuj klucz API do użycia w deepagent-code.
...
6. **Ważne**: Przed użyciem modeli chmurowych w deepagent-code musisz pobrać informacje o modelu lokalnie:
...
DeepAgent Code Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół deepagent-code. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>DeepAgent Code Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **DeepAgent Code Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API deepagent-code.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji deepagent-code.
...
5. Możesz je także dostosować za pomocą konfiguracji deepagent-code. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z deepagent-code:
...
Możesz także dostosować modele za pomocą konfiguracji deepagent-code. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z deepagent-code. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `deepagent-code.json` w katalogu projektu:
...
Pola `limit` pozwalają deepagent-code zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `deepagent-code auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację deepagent-code i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji deepagent-code.
