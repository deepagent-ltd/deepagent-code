...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. DeepAgent Code posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w DeepAgent Code.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `DEEPAGENT_CODE_EXPERIMENTAL_LSP_TOOL=true` (lub `DEEPAGENT_CODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy DeepAgent Code lub gdy zmienna środowiskowa `DEEPAGENT_CODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu DeepAgent Code:
