...
Narzędzia specjalistyczne to funkcje, które tworzysz i które LLM może wywoływać podczas rozmów. Współpracują one z [wbudowanymi narzędziami](/docs/tools) deepagent-code, takimi jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.deepagent-code/tools/` swojego projektu.
- Lub globalnie, umieszczając je w `~/.config/deepagent-code/tools/`.
