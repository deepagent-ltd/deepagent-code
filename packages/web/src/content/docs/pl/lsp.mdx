---
title: Serwery LSP
description: DeepAgent Code integruje się z serwerami LSP.
---

DeepAgent Code może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

DeepAgent Code posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `DEEPAGENT_CODE_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i DeepAgent Code otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji DeepAgent Code.
