...
Dzięki deepagent-code możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie deepagent-code używa naszego własnego motywu `deepagent-code`.
...
deepagent-code ma kilka wbudowanych motywów.
...
- Chcą, aby DeepAgent Code pasował do wyglądu ich terminala
...
deepagent-code obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/deepagent-code/themes/*.json` lub `$XDG_CONFIG_HOME/deepagent-code/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.deepagent-code/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.deepagent-code/themes/*.json`
