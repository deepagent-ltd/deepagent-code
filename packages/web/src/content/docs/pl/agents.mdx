...
W DeepAgent Code obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
DeepAgent Code zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
DeepAgent Code ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

DeepAgent Code ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej DeepAgent Code, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Skonfiguruj agenta w pliku `deepagent-code.json`:
...
- Globalnie: `~/.config/deepagent-code/agents/`
- W projekcie: `.deepagent-code/agents/`
...
Jeśli nie zostanie skonfigurowana, DeepAgent Code używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji DeepAgent Code, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji DeepAgent Code w `provider/model-id`. Na przykład, [DeepAgent Code Zen](/docs/zen), użyj `deepagent-code/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `deepagent-code models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/deepagent-ltd/deepagent-code).
:::
