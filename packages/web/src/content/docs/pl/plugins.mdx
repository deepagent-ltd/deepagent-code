---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć deepagent-code.
---

Wtyczki umożliwiające wyłączenie deepagent-code poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania deepagent-code.
...
- `.deepagent-code/plugins/` - Wtyczki na poziomie projektu
- `~/.config/deepagent-code/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/deepagent-code/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/deepagent-code/deepagent-code.json`)
2. Konfiguracja projektu (`deepagent-code.json`)
3. Globalny katalog wtyczek (`~/.config/deepagent-code/plugins/`)
4. Katalog wtyczek projektu (`.deepagent-code/plugins/`)
...
deepagent-code uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z deepagent-code do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do deepagent-code.
...
:::note
Jeśli korzystasz z aplikacji komputerowej deepagent-code, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj deepagent-code czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do deepagent-code:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod deepagent-code. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://deepagent-code.ai/docs/sdk).
