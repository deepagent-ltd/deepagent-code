...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. DeepAgent Code include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in DeepAgent Code.
...
:::note
Questo strumento e' disponibile solo quando `DEEPAGENT_CODE_EXPERIMENTAL_LSP_TOOL=true` (oppure `DEEPAGENT_CODE_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider DeepAgent Code o quando la variabile d'ambiente `DEEPAGENT_CODE_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di DeepAgent Code:
