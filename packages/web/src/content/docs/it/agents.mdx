...
In DeepAgent Code esistono due tipi di agenti: agenti primari e subagenti.
...
DeepAgent Code include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
DeepAgent Code include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

DeepAgent Code include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di DeepAgent Code, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Configura gli agenti nel file `deepagent-code.json`:
...
- Globale: `~/.config/deepagent-code/agents/`
- Per progetto: `.deepagent-code/agents/`
...
Se non specifichi una temperature, DeepAgent Code usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di DeepAgent Code sia per la config specifica del progetto.
...
L'ID modello nella configurazione DeepAgent Code usa il formato `provider/model-id`. Per esempio, se stai usando [DeepAgent Code Zen](/docs/zen), useresti `deepagent-code/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `deepagent-code models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/deepagent-ltd/deepagent-code).
:::
