...
Con DeepAgent Code puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, DeepAgent Code usa il tema `deepagent-code`.
...
DeepAgent Code include diversi temi integrati.
...
- Vuole che DeepAgent Code corrisponda all'aspetto del terminale
...
DeepAgent Code supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/deepagent-code/themes/*.json` o `$XDG_CONFIG_HOME/deepagent-code/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.deepagent-code/themes/*.json`
4. **Directory di lavoro corrente** - in `./.deepagent-code/themes/*.json`
