...
Puoi aggiungere strumenti esterni a DeepAgent Code usando il _Model Context Protocol_, o MCP. DeepAgent Code supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [DeepAgent Code Config](https://deepagent-code.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/deepagent-code`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
DeepAgent Code gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, DeepAgent Code:
...
Se il server richiede autenticazione, DeepAgent Code ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `deepagent-code mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, DeepAgent Code memorizzera' i token in modo sicuro in `~/.deepagent/code/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in DeepAgent Code insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione DeepAgent Code come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare DeepAgent Code al tuo account Sentry.
