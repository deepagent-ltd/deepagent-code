---
title: Server LSP
description: DeepAgent Code si integra con i tuoi server LSP.
---

DeepAgent Code può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

DeepAgent Code include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `DEEPAGENT_CODE_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e deepagent-code apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di deepagent-code.
