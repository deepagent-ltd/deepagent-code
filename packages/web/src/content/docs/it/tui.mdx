---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di DeepAgent Code.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

DeepAgent Code fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire DeepAgent Code avvia la TUI per la directory corrente.
...
Quando usi la TUI di DeepAgent Code, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a DeepAgent Code. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da DeepAgent Code. _Alias_: `/quit`, `/q`
...
Questo è separato da `deepagent-code.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo DeepAgent Code attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `deepagent-code.default`.
