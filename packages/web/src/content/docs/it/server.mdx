---
title: Server
description: Interagisci con il server deepagent-code via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `deepagent-code serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client deepagent-code.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `deepagent-code.local` |
...
Imposta `DEEPAGENT_CODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `deepagent-code`, oppure imposta `DEEPAGENT_CODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `deepagent-code serve` sia per `deepagent-code web`.
...
Quando esegui `deepagent-code` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server deepagent-code per interagire con deepagent-code in modo programmatico.
:::

Questa architettura permette a deepagent-code di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `deepagent-code serve` per avviare un server standalone. Se la TUI di deepagent-code e gia in esecuzione, `deepagent-code serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di DeepAgent Code.
...
Il server deepagent-code espone le seguenti API.
