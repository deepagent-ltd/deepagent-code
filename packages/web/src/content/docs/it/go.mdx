...
DeepAgent Code Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in DeepAgent Code. Ti abboni a DeepAgent Code Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare DeepAgent Code.
...
DeepAgent Code Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

DeepAgent Code Go funziona come qualsiasi altro provider in DeepAgent Code.

1. Accedi a **<a href={console}>DeepAgent Code Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `DeepAgent Code Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a DeepAgent Code Go.
:::
...
DeepAgent Code Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua DeepAgent Code config
utilizza il formato `deepagent-code-go/<model-id>`. Ad esempio, per Kimi K2.6, useresti
`deepagent-code-go/kimi-k2.6` nella tua configurazione.
...
Abbiamo creato DeepAgent Code Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con DeepAgent Code.
