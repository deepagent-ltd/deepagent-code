---
title: Provider
description: Usa qualsiasi provider LLM in DeepAgent Code.
---

import config from "../../../../config.mjs"
export const console = config.console

DeepAgent Code usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di DeepAgent Code.
...
configurazione di DeepAgent Code.
...
DeepAgent Code Zen e una lista di modelli forniti dal team DeepAgent Code, testati e
verificati per funzionare bene con DeepAgent Code. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da DeepAgent Code Zen.
:::

1. Esegui `/connect` nella TUI, seleziona deepagent-code e vai su [ai.deepagent.ltd/auth](https://ai.deepagent.ltd/auth).
...
Funziona come qualsiasi altro provider in DeepAgent Code ed e totalmente opzionale.
...
Per usare Amazon Bedrock con DeepAgent Code:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui deepagent-code:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `deepagent-code.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in DeepAgent Code non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare deepagent-code per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché deepagent-code funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di deepagent-code.
...
2. Scegli il tuo metodo di autenticazione:
   ...
   3. Nome: `DeepAgent Code`, Scopes: `api`
...
salvare il token nello storage di auth di deepagent-code.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
DeepAgent Code usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare DeepAgent Code
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `deepagent-code.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [deepagent-code-gitlab-auth](https://www.npmjs.com/package/deepagent-code-gitlab-auth).

##### Configurazione

Personalizza tramite `deepagent-code.json`:
...
Per usare il tuo abbonamento GitHub Copilot con deepagent-code:
...
Per usare Google Vertex AI con DeepAgent Code:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui deepagent-code.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da deepagent-code, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [deepagent-code-helicone-session](https://github.com/H2Shami/deepagent-code-helicone-session) per loggare automaticamente ogni conversazione di DeepAgent Code come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di DeepAgent Code elencata come sessione separata.
...
Puoi configurare deepagent-code per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare deepagent-code per usare modelli locali tramite LM Studio.
...
Puoi configurare deepagent-code per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per DeepAgent Code. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/deepagent-code) per dettagli.
:::
...
Per usare Ollama Cloud con DeepAgent Code:
...
3. Copia la chiave API da usare in DeepAgent Code.
...
6. **Importante**: Prima di usare modelli cloud in DeepAgent Code, devi scaricare le informazioni del modello localmente:
...
DeepAgent Code Zen è una lista di modelli testati e verificati forniti dal team DeepAgent Code. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>DeepAgent Code Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **DeepAgent Code Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di DeepAgent Code.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di deepagent-code.
...
5. Puoi anche personalizzarli tramite la configurazione di deepagent-code. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con DeepAgent Code:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di deepagent-code. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con deepagent-code. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `deepagent-code.json` nella directory del progetto:
...
I campi `limit` permettono a DeepAgent Code di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `deepagent-code auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di deepagent-code e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config deepagent-code.
