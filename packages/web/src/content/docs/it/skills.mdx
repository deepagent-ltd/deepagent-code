...
Le skill degli agenti permettono a DeepAgent Code di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
DeepAgent Code cerca in queste posizioni:

- Config di progetto: `.deepagent-code/skills/<name>/SKILL.md`
- Config globale: `~/.config/deepagent-code/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, DeepAgent Code risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.deepagent-code/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/deepagent-code/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.deepagent-code/skills/git-release/SKILL.md` cosi':
...
DeepAgent Code elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `deepagent-code.json`:
...
**Per agenti integrati** (in `deepagent-code.json`):
