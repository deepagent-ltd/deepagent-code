...
description: Usa la configurazione JSON di DeepAgent Code.
---

Puoi configurare DeepAgent Code usando un file di configurazione JSON.

---

## Formato

DeepAgent Code supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/deepagent-code`) - default dell'organizzazione
2. **Config globale** (`~/.config/deepagent-code/deepagent-code.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `DEEPAGENT_CODE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`deepagent-code.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.deepagent-code`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `DEEPAGENT_CODE_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.deepagent-code` e `~/.config/deepagent-code` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/deepagent-code`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di DeepAgent Code in `~/.config/deepagent-code/deepagent-code.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/deepagent-code/tui.json`.
...
Aggiungi `deepagent-code.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando DeepAgent Code si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `DEEPAGENT_CODE_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `DEEPAGENT_CODE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.deepagent-code` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.deepagent-code`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`ai.deepagent.ltd/config.schema.json`**](https://ai.deepagent.ltd/config.schema.json).

La configurazione TUI usa [**`ai.deepagent.ltd/tui.json`**](https://ai.deepagent.ltd/tui.json).
...
Usa `DEEPAGENT_CODE_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `deepagent-code.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `deepagent-code serve` e `deepagent-code web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server DeepAgent Code.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `deepagent-code.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in DeepAgent Code tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, DeepAgent Code prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/deepagent-code/agents/` o `.deepagent-code/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, DeepAgent Code fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`deepagent-code run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/deepagent-code/commands/` o `.deepagent-code/commands/`. [Scopri di piu](/docs/commands).
...
DeepAgent Code scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, deepagent-code **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono DeepAgent Code con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.deepagent-code/plugins/` o `~/.config/deepagent-code/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare DeepAgent Code a usare solo alcuni provider invece di disabilitarli uno per uno.
