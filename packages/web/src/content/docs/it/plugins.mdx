---
title: Plugin
description: Scrivi plugin per estendere DeepAgent Code.
---

I plugin ti permettono di estendere DeepAgent Code agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di DeepAgent Code.
...
- `.deepagent-code/plugins/` - plugin a livello progetto
- `~/.config/deepagent-code/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/deepagent-code/node_modules/`.
...
1. Config globale (`~/.config/deepagent-code/deepagent-code.json`)
2. Config di progetto (`deepagent-code.json`)
3. Directory plugin globale (`~/.config/deepagent-code/plugins/`)
4. Directory plugin di progetto (`.deepagent-code/plugins/`)
...
DeepAgent Code esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di deepagent-code per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere deepagent-code.
...
:::note
Se usi l'app desktop di DeepAgent Code, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a deepagent-code di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a deepagent-code:
...
L'helper `tool` crea uno strumento personalizzato che deepagent-code può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in deepagent-code insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://deepagent-code.ai/docs/sdk) per i dettagli.
