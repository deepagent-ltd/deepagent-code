---
title: プロバイダー
description: DeepAgent Code で LLM プロバイダーを使用する。
---

import config from "../../../../config.mjs"
export const console = config.console

DeepAgent Code は [AI SDK](https://ai-sdk.dev/) および [Models.dev](https://models.dev) を使用して **75 以上の LLM プロバイダー** をサポートし、ローカルモデルの実行をサポートします。

プロバイダーを追加するには、次のことを行う必要があります。

1. `/connect` コマンドを使用して、プロバイダーの API キーを追加します。
2. DeepAgent Code 設定でプロバイダーを構成します。
...
`~/.deepagent/code/auth.json`で。

---

### 設定

DeepAgent Code の `provider` セクションを通じてプロバイダーをカスタマイズできます。
config.
...
DeepAgent Code Zen は、DeepAgent Code チームが提供するモデルのリストです。
DeepAgent Code で適切に動作することがテストおよび検証されています。 [詳細はこちら](/docs/zen)。

:::tip
初めての方は、DeepAgent Code Zen から始めることをお勧めします。
:::

1. TUI で `/connect` コマンドを実行し、`DeepAgent Code Zen` を選択して、[ai.deepagent.ltd/zen](https://ai.deepagent.ltd/zen) にアクセスします。
...
これは DeepAgent Code の他のプロバイダーと同様に機能し、使用は完全にオプションです。

---

## DeepAgent Code Go

DeepAgent Code Go は、DeepAgent Code チームによって提供される、人気のあるオープンコーディングモデルへの信頼性の高いアクセスを提供する低コストのサブスクリプションプランです。これらは DeepAgent Code でうまく機能することがテストおよび検証されています。

1. TUI で `/connect` コマンドを実行し、`DeepAgent Code Go` を選択して、[ai.deepagent.ltd/zen](https://ai.deepagent.ltd/zen) にアクセスします。
...
DeepAgent Code で Amazon Bedrock を使用するには:
...
2. **次のいずれかの方法を使用して認証を構成します**:
   ...
   ***
   ...
   deepagent-code の実行中に次の環境変数のいずれかを設定します。
   ...
   ***
   ...
   プロジェクト固有の設定または永続的な設定の場合は、`deepagent-code.json` を使用します。
...
:::info
DeepAgent Code での Claude Pro/Max サブスクリプションの使用は、[Anthropic](https://anthropic.com) によって正式に許可されていませんが、一般的に機能します。
:::
...
deepagent-code は、OpenAI 互換の API サーバーの背後でローカル LLM を実行するデスクトップアプリケーション [Atomic Chat](https://atomic.chat) 経由でローカルモデルを使うように設定できます（デフォルトのエンドポイントは `http://127.0.0.1:1337/v1`）。
...
:::note
DeepAgent Code が正しく動作するには、デプロイメント名がモデル名と一致する必要があります。
:::
...
DeepAgent Code 設定を通じてモデルを追加することもできます。
...
トークンを DeepAgent Code 認証ストレージに保存します。
:::

##### 自己ホスト型 GitLab

:::note[コンプライアンスメモ]
DeepAgent Code は、セッションタイトルの生成などの一部の AI タスクに小規模なモデルを使用します。
デフォルトでは、Zen によってホストされる gpt-5-nano を使用するように構成されています。 DeepAgent Code をロックするには
自分の GitLab でホストされているインスタンスのみを使用するには、次の行を
`deepagent-code.json` ファイル。セッション共有を無効にすることもお勧めします。
...
詳細については、[deepagent-code-gitlab-auth](https://www.npmjs.com/package/deepagent-code-gitlab-auth) ホームページ。

##### 設定

`deepagent-code.json` を通じてカスタマイズします。
...
GitHub Copilot サブスクリプションを DeepAgent Code で使用するには:
...
DeepAgent Code で Google Vertex AI を使用するには:
...
DeepAgent Code を通じて自動的に構成されていない Helicone の機能またはモデルを見つけた場合は、いつでも自分で構成できます。
...
Helicone の [Sessions](https://docs.helicone.ai/features/sessions) 機能を使用すると、関連する LLM リクエストをグループ化できます。 [deepagent-code-helicone-session](https://github.com/H2Shami/deepagent-code-helicone-session) プラグインを使用して、各 DeepAgent Code 会話を Helicone のセッションとして自動的に記録します。
...
プラグインは、リクエストに `Helicone-Session-Id` ヘッダーと `Helicone-Session-Name` ヘッダーを挿入します。 Helicone のセッションページでは、DeepAgent Code の各会話が個別のセッションとしてリストされています。
...
[llama.cpp の s](https://github.com/ggml-org/llama.cpp) llama-server ユーティリティ] を通じて、ローカルモデルを使用するように deepagent-code を構成できます。
...
LM Studio を通じてローカルモデルを使用するように deepagent-code を構成できます。
...
Ollama を通じてローカルモデルを使用するように deepagent-code を構成できます。

:::tip
Ollama は DeepAgent Code 用に自動的に構成できます。詳細については、[Ollama 統合 docs](https://docs.ollama.com/integrations/deepagent-code)」を参照してください。
:::
...
6. **重要**: DeepAgent Code でクラウドモデルを使用する前に、モデル情報をローカルに取得する必要があります。
...
DeepAgent Code Zen は、DeepAgent Code チームによって提供される、テストおよび検証されたモデルのリストです。 [詳細はこちら](/docs/zen)。

1. **<a href={console}>DeepAgent Code Zen</a>** にサインインし、**API キーの作成** をクリックします。

2. `/connect` コマンドを実行し、**DeepAgent Code Zen** を検索します。

   ```txt
   /connect
   ```

3. DeepAgent Code API キーを入力します。
...
DeepAgent Code 設定を通じて追加のモデルを追加することもできます。
...
5. DeepAgent Code 設定を通じてカスタマイズすることもできます。プロバイダーを指定する例を次に示します。
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) を DeepAgent Code で使用するには:
...
DeepAgent Code 設定を通じてモデルをカスタマイズすることもできます。プロバイダーのルーティング順序を指定する例を次に示します。
...
:::tip
OpenAI と互換性のある任意のプロバイダーを DeepAgent Code で使用できます。最新の AI プロバイダーのほとんどは、OpenAI 互換の API を提供しています。
:::
...
4. プロジェクトディレクトリで `deepagent-code.json` ファイルを作成または更新します。
...
`limit` フィールドを使用すると、DeepAgent Code はコンテキストがどのくらい残っているかを理解できます。標準プロバイダーは、これらを models.dev から自動的に取得します。
...
1. **認証設定を確認します**: `deepagent-code auth list` を実行して、資格情報が正しいかどうかを確認します。
   プロバイダー用のものが設定に追加されます。

これは、認証に環境変数に依存する Amazon Bedrock のようなプロバイダーには当てはまりません。

2. カスタムプロバイダーの場合は、DeepAgent Code 設定を確認し、次のことを行います。
   - `/connect` コマンドで使用されるプロバイダー ID が、DeepAgent Code 設定内の ID と一致することを確認してください。
