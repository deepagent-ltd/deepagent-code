---
title: カスタムツール
description: LLM が DeepAgent Code で呼び出すことができるツールを作成します。
---

カスタムツールは、会話中に LLM が呼び出すことができる作成した関数です。これらは、DeepAgent Code の[組み込みツール](/docs/tools) (`read`、`write`、`bash` など) と連携して動作します。
...
- ローカルでは、プロジェクトの `.deepagent-code/tools/` ディレクトリに配置します。
- または、グローバルに、それらを `~/.config/deepagent-code/tools/` に配置します。
