...
最も一般的なプロバイダーはデフォルトでプリロードされています。 `/connect` コマンドを使用してプロバイダーの資格情報を追加した場合は、DeepAgent Code を開始するときにそれらの資格情報を使用できるようになります。
...
ここでは、DeepAgent Code で適切に動作するいくつかのモデルを順不同で示します。 (これは完全なリストではなく、必ずしも最新であるとは限りません):
...
これらのいずれかをデフォルトのモデルとして設定するには、`model` キーを DeepAgent Code の設定に追加します。
...
ここでの完全な ID は `provider_id/model_id` です。たとえば、[DeepAgent Code Zen](/docs/zen) を使用している場合、GPT 5.1 Codex には `deepagent-code/gpt-5.1-codex` を使用します。
...
多くのモデルは、異なる構成の複数のバリアントをサポートしています。 DeepAgent Code には、一般的なプロバイダーのデフォルトのバリアントが組み込まれています。

### 組み込みバリアント

DeepAgent Code には、多くのプロバイダーのデフォルトのバリアントが付属しています。
...
DeepAgent Code が起動すると、次の優先順位でモデルがチェックされます。

1. `--model` または `-m` コマンドラインフラグ。形式は構成ファイルと同じです: `provider_id/model_id`。

2. DeepAgent Code 構成内のモデルのリスト。
