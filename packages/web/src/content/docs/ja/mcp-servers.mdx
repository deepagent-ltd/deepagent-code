...
_Model Context Protocol_ (MCP) を使用して、DeepAgent Code に外部ツールを追加できます。 DeepAgent Code は、ローカルサーバーとリモートサーバーの両方をサポートします。
...
MCP サーバーは、`mcp` の下の [DeepAgent Code 設定](https://deepagent-code.ai/docs/config/) で定義できます。各 MCP を一意の名前で追加します。 LLM にプロンプ​​トを表示するときに、その MCP を名前で参照できます。
...
組織は、`.well-known/deepagent-code` エンドポイント経由でデフォルトの MCP サーバーを提供できます。これらのサーバーはデフォルトで無効になっている場合があり、ユーザーは必要なサーバーにオプトインできます。
...
DeepAgent Code は、リモート MCP サーバーの OAuth 認証を自動的に処理します。サーバーが認証を必要とする場合、DeepAgent Code は次のことを行います。
...
サーバーが認証を必要とする場合、DeepAgent Code を初めて使用しようとすると、認証を求めるプロンプトが表示されます。そうでない場合は、`deepagent-code mcp auth <server-name>` を使用して [flow](#authenticating) を手動でトリガーできます。
...
`mcp auth` コマンドは、認証のためにブラウザを開きます。承認後、DeepAgent Code はトークンを `~/.deepagent/code/mcp-auth.json` に安全に保存します。
...
MCP は、組み込みツールと並んで、DeepAgent Code のツールとして利用できます。したがって、他のツールと同様に、DeepAgent Code 設定を通じてそれらを管理できます。
...
これにより、ブラウザウィンドウが開き、OAuth フローが完了し、DeepAgent Code が Sentry アカウントに接続されます。
