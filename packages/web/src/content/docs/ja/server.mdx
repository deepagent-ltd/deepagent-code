---
title: サーバー
description: HTTP 経由で DeepAgent Code サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`deepagent-code serve` コマンドは、DeepAgent Code クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `deepagent-code.local` |
...
HTTP 基本認証でサーバーを保護するには、`DEEPAGENT_CODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `deepagent-code` になるか、`DEEPAGENT_CODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`deepagent-code serve` と `deepagent-code web` の両方に当てはまります。
...
`deepagent-code` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
DeepAgent Code サーバーを使用して、プログラムで DeepAgent Code と対話します。
:::

`deepagent-code serve` を実行してスタンドアロンサーバーを起動できます。すでに DeepAgent Code TUI を実行している場合でも、`deepagent-code serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、DeepAgent Code [IDE](/docs/ide) プラグインによって使用されます。
...
DeepAgent Code サーバーは次の API を公開します。
