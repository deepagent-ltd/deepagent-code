...
エージェントスキルにより、DeepAgent Code はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
DeepAgent Code は次の場所を検索します。

- プロジェクト設定: `.deepagent-code/skills/<name>/SKILL.md`
- グローバル設定: `~/.config/deepagent-code/skills/<name>/SKILL.md`
...
プロジェクトのローカルパスの場合、DeepAgent Code は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
途中で、一致する `skills/*/SKILL.md` を `.deepagent-code/` に読み込み、一致する `.claude/skills/*/SKILL.md` または `.agents/skills/*/SKILL.md` を読み込みます。

グローバル定義は、`~/.config/deepagent-code/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md`、および `~/.agents/skills/*/SKILL.md` からもロードされます。
...
次のように `.deepagent-code/skills/git-release/SKILL.md` を作成します。
...
DeepAgent Code では、`skill` ツールの説明に利用可能なスキルがリストされています。
...
`deepagent-code.json` のパターンベースの権限を使用して、エージェントがアクセスできるスキルを制御します。
...
**組み込みエージェントの場合** (`deepagent-code.json` 内):
