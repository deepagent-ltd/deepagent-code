---
title: 設定
description: DeepAgent Code JSON 設定を使用します。
---

JSON 設定ファイルを使用して DeepAgent Code を構成できます。

---

## 形式

DeepAgent Code は、**JSON** と **JSONC** (コメント付きの JSON) 形式の両方をサポートしています。
...
1. **リモート設定** (`.well-known/deepagent-code` から) - 組織のデフォルト
2. **グローバル設定** (`~/.config/deepagent-code/deepagent-code.json`) - ユーザー設定
3. **カスタム設定** (`DEEPAGENT_CODE_CONFIG` 環境変数) - カスタムオーバーライド
4. **プロジェクト設定** (プロジェクト内の `deepagent-code.json`) - プロジェクト固有の設定
5. **`.deepagent-code` ディレクトリ** - エージェント、コマンド、プラグイン
6. **インライン設定** (`DEEPAGENT_CODE_CONFIG_CONTENT` 環境変数) - ランタイムオーバーライド

つまり、プロジェクト設定はグローバルのデフォルトをオーバーライドでき、グローバル設定はリモート組織のデフォルトをオーバーライドできます。

:::note
`.deepagent-code` および `~/.config/deepagent-code` ディレクトリでは、サブディレクトリに **複数名** が使用されています: `agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/`、および `themes/`。下位互換性のために、単数形の名前 (`agent/` など) もサポートされています。
:::
...
組織は、`.well-known/deepagent-code` エンドポイント経由でデフォルト設定を提供できます。これは、それをサポートするプロバイダーで認証するときに自動的に取得されます。
...
グローバル DeepAgent Code 設定を `~/.config/deepagent-code/deepagent-code.json` に配置します。プロバイダー、モデル、権限などのユーザー全体の設定にはグローバル設定を使用します。

TUI 固有の設定には、`~/.config/deepagent-code/tui.json` を使用します。
...
プロジェクトのルートに `deepagent-code.json` を追加します。プロジェクト設定は、標準設定ファイルの中で最も高い優先順位を持ち、グローバル設定とリモート設定の両方をオーバーライドします。
...
DeepAgent Code が起動すると、現在のディレクトリで設定ファイルを検索するか、最も近い Git ディレクトリまで移動します。
...
`DEEPAGENT_CODE_CONFIG` 環境変数を使用してカスタム設定ファイルのパスを指定します。
...
`DEEPAGENT_CODE_CONFIG_DIR` を使用してカスタム設定ディレクトリを指定します。
環境変数。このディレクトリでは、エージェント、コマンド、
モードとプラグインは標準の `.deepagent-code` ディレクトリと同様であり、次のようにする必要があります。
同じ構造に従います。
...
カスタムディレクトリはグローバル config ディレクトリと `.deepagent-code` ディレクトリの後にロードされるため、それらの設定を**オーバーライド**できます。

---

## スキーマ

サーバー/ランタイム構成スキーマは [**`ai.deepagent.ltd/config.schema.json`**](https://ai.deepagent.ltd/config.schema.json) で定義されています。

TUI 設定は [**`ai.deepagent.ltd/tui.json`**](https://ai.deepagent.ltd/tui.json) を使用します。
...
カスタム TUI 設定ファイルを指定するには、`DEEPAGENT_CODE_TUI_CONFIG` を使用します。

`deepagent-code.json` 内の従来の `theme`、`keybinds`、および `tui` キーは非推奨となり、可能であれば自動的に移行されます。
...
`deepagent-code serve` オプションを使用して、`deepagent-code web` および `server` コマンドのサーバー設定を構成できます。
...
- `mdns` - mDNS サービス検出を有効にします。これにより、ネットワーク上の他のデバイスが DeepAgent Code サーバーを検出できるようになります。
- `mdnsDomain` - mDNS サービスのカスタムドメイン名。デフォルトは `deepagent-code.local` です。同じネットワーク上で複数のインスタンスを実行する場合に便利です。
...
`provider`、`model`、および `small_model` オプションを使用して、DeepAgent Code 設定で使用するプロバイダーとモデルを構成できます。
...
`small_model` オプションは、タイトルの生成などの軽量タスク用に個別のモデルを構成します。デフォルトでは、DeepAgent Code は、プロバイダーから安価なモデルが入手可能な場合は、より安価なモデルを使用しようとします。そうでない場合は、メインモデルにフォールバックします。
...
`~/.config/deepagent-code/agents/` または `.deepagent-code/agents/` の Markdown ファイルを使用してエージェントを定義することもできます。 [詳細はこちら](/docs/agents)。
...
デフォルトのエージェントはプライマリエージェントである必要があります (サブエージェントではありません)。これは、`"build"` や `"plan"` のような組み込みエージェント、または定義した[カスタムエージェント](/docs/agents) にすることができます。指定されたエージェントが存在しないか、サブエージェントである場合、DeepAgent Code は警告とともに `"build"` にフォールバックします。

この設定は、TUI、CLI (`deepagent-code run`)、デスクトップアプリ、および GitHub Action のすべてのインターフェイスに適用されます。
...
`~/.config/deepagent-code/commands/` または `.deepagent-code/commands/` の Markdown ファイルを使用してコマンドを定義することもできます。 [詳細はこちら](/docs/commands)。
...
DeepAgent Code は起動時に新しいアップデートを自動的にダウンロードします。 `autoupdate` オプションを使用してこれを無効にできます。
...
デフォルトでは、deepagent-code は明示的な承認を必要とせずに **すべての操作を許可**します。これは、`permission` オプションを使用して変更できます。
...
[Plugins](/docs/plugins) は、カスタムツール、フック、統合を使用して DeepAgent Code を拡張します。

プラグインファイルを `.deepagent-code/plugins/` または `~/.config/deepagent-code/plugins/` に配置します。 `plugin` オプションを使用して npm からプラグインをロードすることもできます。
...
これは、DeepAgent Code を 1 つずつ無効にするのではなく、特定のプロバイダーのみを使用するように制限したい場合に便利です。
