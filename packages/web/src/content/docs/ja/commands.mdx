...
`.deepagent-code/commands/test.md` を作成します。
...
カスタムコマンドは、DeepAgent Code 設定を通じて、または `commands/` ディレクトリに Markdown ファイルを作成することによって追加できます。

---

### JSON

DeepAgent Code [設定](/docs/config)で `command` オプションを使用します:
...
- グローバル: `~/.config/deepagent-code/commands/`
- プロジェクトごと: `.deepagent-code/commands/`
...
DeepAgent Code には、`/init`、`/undo`、`/redo`、`/share`、`/help` などのいくつかの組み込みコマンドが含まれています。 [詳細はこちら](/docs/tui#commands)。
