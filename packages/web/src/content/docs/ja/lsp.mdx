---
title: LSPサーバー
description: DeepAgent Code は LSP サーバーと統合します。
---

DeepAgent Code は Language Server Protocol (LSP) サーバーと統合し、診断を agent へのフィードバックとして使用できます。

---

## 組み込み

DeepAgent Code には、一般的な言語用のいくつかの組み込み LSP サーバーが付属しています。
...
:::note
`DEEPAGENT_CODE_DISABLE_LSP_DOWNLOAD` 環境変数を `true` に設定すると、LSP サーバーの自動ダウンロードを無効にできます。
:::
...
LSP が有効で DeepAgent Code がファイルを開くと、次のようになります。
...
DeepAgent Code 設定の `lsp` セクションを通じて LSP サーバーを有効化し、カスタマイズできます。
