...
ツールを使用すると、LLM がコードベースでアクションを実行できるようになります。 DeepAgent Code には一連の組み込みツールが付属していますが、[カスタムツール](/docs/custom-tools) または [MCP サーバー](/docs/mcp-servers) を使用して拡張できます。
...
DeepAgent Code で利用可能なすべての組み込みツールを次に示します。
...
:::note
このツールは、`DEEPAGENT_CODE_EXPERIMENTAL_LSP_TOOL=true` (または `DEEPAGENT_CODE_EXPERIMENTAL=true`) の場合にのみ使用できます。
:::
...
:::note
このツールは、DeepAgent Code プロバイダーを使用している場合、または `DEEPAGENT_CODE_ENABLE_EXA` 環境変数が真実の値 (`true` または `1` など) に設定されている場合にのみ使用できます。

DeepAgent Code の起動時に有効にするには:
