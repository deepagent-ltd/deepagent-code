---
title: TUI
description: DeepAgent Code ターミナルユーザーインターフェイスの使用。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

DeepAgent Code は、LLM を使用してプロジェクトを作業するための対話型ターミナルインターフェイスまたは TUI を提供します。

DeepAgent Code を実行すると、現在のディレクトリの TUI が開始されます。
...
DeepAgent Code TUI を使用する場合、「`/`」に続いてコマンド名を入力すると、アクションをすばやく実行できます。例えば：
...
DeepAgent Code にプロバイダーを追加します。利用可能なプロバイダーから選択し、その API キーを追加できます。
...
DeepAgent Code を終了します。 _エイリアス_: `/quit`、`/q`
...
これは、サーバー/ランタイムの動作を構成する `deepagent-code.json` とは別です。
...
- `leader_timeout` - leader key を押したあと、DeepAgent Code が次のキー入力を待つ時間を制御します。デフォルトは `2000` です。
...
カスタム TUI 設定パスをロードするには、`DEEPAGENT_CODE_TUI_CONFIG` を使用します。

### Attention

Attention を有効にすると、DeepAgent Code が回答待ち、権限の承認待ち、セッションエラー、またはセッション完了を知らせたいときに、TUI がサウンドやデスクトップ通知で知らせます。`attention.enabled` で有効化します。組み込みイベントは発生時にサウンドを再生します。デスクトップ通知はターミナルウィンドウにフォーカスがない場合にのみ送信され、subagent イベントには使われません。
...
- `sound_pack` - 使用する sound pack ID です。デフォルトは `deepagent-code.default` です。
