...
describe("domain pack quality", () => {
  test("all built-in packs satisfy layout, index, evidence, and minimum L2 density", () => {
    ...
    for (const packDir of discoverPackDirs()) {
      ...
      if (pack.schema_version !== "domain_pack.v1")
        failures.push(`${pack.id}: bad schema_version ${pack.schema_version}`)
      ...
      for (const { file, doc } of docs) {
        ...
        if (doc.type !== "failure_dossier" && !positiveTypes.has(doc.type))
          failures.push(`${pack.id}: ${doc.ref_id} unsupported type ${doc.type}`)
        if (doc.evidence_strength === "weak" || doc.evidence_strength === "none")
          failures.push(`${pack.id}: ${doc.ref_id} weak/none evidence`)
      ...
      for (const entry of index) {
        ...
        if (entry.evidence_strength === "weak" || entry.evidence_strength === "none")
          failures.push(`${pack.id}: ${entry.ref_id} weak/none index evidence`)
        if (
          strictStrengthTypes.has(entry.type) &&
          JSON.stringify(entry.allowed_strengths) !== JSON.stringify(["max", "ultra"])
        ) {
          failures.push(`${pack.id}: ${entry.ref_id} bad allowed_strengths ${JSON.stringify(entry.allowed_strengths)}`)
        }
        if (entry.type === "skill" && !entry.allowed_strengths?.includes("high"))
          failures.push(`${pack.id}: ${entry.ref_id} skill missing high`)
      }
      ...
      for (const [type, minimum] of Object.entries(minimumCounts)) {
        if (counts[type as keyof typeof counts] < minimum)
          failures.push(`${pack.id}: ${type} count ${counts[type as keyof typeof counts]} < ${minimum}`)
      }

      if (!existsSync(path.join(packDir, "evals/smoke/l3-smoke.json")))
        failures.push(`${pack.id}: missing evals/smoke/l3-smoke.json`)
      ...
      if (!existsSync(reportPath)) failures.push(`${pack.id}: missing quality/l3-report.json`)
        ...
        if ((report.scaffold_ratio ?? 1) > 0)
          failures.push(`${pack.id}: scaffold_ratio ${report.scaffold_ratio} > 0 (machine-cloned docs present)`)
      }
    }
  ...
  test("every L3 pack has activation and pack-scoped retrieval smoke", () => {
    ...
    try {
      ...
      for (const packDir of discoverPackDirs()) {
        ...
        const positiveScore =
          Registry.score(smoke.positive_profile, manifests).find((score) => score.packId === pack.id)?.score ?? 0
        const negativeScore =
          Registry.score(smoke.negative_profile, manifests).find((score) => score.packId === pack.id)?.score ?? 0
        ...
        const refs = store
          .retrieve({
            ...
            keywords: smoke.retrieval_query
              .toLowerCase()
              .split(/[^a-z0-9_.#+-]+/)
