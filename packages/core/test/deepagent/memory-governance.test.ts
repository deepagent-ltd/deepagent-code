...
describe("gate 1 — sensitivity detection", () => {
  ...
  test("literal credential values are sensitive even without keywords", () => {
    ...
    expect(Gov.looksSensitive("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.SflKxwRJSMeKKF2QT4fwpMeJf36")).toBe(true)
...
describe("confidence floor scales with blast radius (D7 decision)", () => {
  test("low-blast memory auto-admits at 0.6", () => {
    expect(Gov.meetsConfidenceFloor(candidate({ type: "memory", confidence: 0.6 }), Gov.classify(candidate({ type: "memory" })))).toBe(true)
  })
...
describe("route — the four human-review triggers, everything else automatic", () => {
  ...
  test("gate 1: sensitive -> review", () => {
    const sensitive = { ...base, classification: Gov.classify(candidate({ summary: "token=ghp_abcdefghijklmnopqrstuvwxyz0123" })) }
...
describe("isHighTrust", () => {
  const doc = (over: Partial<Doc>): Doc => ({
    id: "d", type: "memory", scope: "durable:project:p", status: "active", version: 1, superseded_by: null,
    hash: "h", created_round: null, domain: null, tags: [], description: "d",
    provenance: { source: "runner" }, links: [], body: "b", ...over,
  })
