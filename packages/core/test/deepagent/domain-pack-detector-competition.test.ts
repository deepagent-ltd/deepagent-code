...
describe("detector competition (docs/review_38 B3)", () => {
  ...
  test("generic DB migration with backfill does NOT activate code.data-engineering", () => {
    const packs = activated("add database migration with unique constraint backfill transaction rollback and explain plan")
...
describe("high-frequency tool-pack precision (review_4 §六)", () => {
  const fired = (text: string): readonly string[] => {
    Registry.configureRegistry(undefined)
    return Registry.score(liveProfile(text), Registry.discover()).filter((s) => s.score >= 0.5).map((s) => s.packId)
  }
  ...
  test("genuine env-bootstrap task STILL activates code.environment", () => {
    expect(fired("the dependency install fails and the runtime is missing after bootstrap the project")).toContain("code.environment")
  })
  ...
  test("genuine extract-method refactor STILL activates code.refactor", () => {
    expect(fired("extract a helper method from the payment service and rename the getUser function")).toContain("code.refactor")
  })
  test("genuine dependency-upgrade migration STILL activates code.migration", () => {
    expect(fired("migrate the schema and upgrade the react dependency for the breaking change")).toContain("code.migration")
  })
})
...
describe("profileFromInput domain signals (review_4 M3)", () => {
  ...
  const activatedViaTaskText = (text: string): readonly string[] => {
    ...
    return Registry.score(liveProfile(text), manifests).filter((s) => s.score >= 0.5).map((s) => s.packId)
  }
  ...
  test("'ETL pipeline with kafka and spark' activates code.data-engineering", () => {
    expect(activatedViaTaskText("build an ETL pipeline with kafka and spark into the warehouse")).toContain("code.data-engineering")
  })
  test("'reentrancy in solidity smart contract' activates code.blockchain", () => {
    expect(activatedViaTaskText("fix the reentrancy bug in the solidity smart contract evm")).toContain("code.blockchain")
  })
  test("'serverless lambda cold start' activates code.backend.serverless", () => {
    expect(activatedViaTaskText("reduce aws lambda cold start latency in the serverless function")).toContain("code.backend.serverless")
  })
  test("'machine learning training loop overfitting' activates code.ml-ai", () => {
    expect(activatedViaTaskText("fix overfitting in the pytorch training loop reduce gradient noise")).toContain("code.ml-ai")
  })
})
...
describe("batch 2/4 new packs activate (review_4 coverage)", () => {
  ...
  test("FIR filter + FFT on ADC samples → hardware.signal-processing", () => {
    expect(fired("design an FIR filter and run FFT analysis on the ADC samples")).toContain("hardware.signal-processing")
  })
  ...
  test("XDC constraints place and route → hardware.fpga-toolflow", () => {
    expect(fired("close timing with XDC constraints after place and route in vivado")).toContain("hardware.fpga-toolflow")
  })
  ...
  test("UVM constrained-random coverage → hardware.verification", () => {
    expect(fired("write a UVM testbench with constrained-random and functional coverage")).toContain("hardware.verification")
  })
  test("Runge-Kutta ODE solver Monte Carlo → code.simulation", () => {
    expect(fired("implement a Runge-Kutta ODE solver with Monte Carlo and numerical stability")).toContain("code.simulation")
  })
  ...
  test("Airflow DAG sensor backfill → code.data-pipeline-orchestration", () => {
    expect(fired("design a workflow dag in apache airflow with a sensor task and dag scheduling")).toContain("code.data-pipeline-orchestration")
  })
  test("feature store train-serve skew → code.feature-engineering", () => {
    expect(fired("fix train-serve skew in the feature store with temporal leakage")).toContain("code.feature-engineering")
  })
})
...
describe("selectedRefs per-pack quota + dynamic cap (docs/review_38 B4)", () => {
  ...
  test("multi-domain GPU+typecheck task keeps a representative gpu ref (not starved by core/testing)", () => {
    withSeed(() => {
      ...
      const refs = [...(result?.strategyRefs ?? []), ...(result?.methodologyRefs ?? []), ...(result?.knowledgeRefs ?? []), ...(result?.skillRefs ?? [])]
...
describe("batch 1/3/5 new packs activate (review_4 coverage)", () => {
  ...
  test("MongoDB aggregation $lookup sharding → code.database.mongodb", () => {
    expect(fired("design a MongoDB aggregation pipeline with $lookup and a sharding key")).toContain("code.database.mongodb")
  })
  ...
  test("Cassandra partition key tombstones → code.database.cassandra", () => {
    expect(fired("model the Cassandra partition key to avoid tombstones with quorum reads")).toContain("code.database.cassandra")
  })
  test("InfluxDB hypertable cardinality → code.database.timeseries", () => {
    expect(fired("fix tag cardinality explosion in the InfluxDB hypertable retention policy")).toContain("code.database.timeseries")
  })
  ...
  test("git merge conflict interactive rebase → code.git-workflow", () => {
    expect(fired("resolve a git merge conflict during an interactive rebase before force push")).toContain("code.git-workflow")
  })
  test("CMakeLists target_link_libraries → code.build.cmake", () => {
    expect(fired("fix the CMakeLists target_link_libraries and find_package for the library")).toContain("code.build.cmake")
  })
  ...
  test("IAM least privilege Lambda → platform.cloud.aws", () => {
    expect(fired("tighten the IAM policy least privilege for the aws Lambda and S3 bucket")).toContain("platform.cloud.aws")
  })
  ...
  test("Istio VirtualService mTLS → platform.service-mesh", () => {
    expect(fired("configure Istio VirtualService traffic split with mTLS and circuit breaking")).toContain("platform.service-mesh")
  })
  test("ansible playbook idempotent vault → platform.ansible", () => {
    expect(fired("write an idempotent ansible playbook with handlers notify and ansible vault")).toContain("platform.ansible")
  })
  test("PromQL recording rule SLO → platform.monitoring", () => {
    expect(fired("write a PromQL recording rule and an SLO error budget alert in alertmanager")).toContain("platform.monitoring")
  })
  test("CDN edge function cache-control → platform.cdn-edge", () => {
    expect(fired("set cache-control headers and a cloudflare worker edge function with origin shield")).toContain("platform.cdn-edge")
  })
  ...
  test("tenant isolation row-level security → code.backend.multi-tenant", () => {
    expect(fired("enforce tenant isolation with row-level security and tenant-scoped queries")).toContain("code.backend.multi-tenant")
  })
  test("API design resource modeling versioning → code.api-design", () => {
    expect(fired("review the api design resource modeling and api versioning strategy with pagination design")).toContain("code.api-design")
  })
  ...
  test("shopping cart inventory oversell → business.ecommerce", () => {
    expect(fired("fix the shopping cart checkout flow and inventory stock reservation to avoid oversell")).toContain("business.ecommerce")
  })
})
