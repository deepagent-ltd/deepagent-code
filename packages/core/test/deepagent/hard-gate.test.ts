...
describe("hasActiveStep / stepCanComplete", () => {
  ...
  test("a step with no acceptance can always complete; with acceptance it needs a passing validation", () => {
    ...
    expect(
      PlanController.stepCanComplete({ step_id: "s", title: "t", status: "active", acceptance: "tests pass" }, false),
    ).toBe(false)
    expect(
      PlanController.stepCanComplete({ step_id: "s", title: "t", status: "active", acceptance: "tests pass" }, true),
    ).toBe(true)
  })
})
...
describe("completion_report + stop gate (high+)", () => {
  ...
  test("high+ run that never made a plan is not retroactively forced to report", () => {
    const d = stop({
      name: "stop",
      payload: { requiredValidationsRun: true, planStale: false, hardGate: true, planExists: false },
    })
    expect(d.decision).toBe("allow")
  })
