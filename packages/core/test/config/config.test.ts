...
import { ConfigMigrateV1 } from "@deepagent-code/core/v1/config/migrate"
...
describe("Config", () => {
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          return yield* Effect.gen(function* () {
            ...
            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "deepagent-code.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "deepagent-code.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .deepagent-code configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".deepagent-code"), { recursive: true })
            await fs.mkdir(path.join(directory, ".deepagent-code"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "deepagent-code.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "deepagent-code.json"), JSON.stringify({ $schema: "global" })),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".deepagent-code")),
              AbsolutePath.make(path.join(directory, ".deepagent-code")),
            ])
