...
describe("file.ripgrep", () => {
    ...
    Effect.gen(function* () {
      const dir = yield* tmpdir((dir) =>
        Effect.gen(function* () {
          ...
          yield* mkdir(path.join(dir, ".deepagent-code"))
          yield* write(path.join(dir, ".deepagent-code", "thing.json"), "{}")
        }),
      )

      const files = yield* collectFiles({ cwd: dir })
      expect(files.includes("visible.txt")).toBe(true)
      expect(files.includes(path.join(".deepagent-code", "thing.json"))).toBe(true)
    }),
  )
  ...
  it.live("hidden false excludes hidden", () =>
    Effect.gen(function* () {
      ...
      expect(files.includes("visible.txt")).toBe(true)
      expect(files.includes(path.join(".deepagent-code", "thing.json"))).toBe(false)
    }),
  )
