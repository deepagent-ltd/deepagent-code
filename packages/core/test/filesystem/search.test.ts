...
describe("file.search", () => {
  ...
  it.live("falls back to the current filesystem when a warm picker has not observed a new file", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdir()
      const search = yield* Search.Service
      yield* search.warm(dir)
      yield* write(path.join(dir, "late-file.txt"), "late\n")

      expect(yield* search.file({ cwd: dir, query: "late file", limit: 10 })).toContain("late-file.txt")
    }),
  )
