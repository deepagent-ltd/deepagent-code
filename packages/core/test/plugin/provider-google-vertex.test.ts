...
import { GoogleVertexPlugin } from "@deepagent-code/core/plugin/provider/google-vertex"
  ...
  it.effect("ignores OpenAI-compatible providers that are not Google Vertex", () =>
    Effect.gen(function* () {
      ...
      yield* transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.make("deepagent-code"), (provider) => {
          provider.api = {
            type: "aisdk",
      ...
      const provider = yield* catalog.provider.get(ProviderV2.ID.make("deepagent-code"))
