...
import { CloudflareAIGatewayPlugin } from "@deepagent-code/core/plugin/provider/cloudflare-ai-gateway"
      ...
      Effect.gen(function* () {
        ...
        yield* plugin.trigger(
          "aisdk.sdk",
          {
            ...
            options: {
              ...
              metadata: { invoked_by: "test", project: "deepagent-code" },
              cacheTtl: 300,
              cacheKey: "cache-key",
              skipCache: true,
              collectLog: false,
        ...
        expect(aiGatewayCalls[0]).toEqual({
          ...
          options: {
            ...
            collectLog: false,
            headers: {
              "User-Agent": expect.stringContaining("deepagent-code/"),
            },
          },
        })
      }),
    ...
    withEnv(cloudflareEnv(), () =>
      Effect.gen(function* () {
        ...
        yield* plugin.trigger(
          "aisdk.sdk",
          {
            ...
            options: {
              name: "cloudflare-ai-gateway",
              headers: {
                "cf-aig-metadata": JSON.stringify({ invoked_by: "header", project: "deepagent-code" }),
              },
            },
          },
          {},
        ...
        expect(aiGatewayCalls[0]?.options).toMatchObject({
          metadata: { invoked_by: "header", project: "deepagent-code" },
        })
      }),
    ),
  )
