...
describe("DeepAgentEventBus", () => {
  ...
  it.live("grouped subscribers deliver each priority exactly once while anonymous sees the full stream", () =>
    Effect.gen(function* () {
      ...
      const high = yield* bus.publish(input({ idempotencyKey: "priority-high", priority: "high" }))
      const critical = yield* bus.publish(input({ idempotencyKey: "priority-critical", priority: "critical" }))
      const expected = [low.id, normal.id, high.id, critical.id].sort()

      const grouped = Array.from(yield* Fiber.join(groupedFiber))
      const anonymous = Array.from(yield* Fiber.join(anonymousFiber))
      expect(grouped.map((event) => event.id).sort()).toEqual(expected)
