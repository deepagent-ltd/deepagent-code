...
describe("recovery terminal trigger idempotency", () => {
  test("a second recovery terminal insertion for the same activity is a silent no-op", async () => {
    await run(
      Effect.gen(function* () {
        const db = yield* makeDb
        yield* db.run(`PRAGMA foreign_keys = ON`)
        yield* DatabaseMigration.apply(db)
        yield* db.run(`INSERT INTO project (id, worktree, sandboxes, time_created, time_updated) VALUES ('p', '/repo', '[]', 1, 1)`)
        yield* db.run(`INSERT INTO session (id, project_id, slug, directory, title, version, mutation_epoch, time_created, time_updated) VALUES ('s', 'p', 's', '/repo', 'S', '1', 7, 1, 1)`)
        yield* db.run(`INSERT INTO session_intent (intent_id, session_id, source, state, selected_variant, selected_payload_hash, admitted_message_id, delivery, mutation_epoch, time_created, time_selected, time_updated) VALUES ('i', 's', 'composer', 'admitted', 'original', 'ph', 'msg', 'turn', 7, 1, 1, 1)`)
        yield* db.run(`INSERT INTO session_activity_admission (admission_id, session_id, source_kind, legacy_intent_id, admitted_message_id, delivery, payload_fingerprint_kind, payload_fingerprint, execution_mode, created_at) VALUES ('adm', 's', 'legacy_intent', 'i', 'msg', 'turn', 'payload_hash', 'ph', 'legacy', 1)`)
        yield* db.run(`INSERT INTO session_legacy_activity (activity_id, session_id, ordinal, trigger_admission_id, owner_token, state, created_at) VALUES ('act', 's', 0, 'adm', 'owner', 'active', 1)`)
        yield* db.run(`UPDATE session_legacy_activity SET state = 'recovery_required', terminal_reason = 'provider_outcome_indeterminate', settled_at = 1 WHERE activity_id = 'act'`)
        yield* db.run(`INSERT INTO session_legacy_activity_terminal (activity_id, session_id, mutation_epoch, state, reason_code, source, operation_id, membership_ordinal, owner_token, created_at) VALUES ('act', 's', 7, 'recovery_required', 'provider_outcome_indeterminate', 'restart_recovery', 'op:1', 0, 'owner', 1)`)
        const second = yield* db
          .run(`INSERT INTO session_legacy_activity_terminal (activity_id, session_id, mutation_epoch, state, reason_code, source, operation_id, membership_ordinal, owner_token, created_at) VALUES ('act', 's', 7, 'recovery_required', 'provider_outcome_indeterminate', 'same_process_recovery', 'op:2', 0, 'owner', 2) ON CONFLICT DO NOTHING`)
          .pipe(Effect.exit)
        expect(Exit.isSuccess(second)).toBe(true)
        expect(yield* db.get(`SELECT count(*) AS count FROM session_legacy_activity_terminal`)).toEqual({ count: 1 })
      }),
    )
  })
})
