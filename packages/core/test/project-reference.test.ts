...
import { ConfigReference } from "@deepagent-code/core/config/reference"
import { FSUtil } from "@deepagent-code/core/fs-util"
import { Flag } from "@deepagent-code/core/flag/flag"
import { Global } from "@deepagent-code/core/global"
import { Location } from "@deepagent-code/core/location"
import { ProjectReference } from "@deepagent-code/core/project-reference"
import { Repository } from "@deepagent-code/core/repository"
import { RepositoryCache } from "@deepagent-code/core/repository-cache"
...
describe("ProjectReference", () => {
  it.live("uses the broad experimental flag unless references are explicitly configured", () =>
    withEnv(
      { DEEPAGENT_CODE_EXPERIMENTAL: "true", DEEPAGENT_CODE_EXPERIMENTAL_REFERENCES: undefined },
      Effect.sync(() => {
        expect(Flag.DEEPAGENT_CODE_EXPERIMENTAL_REFERENCES).toBe(true)
      }),
    ).pipe(
      Effect.flatMap(() =>
        withEnv(
          { DEEPAGENT_CODE_EXPERIMENTAL: "true", DEEPAGENT_CODE_EXPERIMENTAL_REFERENCES: "false" },
          Effect.sync(() => {
            expect(Flag.DEEPAGENT_CODE_EXPERIMENTAL_REFERENCES).toBe(false)
          }),
        ),
      ),
    ),
...
function withReferences<A, E, R>(body: Effect.Effect<A, E, R>) {
  return withEnv({ DEEPAGENT_CODE_EXPERIMENTAL_REFERENCES: "true" }, body)
}

function withoutReferences<A, E, R>(body: Effect.Effect<A, E, R>) {
  return withEnv({ DEEPAGENT_CODE_EXPERIMENTAL: undefined, DEEPAGENT_CODE_EXPERIMENTAL_REFERENCES: undefined }, body)
}
