 ...
 *
 * HISTORICAL SEAM (V2 IM durable-only migration): its only consumer — the legacy
 * `agent-orchestrator.ts` — is deleted; @mention replies now return through the
 * durable `im_reply_outbox` daemon (deepagent-code src/im/im-reply-outbox.ts) and
 * nothing wires this sink in production. The module is retained only as the
 * `AgentProgressPart` type donor for `agent-executor.ts`.
 *
 * In the standalone kernel this was absent — the orchestrator persisted the reply
 * to its own IM store and broadcast over `/ws/im`.
 *
 * In the Server Edition (deepagent-code-server), the container's IM is NOT the
 * user-facing source of truth — the gateway hub is. The gateway delivered a
 * mention into the container's `/api/v1/im`, the kernel ran the agent, and the
 * reply travelled BACK to the hub. This sink was that outbound seam: when a
 * layer provided it, the orchestrator reported each agent outcome, and the
 * Server Edition implementation POSTed it to the gateway callback
 * (`/internal/im/agent-reply`). The gateway correlated (groupID, messageID)
 * back to its own conversation/message ids.
 *
 * It was OPTIONAL by construction: the orchestrator read it via
 * `Effect.serviceOption`, so its absence changed neither the orchestrator's
