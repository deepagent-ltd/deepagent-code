...
export class Service extends Context.Service<Service, Interface>()("@deepagent-code/v2/Account") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const parseAuthContent = () => {
      try {
        return JSON.parse(process.env.DEEPAGENT_CODE_AUTH_CONTENT ?? "")
      } catch {}
    }

    const load: () => Effect.Effect<Writable, Error> = Effect.fnUntraced(function* () {
      if (process.env.DEEPAGENT_CODE_AUTH_CONTENT) {
