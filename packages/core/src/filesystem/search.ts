...
export class Service extends Context.Service<Service, Interface>()("@deepagent-code/Search") {}
...
export const layer: Layer.Layer<Service, never, FSUtil.Service | Ripgrep.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const acquire = Effect.fn("Search.acquire")(function* (cwd: string) {
      // The deepagent-code test runtime owns an isolated XDG tree that Windows must
      // remove before process exit, so use ripgrep instead of native FFF there.
      if (process.env.DEEPAGENT_CODE_TEST_HOME) return undefined
    ...
    // in 99% of use cases user that is opened deepagent-code at certain directory will
    ...
    const file: Interface["file"] = Effect.fn("Search.file")(function* (input) {
      ...
      if (!fffResult) return yield* fallbackFile(input, dir, kind, limit, rg)
      if (!fffResult.ok) {
        log.warn(`fff ${kind} search failed`, { dir, query, error: fffResult.error })
        return yield* fallbackFile(input, dir, kind, limit, rg)
      }

      const rows = fffResult.value
      if (!rows.length) return yield* fallbackFile(input, dir, kind, limit, rg)
      remember(
        state,
        dir,
        query,
...
const fallbackFile = Effect.fn("Search.fallbackFile")(function* (
  input: FileInput,
  dir: string,
  kind: NonNullable<FileInput["kind"]>,
  limit: number,
  rg: Ripgrep.Interface,
) {
  const files = [...(yield* rg.files({ cwd: dir }).pipe(Stream.runCollect))].map(normalize)
  ...
  const candidates = kind === "file" ? files : kind === "directory" ? directories : [...files, ...directories]
  const query = input.query.trim()
  return query
    ? fuzzysort.go(query, candidates, { limit }).map((item) => item.target)
