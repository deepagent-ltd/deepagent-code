...
export interface Interface {
   ...
   *
   * This exists while the old deepagent-code project service and this core project
...
export class Service extends Context.Service<Service, Interface>()("@deepagent-code/ProjectV2") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const cached = Effect.fnUntraced(function* (dir: string) {
      return yield* fs.readFileString(path.join(dir, "deepagent-code")).pipe(
    ...
    const commit = Effect.fn("Project.commit")(function* (input: { store: AbsolutePath; id: ID }) {
      yield* fs.writeFileString(path.join(input.store, "deepagent-code"), input.id).pipe(Effect.ignore)
    })
