...
export default {
  id: MigrationIdentity.Canonical.finalAuthorities,
  up(tx) {
    return Effect.gen(function* () {
      ...
      `)
      /* Drizzle proposed rebuilding learning_job only to restate checks already installed by
         20260812224000. Rebuilding here would drop its cross-table authority triggers. */
      yield* Effect.void
      /*
      ...
      yield* tx.run(`CREATE INDEX \`learning_job_owner_lease_idx\` ON \`learning_job\` (\`owner\`,\`lease_expires_at\`) WHERE "learning_job"."owner" IS NOT NULL;`) */
      yield* tx.run(
        `CREATE UNIQUE INDEX \`learning_governance_compensation_action_idx\` ON \`learning_governance_compensation\` (\`action_id\`);`,
      )
      yield* tx.run(
        `CREATE UNIQUE INDEX \`learning_governance_compensation_plan_sequence_idx\` ON \`learning_governance_compensation\` (\`plan_id\`,\`sequence\`);`,
      )
      yield* tx.run(
        `CREATE INDEX \`learning_governance_compensation_claim_idx\` ON \`learning_governance_compensation\` (\`state\`,\`created_at\`);`,
      )
      yield* tx.run(
        `CREATE UNIQUE INDEX \`learning_lifecycle_trigger_identity_idx\` ON \`learning_lifecycle_trigger_receipt\` (\`trigger\`,\`session_id\`,\`run_id\`);`,
      )
      yield* tx.run(
        `CREATE INDEX \`learning_lifecycle_trigger_pending_idx\` ON \`learning_lifecycle_trigger_receipt\` (\`created_at\`,\`receipt_id\`) WHERE "learning_lifecycle_trigger_receipt"."state" = 'prepared';`,
      )
      yield* tx.run(
        `CREATE UNIQUE INDEX \`learning_reviewer_attempt_job_idx\` ON \`learning_reviewer_attempt\` (\`job_id\`);`,
      )
      yield* tx.run(
        `CREATE UNIQUE INDEX \`learning_reviewer_attempt_session_idx\` ON \`learning_reviewer_attempt\` (\`review_session_id\`);`,
      )
      yield* tx.run(
        `CREATE INDEX \`learning_reviewer_attempt_state_idx\` ON \`learning_reviewer_attempt\` (\`state\`,\`updated_at\`);`,
      )
      yield* tx.run(
        `CREATE UNIQUE INDEX \`session_v2_provider_parity_receipt_hash_idx\` ON \`session_v2_provider_parity_receipt\` (\`receipt_hash\`);`,
      )
      yield* tx.run(
        `CREATE INDEX \`session_v2_provider_parity_receipt_campaign_idx\` ON \`session_v2_provider_parity_receipt\` (\`campaign_id\`,\`verified\`);`,
      )
      yield* tx.run(
        `CREATE UNIQUE INDEX \`session_v2_provider_turn_receipt_ordinal_idx\` ON \`session_v2_provider_turn_receipt\` (\`session_id\`,\`request_ordinal\`);`,
      )
      yield* tx.run(
        `CREATE INDEX \`session_v2_provider_turn_receipt_input_idx\` ON \`session_v2_provider_turn_receipt\` (\`session_id\`,\`user_message_id\`,\`history_prompt_epoch\`,\`request_input_hash\`);`,
      )
      yield* tx.run(
