...
export default {
  id: "20260923181940_learning_generation",
  up(tx) {
    return Effect.gen(function* () {
      ...
      `)
      yield* tx.run(`CREATE UNIQUE INDEX \`learning_generation_activity_idx\` ON \`learning_generation\` (\`session_id\`,\`activity_id\`);`)
      yield* tx.run(`CREATE INDEX \`learning_generation_pending_idx\` ON \`learning_generation\` (\`trigger\`,\`settled_at\`);`)
    })
  },
