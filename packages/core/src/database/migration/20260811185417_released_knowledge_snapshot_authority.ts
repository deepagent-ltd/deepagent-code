...
export default {
  id: "20260811185417_released_knowledge_snapshot_authority",
  up(tx) {
    return Effect.gen(function* () {
      ...
      `)
      yield* tx.run(
        `CREATE UNIQUE INDEX \`released_knowledge_evaluation_scope_identity_idx\` ON \`released_knowledge_evaluation\` (\`security_namespace_id\`,\`project_scope_key\`,\`evaluation_id\`);`,
      )
      yield* tx.run(
        `CREATE INDEX \`released_knowledge_evaluation_matrix_idx\` ON \`released_knowledge_evaluation\` (\`security_namespace_id\`,\`project_scope_key\`,\`matrix_hash\`);`,
      )
      yield* tx.run(
        `CREATE UNIQUE INDEX \`released_knowledge_snapshot_document_ordinal_idx\` ON \`released_knowledge_snapshot_document\` (\`snapshot_id\`,\`ordinal\`);`,
      )
      yield* tx.run(
        `CREATE UNIQUE INDEX \`released_knowledge_snapshot_scope_identity_idx\` ON \`released_knowledge_snapshot\` (\`security_namespace_id\`,\`project_scope_key\`,\`snapshot_id\`);`,
      )
      yield* tx.run(
        `CREATE INDEX \`released_knowledge_snapshot_parent_idx\` ON \`released_knowledge_snapshot\` (\`parent_snapshot_id\`);`,
      )
      yield* tx.run(`
