...
export default {
  id: "20260812154657_task_structured_output_evidence",
  up(tx) {
    return Effect.gen(function* () {
      ...
      `)
      yield* tx.run(`CREATE INDEX \`task_structured_output_evidence_raw_message_idx\` ON \`task_structured_output_evidence\` (\`raw_result_message_id\`);`)
      yield* tx.run(`CREATE INDEX \`task_structured_output_evidence_result_message_idx\` ON \`task_structured_output_evidence\` (\`result_message_id\`);`)
    })
  },
