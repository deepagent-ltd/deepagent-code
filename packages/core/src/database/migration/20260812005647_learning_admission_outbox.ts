...
export default {
  id: "20260812005647_learning_admission_outbox",
  up(tx) {
    return Effect.gen(function* () {
      ...
      `)
      yield* tx.run(
        `CREATE UNIQUE INDEX \`learning_admission_outbox_dedupe_idx\` ON \`learning_admission_outbox\` (\`dedupe_key\`);`,
      )
      yield* tx.run(
        `CREATE INDEX \`learning_admission_outbox_pending_idx\` ON \`learning_admission_outbox\` (\`created_at\`) WHERE "learning_admission_outbox"."state" = 'pending';`,
      )
      yield* tx.run(`
