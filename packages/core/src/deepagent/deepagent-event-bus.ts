...
import {
  ...
  DeepAgentRateLimitBucketTable,
} from "./deepagent-event-sql"
...
export type TryPublishResult = { readonly published: DeepAgentEvent.Event } | { readonly dropped: "rate_limited" }
...
export interface Interface {
   ...
   */
  readonly recordDrop: (input: { readonly event: DeepAgentEvent.Event; readonly reason: string }) => Effect.Effect<void>
    ...
    Effect.gen(function* () {
      ...
      const groupsFor = (eventType: string, workspaceID: string): Effect.Effect<ReadonlyArray<string>> => {
        ...
        return db
          ...
          .where(
            and(
              or(
                isNull(DeepAgentConsumerGroupTable.type_filter),
                eq(DeepAgentConsumerGroupTable.type_filter, eventType),
              ),
      ...
      const subscribe: Interface["subscribe"] = (input) => {
        ...
        const priorityFiltered = Stream.fromPubSub(highPriorityLive).pipe(
          Stream.filter(
            (event) =>
              (input.type ? event.type === input.type : true) &&
              (event.priority === "critical" || event.priority === "high"),
          ),
        )
        // High/critical events are published to both PubSubs so anonymous observers retain the
        // complete stream. Grouped subscribers consume only normal/low events from `live`; otherwise
        // merging both channels would deliver every high/critical event twice.
        const normalFiltered = filtered.pipe(
          Stream.filter((event) => event.priority !== "critical" && event.priority !== "high"),
        )
        const mergedFiltered = Stream.merge(priorityFiltered, normalFiltered, { haltStrategy: "both" })
      ...
      const emitDlqAlert = (eventID: DeepAgentEvent.ID, subscriptionGroup: string, reason: string, attempts: number) =>
        Effect.gen(function* () {
      ...
      const replay: Interface["replay"] = (input) =>
        Stream.unwrap(
          Effect.gen(function* () {
            ...
            if (input.workspaceID != null) conditions.push(eq(DeepAgentEventTable.workspace_id, input.workspaceID))
            const rows = yield* db
              .select()
              .from(DeepAgentEventTable)
      ...
      const deadLetters: Interface["deadLetters"] = () =>
        db
          ...
          .pipe(
            Effect.orDie,
            Effect.map((rows) => rows.map(trackerOf)),
          )
      ...
      const pendingDeliveryCount: Interface["pendingDeliveryCount"] = (workspaceID) =>
        db
          ...
          .where(
            and(eq(DeepAgentEventDeliveryTable.status, "pending"), eq(DeepAgentEventTable.workspace_id, workspaceID)),
          )
          .get()
          .pipe(
            Effect.orDie,
      ...
      const getByID: Interface["getByID"] = (eventID) =>
        db
          ...
          .pipe(
            Effect.orDie,
            Effect.map((row) => (row ? decodeRow(row) : undefined)),
          )
            ...
            () =>
              Effect.gen(function* () {
                ...
                yield* db
                  ...
                  .where(sql`${DeepAgentEventDeliveryTable.event_id} in (${candidateIDs})`)
                  .run()
                  .pipe(Effect.orDie)
      ...
      const registerConsumerGroup: Interface["registerConsumerGroup"] = (groupId, typeFilter, workspaceID) => {
        ...
        return db
          ...
          .pipe(
            Effect.orDie,
            Effect.asVoid,
            Effect.tap(() => Effect.sync(invalidateDbGroupsCache)),
          )
      }
