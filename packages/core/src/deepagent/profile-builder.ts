// FEAT-002 (docs/34 §4.1/§5): the single authoritative ExtendedProblemProfile builder, lowered into
// core so the agent gateway (core) constructs the SAME profile it hands to knowledge retrieval and
// pack-snapshot recording — one activation authority per run. Previously this logic lived only in
// deepagent-code (profile-detector.ts), which the gateway cannot import; deepagent-code now
// re-exports from here. Pure logic + workspace-fact detection (package.json / README etc.); no
// runtime imports outside node builtins.
...
// called by the gateway/orchestrator before pack activation (docs/34 §5/§9 S5). FEAT-002: this is
// the ONE profile producer the gateway uses for both retrieval and the recorded pack snapshot.
