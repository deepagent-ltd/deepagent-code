...
// is lossless: completed→'settled', interrupted→'interrupted', and
// recovery_required→'recovery_required'. recovery_required is quiescent/nonterminal and remains
// distinct from an ordinary failed/terminal result. expectedVersion is the
...
// carry no objective row). version maps mutation_epoch; settlement state is supplied by the
// caller because the base state is intentionally lossless for recovery_required.
...
export const requestPermission = Effect.fn("DeepAgentActivityAuthority.requestPermission")(function* (
  ...
  if (input.activityKind === "facade" && input.requestKind === "no_progress")
    return yield* new InvalidInputError({ reason: "facade activities do not own no-progress objective authority" })
  const { db } = yield* Database.Service
  return yield* db
    .transaction(
      (tx) =>
        Effect.gen(function* () {
          ...
          const activity =
            input.activityKind === "facade"
              ...
              : yield* tx
                  .select({
                    sessionID: SessionActivityObjectiveTable.session_id,
                    state: SessionActivityObjectiveTable.state,
                    spawnToolCallID: sql<string | null>`NULL`,
                  })
                  .from(SessionActivityObjectiveTable)
                  .where(activityWhere(input))
                  .get()
          if (!activity || !activity.sessionID) return yield* notFound(input)
          ...
          const session = yield* tx
            ...
            .from(SessionTable)
            .where(eq(SessionTable.id, activity.sessionID))
            .get()
          if (!session) return yield* Effect.die(new Error("activity session is missing: " + activity.sessionID))
              ...
              request.activity_kind === "legacy"
                ...
                : request.activity_kind === "facade"
                  ...
                  : yield* tx
                      ...
                      .get()
            if ((request.activity_kind !== "facade" && objective?.state !== "active") || base?.state !== "active")
              return yield* new ConflictError({
      ...
      (request) =>
        Effect.gen(function* () {
          ...
          const objective = yield* tx
            ...
            .get()
          if (request.activity_kind === "facade") {
            const facade = yield* tx
              .select({ state: SessionFacadeActivityTable.state })
              .from(SessionFacadeActivityTable)
              .where(eq(SessionFacadeActivityTable.activity_id, request.activity_id))
              .get()
            if (!facade || facade.state === "recovery_required") return
            if (facade.state !== "active") return
            ...
            if (!recovered)
              return yield* new ConflictError({
                entity: request.activity_id,
                expected: "active facade activity",
                actual: "changed",
              })
            return
          }
...
function recoverActivityForPermissionEffect(
  ...
  return Effect.gen(function* () {
    if (dispatch.activity_kind === "facade") {
      const facade = yield* tx
        .select({ state: SessionFacadeActivityTable.state })
        .from(SessionFacadeActivityTable)
        .where(eq(SessionFacadeActivityTable.activity_id, dispatch.activity_id))
        .get()
      if (!facade || facade.state === "recovery_required") return
      ...
      const recovered = yield* tx
        ...
        .set({
          state: "recovery_required",
          reason_code: "permission_effect_outcome_unknown_after_restart",
          settled_at: now,
          mutation_epoch: sql`${SessionFacadeActivityTable.mutation_epoch} + 1`,
        })
        .where(
          and(
            eq(SessionFacadeActivityTable.activity_id, dispatch.activity_id),
            eq(SessionFacadeActivityTable.state, "active"),
          ),
        )
        .returning({ activityID: SessionFacadeActivityTable.activity_id })
    ...
    const base =
      objective.activity_kind === "legacy"
        ...
        : objective.activity_kind === "facade"
          ? // Facade recovery is quiescent/nonterminal and must remain distinguishable from failed.
            yield* tx
              .update(SessionFacadeActivityTable)
              .set({ state: "recovery_required", reason_code: terminalReason, settled_at: now })
              .where(
                and(
...
function reconcilePermissionFanout(
  ...
  return Effect.gen(function* () {
    ...
    for (const sibling of siblings) {
      ...
      if (decision === "approved_once" && sibling.activity_kind !== "facade" && !objective)
        return yield* Effect.die(new Error("activity objective is missing: " + sibling.activity_id))
      if (
        decision === "approved_once" &&
        sibling.request_kind === "tool" &&
        (sibling.activity_kind === "facade" ? base?.state !== "active" : objective?.state !== "active")
      )
        continue
      ...
      if (!objective || sibling.activity_kind === "facade")
        return yield* Effect.die(new Error("no-progress objective is missing: " + sibling.activity_id))
      const terminalReason = "permission_interrupted"
      const interrupted =
        sibling.activity_kind === "legacy"
          ? yield* tx.get<{ activity_id: string }>(sql`
