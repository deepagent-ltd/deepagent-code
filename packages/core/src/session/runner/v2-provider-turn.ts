...
import { Cause, Context, Duration, Effect, Exit, Layer, Ref, Schedule, Schema, Stream } from "effect"
...
export type Error = ConflictError | UnsafeRetryError | NotFoundError
...
export interface Interface {
  ...
  readonly quarantine: (receipt: Receipt) => Effect.Effect<Receipt, Error>
...
export const layerWith = (options: LayerOptions = {}) =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const quarantine = (receipt: Receipt) =>
        transition({
          receipt,
          from: ["dispatching", "streaming"],
...
export function ownerQualified(db: Database.Interface["db"], campaignId?: string) {
  if (campaignId) return campaignVerified(db, campaignId)
  return Effect.succeed(releaseQualified())
}
