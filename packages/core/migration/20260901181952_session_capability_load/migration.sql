...
CREATE UNIQUE INDEX `session_capability_load_session_capability_body_idx` ON `session_capability_load` (`session_id`,`capability_id`,`body_hash`);--> statement-breakpoint
CREATE INDEX `session_capability_load_session_idx` ON `session_capability_load` (`session_id`,`loaded_at`);