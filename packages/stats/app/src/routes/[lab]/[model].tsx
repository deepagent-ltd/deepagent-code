...
const statsModelFallbackUrl = "https://stats.deepagent-code.ai"
...
export default function StatsModel() {
  ...
  const modelDescription = createMemo(() =>
    stats()
      ? `${modelName()} usage, rank, token mix, cost, geo breakdown, and peer stats across DeepAgent Code.`
      : `${modelName()} model facts, limits, and DeepAgent Code usage availability.`,
  )
      ...
      <div data-slot="model-hero-grid">
        <div data-slot="model-hero-copy">
          ...
          <Show
            when={props.data}
            fallback={
              <p>Model facts from the shared model index. DeepAgent Code usage appears once this model has activity.</p>
            }
          >
            {(data) => (
              <p>
...
function ModelCatalogCallout(props: { catalog: ModelCatalogEntry | null }) {
  return (
    <div data-component="model-rank-panel">
      ...
      <p>No DeepAgent Code usage in the current stats window.</p>
    </div>
  )
}
