...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve deepagent-code route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "deepagent-code")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "deepagent-code")).toBe("openai")
    expect(statProvider("big-pickle", "", "deepagent-code")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "deepagent-code"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("geo aggregates never keep deepagent-code or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "deepagent-code"), country: "US" })).toMatchObject([
