...
# 패키지 매니저
npm i -g deepagent-code@latest        # bun/pnpm/yarn 도 가능
scoop install deepagent-code             # Windows
choco install deepagent-code             # Windows
brew install anomalyco/tap/deepagent-code # macOS 및 Linux (권장, 항상 최신)
brew install deepagent-code              # macOS 및 Linux (공식 brew formula, 업데이트 빈도 낮음)
sudo pacman -S deepagent-code            # Arch Linux (Stable)
paru -S deepagent-code-bin               # Arch Linux (Latest from AUR)
mise use -g deepagent-code               # 어떤 OS든
nix run nixpkgs#deepagent-code           # 또는 github:lessweb/deepagent-code 로 최신 dev 브랜치
```
...
DeepAgent Code 는 데스크톱 앱으로도 제공됩니다. [releases page](https://github.com/lessweb/deepagent-code/releases) 에서 직접 다운로드하거나 [deepagent-code.ai/download](https://deepagent-code.ai/download) 를 이용하세요.
...
DeepAgent Code 에는 내장 에이전트 2개가 있으며 `Tab` 키로 전환할 수 있습니다.
...
[agents](https://deepagent-code.ai/docs/agents) 에 대해 더 알아보세요.

### 문서

DeepAgent Code 설정에 대한 자세한 내용은 [**문서**](https://deepagent-code.ai/docs) 를 참고하세요.

### 기여하기

DeepAgent Code 에 기여하고 싶다면, Pull Request 를 제출하기 전에 [contributing docs](./CONTRIBUTING.md) 를 읽어주세요.

### DeepAgent Code 기반으로 만들기

DeepAgent Code 와 관련된 프로젝트를 진행하면서 이름에 "deepagent-code"(예: "deepagent-code-dashboard" 또는 "deepagent-code-mobile") 를 포함한다면, README 에 해당 프로젝트가 DeepAgent Code 팀이 만든 것이 아니며 어떤 방식으로도 우리와 제휴되어 있지 않다는 점을 명시해 주세요.

---

**커뮤니티에 참여하기** [Discord](https://discord.gg/deepagent-code) | [X.com](https://x.com/deepagent-code)
