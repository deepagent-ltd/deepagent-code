...
const tableBucketName = `deepagent-code-${$app.stage}-lake`
...
const athenaResultsBucket = new aws.s3.Bucket("LakeAthenaResults", {
  bucket: `deepagent-code-${$app.stage}-lake-athena-results`,
  forceDestroy: $app.stage !== "production",
})

const firehoseErrorBucket = new aws.s3.Bucket("LakeFirehoseErrors", {
  bucket: `deepagent-code-${$app.stage}-lake-firehose-errors`,
  forceDestroy: $app.stage !== "production",
})

const athenaWorkgroup = new aws.athena.Workgroup("LakeAthenaWorkgroup", {
  name: `deepagent-code-${$app.stage}-lake-workgroup`,
  forceDestroy: $app.stage !== "production",
  configuration: {
...
const firehose = new aws.kinesis.FirehoseDeliveryStream(
  "LakeFirehose",
  {
    name: `deepagent-code-${$app.stage}-lake-ingest`,
    destination: "iceberg",
